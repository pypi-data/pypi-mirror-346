...
*(Version: 1.4.0, Generated: 2025-05-07 13:37:40)*
...
```python
try:
    with safe_store.safe_store("my_store.db") as store:
...
```python
with safe_store.safe_store("library.db") as store:
    store.add_document(
...
```python
with safe_store.safe_store("research_papers.db", encryption_key="mykey") as store:
...
```python
# Process 1
store1 = safe_store.safe_store("shared.db", lock_timeout=10)
with store1:
    store1.add_document("doc_a.txt") # Acquires lock

# Process 2 (running concurrently)
store2 = safe_store.safe_store("shared.db", lock_timeout=10)
try:
    with store2:
...
*   **Configuring Log Level in `safe_store.__init__`:**
    *   You can easily change the log level for a specific `safe_store` instance using the `log_level` parameter:
    ...
    store_debug = safe_store.safe_store("debug_store.db", log_level=LogLevel.DEBUG)

    # Show only WARNING messages and above
    store_warn = safe_store.safe_store("warn_store.db", log_level=LogLevel.WARNING)
    ```
...
*   **`__enter__(self) -> safe_store`** and **`__exit__(self, exc_type, exc_val, exc_tb)`**
...
try:
    with safe_store.safe_store("mystore.db", encryption_key="bad-key-format!") as store: # Example bad key
        # ...
        pass
except safe_store.ConfigurationError as e:
