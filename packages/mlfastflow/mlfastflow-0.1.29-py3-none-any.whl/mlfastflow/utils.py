...
def profile(
    ...
    """
    try:
        from pandas_profiling import ProfileReport as BaseProfileReport
    except ImportError:
        try:
            from ydata_profiling import ProfileReport as BaseProfileReport
        except ImportError:
            raise ImportError("Please install either pandas-profiling or ydata-profiling")
            
    # Create a custom ProfileReport class that wraps the original and implements fixes
    class ProfileReport(BaseProfileReport):
        def to_html(self):
            """Override to_html to patch the quantile calculations before they happen"""
            # Apply the patch before generating HTML
            from ydata_profiling.model.summarizer import BaseSummarizer
            
            # Store the original calculate_stats method
            original_calculate_stats = BaseSummarizer.calculate_stats
            
            # Define a patched method that ensures '25%' and '75%' are always present
            def patched_calculate_stats(self, series, summarizer):
                stats = original_calculate_stats(self, series, summarizer)
                
                # Ensure all necessary quantiles exist to prevent KeyError
                required_quantiles = ['25%', '75%']
                for q in required_quantiles:
                    if q not in stats:
                        # If the quantile is missing, set it to a reasonable default
                        if '50%' in stats:
                            # If median exists, use that
                            stats[q] = stats['50%']
                        elif 'mean' in stats:
                            # Otherwise use mean
                            stats[q] = stats['mean']
                ...
                # Calculate IQR directly if it's going to be calculated later
                if 'iqr' not in stats and '25%' in stats and '75%' in stats:
                    stats['iqr'] = stats['75%'] - stats['25%']
                        
                return stats
            
            # Apply the patch
            BaseSummarizer.calculate_stats = patched_calculate_stats
            
            try:
                # Call the original method
                result = super().to_html()
                
                # Restore the original method
                BaseSummarizer.calculate_stats = original_calculate_stats
                
                return result
            except Exception as e:
                # Restore the original method even if an error occurs
                BaseSummarizer.calculate_stats = original_calculate_stats
                raise e
