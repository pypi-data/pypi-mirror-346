...
class DynamicWallet:
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        if not self.address or not isinstance(self.address, str):
            logging.critical("[WALLET INIT ERROR] Invalid or missing wallet address.")
            raise SystemExit(1)
    ...
    def fetch_live_tokens(self) -> Dict[str, Dict[str, Decimal]]:
        ...
        try:
            ...
            if self.config.get("track_tokens") and isinstance(self.config.get("token_contracts"), list):
    ...
    def _fetch_token_entry(self, token: Dict[str, Any]) -> Optional[Dict[str, Dict[str, Decimal]]]:
        if token.get("symbol") in self.config.get("frozen_tokens", []):
            logging.info(f"[SKIP TOKEN] {token.get('symbol')} is marked as frozen.")
            return None
        try:
