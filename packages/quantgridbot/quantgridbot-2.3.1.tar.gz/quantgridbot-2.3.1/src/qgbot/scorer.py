...
class TokenScorer:
                ...
                if history:
                    ...
                    if previous > 0:
                        ...
                        if delta >= self.min_volatility_threshold:
                            ...
                            logging.info(f"[DECISION] SIGNAL {token[:6]} volatility Δ={delta:.5f}")
                        else:
                            logging.debug(f"[FLAT] {token} Δ={delta:.5f} below threshold")
    ...
    def best_token(self) -> Optional[str]:
        ...
        try:
            ...
            if ranked:
                ...
                logging.info(f"[DECISION] BEST TOKEN SELECTED → {top[0][:6]} Δ={top[1]:.5f}")
                return top[0]
        except Exception as e:
    ...
    def top_n_tokens(self, n: int = 3) -> List[str]:
        ...
        try:
            ...
            logging.info(f"[TOP {n}] Tokens: {top}")
            return top
        except Exception as e:
