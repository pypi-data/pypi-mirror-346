...
class GridBot:
    ...
    def setup_grid(self, center_price: Decimal, dual=False, center2: Decimal = None):
        try:
            ...
            if dual and center2:
                ...
                logging.info(f"[DECISION] RECENTER dual → ${center_price:.2f} | ${center2:.2f}")
            else:
                self.mode = "single"
                self.primary_grid = [center_price * Decimal(x) for x in np.linspace(float(lower), float(upper), points)]
                self.secondary_grid = []
                ...
                logging.info(f"[DECISION] RECENTER single → ${center_price:.2f}")
    ...
    def check_grids(self, price: Decimal):
        ...
        if len(self.price_history) > 0 and abs(self.price_history[-1] - price) < Decimal("0.0001"):
            logging.debug("[GRIDBOT] Duplicate price tick. Skipping.")
            logging.info(f"[DECISION] SKIP duplicate tick @ {price:.2f}")
            return
        ...
        if now_ts - self.last_trade_time < self.cooldown:
            logging.info(f"[DECISION] SKIP cooldown active ({self.cooldown}s)")
            return
        ...
        for center, grid in zip(centers, [self.primary_grid, self.secondary_grid] if self.mode == "dual" else [self.primary_grid]):
            for level in grid:
                ...
                if spread <= self.slippage_threshold:
                    ...
                    logging.info(f"[DECISION] GRID HIT @ {price:.2f} Δ={spread:.5f}")
                    self.executor.execute_trade(adaptive_volume=Decimal('1.0'))
                    self.last_trade_time = now_ts
                    logging.info(f"[TRADE EXECUTED] Cooldown until {self.last_trade_time + self.cooldown:.2f}")
                    return
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if price > 0:
                    ...
                    logging.info(f"[DECISION] SKIP invalid price tick")
