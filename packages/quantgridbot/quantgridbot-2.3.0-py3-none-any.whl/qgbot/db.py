...
class TradeDatabase:
    def __init__(self, db_path: str = "trades.db"):
        ...
        if not self.db_path.endswith(".db"):
            logging.critical("[DB ERROR] Invalid DB path.")
            raise SystemExit(1)
    ...
    def log_trade(self, action: str, tx_hash: str, eth_price: float, eth_amount: float, timestamp: str = None):
        ...
        if eth_price <= 0 or eth_amount <= 0:
            logging.warning(f"[DB SKIP] Invalid trade values — price={eth_price} amount={eth_amount}")
            return
        ...
        for attempt in range(1, retries + 1):
            try:
                with self._lock:
                    ...
                    logging.info(f"[DECISION] LOGGED {action.upper()} TX {tx_hash[:12]} | {eth_amount:.4f} ETH @ ${eth_price:.2f}")
                    return
    ...
    def backup_db(self, backup_path: str = None):
        ...
        try:
            with self._lock:
                ...
                logging.info(f"[DECISION] BACKUP {backup_path}")
        except Exception as e:
