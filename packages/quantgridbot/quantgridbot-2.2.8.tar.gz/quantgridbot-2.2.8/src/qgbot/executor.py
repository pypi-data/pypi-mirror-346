    ...
    def execute_trade(self, adaptive_volume: Decimal = Decimal("1.0")) -> None:
        ...
        try:
            ...
            if not self._check_slippage(expected, actual):
                logging.info("[DECISION] Slippage too high. No action taken.")
                return
            ...
            if self.config.get("simulate", False):
                logging.info(f"[SIMULATE] {trade_eth:.6f} ETH -> {token_out[:6]}")
                return
    ...
    def swap_eth_to_stable(self, usd_amount: Decimal) -> None:
        try:
            ...
            if not self._check_slippage(expected_price, actual_price):
                logging.warning("[SWAP TO STABLE] Slippage too high. Aborted.")
                return
            ...
            if self.config.get("simulate", False):
                logging.info(f"[SIMULATE] Would swap {eth_amount:.6f} ETH to stablecoin.")
                return
