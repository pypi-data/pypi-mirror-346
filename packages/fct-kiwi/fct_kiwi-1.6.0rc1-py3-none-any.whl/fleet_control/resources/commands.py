...
def schedule_change(json_file, date, date_format, tz, variables, targets) -> dict | None:
    ...
    schedule_base(date_object, message_data, "vars-change", "VarChangeLog", row)


def schedule_update(json_file, date, date_format, tz, targets, fleet, release):
        # Helper function to process JSON file or command-line variables/targets
    ...
    schedule_base(date_object, message_data, "pin-to-release" , "VarChangeLog", row)

def schedule_base(date_object, message_data, pubsub_topic, worksheet_name, row):
    service_account = load_json(SERVICE_ACCOUNT)
    pubsub_message = format_pubsub_message(message_data)
    confirmation = ask_confirmation(message_data, date_object)  # type: ignore
