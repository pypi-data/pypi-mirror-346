...
# def get_custom_set_vars_fleet(fleet: balena.types.models.TypeApplication | int) -> dict[str, dict[str, str | int]]:
#     temp = {"id": fleet} if isinstance(fleet, int) else fleet
#     raw_env_vars = balena_sdk.models.device.env_var.get_all_by_application(temp["id"])
#     raw_service_vars = balena_sdk.models.device.service_var.get_all_by_application(temp["id"])
#     vars_per_bot = {}
#     device_ids = set([item["device"]["__id"] for item in raw_env_vars])  # type: ignore
#     device_ids.update([item["device"]["__id"] for item in raw_service_vars])  # type: ignore
#     vars_per_bot = {id: {"env_vars": {}, "service_vars": {"main": {}}} for id in device_ids}
#     for i in raw_env_vars:
#         bot = i["device"]["__id"]  # type: ignore
#         vars_per_bot[bot].update({i["name"]: i["value"]})
