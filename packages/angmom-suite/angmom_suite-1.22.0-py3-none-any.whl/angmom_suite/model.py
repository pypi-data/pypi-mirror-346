...
class ProjectModelHamiltonian(Store):
    ...
    def evaluate_sf_term_rotation(self, hamiltonian):
        ...
        def cost_function(sf_sub_rot):
            ...
            params = self.model.project_parameters(ham)
            return self.model.compute_error(ham, params)[2]
        ...
        for k in range(max_step):
            ...
            if self.verbose:
                print((k, float(cost), float(opt_state[0][0]), int(opt_state[-2][0])))
...
class SpinHamiltonian(Model):
    ...
    **terms: keyword arguments
    ...
    def compute_projection_norm(self, ham):
        ...
        def proj(op):
            op = op / jnp.linalg.norm(op)
            return jnp.sum(ham * op.conj())

        norm = jnp.linalg.norm(jnp.array([proj(op) for (term, labs), ops in iter(self)
                                          for key, op in ops if term != "diag"]))
        return norm
