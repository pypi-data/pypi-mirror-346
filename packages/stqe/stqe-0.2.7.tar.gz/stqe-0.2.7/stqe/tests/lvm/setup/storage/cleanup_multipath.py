...
def cleanup_multipath():
    ...
    ret, multipath = atomic_run(
        message="Getting mpath device", command=run, return_output=True, cmd="multipath -l -v 1", errors=errors
    )
    ...
    for mpath in cmd.splitlines():
        if "-" not in mpath:
            mpath = "-f " + mpath
        atomic_run(
            message="Removing mpath device %s" % multipath, command=run, cmd="multipath %s" % mpath, errors=errors
        )
