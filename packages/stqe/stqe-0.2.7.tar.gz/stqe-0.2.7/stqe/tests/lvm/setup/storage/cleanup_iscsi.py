...
def cleanup_iscsi():
    ...
    if "brq" in hostname():
        atomic_run(
            "Releasing NFS lock on iscsi disk %s." % disk, command=release_nfs_lock_iscsi, disk_name=disk, errors=errors
        )
    ...
    if "brq" not in hostname():
        atomic_run("Removing image file", command=run, cmd="rm -rf /var/tmp/vdo-general-test.img", errors=errors)
