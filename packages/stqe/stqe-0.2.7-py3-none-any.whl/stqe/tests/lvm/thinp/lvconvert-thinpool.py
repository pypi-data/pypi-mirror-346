...
def _convert():
    ...
    TestObj.tok("lvconvert --thinpool %s/pool -y" % vg_name)
    check_lv_expected_value(TestObj, "pool", vg_name, {"lv_attr": "twi---tz--", "discards": "passdown"})
    TestObj.tok("lvremove -ff %s" % vg_name)
    ...
    TestObj.tok(f"lvconvert -y --thinpool {vg_name}/pool --poolmetadata {vg_name}/metadata")
    check_lv_expected_value(TestObj, "pool", vg_name, {"lv_size": "80.00m", "lv_metadata_size": "40.00m"})
    TestObj.tok("lvremove -ff %s" % vg_name)
