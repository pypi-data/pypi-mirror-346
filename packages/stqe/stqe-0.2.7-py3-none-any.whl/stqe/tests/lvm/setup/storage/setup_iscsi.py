...
def setup_iscsi(test_type="vdo-general"):
    ...
    if "brq" in hostname():
        ...
        atomic_run("Discovering local target", target="127.0.0.1", disc_db=True, command=discovery_st, errors=errors)

        atomic_run("Trying to login to portal: 127.0.0.1", portal="127.0.0.1", command=node_login, errors=errors)
    ...
    if "brq" in hostname():
        ret = atomic_run(
            "NFS locking iscsi disk %s." % disk, command=setup_nfs_lock_iscsi, disk_name=disk, errors=errors
        )
        if ret:
    ...
    atomic_run("Writing var VDO_DEVICE", command=write_var, var={"VDO_DEVICE": "/dev/%s" % disk}, errors=errors)
