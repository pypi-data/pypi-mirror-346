...
def cleanup_raid():
    ...
    atomic_run("Stopping raid %s" % raid, cmd="mdadm --stop %s" % raid, command=run, errors=errors)

    disks = read_var("RAID_DISKS")
    atomic_run(
        "Zeroing superblock on disks: %s" % disks, cmd="mdadm --zero-superblock %s" % disks, command=run, errors=errors
    )
