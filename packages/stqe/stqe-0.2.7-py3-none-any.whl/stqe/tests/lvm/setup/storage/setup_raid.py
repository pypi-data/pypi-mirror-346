...
def setup_raid(level, raid_name, wait_resync=False):
    ...
    if wait_resync:
        ...
        atomic_run(
            "Waiting for resync of raid %s" % raid_name, raid_name=raid_name, command=wait_for_resync, errors=errors
        )

    atomic_run("Writing var VDO_DEVICE", command=write_var, var={"VDO_DEVICE": "/dev/md/%s" % raid_name}, errors=errors)

    atomic_run("Writing var RAID_DISKS", command=write_var, var={"RAID_DISKS": " ".join(disk_paths)}, errors=errors)
