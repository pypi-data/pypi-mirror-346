...
class AddJobSchedule(APIView):                                           
    ...
    def post(self, request):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                print('\n--- pipelineViews: AddJobSchedule: reconfigs:', reconfigs)
                ...
                if len(reconfigs) > 0:
                    reconfigStrList = ''
                    for index, reconfig in enumerate(reconfigs):
                        reconfigStrList += f'{reconfig}'
                        if reconfig != reconfigs[-1]:
                            reconfigStrList += ','
                        
                    newJob += f'&reconfigs={reconfigStrList}'
                else:
                    newJob += f'&reconfigs={reconfigs}' 
...
class GetReconfigsDropdownForPipeline(APIView):
    ...
    def post(self, request, data=None):
        ...
        # reconfigsHtml = '<label for="reconfigs">Select a reconfig file:</label>&emsp;'
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            InternalVar.html = response.json()['reconfigs']
            ...
            def loop(path, reconfigFiles):
                ...
                try:          
                    ...
                    for eachFile in reconfigFiles:
                        reconfigFileFullPath = f'{path}/{eachFile}'
                        reconfigGroupPath = reconfigFileFullPath.split(f'{GlobalVars.reconfigsFolder}/')[-1]
                        # Show just the name of the env without the group name because user expanded the group folder already.
                        reconfigName = eachFile.split('.')[0]
                        html += f'<li><input type="checkbox" name="individualReconfigCheckbox" value="{reconfigGroupPath}" />&emsp;{reconfigName}</li>'
                                    
                except Exception as errMsg:
                    errorMsg = str(errMsg)
                    status = 'failed'
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='GetAllReconfigs', msgType='Error',
            ...
            try:    
                for root, dir, files in os.walk(GlobalVars.reconfigsFolder):
                    # root: /opt/KeystackTests/Reconfigs          
                    ...
                    # /opt/KeystackTests/Reconfigs
                    #shortenPath = root.split(f'{GlobalVars.reconfigsFolder}/')[-1]
                    html = loop(path=root, reconfigFiles=files)
            
                    topFolder = '<ul id="testResultFileTree">'
                    
                    # # Add checkbox for for group selections
                    # topFolder += f'\n\t\t\t<li><input type="checkbox" name="groupReconfigCheckbox" value={shortenPath} />&emsp;<span class="caret2">&ensp;{shortenPath}</span>'
                    topFolder += f'\n\t\t\t<li><span class="caret2">&ensp;{root}</span>'                    
        ...
        return Response(data={'reconfigs': InternalVar.html, 'errorMsg': errorMsg, 'status': status}, status=statusCode)
   
   
class GetReconfigsDropdownForPipeline_backup(APIView):
    swagger_schema = None
