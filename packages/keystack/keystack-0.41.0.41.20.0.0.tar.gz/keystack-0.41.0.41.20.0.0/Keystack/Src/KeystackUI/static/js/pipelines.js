...
// For modifying env
var envPath;
var selectedReconfigsForChanges = null;
var newReconfigFileName = null;
var createNewReconfigsDone = false;

var individualReconfigsArray = [];
//var reconfigGroupArray = [];
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (domain == 'None') {
        ...
        getReconfigsDropdownForPipeline();
...
const resetParams = () => {
    ...
    //document.querySelector("#reconfigs").value = "Select";
    individualReconfigsArray = [];
    //reconfigGroupArray = [];
    document.querySelector('#objectTitle').innerHTML = '';
    selectedPlaybookDomain = null;
    selectedReconfigsForChanges = null;
...
const getSelectedTestParameters = () => {
    ...
    getSelectedReconfigs();
    
    // reconfigs: document.querySelector("#reconfigs").value,
    let selectedTestParameters = {remoteController: sessionStorage.getItem("remoteController"),
                                  ...
                                  reconfigs: individualReconfigsArray,
...
const setReconfigsFile = (object) => {
    /* reconfigsFile={reconfigFile} onclick="setReconfigsFile(this)"
     returns full path to the reconfigs file
    */
    reconfigs = object.getAttribute('reconfigsFile');
    document.querySelector("#insertSelectedReconfigs").innerHTML += `Selected reconfig file:&ensp;${reconfigs}`;
}
...
const getReconfigsDropdownForPipeline = async () => {
    /* Get reconfig dropdown menu for pipeline test parameters */
    try {
        const data = await commons.postData("/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline",  
                                                {remoteController: sessionStorage.getItem("remoteController"),
                                                 domain: document.querySelector("#pageAttributes").getAttribute('domain'),
                                                })

        document.querySelector('#insertReconfigs').innerHTML = data.reconfigs;
        await addListeners({caretName:"caret2"});

    } catch (error) {
        commons.getInstantMessages('pipelines');
        console.error(`getReconfigsList(): ${error}`)
    }    
}

const getSelectedReconfigs = () => {
    // Individial Envs
    const individualReconfigCheckbox = document.querySelectorAll('input[name=individualReconfigCheckbox]:checked');

    for (let x=0; x < individualReconfigCheckbox.length; x++) {
        let reconfigPath = individualReconfigCheckbox[x].getAttribute('value');
        individualReconfigsArray.push(reconfigPath)
        individualReconfigCheckbox[x].checked = false;
    }

    // Env groups
    /*
    const reconfigGroupCheckboxes = document.querySelectorAll('input[name=groupReconfigCheckbox]:checked');
...
const getReconfigContents = async (selectedReconfigFile) => {
    /* Get dropdown selected reconfig file contents */

    try {
        selectedReconfigsForChanges = selectedReconfigFile;

        const data = await commons.postData("/api/v1/fileMgmt/getFileContents",  
                 {remoteController: sessionStorage.getItem("remoteController"),
                  domain: document.querySelector("#pageAttributes").getAttribute('domain'), 
                  filePath: selectedReconfigFile})
        ...
        if (data.status == "failed") {
            ...
            document.querySelector('#insertReconfigFileContents').innerHTML = `<textarea id="reconfigFileContentsTextareaId" cols="102" rows="13">${data.fileContents}</textarea><br><br>`;
        }
    } catch (error) {    
...
const deleteReconfig = async () => {
    /* Delete a reconfig file */
    if (selectedReconfigsForChanges == null) {
        alert('Please select a reconfig from the dropdown menu first')
        return
    } 

    try {
        const data = await commons.postData("/api/v1/pipelines/reconfigs/delete",  
                                                {remoteController: sessionStorage.getItem("remoteController"),
                                                 domain: document.querySelector("#pageAttributes").getAttribute('domain'),
                                                 reconfigFullPath: selectedReconfigsForChanges})

        commons.getInstantMessages('pipelines');
        if (data.status == "success") {
            const status = `<div style='color:green'>Successfully deleted reconfig: ${selectedReconfigsForChanges}</div>`;
            document.querySelector("#modifyTestcaseFileStatus").innerHTML = status;
            getReconfigsList();
            selectedReconfigsForChanges = null;

            document.querySelector('#insertReconfigFileContents').innerHTML = `<textarea id="reconfigFileContentsTextareaId" cols="101" rows="13"></textarea>`
        } else {
        ...
        console.error(`deleteReconfig(): ${error}`)
    }   
}

const saveReconfigChanges = async () => {
    ...
    try {
        ...
        const data = await commons.postData("/api/v1/fileMgmt/modifyFile",  
                                            {remoteController: sessionStorage.getItem("remoteController"),
                                             domain: document.querySelector("#pageAttributes").getAttribute('domain'),
                                             textarea: textarea,
                                             filePath: selectedReconfigsForChanges})

        commons.getInstantMessages('pipelines');

        if (data.status == "success") {
            const status = `<div style='color:green'>Successfully modified reconfig: ${selectedReconfigsForChanges}</div>`;
            document.querySelector("#modifyTestcaseFileStatus").innerHTML = status;
        } else {
            const status = `<div style='color:red'>Failed: ${data.errorMsg}</div>`;
            document.querySelector("#modifyTestcaseFileStatus").innerHTML = status;
...
const createNewReconfigs = async () => {
    /* Create a new reconfig file.
       Provide a template of all the available parameters 
    */
    try {
        const data = await commons.postData("/api/v1/pipelines/reconfigs/getParams",  
                                            {remoteController: sessionStorage.getItem("remoteController"),
                                             domain: document.querySelector("#pageAttributes").getAttribute('domain')})
        ...
        if (data.status == "success") {
            ...
            document.querySelector('#insertReconfigFileContents').innerHTML = `<textarea id="reconfigFileContentsTextareaId" cols="101" rows="13">${data.reconfigParams}</textarea>`;

            // saveNewReconfigs needs to know if user indeed gotten new reconfig params
            createNewReconfigsDone = true;
        ...
        console.error(`createNewReconfig error: ${error}`);
    }
}

const saveNewReconfigs = async () => {
    /* Save new reconfig param and values from the text area */

    try {
        if (createNewReconfigsDone == false) {
            alert('Please press the "Create New" button and make your changes in the text area first')
            return
        }

        newReconfigFileName = document.querySelector('#NewReconfigFileName').value;

        if (newReconfigFileName == null || newReconfigFileName == '') {
        ...
        const data = await commons.postData("/api/v1/pipelines/reconfigs/saveNewReconfigsToFile",
                                            {remoteController: sessionStorage.getItem("remoteController"),
                                             domain: document.querySelector("#pageAttributes").getAttribute('domain'),
                                             textarea: textarea,
                                             reconfigName: newReconfigFileName})

        commons.getInstantMessages('pipelines');
        if (data.status == "success") {
            const status = `<div style='color:green'>Successfully saved new reconfig: ${newReconfigFileName}</div>`;
            document.querySelector("#modifyTestcaseFileStatus").innerHTML = status;
            getReconfigsList();
            newReconfigFileName = document.querySelector('#NewReconfigFileName').value = '';
        ...
        console.error(`saveNewReconfigs error: ${error}`);
    }
}

const getReconfigsList = async () => {
    /* Reconfigurations: dropdown for reconfig files */
    try {
        const data = await commons.postData("/api/v1/pipelines/reconfigs/getReconfigsList",  
                                                {remoteController: sessionStorage.getItem("remoteController"),
                                                 domain: document.querySelector("#pageAttributes").getAttribute('domain'),
                                                })

        document.querySelector('#insertReconfigsDropdown').innerHTML = data.reconfigsList;
        commons.getInstantMessages('pipelines');
    } catch (error) {
        console.error(`getReconfigsList(): ${error}`)
    }    
...
const closeShowModifyTestcasesModal = () => {
    ...
    document.querySelector("#reconfigsList").value = "Select Reconfig File";
    document.querySelector('#insertReconfigFileContents').innerHTML = '';
    //document.querySelector("#newReconfigFileName").value = '';
    newReconfigFileName = null;
    createNewReconfigsDone = false;
}
...
window.createNewReconfigs = createNewReconfigs;
window.saveNewReconfigs = saveNewReconfigs;
window.saveReconfigChanges = saveReconfigChanges;
window.deleteReconfig = deleteReconfig;
...
window.getReconfigContents = getReconfigContents;
...
window.setReconfigsFile = setReconfigsFile;
window.getReconfigsDropdownForPipeline = getReconfigsDropdownForPipeline;
