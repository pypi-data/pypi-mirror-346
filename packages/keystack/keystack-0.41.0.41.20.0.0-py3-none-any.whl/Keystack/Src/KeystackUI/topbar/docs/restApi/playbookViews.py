    ...
    def post(self, request, data=None):
        ...
        try:
            ...
            if request.GET:
                ...
                self.reconfigs             = request.GET.get('reconfigs', [])
        ...
        if request.data:
            ...
            try:
                ...
                self.playbook                      = request.data.get('playbook', None)
                 # ['demo2.yml', 'demo1.yml', 'qa/sample2.yml']
                self.reconfigs                     = request.data.get('reconfigs', [])
        ...
        # If job came from cron scheduler, self.reconfigs is a string. Needs to be a list.
        if type(self.reconfigs) == str:
            self.reconfigs = self.reconfigs.split(',')
        ...
        if self.remoteControllerIp and self.remoteControllerIp != self.mainControllerIp:                
            ...
            if runOnLocalController:
                ...
                if self.reconfigs:  
                    for reconfig in self.reconfigs:                  
                        command += f' -reconfigs {reconfig}'
