...
class LockManager:
    ...
    def _key(self, slot_type: str, task_name: str, signature: str) -> str:
        ...
        return f"{self._prefix}:{task_name}:{slot_type}:{signature}"
    ...
    def acquire(
        ...
        if new_count == 1:
            ...
            # over limit: rollback and deny
            self._client.decr(key)
            return False
        else:
            # ensure TTL is at least what's configured
            if current_ttl < ttl:
                self._client.expire(key, ttl)
        return True
...
class LimitedTask(Task):
    ...
    def apply_async(
        ...
        **options: Any,
        ...
        if not self._lock_manager.acquire_queue(self.name, sig, queue_limit, ttl):
            # Over queue limit: skip enqueue
            return None

        headers = options.setdefault("headers", {})
        headers.setdefault("_limited_sig", sig)
    ...
    def __call__(
        ...
        if sig:
            # Task dequeued: release queue slot
            ...
            if not self._lock_manager.acquire_run(
                ...
                # Concurrency full: requeue if allowed
                queue_limit = self._get_queue_limit()
                if queue_limit > 0:
                    # Re-enqueue
                    super().apply_async(
            ...
            # Mark that run slot was acquired
    ...
    def after_return(
        ...
        # Release run slot if it was acquired
