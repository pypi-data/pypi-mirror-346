...
def _validate_json_file(file_path: str) -> bool:
    """Valida che il file JSON sia valido e leggibile."""
    try:
...
def _setup():
    ...
    if not Path(config_file).is_file():
        ...
        config['main_section'] = {
            ...
            'max_backups': '20',  # Numero massimo di backup da mantenere
            'log_max_bytes': '10485760',  # 10MB in bytes
            'log_backup_count': '5',  # Numero di file di backup da mantenere
            'log_compress': 'true'  # Comprimi i file di log ruotati
        }
        config['backend'] = {
            'name': '__backend__',
            'cmd': cmd_backend,
...
def main():
    ...
    # Verifica la validità del file JSON prima di procedere
    if Path(db_path).exists() and not _validate_json_file(db_path):
        ...
        if latest_backup:
            ...
            print(f"[yellow]Suggestion:[/yellow] You can restore from the latest backup using either command:")
            print(f"    gunzip -c {latest_backup} > {db_path}")
            print("    # or")
            print(f"    zcat {latest_backup} > {db_path}")
        else:
            print(f"[red]ERROR:[/red] Database file {db_path} is corrupted and no backups are available.")
            print("[yellow]Suggestion:[/yellow] Remove the corrupted file and restart the daemon to create a new one.")
        sys.exit(1)
