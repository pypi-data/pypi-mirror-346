...
def test_memory_logger_log_multiple_events(memory_logger):
    ...
    for event_type, payload in events:
        memory_logger.log(agent_id=payload["agent_id"], event_type=event_type, payload=payload)
...
def test_memory_logger_invalid_event(memory_logger):
    ...
    with pytest.raises(ValueError, match="Event must contain 'agent_id'"):
        memory_logger.log(agent_id="", event_type="test_event", payload={"data": "test"})
...
def test_memory_logger_redis_connection_error():
    with patch("orka.memory_logger.redis.from_url", side_effect=Exception("Connection error")):
