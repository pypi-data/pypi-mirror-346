...
def test_router_node_bool_key_variants():
    node = RouterNode(node_id="test", params={"decision_key": "d", "routing_map": {True: ["yes"], False: ["no"]}}, queue="test")
    # Should match True
...
async def test_fork_node_raises_on_no_targets():
    ...
    fork_node = ForkNode(node_id="fork_test", prompt=None, queue="test", memory_logger=memory)
    class DummyOrchestrator:
        fork_manager = None
        def enqueue_fork(self, *a, **k): pass
...
def test_join_node_complete_merges_and_cleans():
    ...
    join = JoinNode(node_id="join", prompt=None, queue="test", memory_logger=FakeMemoryLogger(), group=None)
