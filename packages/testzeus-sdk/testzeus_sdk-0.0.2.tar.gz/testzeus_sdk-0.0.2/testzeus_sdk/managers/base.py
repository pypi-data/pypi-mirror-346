...
class BaseManager(Generic[T]):
    ...
    def __init__(
        self, client: TestZeusClient, collection_name: str, model_class: Type[T]
    ) -> None:
        """
    ...
    async def get_one(
        self, id_or_name: str, expand: Optional[Union[str, List[str]]] = None
    ) -> T:
        """
        ...
        if self._is_valid_id(id_or_name):
            result = self.client.pb.collection(self.collection_name).get_one(
                id_or_name, query_params={"expand": expand_str}
            )
        else:
            ...
            if not tenant_id:
                raise ValueError(
                    "User must be authenticated with a tenant to find by name"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Could not find {self.collection_name} with name: {id_or_name}"
                ) from e
    ...
    async def create(self, data: Dict[str, Any]) -> T:
        ...
        if "tenant" not in data:
            ...
            if tenant_id:
                ...
                print(
                    f"Warning: No tenant ID available when creating {self.collection_name}"
                )
        ...
        try:
            result = self.client.pb.collection(self.collection_name).create(
                processed_data
            )
            return self.model_class(record_to_dict(result))
        except Exception as e:
    ...
    async def update(self, id_or_name: str, data: Dict[str, Any]) -> T:
        ...
        if "tenant" not in data:
            ...
            if tenant_id:
                ...
                print(
                    f"Warning: No tenant ID available when updating {self.collection_name}"
                )
        ...
        try:
            ...
            result = self.client.pb.collection(self.collection_name).update(
                entity_id, processed_data
            )
            return self.model_class(record_to_dict(result))
        except Exception as e:
            # Provide more information about the error
            raise ValueError(
                f"Failed to update {self.collection_name} {id_or_name}: {str(e)}"
            ) from e
