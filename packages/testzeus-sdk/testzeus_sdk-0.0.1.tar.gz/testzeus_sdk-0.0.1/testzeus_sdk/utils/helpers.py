...
async def expand_test_run_tree(client: TestZeusClient, test_run_id: str) :
    """
    ...
    test_run_dash_output_steps = await client.test_run_dash_output_steps.get_list(filters={'test_run_dash_output':[test_run_dash_output.id for test_run_dash_output in test_run_dash_outputs['items']]})
    ...
    test_run_dash_outputs_attachments = await client.test_run_dash_outputs_attachments.get_list(filters={'test_run_dash_output':[test_run_dash_output.id for test_run_dash_output in test_run_dash_outputs['items']]})
...
def convert_name_refs_to_ids(
    ...
    for field, collection in ref_fields.items():
        if (
            ...
            and not client.managers.base._is_valid_id(result[field])
        ):
            # Convert name to ID
