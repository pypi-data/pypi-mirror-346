...
class TestRunManager(BaseManager[TestRun]):
    ...
    async def cancel(self, id_or_name: str) -> TestRun:
        ...
        return await self.update(str(test_run.id), {"status": "cancelled"})
    ...
    async def download_all_attachments(
        self, id_or_name: str, output_dir: str = "."
    ) -> List[str]:
        """
        ...
        for attachment in attachments:
            filepath = (
                await self.client.test_run_dash_outputs_attachments.download_attachment(
                    attachment["id"], output_dir
                )
            )
            if filepath:
