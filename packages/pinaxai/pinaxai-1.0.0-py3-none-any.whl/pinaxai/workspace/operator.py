...
def start_workspace(
    ...
    """Start an Pinaxai Workspace. This is called from `pa ws up`"""
    if ws_config is None:
...
def stop_workspace(
    ...
    """Stop an Pinaxai Workspace. This is called from `pa ws down`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def update_workspace(
    ...
    """Update an Pinaxai Workspace. This is called from `pa ws patch`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def set_workspace_as_active(ws_dir_name: Optional[str]) -> None:
    ...
    if active_ws_config is None:
        ...
        print_info("If this workspace has not been setup, please run `pa ws setup` from the workspace directory")
        return
