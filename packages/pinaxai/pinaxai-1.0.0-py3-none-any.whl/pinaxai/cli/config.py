...
class PinaxaiCliConfig:
    ...
    def clear_user_cache(self) -> None:
        ...
        logger.info("Workspaces cleared, please setup again using `pa ws setup`")
    ...
    def _add_or_update_ws_config(
        ...
        if existing_ws_config is None:
            logger.error(f"Could not find workspace at: {ws_root_str}, please run `pa ws setup`")
            return None
    ...
    def from_saved_config(cls) -> Optional["PinaxaiCliConfig"]:
        try:
            ...
            logger.warning(e)
            logger.warning("Please setup the workspace using `pa ws setup`")
            return None
    ...
    def print_to_cli(self, show_all: bool = False):
        ...
        if self.active_ws_dir:
            ...
            print_info(
                "Please create a workspace using `pa ws create` or setup an existing workspace using `pa ws setup`"
            )
