...
class Simba:
    ...
    def init_simba(self, input_file):
        ...
        logging.info("┃" + "".center(68) + "┃")
        logging.info("┃" + "Version 1.0.0 | 2025".center(68) + "┃")
        logging.info("┃" + "".center(68) + "┃")
        logging.info("┗" + ("━" * 68) + "┛")
        logging.info("\n")
        ...
        try:   
            ...
            clipped_exponent = np.minimum(3.02 * input_data['Av'], max_safe_exponent)
            self.environment.G_0_unatt = input_data['G_0'] * (np.exp(clipped_exponent))
        ...
        try:
            ...
            if self.environment.G_0 > 5e5:
                self.species.abundance[i_PAH0] = 4.17e-07 * 1e-10
    ...
    def compute_rate_coefficients(self, y):
        ...
        Notes:
            - Rate coefficients are computed based on current gas and dust temperatures
            ...
            - Temperature limits are enforced for relevant reaction types
        ...
        # Safeguard for n_gr to prevent underflow
        n_gr = np.maximum(n_gr, 1e-30)
        ...
        # Final safety check for NaN or Inf values that might have slipped through
        if np.any(np.isnan(self.reactions.k)) or np.any(np.isinf(self.reactions.k)):
            ...
            self.reactions.k = np.nan_to_num(self.reactions.k, nan=1e-99, posinf=1e20, neginf=1e-99)
    ...
    def _try_integration(self, config):
        ...
        self._progress_bar = tqdm(ascii=" ■", ncols=70, total=100, bar_format="   [{bar}] {percentage:3.0f}% [{elapsed}<{remaining}]", colour='#6495ED')
        ...
        def wrapper_derivatives(t, y):
            ...
            # Clip rate coefficients to improve numerical stability
            self.reactions.k = np.clip(self.reactions.k, 1e-20, 1e20)
