...
def safe_write_to_jsons(file_path:str=None, data:(dict or list or str)=None,*args, ensure_ascii=False, indent=4, error=False, error_msg=None, **kwargs):
...
def safe_write_to_jsons(file_path:str=None, data:(dict or list or str)=None,*args, ensure_ascii=False, indent=4, error=False, error_msg=None, **kwargs):
    ...
    if params:
        ...
        if open_it:
            ...
            os.replace(temp_file_path, params[0])
def safe_dump_to_fil(data, file_path=None, ensure_ascii=False, indent=4, *args, **kwargs):
...
def safe_load_from_file(file_path):
    return safe_read_from_json(file_path)
def safe_read_from_file(file_path):
    return safe_read_from_json(file_path)
