...
def make_temp_dir(file_path):
    ...
    return get_new_backup_dir(backup_dir=tmp_dir)
def convert_variables_files(strings,interface_file,react_file):
    get_src_dir_from_file_path(file_path)
    ...
    temp_dir = make_temp_dir(file_path)
    src_dir = get_src_dir_from_file_path(file_path)
    replacement_file_dir = os.path.join(temp_dir)
    react_basename = os.path.basename(react_file)
    replacement_file_path = os.path.join(replacement_file_dir,react_basename)
    write_to_file(data=react_data,file_path=replacement_file_path)
    
    write_to_file(contents=react_data,file_path=react_file_path)
