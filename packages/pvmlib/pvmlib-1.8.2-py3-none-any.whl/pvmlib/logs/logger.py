...
class LogData:
    ...
    def __initialize(self, origin: str = "INTERNAL"):
        ...
        try:
            ...
            self.gcp_handler = CloudLoggingHandler(self.gcp_client, name=self.app.name)
            self.gcp_handler.setLevel(logging.INFO)
            logger.addHandler(self.gcp_handler)
    ...
    def log(
        ...
        console_log, gcp_log_payload = self._format_log(log_entry, filename, method_name, context.get_tracing_id())

        if self.gcp_client:
            self.gcp_logger.log_struct(gcp_log_payload, severity=level)
        else:
            self.logger.log(level, console_log)
    ...
    def _format_log(
        ...
        Returns:
            ...
            dict: The payload for Google Cloud Logging.
        """
        ...
        def format_log_message(log_data: DataLogger) -> str:
            ...
            """
            return json.dumps(log_data.model_dump())

        part_four_log = format_log_message(log_entry)
        console_log = f'{part_one_log}{part_thow_log}{part_three_log}{part_four_log}'  # Formato para la consola

        # Create the payload for GCP Logging
        gcp_log_payload = {
            ...
            "additional_info": log_entry.additional_info,  # Ensure this is JSON serializable
            "exception": log_entry.exception.model_dump() if log_entry.exception else None,
        }
        return console_log, gcp_log_payload
