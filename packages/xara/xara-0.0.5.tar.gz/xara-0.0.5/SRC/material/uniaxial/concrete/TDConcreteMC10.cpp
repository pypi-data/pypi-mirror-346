...
OPS_ADD_RUNTIME_VPV(OPS_TDConcreteMC10)
{
  ...
  if (numArgs == 17) {
    ...
    //ntosic
    theMaterial = new TDConcreteMC10(
        iData, dData[0], dData[1], dData[2], dData[3], dData[4], dData[5],
...
TDConcreteMC10::TDConcreteMC10(int tag, double _fc, double _ft, double _Ec,
                               ...
                               double _cem)
    : UniaxialMaterial(tag, MAT_TAG_TDConcreteMC10), fc(_fc), ft(_ft), Ec(_Ec),
      Ecm(_Ecm), beta(_beta), age(_age), epsba(_epsba), epsbb(_epsbb),
      epsda(_epsda), epsdb(_epsdb), phiba(_phiba), phibb(_phibb), phida(_phida),
      phidb(_phidb), tcast(_tcast), cem(_cem)
{
  ecminP = 0.0;
  ecmaxP = 0.0; //ntosic
  deptP  = 0.0;
...
{
  TDConcreteMC10 *theCopy = new TDConcreteMC10(
      this->getTag(), fc, ft, Ec, Ecm, beta, age, epsba, epsbb, epsda, epsdb,
      phiba, phibb, phida, phidb, tcast, cem); //ntosic
...
TDConcreteMC10::setCreepBasicStrain(double time, double stress)
{
  ...
  for (int i = 1; i <= count; i++) {
    ...
    runSum +=
        PHIB_i[i] * DSIG_i[i] /
        Ecm; //CONSTANT STRESS within Time interval //ntosic: changed to Ecm from Ec (according to Model Code formulation of phi basic)
  }
...
TDConcreteMC10::setCreepDryingStrain(double time, double stress)
{
  ...
  for (int i = 1; i <= count; i++) {
    ...
    runSum +=
        PHID_i[i] * DSIG_i[i] /
        Ecm; //CONSTANT STRESS within Time interval //ntosic: changed to Ecm from Ec (according to Model Code formulation of phi drying)
  }
...
TDConcreteMC10::setPhiDrying(double time, double tp)
{
  ...
  double tmtp      = time - tp;
  double tpa       = tp * pow(9.0 / (2.0 + pow(tp, 1.2)) + 1.0, cem);
  double phiDrying = phida / (0.1 + pow(tpa, 0.2)) *
                     pow(tmtp, 1.0 / (2.3 + 3.5 / pow(tpa, 0.5))) /
...
{
  double shrinkBasic =
      epsba * (1 - exp(-0.2 * epsbb *
                       pow(time, 0.5))); //ntosic: Model Code 2010 Equations
  return shrinkBasic;
}
...
TDConcreteMC10::setShrinkDrying(double time)
{
  ...
  if (time - (tD) < 0) {
    ...
    shrinkDrying =
        epsda * pow(time - (tD), 0.5) /
        pow(epsdb + time - (tD), 0.5); //ntosic: Model Code 2010 Equations
  }
  return shrinkDrying;
}
...
TDConcreteMC10::setTrialStrain(double trialStrain, double strainRate)
{
  ...
  if (t - tcast <
      (2.0 -
       0.0001)) { //Assumed that concrete can only carry load once hardened at 2 days following casting
    eps_crb   = 0.0; //ntosic
    eps_crd   = 0.0; //ntosic
    eps_shb   = 0.0; //ntosic
    eps_shd   = 0.0; //ntosic
    ...
    if (ops_Creep == 1) {
      if (fabs(t - TIME_i[count]) <=
          0.0001) { //If t = t(i-1), use creep/shrinkage from last calculated time step
...
TDConcreteMC10::setStress(double strain, double &stiff)
{
  ...
  if (strain <= ecmin) { // Concrete in compression loading
    ...
    if (strain < 0.0) { // Compression Unloading
      ...
      double et0 = ft / Ec;
      if (strain >=
          ecmax) { //Tens Load or reload if strain is larger than before
        ...
        if (strain <= et0 &&
            ecmax <= et0) { //Linear unloading/reloading, i.e, not cracked
          this->Tens_Envlp(strain, stress, stiff);
...
Response *
TDConcreteMC10::setResponse(const char **argv, int argc, OPS_Stream &theOutput)
{
  ...
  // limit:  no more than 10000 random variables/sensitivity parameters
    ...
    theResponse =
        new MaterialResponse(this, gradient + 10000, this->getStress());
  }
  // strain sensivitiy
    ...
    theResponse =
        new MaterialResponse(this, gradient + 20000, this->getStrain());
  }
...
TDConcreteMC10::getResponse(int responseID, Information &matInfo)
{
  ...
  static Vector CreepStressStrainTangent(
      8); //Added by AMK //ntosic: modified number
