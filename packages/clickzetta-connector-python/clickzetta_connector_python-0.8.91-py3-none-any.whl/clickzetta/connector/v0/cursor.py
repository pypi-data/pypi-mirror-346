...
class Cursor(object):
    ...
    def _is_insert_into(self, operation_wo_comments):
        if RE_SQL_INSERT_STMT.match(operation_wo_comments):
            return True
        return False
    ...
    def executemany(self, operation: str, seqparams: Union[Sequence[Any], dict, None] = None, parameters=None,
        ...
        Args:
            ...
            insert_batch_size: The batch size for insert into operation.
        ...
        if not self._is_insert_into(operation_wo_comments):
            _log.debug("Non-INSERT INTO statement detected, executing without batching")
            sql_length = len(operation)
            if sql_length > 5242880:
                raise ProgrammingError(f"FAILED SQL length exceeds limit 5242880, current length {sql_length}")

            batch_values = []
            for param in seqparams:
                p = self.connection._process_params_qmarks(param)
                count = -1
                def replacer(_):
                    nonlocal count
                    count += 1
        ...
        for i in range(0, len(seqparams), insert_batch_size):
            batch = seqparams[i:i + insert_batch_size]
            batch_values = []
            for param in batch:
