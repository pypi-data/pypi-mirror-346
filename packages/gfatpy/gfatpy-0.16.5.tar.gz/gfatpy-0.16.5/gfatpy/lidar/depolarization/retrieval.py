...
def add_depolarization_products_of_channel(
    ...
    for mode_ in polarized_channels[f"{telescope_}f"][wavelength_].keys():
        ...
        if (
            ...
            eta_star_key = f"eta_star_mean_{wavelength_}{telescope_}{mode_}"
            if eta_star_key not in calibration:
                eta_star_key = eta_star_key.replace("g", "p")  # Try replacing 'g' with 'p'
            try:
                eta_star = calibration[eta_star_key].item()
            except KeyError:
                raise KeyError(
                    f"{eta_star_key} must be a key in the calibration xr.dataset."
                )
