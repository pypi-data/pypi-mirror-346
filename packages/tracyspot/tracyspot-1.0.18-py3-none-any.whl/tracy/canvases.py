...
class KymoCanvas(ImageCanvas):
    ...
    def draw_trajectories_on_kymo(self, showsearchline=True):
        ...
        # only do this when you’re in “value”‐mode
        if self.navigator.trajectoryCanvas._column_types.get(col) == "value":
            # 1) collect unique values in the order they appear
            seen = []
            for traj in self.navigator.trajectoryCanvas.trajectories:
                v = traj.get("custom_fields", {}).get(col, "")
                if v and v not in seen:
                    seen.append(v)
            unique_vals = seen

            # 2) concatenate Accent, tab10 and tab20 into one long list of hex colours
            def cmap_to_hex(name):
                cmap = cm.get_cmap(name)
                return [mcolors.to_hex(cmap(i)) for i in range(cmap.N)]

            colours = cmap_to_hex("Accent") + cmap_to_hex("tab10") + cmap_to_hex("tab20")
            
            # 3) assign each value the next colour in that list (wrap if needed)
            color_map = {
                val: colours[i % len(colours)]
                for i, val in enumerate(unique_vals)
            }
        else:
            color_map = {}

        # Clear any old legend items
        for i in reversed(range(self.navigator.legendLayout.count())):
            w = self.navigator.legendLayout.itemAt(i).widget()
            if w:
                w.setParent(None)

        # If we’re in “value” mode, build swatches + labels and show:
        col = self.color_by_column
        col_type = self.navigator.trajectoryCanvas._column_types.get(col)
        if col_type == "value":
            for v in unique_vals:
                swatch = QLabel(self.navigator.legendWidget)
                swatch.setFixedSize(12,12)
                swatch.setStyleSheet(f"""
                    background-color: {color_map[v]};
                    border: 1px solid #333;
                    border-radius: 2px;
                """)
                label = QLabel(v, self.navigator.legendWidget)
                label.setStyleSheet("color: #222; font-size: 14px;")
                self.navigator.legendLayout.addWidget(swatch,  0, Qt.AlignVCenter)
                self.navigator.legendLayout.addWidget(label,  0, Qt.AlignVCenter)
            self.navigator.legendWidget.show()
            self.navigator.legendWidget.adjustSize()
        elif col_type == "binary":
            # single‐swatch legend for your binary flag
            swatch = QLabel(self.navigator.legendWidget)
            swatch.setFixedSize(12,12)
            swatch.setStyleSheet("""
                background-color: #FFC107;   /* same “marked” colour */
                border: 1px solid #333;
                border-radius: 2px;
            """)
            lbl = QLabel(col, self.navigator.legendWidget)
            lbl.setStyleSheet("color: #222; font-size: 14px;")
            self.navigator.legendLayout.addWidget(swatch, 0, Qt.AlignVCenter)
            self.navigator.legendLayout.addWidget(lbl,    0, Qt.AlignVCenter)
            self.navigator.legendWidget.show()
            self.navigator.legendWidget.adjustSize()

        else:
            # neither value nor binary → hide the legend
            self.navigator.legendWidget.hide()
        ...
        for idx, traj in enumerate(self.navigator.trajectoryCanvas.trajectories):
            ...
            if col is None:
                ...
                if self.navigator.trajectoryCanvas._column_types[col] == "binary":
                    val = traj["custom_fields"].get(col, "")
                    main_color = "#FFC107" if val else "magenta"
                else:  # value column
                    val = traj["custom_fields"].get(col, "")
                    main_color = color_map.get(val, "magenta")
