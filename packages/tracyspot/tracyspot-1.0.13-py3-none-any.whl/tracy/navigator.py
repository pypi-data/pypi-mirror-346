...
class KymographNavigator(QMainWindow):
    ...
    def create_ui(self):
        ...
        contrastresetfilter = BubbleTipFilter("Reset contrast", self)
        ...
        sumfilter = BubbleTipFilter("Show the maximum projection (shortcut: m)", self)
        ...
        reffilter = BubbleTipFilter("Show the reference image", self)
        ...
        switch_filter = BubbleTipFilter("Toggle between picking spots and drawing kymographs", self)
        ...
        self.topRightSplitter.setCollapsible(1, False)
    ...
    def eventFilter(self, obj, event):
        ...
        # hover tip on the histogram canvas
        if obj is getattr(self, "histogramCanvas", None):
            if event.type() == QEvent.Enter:
                tip_w = self.histogramTip.sizeHint().width()
                tip_h = self.histogramTip.sizeHint().height()
                cw, ch = self.histogramCanvas.width(), self.histogramCanvas.height()
                self.histogramTip.move((cw - tip_w)//2, (ch - tip_h)//2)
                self.histogramTip.setVisible(True)
                return False
            elif event.type() == QEvent.Leave:
                self.histogramTip.setVisible(False)
                return False
