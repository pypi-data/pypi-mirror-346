...
class BubbleTip(QWidget):
    def __init__(self, text: str, parent=None):
        ...
        self.text     = text
        self.padding  = 8
        self.triangle = 10   # store it here!

        fm = QFontMetrics(self.font())
        tw = fm.horizontalAdvance(text)
        th = fm.height()

        # body sits to the right of the triangle
        self.body = QRectF(self.triangle, 0,
                           tw + 2*self.padding,
                           th + 2*self.padding)

        total_w = int(self.triangle + self.body.width())
        total_h = int(self.body.height())
    ...
    def paintEvent(self, event):
        ...
        rect_path.addRoundedRect(self.body, 8, 8)
        ...
        pts = [
            QPointF(0,               mid_y),
            QPointF(self.body.left(), mid_y - t/2),
            QPointF(self.body.left(), mid_y + t/2),
        ]
        tri_path = QPainterPath()
        tri_path.addPolygon(QPolygonF(pts))
        tri_path.closeSubpath()

        # 3) merge both
        shape = rect_path.united(tri_path)
        ...
        # 5) text
        p.setPen(Qt.black)
        inner = self.body.adjusted(self.padding,
                                   self.padding,
                                   -self.padding,
                                   -self.padding)
...
class BubbleTipFilter(QObject):
    ...
    def _showBubble(self):
        # only if still on the button
        if not self._wobj:
            return
        gp = QCursor.pos()
        btn_tl   = self._wobj.mapToGlobal(QPoint(0,0))
        btn_rect = QRect(btn_tl, self._wobj.size())
        if not btn_rect.contains(gp):
            return

        # create & position bubble
        b = BubbleTip(self.text, parent=self._wobj.window())
        btn_tr = self._wobj.mapToGlobal(self._wobj.rect().topRight())
        x = btn_tr.x()
        y = btn_tr.y() + (self._wobj.height() - b.height())//2
        b.move(x, y)
        b.show()
...
class CenteredBubble(QWidget):
    def __init__(self, text: str, parent=None, pen_width=1):
        ...
        # compute the body size (text + padding)
        body_w = tw + 2*self.padding
        body_h = th + 2*self.padding

        # total widget size = body + pen_width on *both* sides
        total_w = int(body_w + self.pen_width)
        total_h = int(body_h + self.pen_width)
        self.resize(total_w, total_h)

        # set the rectangle where we'll draw the rounded‐rect
        # inset by half the pen so stroke is fully inside
    ...
    def paintEvent(self, event):
        ...
        # draw text
        p.setPen(Qt.black)
        inner = self.rect_body.adjusted(self.padding,
                                        self.padding,
                                        -self.padding,
                                        -self.padding)
        p.drawText(inner, Qt.AlignCenter, self.text)
...
class CenteredBubbleFilter(QObject):
    def __init__(self, text: str, parent=None, delay_ms=1000, visible_ms=3000):
        super().__init__(parent)
        self.text       = text
        self.bubble     = None
        self.target     = None
        ...
        self.showTimer.setSingleShot(True)
        self.showTimer.setInterval(delay_ms)
        self.showTimer.timeout.connect(self._show)

        # Auto-hide after visible_ms
        self.hideTimer  = QTimer(self)
        self.hideTimer.setSingleShot(True)
        self.hideTimer.setInterval(visible_ms)
    ...
    def eventFilter(self, obj, ev):
        # Mouse enters the target → start the showTimer
        if ev.type() == QEvent.Enter and obj is self.target_or_none(obj):
            self.target = obj
            self.showTimer.start()
        # Mouse leaves the target → cancel everything, hide immediately
        elif ev.type() == QEvent.Leave and obj is self.target:
            self.showTimer.stop()
            self.hideTimer.stop()
            self._hide()
            self.target = None
    ...
    def target_or_none(self, obj):
        # Only consider widgets we explicitly attached to
        return obj if hasattr(obj, 'installEventFilter') else None

    def _show(self):
        if not self.target:
            return
        # make sure mouse is still on the canvas
        gp = QCursor.pos()
        tgt_rect = QRect(self.target.mapToGlobal(QPoint(0,0)),
                        self.target.size())
        if not tgt_rect.contains(gp):
            return

        # if we didn’t build it yet, make it a *child* of the canvas
        if not self.bubble:
            self.bubble = CenteredBubble(self.text,
                                        parent=self.target)
        # position and raise it
