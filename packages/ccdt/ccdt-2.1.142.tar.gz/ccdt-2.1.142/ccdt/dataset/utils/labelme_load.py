...
class LabelmeLoad(object):
    ...
    async def recursive_walk(self, root_dir: str, args) -> List[str]:  # 增加函数注解，函数的返回值类型被指定为List[dict]，表示返回值是一个字典列表。
        ...
        if args == "create_dir" or args == "move":
            ...
            semaphore = asyncio.Semaphore(1000)  # 限制并发数为1000，根据实际情况调整
            # 增加文件句柄限制调整，是asyncio.run(adjust_file_handle_limit())的时候就要进行处理
            # asyncio.run(adjust_file_handle_limit())在程序启动时调用：
            # async def adjust_file_handle_limit():
            #     import resource
            #     try:
            #         soft_limit, hard_limit = resource.getrlimit(resource.RLIMIT_NOFILE)
            #         print(f"原始文件描述符限制: soft={soft_limit}, hard={hard_limit}")
            #         new_limit = min(hard_limit, 65535)  # 设置合理的上限值
            #         resource.setrlimit(resource.RLIMIT_NOFILE, (new_limit, hard_limit))
            #         print(f"文件描述符限制已调整: soft={new_limit}, hard={hard_limit}")
            #     except Exception as e:
            #         print(f"修改文件描述符限制失败: {e}")
            ...
            # 分批异步任务调度
            print("分批异步任务调度开始")
            tasks = await self.process_files_in_batches(file_paths, root_dir, semaphore)
            ...
            all_images_file_path = await self.show_progress(tasks, desc="异步处理文件")
    ...
    async def process_files_in_batches(self, file_paths, root_dir, batch_size):
        ...
        for i in range(0, len(file_paths), batch_size):
            ...
            tasks = [self.process_file_with_semaphore(file_path, root_dir, batch_size) for file_path in batch]
    ...
    @staticmethod
    async def show_progress(tasks, desc="进度", unit="file"):
        """
        使用异步进度条，避免阻塞
        @param tasks:
        @param desc:
