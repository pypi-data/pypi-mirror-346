...
from deepcodec.video.frame import PictureType
...
class TestCodecContext(TestCase):
    ...
    def test_codec_delay(self) -> None:
        with deepcodec.open(fate_suite("mkv/codec_delay_opus.mkv")) as container:
            assert container.streams.audio[0].codec_context.delay == 312
        with deepcodec.open(fate_suite("h264/interlaced_crop.mp4")) as container:
    ...
    def test_decoder_extradata(self):
        ctx = deepcodec.codec.Codec("h264", "r").create()
    ...
    def test_decoder_gop_size(self) -> None:
        ctx = deepcodec.codec.Codec("h264", "r").create("video")
    ...
    def test_encoder_extradata(self) -> None:
        ctx = deepcodec.codec.Codec("h264", "w").create()
        assert ctx.extradata is None
        assert ctx.extradata_size == 0
    ...
    def test_encoder_pix_fmt(self) -> None:
        ctx = deepcodec.codec.Codec("h264", "w").create("video")
    ...
    def test_bits_per_coded_sample(self):
        with deepcodec.open(fate_suite("qtrle/aletrek-rle.mov")) as container:
            ...
            for packet in container.demux(stream):
                for frame in packet.decode():
                    pass
                assert isinstance(packet.stream, deepcodec.VideoStream)
        ...
        with deepcodec.open(fate_suite("qtrle/aletrek-rle.mov")) as container:
            ...
            with pytest.raises(deepcodec.error.InvalidDataError):
...
class TestEncoding(TestCase):
    ...
    def image_sequence_encode(self, codec_name: str) -> None:
        ...
        ctx = deepcodec.Codec(codec_name, "r").create("video")
    ...
    def video_encoding(
        ...
        ctx = deepcodec.Codec(dec_codec_name, "r").create("video")
        ctx.open()
