...
def make_h264_test_video(path: str) -> None:
    ...
    output_container = deepcodec.open(path, "w")
    ...
    for _ in range(24):
        frame = deepcodec.VideoFrame.from_ndarray(
...
class TestDecode(TestCase):
    ...
    def test_decode_audio_sample_count(self) -> None:
        ...
        assert isinstance(audio_stream, deepcodec.AudioStream)
    ...
    def test_decoded_time_base(self) -> None:
        ...
        for packet in container.demux(stream):
            for frame in packet.decode():
                assert not isinstance(frame, deepcodec.subtitles.subtitle.SubtitleSet)
    ...
    def test_side_data(self) -> None:
        container = deepcodec.open(fate_suite("mov/displaymatrix.mov"))
        frame = next(container.decode(video=0))
        assert frame.rotation == -90

    def test_hardware_decode(self) -> None:
        hwdevices_available = deepcodec.codec.hwaccel.hwdevices_available()
        ...
        hwaccel = deepcodec.codec.hwaccel.HWAccel(
            device_type=HWACCEL_DEVICE_TYPE, allow_software_fallback=False
        )

        container = deepcodec.open(test_video_path, hwaccel=hwaccel)
        video_stream = container.streams.video[0]
