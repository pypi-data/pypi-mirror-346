...
- The `--sample` must include the bremsstrahlung category in the end. Those are `0brem`, `1brem_tag`, `1brem_probe` and `2brem`. In the future, we plan to join these four categories into two, because the relevant change is whether the probe electron has brem or not. Block 1, 5 and 7 available.
