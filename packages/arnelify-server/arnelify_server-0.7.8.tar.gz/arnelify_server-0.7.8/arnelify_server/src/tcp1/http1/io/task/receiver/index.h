#ifndef ARNELIFY_SERVER_HTTP1_RECEIVER_H
#define ARNELIFY_SERVER_HTTP1_RECEIVER_H
...
class Http1Receiver final {
 private:
  ...
  int setFilename(const std::string& value) {
    ...
    if (this->opts.HTTP1_KEEP_EXTENSIONS) {
      const std::string file = oss.str() + this->fileExt;
      this->filePath = this->opts.HTTP1_UPLOAD_DIR / file;
      return this->SIGNAL_FINISH;
    }

    this->filePath = this->opts.HTTP1_UPLOAD_DIR / oss.str();
    return this->SIGNAL_FINISH;
  }
  ...
  int setHeader(const std::string& key, const std::string& value) {
    ...
    const bool isAcceptEncoding =
        key == "Accept-Encoding" || key == "accept-encoding";
    if (isAcceptEncoding) {
      this->acceptEncoding = value;
      return this->SIGNAL_FINISH;
    }

    const bool isContentLength =
    ...
    const bool isContentType = key == "Content-Type" || key == "content-type";
    if (isContentType) {
      if (this->hasBody) {
    ...
    const bool isCookie = key == "Cookie" || key == "cookie";
    if (isCookie) {
    ...
    while (metaEnd != std::string::npos) {
      ...
      if (hasContentDisposition) {
        ...
        const bool isMaxFields =
            this->fields.size() > this->opts.HTTP1_MAX_FIELDS;
        if (isMaxFields) {
        ...
        if (hasFilenameStart) {
          ...
          if (hasFile) {
            ...
            const bool isMaxFiles =
                this->filesCounter > this->opts.HTTP1_MAX_FILES;
            if (isMaxFiles) {
      ...
      if (hasBefore) {
        ...
        if (this->isWrite) {
          const bool isAllowEmptyFiles =
              !this->opts.HTTP1_ALLOW_EMPTY_FILES && !beforeLen;
          ...
          const bool isMaxFileSize =
              this->fileSize > this->opts.HTTP1_MAX_FILE_SIZE_MB * 1024 * 1024;
          if (isMaxFileSize) {
          ...
          const bool isMaxFilesSizeTotal =
              this->filesSizeTotal >
              this->opts.HTTP1_MAX_FILES_SIZE_TOTAL_MB * 1024 * 1024;
          ...
          const bool isMaxFieldsSizeTotal =
              ...
              this->opts.HTTP1_MAX_FIELDS_SIZE_TOTAL_MB * 1024 * 1024;
 ...
 public:
  Http1Receiver(Http1ReceiverOpts& o)
      : SIGNAL_ACCEPTED(0),
        SIGNAL_ERROR(1),
        SIGNAL_FINISH(2),
