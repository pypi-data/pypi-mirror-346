import re
from eBonsParser.models import *
from eBonsParser.utils import extract_text_from_pdf

class Rewe(Store):
    """
    Represents a Rewe store.
    """
    ...
    def _uid_extract(self, raw_text: str) -> str:
        ...
        """
        pass
        uid_match = re.search(r"^(UID\s?Nr\.?:?|UID:?.?\s?)\s*(.*)", raw_text, re.MULTILINE)
        if uid_match:
            return uid_match.group(2).strip()
        else:
    ...
    def _sum_extract(self, raw_text: str) -> float:
        ...
        """
        sum_match = re.search(r"^(SUMME\s?EUR)\s*(.*)", raw_text, re.MULTILINE)
        if sum_match:
            normalized_line = re.sub(r"(\d+),(\d{2})", r"\1.\2", sum_match.group(2)).strip()
    ...
    def _payment_extract(self, raw_text: str) -> PaymentMethod:
        ...
        """
        payment_match = re.search(r"^(Geg\.?\s*)([\w\-]+)\s*EUR\s?(.*)", raw_text, re.MULTILINE)
        if payment_match:
    ...
    def _bonNr_extract(self, raw_text: str) -> float:
        ...
        """
        ebon_match = re.search(r"\bBon-Nr\.?:\s*(\d{1,10})\b", raw_text)
        if ebon_match:
