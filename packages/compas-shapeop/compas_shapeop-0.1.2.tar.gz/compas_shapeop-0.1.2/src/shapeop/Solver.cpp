  ...
  SHAPEOP_OMP_PARALLEL
  {
    if (dynamic_) {
      SHAPEOP_OMP_FOR for (int i = 0; i < static_cast<int>(momentum_.cols()); ++i) {
        ...
        for (int j = 0; j < static_cast<int>(forces_.size()); ++j) momentum_.col(i) += (delta_ * delta_) * forces_[j]->get(points_, i); 
  ...
  for (unsigned int it = 0; it < iteration; ++it) {
    SHAPEOP_OMP_PARALLEL
    {
      ...
      SHAPEOP_OMP_FOR for (int i = 0; i < 3; ++i)
        points_.row(i) = solver_->solve(At_ * projections_.row(i).transpose() + M_ * momentum_.row(i).transpose(), points_.row(i).transpose()).transpose(); 
    }
  }
  if (dynamic_) {
    SHAPEOP_OMP_PARALLEL
