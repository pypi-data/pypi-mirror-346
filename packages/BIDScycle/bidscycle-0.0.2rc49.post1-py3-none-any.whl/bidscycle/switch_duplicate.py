#!/usr/bin/env python3
"""
bidscycle.switch_duplicates
===========================

Rename every __dub file that matches a set of BIDS‑entity filters and the duplicate number with the correct BIDS structure

 • Matching side‑car *.json* files are renamed as well
...
__all__ = ["switch_duplicate"]

# --------------------------------------------------------------------------- #
# helpers                                                                     #
# --------------------------------------------------------------------------- #
# Custom BIDS layout configuration to handle duplicates
custom_cfg = {
    "name": "bids-with-dup",   # unique ID
    "entities": [
        {   # duplicate index, e.g. _dup-01
            "name":   "dup",
            "pattern": r"_dup-(?P<dup>[0-9]+)",
            "dtype":  "str",
        },
    ],
...
def _dedup_path(src: Path) -> Path:
    """
    Return the original file name with the __dup-XX part removed.
    For example: sub-01_ses-1a_T1w__dup-01.nii.gz -> sub-01_ses-1a_T1w.nii.gz
    """
    full_ext = "".join(src.suffixes)         # ".nii.gz"
    stem = src.name[: -len(full_ext)]        # strip extension from filename
    new_stem = re.sub(r"__dup-[0-9]+", "", stem)  # remove __dup-XX
    return src.with_name(f"{new_stem}{full_ext}")
...
def switch_duplicate(
    *,
    ...
    filters
        List like ['subject=01', 'run=2,3', 'dup=01'].
    commit_msg
    ...
    """
    layout = BIDSLayout(dataset, config=["bids", custom_cfg], validate=False) #using the custom config to handle duplicates
    ...
    # ------------------ build rename plan ---------------------------------- #
    for f in matches:
        src_nifti = Path(f.path) # define the source nifti file
        session_dir = src_nifti.parents[2] # …/sub-XX/ses-YY

        dst_nifti = _dedup_path(src_nifti) # define the destination nifti file
        rename_pairs.append((src_nifti, dst_nifti)) #save as a tuple

        json_path = _json_sidecar(src_nifti)
        if json_path.exists():
            rename_pairs.append((json_path, _dedup_path(json_path)))
    # ------------------ execute renames ------------------------------------ #
    new_files: List[Path] = []
