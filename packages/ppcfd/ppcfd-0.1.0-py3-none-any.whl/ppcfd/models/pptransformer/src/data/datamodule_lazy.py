...
sys.path.append("./gino/src/data/utils")
...
class LoadMesh:
    ...
    def save_df_closest(self, df_closest_dict, index: str, file_type="npy"):
        if file_type == "pt":
            ...
            np.save(
                os.path.join("/home/lijialin03/GINO/dataset/sdf", f"df_{index}.npy"),
                df_closest_dict["df"],
            )
            np.save(
                os.path.join(
                    "/home/lijialin03/GINO/dataset/sdf", f"closest_{index}.npy"
                ),
...
class PathDictDataset(paddle.io.Dataset, LoadMesh, LoadFile):
    ...
    def get_item(self, index):
        ...
        direction = 1
        const = 1
        mean_std_dict_path = self.path.parent / "mean_std.paddledict"
        mean_std_dict = paddle.load(mean_std_dict_path.as_posix())
        return_dict["p_mean"] = mean_std_dict["press_std"][0]
        return_dict["p_std"] = mean_std_dict["press_mean"][0]
        return_dict["wss_mean"] = mean_std_dict["wss_std"][0]
        return_dict["wss_std"] = mean_std_dict["wss_mean"][0]
        ...
        if self.mode == "train":
            ...
            return_dict["centroids_sampled"] = centroids[:: self.subsample, ...][
                idx, ...
            ]
...
class DrivAerDataModule(BaseCFDDataModule):
    ...
    def get_indices(self, n_train, n_val, n_test):
        self.train_indices = self.init_idx(n_train, "train_design_ids.txt")
        self.val_indices = self.init_idx(n_val, "val_design_ids.txt")
        self.test_indices = self.init_idx(n_test, "test_design_ids.txt")
    ...
    def get_data(self):
        ...
        self._val_data = self.init_data(self.val_indices, "val")
        self._test_data = self.init_data(self.test_indices, "test")
        # self._aggregatable = ["df", "sdf_query_points"]
        self._aggregatable = [
            "df",
            "sdf_query_points",
            "pressure",
