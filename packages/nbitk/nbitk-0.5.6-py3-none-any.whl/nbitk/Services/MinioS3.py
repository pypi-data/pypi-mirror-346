...
def compute_md5(file_path_or_iobase: Union[str, io.IOBase], quiet: bool = False) -> str:
    """
    ...
    if isinstance(file_path_or_iobase, str):
...
class MinioS3:
    ...
    def download_file(
        ...
        try:
            with open(output_file, "wb") as fw:
                fw.write(self.get_object(bucket_name, object_name, quiet=quiet))
        except S3Error:
    ...
    def put_object(
        ...
        # compute hash, to compare with object hash computed by the server.
        ...
        if md5sum != result.etag:
            # the file is copy but the content is diff from the source file
            self.client.remove_object(bucket_name, object_name)
            raise Md5sumValidationError(f"The MD5sum calculated by the server did not match"
                                        f" the MD5sum of object '{object_name}'."
    ...
    def copy_object(
        ...
        md5sum = self.stat_object(source_bucket_name, source_object_name).etag
        # copy only if the source object matches the etag/md5sum
        copy_source = CopySource(source_bucket_name, source_object_name, match_etag=md5sum)
        ...
        if md5sum != result.etag:
            ...
            raise Md5sumValidationError(f"The MD5sum calculated by the server did not match"
                                        f" the MD5sum of object '{source_object_name}. "
