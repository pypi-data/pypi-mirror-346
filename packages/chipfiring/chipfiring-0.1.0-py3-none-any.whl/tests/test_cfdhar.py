...
class TestDharAlgorithm:
    ...
    def test_init_invalid_q(self, simple_graph):
        ...
        config = CFDivisor(simple_graph, [("A", 2), ("B", 1), ("C", 0), ("D", 1)])
        with pytest.raises(ValueError, match="Distinguished vertex E not found in graph"):
    ...
    def test_run_with_debt(self, simple_graph):
        ...
        assert all(dhar.configuration.get_degree(v.name) >= 0 for v in dhar.configuration.degrees.keys())
    ...
    def test_debt_concentration_with_bob_as_q(self, sequence_test_graph):
        ...
        divisor = CFDivisor(sequence_test_graph, [
            ("Alice", 2), 
            ("Bob", -3), 
            ("Charlie", 4), 
            ("Elise", -1)
