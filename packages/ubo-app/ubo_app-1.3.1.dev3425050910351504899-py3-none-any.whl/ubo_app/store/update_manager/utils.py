...
async def check_version(beta_versions: bool) -> None:  # noqa: FBT001
    ...
    try:
        async with (
            ...
            if beta_versions:
                recent_versions: list[str] = data['releases'].keys()
                ...
                recent_versions = [
                    version for version in data['releases'] if '.dev' not in version
                ]
                recent_versions = sorted(
                    recent_versions,
                    key=lambda x: data['releases'][x][0]['upload_time'],
...
Then another reboot will be done to complete the update process.""",
    ...
    async def download_files() -> None:
        ...
        process = await asyncio.create_subprocess_exec(
            ...
            'curl',
            '-Lk',
            f'https://files.pythonhosted.org/packages/source/u/ubo_app/ubo_app-{target_version}.tar.gz',
            '--output',
        ...
        with tarfile.open(
            ...
            members = [m for m in tar.getmembers() if m.name == 'install.sh']
            if not members:
                msg = 'Failed to extract install.sh'
                raise RuntimeError(msg)
            tar.extract(members[0], path=UPDATE_ASSETS_PATH)
        ...
        process = await asyncio.create_subprocess_exec(
            ...
            'setuptools',
            'wheel',
            'ubo-app' if target_version is None else f'ubo-app=={target_version}',
    ...
    recent_versions_item = SubMenuItem(
        ...
        sub_menu=HeadlessMenu(
            ...
            items=[
                DispatchItem(
                    label=version,
                    store_action=UpdateManagerRequestUpdateAction(version=version),
                    icon='󰜉',
                )
