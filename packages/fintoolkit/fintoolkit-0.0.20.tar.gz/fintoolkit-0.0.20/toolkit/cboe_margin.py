    ...
    **kwargs,
    ...
    if quantity > 0:
        if otc and expiration > 9 / 12:
            intrinsic = itm(
                otype=otype, strike=strike, underlying=underlying, reduced=reduced
            )
        ...
        return (
            ...
            * np.maximum(
                ...
                value
                + 0.1 * leverage * (underlying * reduced if otype == "c" else strike),
            )
        )
...
def max_loss(portfolio):
    ...
    return -min(
        0,
        portfolio.apply(
            lambda row: row.quantity * row.multiplier * itm(**row, underlying=strikes),
            axis=1,
        )
        .sum()
        .min(),
...
def margin_strategy(portfolio, underlying=0, eligible=False):
    ...
    if (
        eligible
        and len(s) == 4
        and s.expiration.nunique() == 1
        and s.strike.nunique() == 2
        and s.iloc[0].quantity == -s.iloc[1].quantity
        and s.iloc[0].strike == s.iloc[1].strike
        and s.iloc[0].otype != s.iloc[1].otype
        and s.iloc[2].quantity == -s.iloc[3].quantity
        and s.iloc[2].strike == s.iloc[3].strike
        and s.iloc[2].otype != s.iloc[3].otype
    ):
        ...
        if len(long) == 1 and len(short) == 0:
            ...
            margin_value = (
                long.iloc[0].quantity
                * long.iloc[0].multiplier
                * np.minimum(
                    (0.1 if long.iloc[0].otype == "p" else 1.1) * long.iloc[0].strike
                    + otm(**long.iloc[0], underlying=underlying),
                    (0.25 if long.iloc[0].otype == "p" else 1.3) * underlying,
                )
            )
            ...
            if long.iloc[0].strike == short.iloc[0].strike:
                margin_value = (
                    ...
                    * long.iloc[0].multiplier
                    * (
                        (0.1 if under.iloc[0].quantity > 0 else 1.1)
                        * long.iloc[0].strike
                        + (
                            itm(**short.iloc[0], underlying=underlying)
                            if short.iloc[0].otype == "p"
                            else 0
                        )
                    )
                )
                ...
                margin_value = (
                    ...
                    * long.iloc[0].multiplier
                    * np.minimum(
                        0.1 * long.iloc[0].strike
                        + otm(**long.iloc[0], underlying=underlying),
                        0.25 * short.iloc[0].strike,
                    )
                )
        else:
        ...
        margin_value = (
            margin_strategy(long)
            + short.apply(
                lambda row: margin(**row, underlying=underlying), axis=1
            ).sum()
        )
    elif (
        len(short) == 2
        and len(long) == 0
        and short.iloc[0].otype != short.iloc[1].otype
    ):
        margins = short.apply(lambda row: margin(**row, underlying=underlying), axis=1)
        smaller = short.iloc[1 - margins.idxmax()]
        margin_value = (
            margins.max() - smaller.quantity * smaller.multiplier * smaller.value
        )
    else:
        # Base Case
        margin_value = (
            max_loss(portfolio) + (long.quantity * long.multiplier * long.value).sum()
        )
