...
def portfolio_return(
    weights: pd.Series | pd.DataFrame, returns: pd.Series | pd.DataFrame
...
def portfolio_volatility(
    weights: pd.Series | pd.DataFrame, cov: pd.DataFrame
...
def risk_contribution(
    ...
    """
    return (weights.T @ cov * weights.T).div(
        portfolio_volatility(weights, cov) ** 2, axis=0
    )
...
def inverse_vol(cov: pd.DataFrame) -> pd.Series:
    ...
    """
    inverse = 1.0 / pd.Series(np.diag(cov), index=cov.index) ** 0.5
...
def max_sharpe(
    ...
    fully_invest = {"type": "eq", "fun": lambda w: np.sum(w) - 1}
    wtg = optimize.minimize(
        neg_sharpe,
        equal_weight(er),
        args=(er, cov, rfr),
        method="SLSQP",
...
def min_vol(cov: pd.DataFrame) -> pd.Series:
    ...
    """
    return max_sharpe(
        pd.Series(np.repeat(1.0, cov.shape[0]), index=cov.index), cov, rfr=0
    )
...
def min_vol_at(
    ...
    """
    fully_invest = {"type": "eq", "fun": lambda w: np.sum(w) - 1}
    target_return = {"type": "eq", "fun": lambda w: portfolio_return(w, er) - target}
    wtg = optimize.minimize(
