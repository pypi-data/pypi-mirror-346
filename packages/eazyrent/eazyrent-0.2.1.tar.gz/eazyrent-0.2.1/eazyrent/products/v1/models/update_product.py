...
UPDATEPRODUCT_ANY_OF_SCHEMAS = ["UpdateForRentProduct", "UpdateForSaleProduct"]
...
class UpdateProduct(BaseModel):
    ...
    # data type: UpdateForRentProduct
    anyof_schema_1_validator: Optional[UpdateForRentProduct] = None
    # data type: UpdateForSaleProduct
    anyof_schema_2_validator: Optional[UpdateForSaleProduct] = None
    if TYPE_CHECKING:
        actual_instance: Optional[
            Union[UpdateForRentProduct, UpdateForSaleProduct]
        ] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"UpdateForRentProduct", "UpdateForSaleProduct"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = UpdateProduct.model_construct()
        error_messages = []
        # validate data type: UpdateForRentProduct
        if not isinstance(v, UpdateForRentProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `UpdateForRentProduct`"
            )
        else:
            return v

        # validate data type: UpdateForSaleProduct
        if not isinstance(v, UpdateForSaleProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `UpdateForSaleProduct`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in UpdateProduct with anyOf schemas: UpdateForRentProduct, UpdateForSaleProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[UpdateForRentProduct] = None
        try:
            instance.actual_instance = UpdateForRentProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[UpdateForSaleProduct] = None
        try:
            instance.actual_instance = UpdateForSaleProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into UpdateProduct with anyOf schemas: UpdateForRentProduct, UpdateForSaleProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], UpdateForRentProduct, UpdateForSaleProduct]]:
