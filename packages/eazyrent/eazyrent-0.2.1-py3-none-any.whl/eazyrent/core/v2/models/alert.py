...
class Alert(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["code", "status", "comment", "alert_type"]

    @field_validator("status")
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["SUCCESS", "WARNING", "ERROR", "NEUTRAL"]):
            raise ValueError(
                "must be one of enum values ('SUCCESS', 'WARNING', 'ERROR', 'NEUTRAL')"
            )
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of alert_type
        if self.alert_type:
            _dict["alert_type"] = self.alert_type.to_dict()
        ...
        _obj = cls.model_validate(
            {
                ...
                "alert_type": BaseAlertType.from_dict(obj["alert_type"])
                if obj.get("alert_type") is not None
                else None,
            }
        )
        return _obj
