...
PAGINATEDRESPONSEUNIONLISTFORRENTPRODUCTLISTFORSALESPRODUCTRESULTSINNER_ANY_OF_SCHEMAS = [
    "ListForRentProduct",
    "ListForSalesProduct",
]


class PaginatedResponseUnionListForRentProductListForSalesProductResultsInner(
    BaseModel
):
    """
    ...
    # data type: ListForRentProduct
    anyof_schema_1_validator: Optional[ListForRentProduct] = None
    # data type: ListForSalesProduct
    anyof_schema_2_validator: Optional[ListForSalesProduct] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ListForRentProduct, ListForSalesProduct]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ListForRentProduct", "ListForSalesProduct"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = (
            PaginatedResponseUnionListForRentProductListForSalesProductResultsInner.model_construct()
        )
        error_messages = []
        # validate data type: ListForRentProduct
        if not isinstance(v, ListForRentProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListForRentProduct`"
            )
        else:
            return v

        # validate data type: ListForSalesProduct
        if not isinstance(v, ListForSalesProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListForSalesProduct`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in PaginatedResponseUnionListForRentProductListForSalesProductResultsInner with anyOf schemas: ListForRentProduct, ListForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ListForRentProduct] = None
        try:
            instance.actual_instance = ListForRentProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ListForSalesProduct] = None
        try:
            instance.actual_instance = ListForSalesProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into PaginatedResponseUnionListForRentProductListForSalesProductResultsInner with anyOf schemas: ListForRentProduct, ListForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ListForRentProduct, ListForSalesProduct]]:
