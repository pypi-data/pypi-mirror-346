...
RESPONSEPARTIALUPDATEPRODUCT_ANY_OF_SCHEMAS = ["ForRentProduct", "ForSalesProduct"]


class ResponsePartialUpdateProduct(BaseModel):
    """
    ResponsePartialUpdateProduct
    """

    # data type: ForRentProduct
    anyof_schema_1_validator: Optional[ForRentProduct] = None
    # data type: ForSalesProduct
    anyof_schema_2_validator: Optional[ForSalesProduct] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ForRentProduct, ForSalesProduct]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ForRentProduct", "ForSalesProduct"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponsePartialUpdateProduct.model_construct()
        error_messages = []
        # validate data type: ForRentProduct
        if not isinstance(v, ForRentProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ForRentProduct`"
            )
        else:
            return v

        # validate data type: ForSalesProduct
        if not isinstance(v, ForSalesProduct):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ForSalesProduct`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ResponsePartialUpdateProduct with anyOf schemas: ForRentProduct, ForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ForRentProduct] = None
        try:
            instance.actual_instance = ForRentProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[ForSalesProduct] = None
        try:
            instance.actual_instance = ForSalesProduct.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ResponsePartialUpdateProduct with anyOf schemas: ForRentProduct, ForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], ForRentProduct, ForSalesProduct]]:
