...
class Applicant(BaseModel):
    ...
    physical_guarantors: Optional[List[Applicant]] = None
    ...
    documents: Optional[List[ListDocument]] = None
    financial_resume: Optional[Any] = None
    form_available: Optional[StrictBool] = Field(
        default=False, description="If applicant has a form associated"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if financial_resume (nullable) is None
        # and model_fields_set contains the field
        if (
            self.financial_resume is None
            and "financial_resume" in self.model_fields_set
        ):
            _dict["financial_resume"] = None
        ...
        _obj = cls.model_validate(
            {
                ...
                "physical_guarantors": [
                    Applicant.from_dict(_item) for _item in obj["physical_guarantors"]
                ]
                ...
                "documents": [
                    ListDocument.from_dict(_item) for _item in obj["documents"]
                ]
                if obj.get("documents") is not None
                else None,
                "financial_resume": obj.get("financial_resume"),
                "form_available": obj.get("form_available")
                if obj.get("form_available") is not None
                else False,
            }
        )
        return _obj
...
Applicant.model_rebuild(raise_errors=False)
