...
class UpdateUserGrants(BaseModel):
    """
    UpdateUserGrants
    """  # noqa: E501

    grants: Optional[List[StrictStr]] = None
    __properties: ClassVar[List[str]] = ["grants"]

    @field_validator("grants")
    def grants_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(["org:admin", "org:staff", "org:user"]):
                raise ValueError(
                    "each list item must be one of ('org:admin', 'org:staff', 'org:user')"
                )
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({"grants": obj.get("grants")})
        return _obj
