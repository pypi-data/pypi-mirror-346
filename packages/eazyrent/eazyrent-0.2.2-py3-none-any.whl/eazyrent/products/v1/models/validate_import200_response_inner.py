...
VALIDATEIMPORT200RESPONSEINNER_ANY_OF_SCHEMAS = [
    "CreateForRentProductOutput",
    "CreateForSaleProductOutput",
]


class ValidateImport200ResponseInner(BaseModel):
    """
    ValidateImport200ResponseInner
    """

    # data type: CreateForSaleProductOutput
    anyof_schema_1_validator: Optional[CreateForSaleProductOutput] = None
    # data type: CreateForRentProductOutput
    anyof_schema_2_validator: Optional[CreateForRentProductOutput] = None
    if TYPE_CHECKING:
        actual_instance: Optional[
            Union[CreateForRentProductOutput, CreateForSaleProductOutput]
        ] = None
    else:
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ValidateImport200ResponseInner.model_construct()
        error_messages = []
        # validate data type: CreateForSaleProductOutput
        if not isinstance(v, CreateForSaleProductOutput):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `CreateForSaleProductOutput`"
            )
        else:
            return v

        # validate data type: CreateForRentProductOutput
        if not isinstance(v, CreateForRentProductOutput):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `CreateForRentProductOutput`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ValidateImport200ResponseInner with anyOf schemas: CreateForRentProductOutput, CreateForSaleProductOutput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[CreateForSaleProductOutput] = None
        try:
            instance.actual_instance = CreateForSaleProductOutput.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[CreateForRentProductOutput] = None
        try:
            instance.actual_instance = CreateForRentProductOutput.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ValidateImport200ResponseInner with anyOf schemas: CreateForRentProductOutput, CreateForSaleProductOutput. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        ...
        Union[Dict[str, Any], CreateForRentProductOutput, CreateForSaleProductOutput]
    ]:
