...
DOCUMENTTYPE_ANY_OF_SCHEMAS = ["ListAcceptedDocument", "str"]
...
class DocumentType(BaseModel):
    ...
    # data type: ListAcceptedDocument
    anyof_schema_1_validator: Optional[ListAcceptedDocument] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ListAcceptedDocument, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = {"ListAcceptedDocument", "str"}
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = DocumentType.model_construct()
        error_messages = []
        # validate data type: ListAcceptedDocument
        if not isinstance(v, ListAcceptedDocument):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `ListAcceptedDocument`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in DocumentType with anyOf schemas: ListAcceptedDocument, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ListAcceptedDocument] = None
        try:
            instance.actual_instance = ListAcceptedDocument.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into DocumentType with anyOf schemas: ListAcceptedDocument, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ListAcceptedDocument, str]]:
