...
RESPONSEGETPRODUCT_ANY_OF_SCHEMAS = ["ForRentProduct", "ForSalesProduct"]


class ResponseGetProduct(BaseModel):
    """
    ResponseGetProduct
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ResponseGetProduct.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in ResponseGetProduct with anyOf schemas: ForRentProduct, ForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ResponseGetProduct with anyOf schemas: ForRentProduct, ForSalesProduct. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
