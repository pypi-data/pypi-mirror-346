...
class PowerShell(ProxyBase):
    ...
    def __new__(cls, obj=None):
        ...
        if obj is None:
            ...
            # streams.Error.DataAdded       += self._stream_output_event(streams.Error)
            streams.Warning.DataAdded     += self._stream_output_event(streams.Warning)
            streams.Verbose.DataAdded     += self._stream_output_event(streams.Verbose)
            streams.Debug.DataAdded       += self._stream_output_event(streams.Debug)
            streams.Information.DataAdded += self._stream_output_event(streams.Information)
            streams.Progress.DataAdded    += self._stream_output_event(streams.Progress)
            ...
            output_collection.DataAdded   += self._stream_output_event(output_collection)
    ...
    def _stream_output_event(self, stream):
        def stream_event(sender: System.Object,
            ...
            for item in sender.ReadAll():
                if isinstance(item, Automation.ErrorRecord):  # NOK !!!
                    ...
                    cprint(message, fore_256="red")
                elif isinstance(item, Automation.WarningRecord):
                    cprint(f"WARNING: {item.Message}", fore_256="light_yellow")
                elif isinstance(item, Automation.VerboseRecord):
                    cprint(f"VERBOSE: {item.Message}", fore_256="light_yellow")
                elif isinstance(item, Automation.DebugRecord):
                    cprint(f"DEBUG: {item.Message}",   fore_256="light_yellow")
                elif isinstance(item, Automation.InformationRecord):
                    cprint(f"{item.MessageData}")
    ...
    def DebugPreference(self):
        return self.Runspace.SessionStateProxy.GetVariable("DebugPreference")
    ...
    def Write_Information(self, Msg, **kwargs):
        ...
        if preference != Automation.ActionPreference.SilentlyContinue:
