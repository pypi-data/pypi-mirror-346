...
def env_get_enum(key: str,
                 ...
                 use_names: Literal["lower", "upper", "plain"] = None,
    ...
    The parameter *use_names* determines whether the lowercase, uppercase, or plain names, or the values,
    of the elements in the *enum_class* should be used (defaults to values). If *values* is specified,
    the value obtained is checked for occurrence therein. On failure, *None* is returned.

    :param key: the key which the value is associated with
    :param enum_class: the *enum* class to consider (must be a subclass of *IntEnum* or *StrEnum*)
    :param use_names: specifies whether *enum*'s names should be used (defaults to *enum*'s values)
    ...
    if use_names:
        ...
        vals: list[str]
        if use_names == "lower":
            val = def_value.name.lower() if def_value else None
            vals = [e.name.lower() for e in (values or enum_class)]
        elif use_names == "upper":
            val = def_value.name.upper() if def_value else None
            vals = [e.name.upper() for e in (values or enum_class)]
        else:
            val = def_value.name if def_value else None
            vals = [e.name for e in (values or enum_class)]
