...
def validate_enum(errors: list[str] | None,
    ...
    the value obtained is checked for occurrence therein.
    ...
    if use_names:
        ...
        vals: list[str]
        if use_names == "lower":
            val = default.name.lower() if default else None
            vals = [e.name.lower() for e in (values or enum_class)]
        elif use_names == "upper":
            val = default.name.upper() if default else None
            vals = [e.name.upper() for e in (values or enum_class)]
        else:
            val = default.name if default else None
