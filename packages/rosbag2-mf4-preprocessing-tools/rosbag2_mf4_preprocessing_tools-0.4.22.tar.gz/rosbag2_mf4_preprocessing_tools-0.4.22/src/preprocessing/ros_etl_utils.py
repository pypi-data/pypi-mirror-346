...
def prepare_transform_arguments(
    config: Dict[str, Any], extracted_results: List[Optional[Dict[str, Any]]]
) -> List[Dict[str, Any]]:
    kwargs_list = []
    ...
    if not isinstance(extracted_results, list):
        logger.error(f"Expected a list of results from extraction, got {type(extracted_results)}.")
        return []
    ...
    for result in extracted_results:
        if (
            ...
            and os.path.exists(result["intermediate_data_path"])
        ):
            kwargs_list.append({"extracted_metadata": result, "config": config})
            successful_extractions += 1
        else:
            input_path = result.get("input_folder_path", "unknown") if isinstance(result, dict) else "unknown"
            status = result.get("status", "missing_status") if isinstance(result, dict) else "unknown_format"
            reason = f"status '{status}' or intermediate file issue"
            if isinstance(result, dict) and result.get("intermediate_data_path") and not os.path.exists(result["intermediate_data_path"]):
                reason = f"intermediate file {result['intermediate_data_path']} does not exist"
            logger.warning(
                f"Skipping transform for extraction result from {input_path}. Reason: {reason}."
            )
            skipped_extractions += 1
    logger.info(
        f"Prepared {len(kwargs_list)} arguments for transform task ({successful_extractions} successful extractions, {skipped_extractions} skipped)."
    )
    return kwargs_list
