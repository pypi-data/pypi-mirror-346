# def hello():
#     print('Hello from qiskit_noise_analysis')
...
class ResourceEstimation:
    ...
    def __init__(self):
        ...
        # default values
        self.default_circuits = [self.generate_grover_circuit]
        self.default_basis_gates = ["id", "rz", "sx", "x", "cx"]   # Note 'reset', 'delay' and 'measure' are always present in transpilation
    ...
    def get_raw_result(self):   # rename due to clash with big function
    ...
    def generate_grover_circuit(self):
        '''
        Create a Grover circuit of size 5
        '''
        return None
    ...
    def demo(self):
        ...
        """
        self.generate_report(self.default_circuits, self.default_basis_gates, self.default_resources)
    ...
    def validate_basis_gates_list(self,  basis_gates):
        """
        Validate basis gates (Implement Later)    
        """
        return True
    
    def validate_resources_list(self, resources):
        """
        Validate resources (Implement Later)    
        """
        return True
    ...
    def prepare_visualization_dict(self, estimation):
        ...
        # Filter out rows where both circ1 and circ2 are zero
    ...
    def get_estimation(self, circuits, basis_gates=None, resource_set=None, circuit_names=None):
        ...
        # initializing the inputs with defaults inputs 
        ...
        if(not self.validate_circuit_list(circuits)):
            raise ValueError("Invalid circuit provided")
        if(not self.validate_basis_gates_list(basis_gates)):
            raise ValueError("Invalid basis gate set provided")
        if(not self.validate_resources_list(resource_set)):
            raise ValueError("Invalid circuit provided")
