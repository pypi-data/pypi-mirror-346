...
# If busybox image is compiled with FEATURE_TUNCTL_UG=y (Support owner:group
# assignment) tunctl sets interface owner even when -u flag is not provided.
# tunctl runs as root in the container and sets interface owner to
# root. If QEMU runs as non-root user it fails to configure the interface:
#
#     could not configure /dev/net/tun (interface): Operation not
#     permitted
#
# To support running QEMU as non-root we set interface owner to the current
# user.
#
# tunctl flag -u requires username so we create user "alpy" in the container

_CREATE_TAP_INTERFACE_SCRIPT = """\
if tunctl --help 2>&1 | grep owner
then
    echo "alpy:x:$OWNER:$OWNER:alpy:/home:/bin/false" >> /etc/passwd &&
        tunctl -t "$INTERFACE" -u alpy
else
    tunctl -t "$INTERFACE"
fi
"""
...
class NodeTap:
    ...
    :type timeout: int
    :param busybox_image: ID or name of busybox Docker image
    ...
    def _run_in_container(self, command, **kwargs):
        container = self._docker_client.containers.create(
            self._busybox_image, command, **kwargs
        )
        try:
            container.start()
    ...
    def _create_interface(self):
        if euid := os.geteuid():
            self._run_in_container(
                ["sh", "-c", _CREATE_TAP_INTERFACE_SCRIPT],
                network_mode="host",
                cap_add=["NET_ADMIN"],
                devices=["/dev/net/tun"],
                environment={
...
def make_numbered_nodes(
    *, busybox_image, docker_client, tap_interfaces, timeout
):
...
def make_skeleton(*, busybox_image, docker_client, tap_interfaces, timeout):
