...
def svm_undersampling_algorithm(x_train, y_train, x_test, y_test,
    ...
    (con colonna 'y720') e la lista dei loro indici originali.
    ...
    free_sv_df["y720"] = y_train.iloc[free_support_vector_indices_list].values
...
def fcca_with_brf_validation(
    ...
    search.fit(x_train_down_fcca, y_train_down_fcca)
    ...
    p1      = fcca_cfg.get("p1", 1)
    lambda1 = fcca_cfg.get("lambda1", 1)
    lambda2 = fcca_cfg.get("lambda2", 0)
    ...
    for lambda0 in lambda0_values:
        for p0 in p0_values:
            ...
            # fit sui downsampled
            x_tr_d, y_tr_d = discretizer.fit_transform(x_train_down_fcca, y_train_down_fcca)
            # trasformazione preliminare del test (senza soglie)
            x_te_d, y_te_d = discretizer.transform(x_test_fcca, y_test_fcca)
            ...
            for tao_q in tao_q_values:
                ...
                x_tr_q, y_tr_q = discretizer.transform(x_train_down_fcca, y_train_down_fcca, thresholds)
                x_te_q, y_te_q = discretizer.transform(x_test_fcca,       y_test_fcca,       thresholds)
                ...
                cr = discretizer.compression_rate(x_test_fcca, y_test_fcca, thresholds)
                ir = discretizer.inconsistency_rate(x_test_fcca, y_test_fcca, thresholds)
...
def plot_fcca_results():
    ...
    """
    main_folder = "FCCA_results"
    folder_pattern = re.compile(r"lambda0_([\d\.]+)_Q_([\d\.]+)_p0_([\d\.]+)")
    ...
    for folder_name in os.listdir(main_folder):
        ...
        lambda0_val, tau_q_val, p0_val = map(float, m.groups())
        ...
        # --- BRF performance ---
        brf_path = os.path.join(folder_path, "BRFC_Performance.xlsx")
...
def _tree_to_dot(node, node_id=0, dot_lines=None):
    ...
    if "prediction" in node:
        ...
        true_id = next_id
        dot_lines, next_id = tree_to_dot_enhanced(node["true"], true_id, dot_lines)
        dot_lines.append(f'  node{current_id} -> node{true_id} [label="True"];')

        # Processa il ramo "false"
        false_id = next_id
        dot_lines, next_id = tree_to_dot_enhanced(node["false"], false_id, dot_lines)
...
def run_gosdt():
    ...
    out = cfg.get("output_dir", "logs/gosdt")
    ...
    params = {k:v for k,v in cfg.items() if k not in ("enabled","input_dir","output_dir")}
    ...
    dot_data_stylish_train = generate_stylish_dot(enhanced_tree_train, x_train.columns.tolist())
    ...
    enhanced_tree_test = add_counts_to_tree(tree0, x_test, y_test, x_test.columns.tolist())
    dot_data_stylish_test = generate_stylish_dot(enhanced_tree_test, x_test.columns.tolist())
...
def run_pipeline(config_path: str = None):
    ...
    if ds_cfg.get("enabled", False):
        free_sv_df, free_sv_idxs = svm_undersampling_algorithm(
            ...
            feat_cols, class_weights, ds_cfg
        )
        ...
        X_train_downsample = free_sv_df.drop(columns=['y720'])
        y_train_downsample = free_sv_df['y720']
    ...
    # dopo la BRF di validazione undersampling
