...
class ClustFCC(BiobbObject):
    ...
    Args:
        ...
        output_cluster_zip_path (str): Path to the output PDB file collection in zip format. File type: output. `Sample file <https://raw.githubusercontent.com/bioexcel/biobb_haddock/master/biobb_haddock/test/reference/haddock/ref_clustfcc.zip>`_. Accepted formats: zip (edam:format_3987).
        output_haddock_wf_data_zip (str) (Optional): Path to the output zipball containing all the current Haddock workflow data. File type: output. `Sample file <https://github.com/bioexcel/biobb_haddock/raw/master/biobb_haddock/test/data/haddock/haddock_wf_data_clustfcc.zip>`_. Accepted formats: zip (edam:format_3987).
    ...
    @launchlogger
    def launch(self) -> int:
        """Execute the :class:`ClustFCC <biobb_haddock.haddock.clust_fcc>` object."""
        # tmp_files = []
        ...
        if self.container_path:
            ...
            shutil.copy2(
                self.output_cfg_path, str(self.stage_io_dict.get("unique_dir", ""))
            )
