...
class BaseAgent:
    ...
    async def _update_state(self, current_state: State, parsed_response: Dict, node: str) -> State:
        ...
        if tool_name and tool_name not in ["None", None]:
            ...
            if self.logger:
                self.logger.info("-------------------------------------Tool Input Parsed---------------------------------")
                self.logger.info(f"Tool: {current_state['current_tool']}, Input: {current_state['tool_input']}")
        else:
    ...
    async def execute_tool(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if has_ainvoke:
                if is_underlying_async:
                    ...
                    if self.logger: self.logger.info(f"Choosing ainvoke for '{tool_name}' (has_ainvoke=True, is_underlying_async=True).")
                elif not has_func:
                    # Case 2: Cannot inspect .func, but ainvoke exists.
                    # TRUST ainvoke as the intended async path (handles StructuredTool etc.)
                    should_use_ainvoke = True
                    if self.logger: self.logger.info(f"Choosing ainvoke for '{tool_name}' (has_ainvoke=True, has_func=False). Trusting ainvoke.")
            ...
            if should_use_ainvoke:
                ...
                if self.logger: self.logger.info(f"Executing '{tool_name}' using tool.ainvoke.")
        ...
        if hasattr(tool, 'return_direct') and tool.return_direct and state.get('satisfied', True):
             if self.logger: self.logger.info(f"Tool '{tool_name}' requested return_direct. Setting final answer.")
             state.update({
