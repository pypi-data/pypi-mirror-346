...
class Agent(BaseAgent): # Inherit from the modified BaseAgent
    def __init__(self, agent_name: str, llm_router: BaseGenerativeModel, llm_evaluator: BaseGenerativeModel,
        ...
        self.MAX_TOOL_LOOP = kwargs.get('max_tool_loop', 3)
    ...
    async def node_handler(self, state: State, llm: BaseGenerativeModel, prompt: str, component_context: Optional[List] = None, node: Optional[str] = None) -> State:
        ...
        try:
            ...
            if self.logger:
                ...
                self.logger.info(f"Node {node_name} LLM response answer (truncated): {str(log_answer)[:200]}...")
    ...
    async def checkroutingcondition(self, state: State) -> Literal["continue", "end", "reflection"]:
        ...
        MAX_REFLECTIONS = 3 # Define max reflections
    ...
    async def initiate_agent_astream(self, query: str, passed_from: Optional[str] = None) -> AsyncGenerator[Dict, None]:
        ...
        try:
            ...
            async for state in self.app.astream(initial_state, config=config, stream_mode=["updates"]):
                yield state
