...
async def get_abilities(cwd, bot_setting):
    tool_call = bot_setting["toolCall"]
    abilities = []
    mcp_servers = []
    
    item_list = tool_call["botConfig"]
    for item in item_list:
        item = ref_bot(item["address"], item["config"])
        if item:
            abilities.append(item)
            
    item_list = tool_call["mcpConfig"]
    for item in item_list:
        item = ref_mcp(item["address"], item_list["config"], cwd)
        if item:
            mcp_server = await item.__aenter__()
            mcp_servers.append(mcp_server)

    item_list = tool_call["toolConfig"]
    for item in item_list:
        items = ref_tools(item["address"], item_list["config"], cwd)
        abilities.extend(items)
    
    item_list = tool_call["workflowConfig"]
    for item in item_list:
        item = ref_workflow(item["address"], item_list["config"])
        if item:
            abilities.append(item)
