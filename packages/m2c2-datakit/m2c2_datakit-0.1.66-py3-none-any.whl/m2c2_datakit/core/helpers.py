...
def summarize(df: pd.DataFrame, grouping, summarization_func, **kwargs) -> pd.DataFrame:
    validate_input(df)
    summary_df = df.groupby(grouping).apply(summarization_func, **kwargs).reset_index()
