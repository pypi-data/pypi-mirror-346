...
class Body(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        error_messages = []
        # validate data type: AppLinkStateChangedWebhook
        if not isinstance(v, AppLinkStateChangedWebhook):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `AppLinkStateChangedWebhook`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in Body with anyOf schemas: AppLinkStateChangedWebhook, ProductStateChangedWebhook, TransactionUpdatesAvailableWebhook. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[AppLinkStateChangedWebhook] = None
        try:
            instance.actual_instance = AppLinkStateChangedWebhook.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Body with anyOf schemas: AppLinkStateChangedWebhook, ProductStateChangedWebhook, TransactionUpdatesAvailableWebhook. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
