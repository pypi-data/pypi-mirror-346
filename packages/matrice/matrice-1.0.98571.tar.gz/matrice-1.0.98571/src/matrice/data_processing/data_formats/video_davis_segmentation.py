...
def extract_objects_from_mask(mask_path):
    ...
    for path in dataset_paths:
        if path.endswith(".png"):
            ...
            (
                ...
                video_height,
                video_width,
            ) = extract_objects_from_mask(path)
            if video_name not in complete_videos:
                ...
                complete_videos[video_name] = {
                    ...
                    "splitType": ("multiple" if has_multiple_splits else split),
                    "frame_rate": 30,
...
def add_davis_dataset_items_details(batch_dataset_items, frames_details):
    ...
    processed_batch = []
    logging.debug(
        "batch_dataset_items in add davis dataset items details: %s",
        batch_dataset_items,
    )
    for dataset_item in batch_dataset_items:
        if not dataset_item.get("fileInfoResponse", {}).get("frames"):
            continue
