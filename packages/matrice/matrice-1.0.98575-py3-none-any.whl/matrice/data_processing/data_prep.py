...
def get_image_annotations(
    ...
    logging.info("Version of dataset %s", dataset_version)
    logging.info("Dataset item %s", dataset_item)
    for info in dataset_item["versionInfo"]:
        if info["version"] == dataset_version:
...
def write_yolo_labels(
    ...
    for image in dataset_items:
        ...
        if not item_set_type:
            logging.warning("Skipping %s - no valid set type", img_name)
            continue

        annotations = get_image_annotations(image, dataset_version)
        if not annotations:
            logging.info("Skipping %s - no annotations found", img_name)
            continue  # Skip creating file if no annotations

        os.makedirs(f"{local_path}/labels/{item_set_type}", exist_ok=True)
        anno_txt = f"{local_path}/labels/{item_set_type}/{'.'.join(img_name.split('.')[:-1])}.txt"

        with open(anno_txt, "w", encoding="utf-8") as file:
            for anno in annotations:
                try:
                    x_coord, y_coord, w_norm, h_norm = convert_bbox_coco2yolo(
                        image["width"],
                        image["height"],
                        anno["bbox"],
                    )
                    ...
                    if category_id is None:
                        logging.warning("Unknown category %s in %s", category, img_name)
                        continue
                    ...
                    logging.error("Error processing annotation in %s: %s", img_name, str(exc))
