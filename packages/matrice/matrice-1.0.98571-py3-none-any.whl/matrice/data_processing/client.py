...
def get_video_partition_status(
    ...
    if not skip_annotation_partition:
        annotation_partition = get_annotations_partition(annotation_files)
    images_partitions = get_partitions(image_files)
    return annotation_partition, images_partitions
...
def get_video_davis_partition_batches(
    ...
    def create_file_info(file_path):
        os.path.basename(os.path.dirname(file_path))
        video_identifier = os.path.basename(os.path.dirname(file_path))
        file_name = os.path.basename(file_path)
        frame_number = int(os.path.splitext(file_name)[0])
        return {
            "partition_num": partition["partitionNum"],
...
def upload_video_imagenet_batch_files(batch, max_attempts=5):
    ...
    videos = [file_info for file_info in batch]
    for file_info in videos:
        success = upload_file(
            file_info["local_file_path"],
            file_info["presigned_url"],
...
def batch_create_video_youtube_bb_dataset_items(
    ...
    num_created = 0  # Counter for created dataset items
    ...
    for video in batch:
        ...
        logging.info("Payload for create dataset items: %s", payload)

        resp = rpc.post(
            f"/v2/dataset/add_video_dataset_items?projectId={project_id}",
            payload=payload,
        )
        logging.info("Response from create dataset items: %s", resp)

        # Increment counter if creation appears successful
        if resp and resp.get("success"):  # Or check your actual success key
            num_created += 1

    logging.info("Total dataset items created: %d", num_created)
    return batch
...
def batch_create_video_imagenet_dataset_items(
    ...
    get_relative_path_fn = (
        get_davis_relative_path
        if input_type in ["davis", "video_mscoco"]
...
def get_client_video_processing_pipeline(
    ...
    for partition in images_partitions:
        if input_type == "youtube_bb":
            for batch in get_video_youtube_bb_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
            ...
            for batch in get_video_mot_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
            ...
            for batch in get_video_davis_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
            ...
            for batch in get_video_imagenet_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
            ...
            for batch in get_video_kinetics_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
                ...
                stage_1_queue.put(batch)
        elif input_type == "video_mscoco":
            for batch in get_video_mscoco_partition_batches(
                partition,
                batch_size,
                dataset_id,
                dataset_version,
