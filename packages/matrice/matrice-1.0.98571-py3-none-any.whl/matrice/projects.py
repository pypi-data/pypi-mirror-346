...
class Projects:
    ...
    def _create_dataset(
        ...
        if source_type == "lu":
            ...
            if not is_dataset_compressed:
                skip_annotation_pipeline = (
                    input_type in ["imagenet", "unlabeled"] or
                    ("labelbox" in input_type)
                )

                if dataset_type.lower() in ["video_detection", "frames"]:
                    logging.info("Processing video detection dataset")
                    annotation_partition, images_partitions = get_video_partition_status(
                        dataset_path,
                        skip_annotation_pipeline
                    )
                    partition_stats = [annotation_partition, *images_partitions]
                    logging.debug(f'partition stats: {partition_stats}')
                else:
                    logging.info("Processing image dataset")
                    annotation_partition, images_partitions = get_partition_status(
                        dataset_path,
                        skip_annotation_pipeline
                    )
                    partition_stats = [annotation_partition, *images_partitions]
        ...
        # Process URL source
            ...
            try:
                dataset_size, err, msg = get_dataset_size_in_mb_from_url(
                    self.session, source_url, self.project_id
                )
                if err:
                    dataset_size = 0
                    logging.warning(f"Could not get dataset size: {msg}")
            except Exception as e:
                logging.error(f"Error getting dataset size: {e}")
                dataset_size = 0
        ...
        logging.debug(f"Create dataset request: {create_dataset_request}")
        resp = self.rpc.post(path=path, headers=headers, payload=create_dataset_request)
        logging.debug(f"Create dataset response: {resp}")

        if not resp["success"]:
            error_msg = resp.get("message", "Unknown error")
            logging.error(f"Dataset creation failed: {error_msg}")
            raise Exception(f"Dataset creation failed: {error_msg}")

        # Process successful creation
        dataset_id = resp["data"]["_id"]
        logging.info(f"Dataset created successfully with ID: {dataset_id}")
