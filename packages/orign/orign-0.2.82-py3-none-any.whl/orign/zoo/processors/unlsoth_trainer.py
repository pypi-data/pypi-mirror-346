        ...
        if latest_checkpoint:
            ...
            if os.path.exists(trainer_state_path):
                try:
                    ...
                    if state_data:
                        print(
                            f"  Checkpoint trainer_state - epoch: {state_data.get('epoch')}"
                        )
                        print(
                            f"  Checkpoint trainer_state - global_step: {state_data.get('global_step')}"
                        )
                        # log_history is a list, show the last entry if it exists
                        log_history = state_data.get("log_history", [])
                        if log_history:
                            print(
                                f"  Checkpoint trainer_state - last log_history entry: {log_history[-1]}"
                            )
                except Exception as e:
                    print(
