...
class TrainedModelRegistryRequest(BaseModel):
    ...
    """ # noqa: E501
    access_key: Optional[StrictStr] = Field(default=None, description="The access key, username or team name for the registry. * `s3` - The access key/username. * `ngc` - The optional NGC team name. * `openllm` - The value is ignored. * `huggingface` - The value is ignored. * `pfs` - The optional repository name.", alias="accessKey")
    bucket: Optional[StrictStr] = Field(default=None, description="The bucket or organization name.  * `s3` - The required S3 bucket name. * `ngc` - The required NGC org name.  Use `nim` for NVIDIA-provided NIMs. * `openllm` - The value is ignored. * `huggingface` - The value is ignored. * `pfs` - The required project name.")
    endpoint_url: Optional[StrictStr] = Field(default=None, description="The registry endpoint (host): * `s3` - The S3 registry endpoint. Required. * `openllm` - The huggingface.co compatible endpoint (default https://huggingface.co). * `huggingface` - The huggingface.co compatible endpoint (default https://huggingface.co). * `ngc` - The NVIDIA NGC-compatible api endpoint (default https://api.ngc.nvidia.com). * `pfs` - The required HPE MLDM cluster (pachd) address.", alias="endpointUrl")
    ...
    project: Optional[StrictStr] = Field(default=None, description="The type of this model registry. Must be one of the values: (s3, http, openllm, ngc, pfs). * `s3` - Configuration to enable direct model download from an s3 bucket. * `openllm` - Configuration to enable direct model download of OpenLLM models from huggingface.co. * `huggingface` - Configuration to enable direct model download of vLLM models from huggingface.co. * `ngc` -  Configuration to enable direct model download from the NVIDIA NGC: AI Development Catalog. * `pfs` -  Configuration to enable direct model download from an HPE MLDM Pachyderm File System. The optional name or ID of the containing project. If unspecified, the project is determined by the project in the session, otherwise it uses the default project configured for the user.")
