...
class TrainedModelRegistry(BaseModel):
    ...
    """ # noqa: E501
    access_key: Optional[StrictStr] = Field(default=None, description="The access key, username or team name for the registry. * `s3` - The access key/username. * `openllm` - The value is ignored. * `huggingface` - The value is ignored. * `ngc` - The optional NGC team name. * `pfs` - The optional repository name.", alias="accessKey")
    bucket: Optional[StrictStr] = Field(default=None, description="The bucket or organization name, depending on which of the following values is selected as the model registry type.  * `s3` - The required S3 bucket name. * `openllm` - The value is ignored. * `huggingface` - The value is ignored. * `ngc` - The required NGC org name.  Use `nim` for NVIDIA-provided NIMs. * `pfs` - The required project name.")
    ...
    type: StrictStr = Field(description="The type of this model registry. Must be one of the values: (s3, http, openllm, ngc, pfs). * `s3` - Configuration to enable direct model download from an s3 bucket. * `openllm` - Configuration to enable direct model download of OpenLLM models from huggingface.co. * `huggingface` - Configuration to enable direct model download of vLLM models from huggingface.co. * `ngc` -  Configuration to enable direct model download from the NVIDIA NGC: AI Development Catalog. * `pfs` -  Configuration to enable direct model download from an HPE MLDM Pachyderm File System.")
