...
from aioli.cli.project import lookup_project_by_id
...
def list_models(args: Namespace) -> None:
    def format_json(
        ...
        for m in response:
            ...
            d.pop("modifiedAt")
            if "project" in d:
                d["project"] = lookup_project_by_id(d["project"], proj_api).name
            if "registry" in d:
                registry = lookup_registry_by_id(d["registry"], registries_api)
                d["registry"] = registry.name
                if registry.project:
                    registry_project = lookup_project_by_id(registry.project, proj_api)
                    d["registryProject"] = registry_project.name
            models.append(d)
    ...
    def format_models(
        ...
        def format_model(e: PackagedModel, reg_api: aiolirest.RegistriesApi) -> List[Any]:
            ...
            rpname = ""
            if e.project:
                pname = lookup_project_by_id(e.project, proj_api).name
            if e.registry:
                registry = lookup_registry_by_id(e.registry, reg_api)
                rname = registry.name
                if registry.project:
                    registry_project = lookup_project_by_id(registry.project, proj_api)
                    rpname = registry_project.name
...
def show_model(args: Namespace) -> None:
    ...
    if model.registry:
        ...
        if registry.project:
            d["registryProject"] = lookup_project_by_id(registry.project, projects_api).name

    d["project"] = lookup_project_by_id(d["project"], projects_api).name
