...
class _SectionInterface:
    ...
    def initialize(self):
        ...
        shear = "shear" in self._section.lower()
        for i,mat in enumerate(self._materials):
            if shear:
                m = mat
                model.nDMaterial(m["type"], i+1, **{k: v for k, v in m.items() if k != "type"})
            else:
                m = mat
                model.uniaxialMaterial(m["type"], i+1, **{k: v for k, v in m.items() if k != "type"})


        model.section(self._section, 1, GJ=1e3)
        for i in range(len(self._materials)):
            for fiber in self._shape.create_fibers(warp=shear, group=i):
