  ...
  [vv]    Bishear            {Ivv :>10.4}
    ...
    def fibers(self, origin=None, center=None, types=None, warp=True, group=0):
        ...
        if origin is not None:
            if origin == "centroid":
                yield from self.translate(self._analysis.centroid()).fibers(center=center,  origin=None)
            elif origin == "shear-center":
                yield from self.translate(self._analysis.shear_center()).fibers(center=center,  origin=None)
            else:
                yield from self.translate(origin).fibers(center=center,  origin=None)
            return
