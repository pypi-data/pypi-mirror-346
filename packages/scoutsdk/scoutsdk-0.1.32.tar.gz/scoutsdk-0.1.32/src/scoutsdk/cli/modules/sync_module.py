...
class SyncModule(BaseModule):
    ...
    def _load_config(self, config_path: str) -> SyncConfig:
        ...
        for ref_path in assistants_ref:
            assistant_json_path = Path(ref_path) / "assistant.json"
            if not assistant_json_path.exists():
                logger.warning(f"Assistant ref file not found: {assistant_json_path}")
                continue
            try:
                with open(assistant_json_path, "r") as af:
                    assistant_data = json.load(af)
                    if isinstance(assistant_data, list):
                        for a in assistant_data:
                            self._fix_assistant_paths_with_ref_path(a, ref_path)
                            assistants.append(a)
                    else:
                        self._fix_assistant_paths_with_ref_path(
                            assistant_data, ref_path
                        )
                        assistants.append(assistant_data)
            except Exception as e:
                raise ModuleError(
                    f"Error loading assistant ref {assistant_json_path}: {str(e)}"
                )
    ...
    def _fix_assistant_paths_with_ref_path(self, assistant: dict, ref_path: str):
        def join_path(p):
            return str(Path(ref_path) / p) if p else p
