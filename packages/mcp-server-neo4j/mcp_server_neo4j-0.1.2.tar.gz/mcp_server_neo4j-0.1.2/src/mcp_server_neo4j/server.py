...
class Neo4jServer:
    def __init__(self, uri: str, username: str, password: str, database: str = "neo4j"):
        """初始化Neo4j服务器连接"""
        try:
            logger.info(f"正在连接Neo4j数据库: {uri}, 数据库: {database}")
    ...
    def run_query(self, query: str, parameters: Dict[str, Any] = None) -> Neo4jResult:
        """执行Cypher查询"""
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                return Neo4jResult(data=records, summary=summary)
        except Exception as e:
        ...
        try:
            ...
            match name:
                case Neo4jTools.RUN_QUERY.value:
                    ...
                    result = neo4j_server.run_query(query, parameters)
