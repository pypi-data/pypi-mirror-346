...
from pipelex.tools.log.log_levels import LOGGING_LEVEL_DEV, LOGGING_LEVEL_DEV_NAME_FOR_GCLOUD
...
class LogDispatch:
    ...
    def _gcloud_severity_str_from_int(self, severity: int) -> str:
        """
        Converts an integer severity level to a string representation for Google Cloud logging.

        Args:
            severity (int): The severity level as an integer.

        Returns:
            str: The severity level as a string.
        """
        if severity == LOGGING_LEVEL_DEV:
            return LOGGING_LEVEL_DEV_NAME_FOR_GCLOUD
        else:
            return str(logging.getLevelName(severity))
    ...
    def _complete_dict_for_gcloud(self, dict_info: Dict[str, Any]) -> Dict[str, Any]:
        """
        Completes a dictionary with additional information for Google Cloud logging.

        Args:
            dict_info (Dict[str, Any]): The original dictionary to be completed.

        Returns:
            Dict[str, Any]: The completed dictionary with additional information.
        """
        completed_dict = dict_info.copy()
        completed_dict["env"] = runtime_manager.environment
        completed_dict["project"] = self.project_name
