...
from pipelex.tools.class_registry import ClassRegistryInheritanceError, ClassRegistryNotFoundError, class_registry
...
class LibraryManager:
    ...
    def _load_combo_libraries(self, library_paths: List[str]):
        ...
        for toml_path in toml_file_paths:
            ...
            if domain_definition is None:
                raise LibraryParsingError(f"Library '{library_name}' has no definition set for domain '{domain_code}'")
    ...
    def _load_library_dict(self, library_name: str, library_dict: Dict[str, Any], component_type: str):
        if domain := library_dict.pop("domain", None):
            # domain is set at the root of the library
            self._load_library_components_from_recursive_dict(domain=domain, recursive_dict=library_dict, component_type=component_type)
        else:
            raise LibraryParsingError(f"Library '{library_name}' has no domain set")

    def _load_library_components_from_recursive_dict(self, domain: str, recursive_dict: Dict[str, Any], component_type: str):
        for key, obj in recursive_dict.items():
            # root of domain
            if not isinstance(obj, dict):
                if not isinstance(obj, str):
                    raise LibraryError(f"Only a dict or a string is expected at the root of domain but '{domain}' got type '{type(obj)}'")
                if key not in self.allowed_root_attributes:
                    raise LibraryParsingError(f"Domain '{domain}' has an unexpected root attribute '{key}'")
                continue
            ...
            if key == component_type:
                try:
                    if key == LibraryComponent.CONCEPT:
                        self._load_concepts(domain=domain, obj_dict=obj_dict)
                    elif key == LibraryComponent.PIPE:
                        self._load_pipes(domain=domain, obj_dict=obj_dict)
                    else:
                        continue
                    ...
                    raise error_class(f"Error loading a {component_type} from domain '{domain}' because of: {error_msg}") from exc
            elif key not in [LibraryComponent.CONCEPT, LibraryComponent.PIPE]:
                # Not a concept but a subdomain
                self._load_library_components_from_recursive_dict(domain=domain, recursive_dict=obj_dict, component_type=component_type)
            else:
                # Skip keys that don't match our criteria
                continue

    def _load_concepts(self, domain: str, obj_dict: Dict[str, Any]):
        for concept_code, concept_obj in obj_dict.items():
            if isinstance(concept_obj, str):
                # we only have a definition
                concept_from_def = ConceptFactory.make_concept_from_definition(domain=domain, code=concept_code, definition=concept_obj)
                ...
                concept_from_dict = ConceptFactory.make_from_details_dict(domain=domain, code=concept_code, details_dict=concept_obj_dict)
                self.concept_library.add_new_concept(concept=concept_from_dict)
            else:
                raise ConceptLibraryError(f"Unexpected type for concept_code '{concept_code}' in domain '{domain}': {type(concept_obj)}")

    def _load_pipes(self, domain: str, obj_dict: Dict[str, Any]):
