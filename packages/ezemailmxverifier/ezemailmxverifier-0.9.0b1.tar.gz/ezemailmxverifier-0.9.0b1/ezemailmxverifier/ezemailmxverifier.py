...
"""Easy Email MX Verifier v0.9.0 - A complete RFC 2822 style email validation for Python."""
"""
...
class EzEmailMXVerifierErrorStrings(object):
    ...
    def __init__(self):
        self.error_strings = {
            ...
            "dns_resolve_hostname_success": "Hostname {mx} resolved to {ip_address} for domain {domain}",
            "dns_resolve_hostname_exception": "Resolve hostname exception for domain {domain}: {error}",
            ...
            "valid_email_no_valid_mx": "Valid Email sintax but no valid MX records found for domain {domain}",
        }
        self.dns_rcodes = {
...
class EzEmailMXVerifier(object):
    ...
    >>> print(response, error_message)
    """
    def __init__(self,dns_server:str="8.8.8.8",fallback_dns_server:str="1.1.1.1",timeout:float=0.5,timeout_max_retries:int=2,safe_timeout_enabled:bool=False,
                 ...
                 debug=False,debug_flag_file_watchdog_interval:int=5,debug_flag_file_dir:str=os.path.dirname(__file__),debug_save_dir:str=tempfile.gettempdir(),
                 verify_tld:bool=True,tld_datfile_dest_dir:str=os.path.dirname(__file__),tld_datfile_auto_update_days:int=7)->None:
        """
        ...
        if debug_flag_file_watchdog_interval > 0 and not self.__debug_flag:
            threading.Thread(target=self.__worker_debug_flag_watchdog, daemon=True, args=(debug_flag_file_watchdog_interval,self.__debug_flag_file_dir)).start()
        ...
        self.error_string.update_error_strings({"safe_timeout_reached_mx_hostname": "Safe timeout reached verifying hostname {hostname} for domain {domain} {error}"})
        ...
        self.__soa_domain_cache = EzEmailMXVerifierCache()            
    ...
    def __worker_debug_flag_watchdog(self,interval:int=5,debug_flag_file_dir:str=None)->None:
        ...
        if debug_flag_file_dir is None or (debug_flag_file_dir is not None and not os.path.isdir(debug_flag_file_dir)):
            debug_flag_file_dir = os.path.join(os.path.dirname(__file__),'ezemailmxverifier_debug')
        else:
            debug_flag_file_dir = os.path.join(debug_flag_file_dir,'ezemailmxverifier_debug')
        while True:
            try:
                ...
                if current_debug_flag_state != self.__debug_flag:
                    ...
                    if self.__debug_flag:
                        ...
                        self.__debug("Debug flag file found! Debug function set to True.")
                    else:
                        self.__debug("Debug flag file removed! Debug function set to False.")
    ...
    def __worker_cache(self,interval:int=5):
        """Worker to run in background to show and save all cache information when running in debug mode"""
        self.__debug(self.__cRed("Running background thread to monitor the application cache..."))
        while True:
            try:
                time.sleep(interval)
                ...
                if self.use_mx_cache == True:
                    mx_cache_stats = self.__mx_cache.cache_info()
                    ...
                    with open(mx_cache_save_path, 'w') as f:
                        json.dump(self.__mx_cache.get_cache(), f, indent=3, sort_keys=False)
                if self.use_resolver_cache == True:
                    resolver_cache_stats = self.__resolver_cache.cache_info()
                    ...
                    with open(resolver_cache_save_path, 'w') as f:
                        json.dump(self.__resolver_cache.get_cache(), f, indent=3, sort_keys=False)
                if self.use_soa_domain_cache == True:
                    soa_domain_stats = self.__soa_domain_cache.cache_info()
                    ...
                    with open(soa_domain_cache_save_path, 'w') as f:
                        json.dump(self.__soa_domain_cache.get_cache(), f, indent=3, sort_keys=False)
    ...
    def verify_domain(self,domain:str,return_boolean:bool=None)->typing.Tuple[bool,str,dict]:
        ...
        if self.use_soa_domain_cache == True:
            start_time = time.monotonic()
            cached_response = self.__soa_domain_cache.find_domain(domain)
        ...
        if sintax_result:
            ...
            if result[0] == True and self.use_soa_domain_cache == True:
                start_time = time.monotonic()
                self.__soa_domain_cache.add_domain(domain, result[2])
                self.__debug(f">>> Added to cache SOA records for domain {domain} [{'%.9f'%(time.monotonic()-start_time)}]")
                # cache validation
                # assert self.__soa_domain_cache.find_domain(domain) == result[2]
        if return_boolean:
            return result[0] 
        return result
    ...
    def __verify_mx_records(self,domain:str)->typing.Tuple[bool,typing.List[str]]:
        ...
        if self.use_mx_cache:
            start_time = time.monotonic()
            mx_servers = self.__mx_cache.find_domain(domain)
        ...
        try:            
            ...
            with concurrent.futures.ThreadPoolExecutor(10) as executor:
                for mx, priority in mx_records:
                    ...
                    if self.use_resolver_cache:
                        start_time = time.monotonic()
                        ips_response = self.__resolver_cache.find_domain(mx)
                    ...
                    try:
                        ...
                        try:
                            ...
                            if response_host_domain == True and isinstance(response_ipaddr_or_err,list) and len(response_ipaddr_or_err) > 0:
                                self.__debug(self.error_string('dns_resolve_hostname_success').format(mx=mx,ip_address=response_ipaddr_or_err,domain=domain))
                                valid_mx_servers.append((mx,priority,response_ipaddr_or_err))
                                if self.use_resolver_cache:
                                    start_time = time.monotonic()
                                    self.__resolver_cache.add_domain(mx,response_ipaddr_or_err)
                                    self.__debug(f">>> Added to resolver cache the IPs for hostname {mx} [{'%.9f'%(time.monotonic()-start_time)}]")
                            else:
                                self.__debug(self.error_string('dns_resolve_hostname_exception').format(mx=mx,domain=domain,error=response_ipaddr_or_err))
                        except concurrent.futures.TimeoutError as ERR:
                            error_message = self.error_string('dns_resolve_timeout_for_mx').format(mx=mx,domain=domain,error=str(ERR))
                            self.__debug(error_message)
                            return False, error_message, []
                        except Exception as ERR:
                            if str(ERR).lower().find('safe timeout') != -1:
                                self.__debug(self.error_string('safe_timeout_reached_mx_hostname').format(hostname=mx,domain=domain,error=str(ERR)))
                                if len(valid_mx_servers) > 0:
                                    return True, self.error_string('valid_email_mx_ok').format(domain=domain), valid_mx_servers
                                else:
                                    return False, self.error_string('safe_timeout_reached_mx_hostname').format(hostname=mx,domain=domain,error=str(ERR)), []
            ...
            if self.use_mx_cache and len(valid_mx_servers) > 0:
                start_time = time.monotonic()
                self.__mx_cache.add_domain(domain,valid_mx_servers)
