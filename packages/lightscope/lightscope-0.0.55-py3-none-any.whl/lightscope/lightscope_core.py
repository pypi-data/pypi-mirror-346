...
ETH_HDR_LEN = 14          # Ethernet header length
ETH_P_IP    = 0x0800
ETH_P_8021Q = 0x8100      # VLAN-tagged frame

import dpkt, socket, struct, datetime
ETH_P_IPV6 = 0x86DD

def parse_packet_info_fast(buf: bytes, pkt_no: int) -> PacketInfo:
    # --- detect raw-IP vs Ethernet ---
    first_nibble = buf[0] >> 4
    if first_nibble in (4, 6):
        off = 0
        eth_type = ETH_P_IP if first_nibble == 4 else ETH_P_IPV6
    else:
    ...
    # --- IPv6 path -----------------------------------------------
        ...
        ip6 = dpkt.ip6.IP6(buf[off:])
        # walk any extension headers until next == TCP
        nxt = ip6.nxt
        data = ip6.data
        while isinstance(data, (dpkt.ip6.IP6ExtHdrHopOpts,
                                dpkt.ip6.IP6ExtHdrFragment,
                                dpkt.ip6.IP6ExtHdrRouting,
                                dpkt.ip6.IP6ExtHdrDest)):
            nxt = data.nxt
            data = data.data
        if nxt != dpkt.ip.IP_PROTO_TCP:
            raise Exception("Not TCP/IPv6")
        tcp = data  # now a dpkt.tcp.TCP
