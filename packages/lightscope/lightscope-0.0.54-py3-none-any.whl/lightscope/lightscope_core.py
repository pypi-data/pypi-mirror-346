...
def parse_packet_info_fast(buf: bytes, pkt_no: int) -> PacketInfo:
    """
    Parse a buffer that may be an Ethernet frame or a raw IPv4 packet,
    but only ever returns TCP packets. Everything else raises.
    """
    # --- detect raw-IP vs Ethernet ---------------------------------------
    first_byte = buf[0]
    if (first_byte >> 4) == 4:
        # looks like an IPv4 header right away
        off = 0
    else:
        # assume Ethernet
        ...
        if eth_type != ETH_P_IP:
            raise Exception(f"Not IPv4 (ethertype=0x{eth_type:04x})")

    # --- parse the IP header ---------------------------------------------
    ip = dpkt.ip.IP(buf[off:])
    if ip.p != dpkt.ip.IP_PROTO_TCP:
        # drop anything that isn't TCP
        raise Exception(f"Not TCP (proto={ip.p})")
