...
def read_from_interface_windows(network_interface,
    ...
    def sender_thread():
        ...
        while True:
            ...
            if (now - prior_time) >= IDLE_FLUSH_SECS:
                print(f"{interface_human_readable} sender_thread: len(send_deque) {len(send_deque)} pps {packets_processed}", flush=True)
                packets_processed = 0
                prior_time = now
...
##TODO DEBUG
def main():
    # on Windows, allow spawning child procs
    if platforminfo.system() == "Windows":
        multiprocessing.freeze_support()

    # now start your normal run
    lightscope_run()
