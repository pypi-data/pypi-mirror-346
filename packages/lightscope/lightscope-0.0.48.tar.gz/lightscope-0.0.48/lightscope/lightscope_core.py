...
class Ports:
    ...
    def packet_handler(self, unprocessed_packets):
        ...
        while True:
            ...
            print(self.currently_open_ip_list.items(),flush=True)
            print(self.previously_open_ip_list_A.items(),flush=True)
            print(self.previously_open_ip_list_B.items(),flush=True)
...
def choose_mac_linux_interface():
    ...
    # match both regular and VPN interfaces (utun, ppp, tun, en*)
    iface_pattern = re.compile(r'^(en|utun|ppp)\d+$')
    ...
    for iface, s in stats.items():
        ...
        # pull any addresses so we can keep the same signature shape:
        ipv4s = [a.address for a in addrs.get(iface, [])
                 if a.family == socket.AF_INET]
        ipv6s = [a.address for a in addrs.get(iface, [])
