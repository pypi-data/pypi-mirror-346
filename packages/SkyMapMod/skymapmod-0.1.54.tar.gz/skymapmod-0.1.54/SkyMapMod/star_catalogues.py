...
def star_spectrum(l, b, lmbd):
    ...
    Примечания:
        ...
        - Учитывается порядок амплитуды с помощью множителя 1e-30
    ...
    A = star_brightness[i][j] #учитывается порядок амплитуды, которая хранится в больших числах
    T = star_temperatures[i][j]
    print('Информация по звезде в этом пикселе: T = ', T, ', A = ', A, sep='')
    print('Техническая строчка: i = ', i, ', j = ', j, sep='')
    spectrum = A * black_body_phot(lmbd, T) 10**(-30) 
    return spectrum