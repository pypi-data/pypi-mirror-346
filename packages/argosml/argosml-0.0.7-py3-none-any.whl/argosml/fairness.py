...
def fairness(X_train, y_train, model, protected_variable, metric_name='statistical_parity_ratio', privileged_class=1):
...
def fairness_metrics(X_train, y_train, model, protected_variables, threshold):
  ...
  for protected_variable in protected_variables:
    for metric in options:
      ...
      metrics[f'{_to_camel_case(name)}_{metric}'] = fairness(X_train, y_train, model, name, metric_name=metric, privileged_class=reference_group)
