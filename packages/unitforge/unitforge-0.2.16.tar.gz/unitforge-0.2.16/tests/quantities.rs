...
mod test_quantity_enum {
    use unitforge::{Distance, DistanceUnit, Force, ForceDistanceUnit, ForceUnit, Mass, MassUnit, PhysicsQuantity, Quantity, StiffnessUnit, Unit};
    use crate::approx_eq;
    ...
    fn test_add() {
        ...
        let c = (a + b).unwrap();
        assert!(approx_eq(c.to(Unit::MassUnit(MassUnit::kg)).unwrap(), 22., 1E-10));
    }
    ...
    fn test_sub() {
        ...
        let b = Quantity::MassQuantity(Mass::new(10., MassUnit::kg));
        let c = (a - b).unwrap();
        assert!(approx_eq(c.to(Unit::MassUnit(MassUnit::kg)).unwrap(), 2., 1E-10));
    }
    ...
    fn test_mul() {
        ...
        let c = (a * b).unwrap();
        assert!(approx_eq(c.to(Unit::ForceDistanceUnit(ForceDistanceUnit::Nm)).unwrap(), 12., 1E-10));
    }
    ...
    fn test_div() {
        ...
        let b = Quantity::DistanceQuantity(Distance::new(4., DistanceUnit::m));
        let c = (a / b).unwrap();
        assert!(approx_eq(c.to(Unit::StiffnessUnit(StiffnessUnit::N_m)).unwrap(), 3./4., 1E-10));
    }

    #[test]
    fn test_abs() {
        let a = Quantity::DistanceQuantity(Distance::new(-4., DistanceUnit::m));
        assert!(approx_eq(a.abs().to(Unit::DistanceUnit(DistanceUnit::m)).unwrap(), 4., 1E-10));
    }
