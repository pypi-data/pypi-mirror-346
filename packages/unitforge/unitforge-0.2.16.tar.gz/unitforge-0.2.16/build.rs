...
fn write_quantity_code(dest_path: &PathBuf, quantity_structs: &Vec<(String, String)>, quantities_template_path: String) {
    ...
    for (struct_name, content) in quantity_structs {
        ...
        quantity_fmt_matches += &format!("            Quantity::{}Quantity(v) => write!(f, \"{{}}\", v),\n", struct_name);
        quantity_comparisons += &format!("            ({}Quantity(lhs), {}Quantity(rhs)) => lhs.partial_cmp(rhs),\n", struct_name, struct_name);
        unit_variants += &format!("    {}Unit({}Unit),\n", struct_name, struct_name);
        to_quantity_variants += &format!("            Unit::{}Unit(unit) => Quantity::{}Quantity({}::new(value, *unit)),\n", struct_name, struct_name, struct_name);
        quantity_abs_variants += &format!("            Quantity::{}Quantity(value) => Quantity::{}Quantity(value.abs()),\n", struct_name, struct_name);
        unit_name_variants += &format!("            Unit::{}Unit(unit) => unit.name(),\n", struct_name);
