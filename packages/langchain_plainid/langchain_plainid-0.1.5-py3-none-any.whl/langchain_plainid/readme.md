...
```python
    ...
    plainid_retriever = PlainIDRetriever(vectorstore=vector_store, filter_provider=mock_filter_provider)
...
We support different vector stores, but some of them have limitations in filtering or querying data. Below is the list of tested vector stores and their limitations.

### FAISS

It doesn't support $like operators, so no STARTSWITH, ENDSWITH, CONTAINS operators in PlainID ruleset.

### Chroma

It doesn't support $like, $in, $nin operators, so no IN, NOT_IN, STARTSWITH, ENDSWITH, CONTAINS operators in PlainID ruleset.
