...
class TabuSearch:
    def __init__(
        ...
        update_history:Optional[Callable[[Solution]]] = None,
        ...
        while not self._should_stop():
            ...
            if best_candidate:
                ...
                if self.update_history:
                    self.history=self.update_history(best_candidate)
                else:
    ...
    def _should_stop(self) -> bool:
        ...
        if time.time() - self.start_time > self.max_time:
            self._log("Stopping - Max time reached")
            return True
    ...
    def _update_tabu_list(self, best_candidate: Solution):
        ...
        move_hash = self._get_move_hash(best_candidate.representation)
    ...
    def _get_move_hash(self, candidate: Solution) -> int:
        """Génère un hash pour le mouvement entre solutions."""
        # Implémentation spécifique au problème
        # Exemple générique: hash de la différence entre les solutions
        diff = frozenset(
            candidate.representation.symmetric_difference(self.current_solution.representation)
        )
    ...
    def _update_frequency(self, solution: Solution):
        ...
        h = hash(solution.representation)
        self._frequency[h] += 1
