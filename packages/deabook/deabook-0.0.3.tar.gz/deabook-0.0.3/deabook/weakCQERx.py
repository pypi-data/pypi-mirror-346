    ...
    def __afriat_rule(self):
        ...
        if self.rts == RTS_VRS:
            def afriat_rule(model, i, h):
                ...
                return __operator(
                    model.alpha[i] - sum(model.gamma[i, l] *  self.yref.loc[i, self.ycol[l]] for l in model.L)
                        + sum(model.delta[i, j] * self.bref.loc[i, self.bcol[j]] for j in model.J),
                    model.alpha[h] - sum(model.gamma[h, l] * self.yref.loc[i, self.ycol[l]] for l in model.L)
...
class weakCERx(weakCQRx):
    """Convex expectile regression (weakCERx)
    """
