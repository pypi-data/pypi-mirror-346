...
from .utils import tools,CNLSSDFDDF_FRONTIER
...
class StoNED:
    ...
    def get_technical_efficiency_ratio(self, method=RED_MOM):
        """
        Args:
            method (String, optional): RED_MOM (Method of moments) or RED_QLE (Quassi-likelihood estimation). Defaults to RED_MOM.
        ...
        """
        tools.assert_optimized(self.model.optimization_status)
        ddfhat = self.get_technical_inefficiency(method)
        if self.model.__class__.__name__ in ["CNLSSDweak","CNLSDDFweak"]:

            if sum(self.gy) >= 1:
                self.TE = self.basexy/(self.basexy+self.epsilonhat)
                return self.TE
                ...
                self.TE = (-1*np.asarray(self.basexy) + self.epsilonhat) /(-1*np.asarray(self.basexy) + self.epsilonhat + ddfhat)
                return self.TE
            ...
            if sum(self.gy) >= 1:
                self.TGR = self.basexy/(self.basexy+self.epsilonhat)
                df = pd.DataFrame({
                    'TGR': self.TGR,
    ...
    def __quassi_likelihood(self, residual):
        ...
        def __quassi_likelihood_estimation(lamda, eps):
            ...
            return -log_likelihood + penalty  # Add penalty to the negative log likelihood
        ...
        if self.model.fun == FUN_PROD:
            self.residual_minus = residual-self.mu
            # print(22222222222,self.residual_minus)
        elif self.model.fun == FUN_COST:
            self.residual_minus = -self.mu+residual
