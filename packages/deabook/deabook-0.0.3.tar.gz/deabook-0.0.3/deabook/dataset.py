...
class production_data:
    ...
    def __init__(self, dmu, x, y, b=None, z=None):
        ...
        self.b = b
        self.z = z
def load_zhizaoye(year=None, x_select=['L',	'K'	,'E'], y_select=['Y'], b_select=['C']):
    ...
    dataframe = pd.read_excel(
        file_path+"/data/aaa.xls")#.query("2011<=Year==2014 " )
    if year != None:
        dataframe = dataframe[dataframe['year'] == year]
    else:
        dataframe['hangye'] = dataframe['hangye'] + \
            dataframe['year'].apply(str)
    dmu = np.asanyarray(dataframe['hangye']).T
    x = np.column_stack(
        [np.asanyarray(dataframe[selected]).T for selected in x_select])
    y = np.column_stack(
        [np.asanyarray(dataframe[selected]).T for selected in y_select])/1000
    if b_select != None:
        b = np.column_stack(
...
def load_GHG_abatement_cost(year=None, x_select=['HRSN', 'CPNK'], y_select=['VALK'], b_select=['GHG']):
    ...
    dataframe = pd.read_csv(
        file_path+"/data/abatementCost.csv").query("2011<=Year==2014 " )
    if year != None:
        dataframe = dataframe[dataframe['Year'] == year]
    else:
