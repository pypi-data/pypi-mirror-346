...
class DEA:
    ...
    def optimize(self, email=OPT_LOCAL, solver=OPT_DEFAULT):
        ...
        for ind, problem in self.modeldict.items():
            ...
            # self.modeldict[ind] = problem
            print("SSSSSSSSSSSSS")
            print(list(problem.rho[:].value))
...
class DEADUAL(DEA):
            ...
            if self.rts == RTS_VRS1:
                def first_rule(model, r):
                    return sum(model.delta[ j] * self.xref[r][j] for j in model.J) \
                        - sum(model.gamma[ k] * self.yref[r][k] for k in model.K) + model.alpha >= 0
                return first_rule
            elif self.rts == RTS_CRS:
                def first_rule(model, r):
                    return sum(model.delta[j] * self.xref[r][j] for j in model.J) \
                        - sum(model.gamma[ k] * self.yref[r][k] for k in model.K) >= 0
                return first_rule
        elif sum(self.gy) >= 1:
            if self.rts == RTS_VRS1:
                def first_rule(model,  r):
    ...
    def __second_rule(self):
        ...
        if sum(self.gx) >= 1:
            def second_rule(model):
                return sum(model.delta[ j] * self.x[self.ii][j]*self.gx[j] for j in model.J) <= 1
            return second_rule
        elif sum(self.gy) >= 1:

            def second_rule(model):
                return sum(model.gamma[ k] * self.y[self.ii][k]*self.gy[k] for k in model.K) == 1
            return second_rule
    ...
    def optimize(self, email=OPT_LOCAL, solver=OPT_DEFAULT):
        ...
        for ind, problem in self.modeldict.items():
            ...
            # self.modeldict[ind] = problem
            print("SSSSSSSSSSSSS")
            print(list(problem.gamma[:].value))
        # self.problem_status, self.optimization_status = tools.optimize_model(
        #     self.__model__, email, CET_ADDI, solver)
...
class DDFDUAL(DEADUAL):
    ...
    def __first_rule(self):
        ...
        if self.rts == RTS_VRS1:
            ...
            def first_rule(model,  r):
                return sum(model.delta[ j] * self.xref[r][j] for j in model.J) - \
                    sum(model.gamma[ k] * self.yref[r][k] for k in model.K) >= 2
            return first_rule
