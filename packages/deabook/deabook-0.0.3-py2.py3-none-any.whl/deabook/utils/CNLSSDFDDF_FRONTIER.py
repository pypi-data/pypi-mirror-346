...
class CNLSSD:
    ...
    def __init__(self, data, sent, z=None, gy=[1], gx=[0], cet=CET_MULT, fun=FUN_PROD, rts=RTS_VRS1):
        ...
        print(self.__model__.__class__.__name__)
        # Optimize model
        ...
        if self.cet == CET_MULT:
            if type(self.z) != type(None):
                def regression_rule(model, i):
                    return model.epsilon[i] - log(self.basexy[i]) == log(model.frontier[i] + 1) \
        ...
        if self.cet == CET_MULT:
            if self.rts == RTS_VRS1:
                def log_rule(model, i):
                    return model.frontier[i] == model.alpha[i] - 1 +\
...
class CNLSDDF(CNLSSD):
    ...
    def __init__(self, data, sent, muhat, epsilonhat, z=None, gy=[1], gx=[1], fun=FUN_PROD, rts=RTS_VRS1):
        """CNLS DDF model
        ...
        for i in self.I:
            ...
            # Initialize the sets
            # self.__model__.I = Set(initialize=range(len(self.y)))
            self.__model__.R = Set(initialize=range(len(self.y)))
            ...
            # Initialize the variables
            if self.rts == RTS_VRS1:
                self.__model__.alpha = Var( doc='alpha')
            self.__model__.delta = Var(self.__model__.J, bounds=(0.0, None), doc='delta')
            self.__model__.epsilon = Var(doc='residuals')
            self.__model__.gamma = Var(self.__model__.K, bounds=(0.0, None), doc='gamma')
            ...
            self.__model__.objective = Objective(rule=self.__objective_rule(),
                                                 sense=minimize,
                                                 doc='objective function')
            self.__model__.regression_rule = Constraint(self.__model__.R,
    ...
    def __objective_rule(self):
        ...
        def objective_rule(model):
            if self.rts == RTS_VRS1:
                ...
                return model.alpha \
                + sum(model.delta[j] * self.x[self.ii][j] for j in model.J) \
    ...
    def __regression_rule(self):
        ...
        if self.rts == RTS_VRS1:
            if type(self.z) != type(None):
                ...
                def regression_rule(model, i):
                    return sum(model.delta[ j] * self.x[i][j] for j in model.J) \
                        - sum(model.gamma[ k] * self.y[i][k] for k in model.K) + model.alpha \
                        - sum(model.omega[m] * self.z[i][m] for m in model.M) >= \
                        self.muhat+self.epsilonhat[i]
            ...
            if type(self.z) != type(None):
                ...
                def regression_rule(model, i):
                    return sum(model.delta[ j] * self.x[i][j] for j in model.J) \
                        - sum(model.gamma[ k] * self.y[i][k] for k in model.K) \
                        - sum(model.omega[m] * self.z[i][m] for m in model.M) >= \
                        self.muhat+self.epsilonhat[i]
    ...
    def __translation_property(self):
        ...
        def translation_rule(model):
            return sum(model.delta[ j] * self.gx[j]* self.x[self.ii][j]  for j in model.J) \
                + sum(model.gamma[ k] * self.gy[k] *self.y[self.ii][k]  for k in model.K) == 1
