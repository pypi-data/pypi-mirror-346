...
from .constant import CET_ADDI, FUN_PROD, FUN_COST, OPT_DEFAULT, RTS_CRS, RTS_VRS1,OPT_LOCAL
import time
from . import CNLS
...
class CNLSDDFG(CNLS.CNLS):
    """Convex Nonparametric Least Square (CNLSDDF) and Genetic algorithm
    """
    def __init__(self, y, x, b=None, z=None, gy=[1], gx=[1], gb=None, fun=FUN_PROD, rts=RTS_VRS1):
        """CNLSDDFG model
    ...
    def __convergence_test(self, alpha, beta, gamma, delta):
        ...
        b = np.asarray(self.b)
        activetmp1 = 0.0
        if self.rts == RTS_VRS1 and self.fun == FUN_PROD:
        # go into the loop
        ...
        elif self.rts == RTS_VRS1 and self.fun == FUN_COST:
        # go into the loop
            for i in range(len(x)):
                activetmp = 0.0
                # go into the sub-loop and find the violated concavity constraints
