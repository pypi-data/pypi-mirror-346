        ...
        if self.cet == CET_MULT:
            if type(self.z) != type(None):
                def regression_rule(model, i):
                    return model.epsilon[i] - log(self.basexyb[i]) \
            ...
            def regression_rule(model, i):
                ...
                return model.epsilon[i] - log(self.basexyb[i]) == log(model.frontier[i] + 1)
            return regression_rule
...
class CNLSDDFweak(CNLSSDweak):
    ...
    def __init__(self, data, sent, z=None, gy=[1], gx=[1], gb=[1], fun=FUN_PROD, rts=RTS_VRS1,
        ...
        """
        # TODO(error/warning handling): Check the configuration of the model exist
        self.y, self.x, self.b, self.z,self.gy, self.gx, self.gb, self.basexyb = tools.assert_CNLSDDFweak(data,sent,z,gy,gx,gb)
        self.fun = fun
        self.rts = rts
        self.data = data
        ...
        # if type(epsilonp) != type(None):
        #     self.__model__.epsilon = Var(self.__model__.I, bounds=(epsilonp[0], epsilonp[1]),doc='residuals')
        # else:
    ...
    def __objective_rule(self):
        ...
        def objective_rule(model):
            return sum(model.epsilon[i] ** 2 for i in model.I)
            # \
            # + 0.9 * sum(model.delta[ij] ** 2 for ij in model.I * model.J) \
            # + 0.9 * sum(model.gamma[ik] ** 2 for ik in model.I * model.K) \
            # + 0.9 * sum(model.kappa[il] ** 2 for il in model.I * model.L)
        return objective_rule
    ...
    def __reduction_factor_rule(self):
        """Return the proper reduction_factor inequality constraint"""
        def reduction_factor_rule(model, i):
            return model.alpha[i] + sum(model.delta[i, j] * self.x[i][j] for j in model.J) >= 0
    ...
    def __translation_property2(self):
        ...
        def translation_rule2(model, i):
            return -self.basexyb[i] >=  model.epsilon[i]
