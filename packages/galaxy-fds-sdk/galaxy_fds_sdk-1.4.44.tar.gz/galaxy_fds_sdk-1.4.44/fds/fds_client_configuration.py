class FDSClientConfiguration(object):
  ...
  def set_endpoint(self, endpoint):
    ...
    if endpoint.endswith(self.URI_SUFFIX):
      suffix = self.URI_SUFFIX
    elif endpoint.endswith(self.URI_NET_SUFFIX):
      suffix = self.URI_NET_SUFFIX
    elif endpoint.endswith(self.URI_HOST_STYLE_COM_SUFFIX):
      suffix = self.URI_HOST_STYLE_COM_SUFFIX
      self._enable_https = True
    else:
      raise ValueError("illegal endpiont: " + endpoint)

    self._region_name = self._endpoint[0:len(endpoint) - len(suffix)]
    if self._cdn_endpoint == None and not self.shouldUseVirtualAddressing():
      self._cdn_endpoint = self.URI_CDN + '.' + self._region_name + self.URI_CDN_SUFFIX

  def get_endpoint(self):
    return self._endpoint

  def set_cdn_endpoint(self, endpoint):
    self._cdn_endpoint = endpoint
  ...
  def _build_base_uri(self, enable_cdn):
    if enable_cdn and not self._cdn_endpoint:
      raise Exception("Please use set_cdn_endpoint(cdnDomain) to set cdn domain first if you want to use cdn.")
  ...
  def shouldUseVirtualAddressing(self):
    return self._endpoint.endswith(self.URI_HOST_STYLE_COM_SUFFIX)
