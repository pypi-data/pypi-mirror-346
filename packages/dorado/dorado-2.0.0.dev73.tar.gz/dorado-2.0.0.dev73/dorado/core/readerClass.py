...
class aico_reader(reader):
    ...
    def mkceres(self,  date, sub = 'raw', target = None, calibrated = False, aligned = False):
            ...
            if len(biases) == 0:
                try: # If Dorado failed before to resolve the date, pull from ceres
                    bias = self.getBias(mjdstr) # find bias that are near in time
                except:
                    mjdstr = str(int(np.round(cere.date.mjd, 0)))
                    bias = self.getBias(mjdstr)
    ...
    def mkFlat(self, flats):
            ...
            if save:
                ...
                # start=datetime.now()
                flat = ccdproc.combine(flats, method='median', mem_limit=2e9, sigma_clip=True)
    ...
    def mkBias(self, biasIFC):
            ...
            if save:
                bias = ccdproc.combine(biasIFC, method = 'average', mem_limit=2e9) #, unit = Dorado.unit) ## NOTE edited
    ...
    def _save_phi(self, cr, phi, save_series, save_base):
        ...
        if save_series:
            ...
            for p in range(len(fildat.data)):
                image             = fildat.data[p]
                tstr              = Time(image.header['DATE-OBS'], format='fits', 
                                        out_subfmt='date_hms').strftime('%Y_%m_%d-%H_%M_%S%z')
