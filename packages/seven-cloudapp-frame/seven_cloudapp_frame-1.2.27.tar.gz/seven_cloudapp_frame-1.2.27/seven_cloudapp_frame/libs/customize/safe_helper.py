...
@LastEditTime: 2025-05-09 17:35:24
...
class SafeHelper:
    ...
    @classmethod
    def operate_current_limit_white_list(self, app_id, current_limit_count, operate_type=1):
        """
        ...
        redis_config = self.get_redis_config()
        redis_init = RedisExHelper.init(config_dict=redis_config, decode_responses=True)
        hash_name = f"current_limit_white_list:db_{project_name}"
        if operate_type == 1:
            redis_init.hset(hash_name, app_id, current_limit_count)
        else:
            redis_init.hdel(hash_name, app_id)
