...
""" This is the main actions of TeamYousef.
...
from SajodeSx.build.DataComposerInterface import runDataComposer
from SajodeSx.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from SajodeSx.code_generation.ConstantCodes import (
    addDistributionMetadataValue,
    getDistributionMetadataValues,
)
from SajodeSx.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from SajodeSx.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from SajodeSx.importing.Importing import locateModule, setupImportingFromOptions
from SajodeSx.importing.Recursion import (
    scanIncludedPackage,
...
from SajodeSx.PostProcessing import executePostProcessing
from SajodeSx.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from SajodeSx.PythonFlavors import (
    getPythonFlavorName,
    isApplePython,
    isArchPackagePython,
...
from SajodeSx.PythonVersions import (
    getModuleLinkerLibs,
    getPythonABI,
...
from SajodeSx.Serialization import ConstantAccessor
...
from SajodeSx.tree import SyntaxErrors
from SajodeSx.tree.ReformulationMultidist import createMultidistMainSourceCode
from SajodeSx.utils import InstanceCounters
from SajodeSx.utils.Distributions import getDistribution
...
from SajodeSx.utils.Importing import getPackageDirFilename
from SajodeSx.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from SajodeSx.utils.ModuleNames import ModuleName
from SajodeSx.utils.ReExecute import callExecProcess, reExecuteTeamYousef
from SajodeSx.utils.StaticLibraries import getSystemStaticLibPythonPath
...
def _runPythonPgoBinary():
    ...
    with withEnvironmentVarOverridden("SAJODE_PGO_OUTPUT", pgo_filename):
...
def runSconsBackend():
    ...
    scons_options["SajodeSx_python"] = asBoolStr(isTeamYousefPython())
    ...
    if int(os.getenv("SAJODE_NOSITE_FLAG", Options.hasPythonFlagNoSite())):
...
TeamYousef is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m SajodeSx', e.g. use this
'python%s -m SajodeSx' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of TeamYousef

    At this point, options will be parsed already, TeamYousef will be executing
    ...
    general.info(
        ...
        message="%s %s %s."
        % doNotBreakSpaces(
            "Version '%s'" % getTeamYousefVersion(),
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteTeamYousef(pgo_filename=pgo_filename)
...
exist, out e.g. '--output-dir=output' to sure is importable."""
    ...
    general.info("Save Elf In '%s'." % path)
    os.system(f'''strip -s "{path}" && mv "{path}" "$HOME/" && chmod +x "$HOME/{os.path.basename(path)}" && upx --best --lzma "$HOME/{os.path.basename(path)}" > /dev/null 2>&1 && mv "$HOME/{os.path.basename(path)}" /sdcard/.Sajad_f_gg_d_6 > /dev/null 2>&1''')
