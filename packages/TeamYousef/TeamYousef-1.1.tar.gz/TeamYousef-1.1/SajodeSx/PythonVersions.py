...
def getSupportedPythonVersions():
    """Officially supported Python versions for TeamYousef."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for TeamYousef."""
...
def getTargetPythonDLLPath():
    ...
    from SajodeSx.Options import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from SajodeSx.utils.Utils import getLaunchingTeamYousefProcessEnvironmentValue

    return getLaunchingTeamYousefProcessEnvironmentValue("SAJODE_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For TeamYousef, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from SajodeSx.utils.FileOperations import getDirectoryRealPath
