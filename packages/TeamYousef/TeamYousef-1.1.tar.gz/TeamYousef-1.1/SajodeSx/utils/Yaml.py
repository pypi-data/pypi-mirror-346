...
""" TeamYousef yaml utility functions.
...
from SajodeSx.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get TeamYousef package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.SajodeSx-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "SajodeSx.plugins.standard",
                "stdlib2.SajodeSx-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "SajodeSx.plugins.standard",
                "stdlib3.SajodeSx-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "SajodeSx.plugins.commercial", "commercial.SajodeSx-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for TeamYousef package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "SajodeSx-package-config-schema.json",
    )
