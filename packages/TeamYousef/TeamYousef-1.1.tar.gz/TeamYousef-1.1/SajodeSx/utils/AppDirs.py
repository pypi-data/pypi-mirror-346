...
def _getCacheDir():
    ...
    if _cache_dir is None:
        _cache_dir = os.getenv("SAJODE_CACHE_DIR")

        if _cache_dir:
            _cache_dir = os.path.expanduser(_cache_dir)
        elif appdirs is not None:
            _cache_dir = appdirs.user_cache_dir("TeamYousef", None)
        else:
            _cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "TeamYousef")
        ...
        if _cache_dir.startswith(
            ...
            _cache_dir = os.path.join(tempfile.gettempdir(), "TeamYousef")
...
'SAJODE_CACHE_DIR' set to a writable directory."""
                % _cache_dir
            )
...
def getCacheDirEnvironmentVariableName(cache_basename):
    ...
    return "SAJODE_CACHE_DIR_" + env_name
