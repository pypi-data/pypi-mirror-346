...
from SajodeSx import Tracing
from SajodeSx.__past__ import urlretrieve
from SajodeSx.Progress import withTeamYousefDownloadProgressBar
...
def getDownload(name, url, download_path):
    ...
    with withTeamYousefDownloadProgressBar(desc="Download %s" % name) as reporthook:
        try:
            try:
...
def getCachedDownload(
    ...
    SajodeSx_download_dir = getDownloadCacheDir()

    SajodeSx_download_dir = os.path.join(
        SajodeSx_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        SajodeSx_download_dir = os.path.join(SajodeSx_download_dir, is_arch_specific)

    if specificity:
        SajodeSx_download_dir = os.path.join(SajodeSx_download_dir, specificity)

    download_path = os.path.join(SajodeSx_download_dir, os.path.basename(url))
    exe_path = os.path.join(SajodeSx_download_dir, binary)

    makePath(SajodeSx_download_dir)
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, SajodeSx_download_dir)
...
def getCachedDownloadedMinGW64(target_arch, assume_yes_for_downloads, download_ok):
    ...
    gcc_binary = getCachedDownload(
        ...
        unzip=True,
        flatten=False,
        message="TeamYousef will use gcc from MinGW64 of winlibs to compile on Windows.",
        reject="Only this specific gcc is supported with TeamYousef.",
