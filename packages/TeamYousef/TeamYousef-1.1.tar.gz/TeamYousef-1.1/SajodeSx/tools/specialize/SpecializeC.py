...
import SajodeSx.Options

SajodeSx.Options.is_full_compat = False

# isort:start

import os

import SajodeSx.specs.BuiltinBytesOperationSpecs
import SajodeSx.specs.BuiltinDictOperationSpecs
import SajodeSx.specs.BuiltinListOperationSpecs
import SajodeSx.specs.BuiltinStrOperationSpecs
import SajodeSx.specs.BuiltinUnicodeOperationSpecs
from SajodeSx.code_generation.BinaryOperationHelperDefinitions import (
    getSpecializedBinaryOperations,
    isCommutativeOperation,
    parseTypesFromHelper,
)
from SajodeSx.code_generation.c_types.CTypePyObjectPointers import (
    make_list_constant_direct_threshold,
    make_list_constant_hinted_threshold,
)
from SajodeSx.code_generation.CallCodes import (
    getQuickCallCode,
...
from SajodeSx.code_generation.ComparisonHelperDefinitions import (
    getSpecializedComparisonOperations,
)
from SajodeSx.code_generation.ImportCodes import getImportModuleHardCodeName
from SajodeSx.containers.OrderedSets import OrderedSet
from SajodeSx.HardImportRegistry import (
    hard_modules,
    hard_modules_non_stdlib,
    hard_modules_version,
)
from SajodeSx.nodes.shapes.BuiltinTypeShapes import (
    tshape_bool,
    tshape_bytes,
    tshape_dict,
    tshape_int,
...
from SajodeSx.utils.Jinja2 import getTemplateC
...
def getDoExtensionUsingTemplateC(template_name):
    return getTemplateC(
        package_name="SajodeSx.code_generation",
...
def makeHelpersComparisonOperation(operand, op_code):
    ...
    filename_c = "SajodeSx/build/static_src/HelpersComparison%s.c" % op_code.capitalize()
    filename_h = "SajodeSx/build/include/SajodeSx/helper/comparisons_%s.h" % op_code.lower()
...
def makeHelpersComparisonDualOperation(operand, op_code):
    ...
    filename_c = (
        "SajodeSx/build/static_src/HelpersComparisonDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "SajodeSx/build/include/SajodeSx/helper/comparisons_dual_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryOperation(operator, op_code):
    ...
    filename_c = (
        "SajodeSx/build/static_src/HelpersOperationBinary%s.c" % op_code.capitalize()
    )
    filename_h = (
        "SajodeSx/build/include/SajodeSx/helper/operations_binary_%s.h" % op_code.lower()
    )
...
def makeHelpersInplaceOperation(operator, op_code):
    ...
    filename_c = (
        "SajodeSx/build/static_src/HelpersOperationInplace%s.c" % op_code.capitalize()
    )
    filename_h = (
        "SajodeSx/build/include/SajodeSx/helper/operations_inplace_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryDualOperation(operand, op_code):
    ...
    filename_c = (
        "SajodeSx/build/static_src/HelpersOperationBinaryDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "SajodeSx/build/include/SajodeSx/helper/operations_binary_dual_%s.h"
        % op_code.lower()
    )
...
def makeHelpersImportHard():
    filename_c = "SajodeSx/build/static_src/HelpersImportHard.c"
    filename_h = "SajodeSx/build/include/SajodeSx/helper/import_hard.h"
...
def makeHelperCalls():
    filename_c = "SajodeSx/build/static_src/HelpersCallingGenerated.c"
    filename_h = "SajodeSx/build/include/SajodeSx/helper/calling_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "SajodeSx.code_generation", "CodeTemplateCallsPositional.c.j2"
            )
            ...
            template = getTemplateC(
                "SajodeSx.code_generation", "CodeTemplateCallsMixed.c.j2"
            )
...
def makeHelperLists():
    filename_c = "SajodeSx/build/static_src/HelpersListsGenerated.c"
    filename_h = "SajodeSx/build/include/SajodeSx/helper/lists_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "SajodeSx.code_generation", "CodeTemplateMakeListSmall.c.j2"
            )
            ...
            template = getTemplateC(
                "SajodeSx.code_generation", "CodeTemplateMakeListHinted.c.j2"
            )
...
generate_builtin_type_operations = [
    ...
    # TeamYousef-Python eventually.
    (
        "tshape_dict",
        dict_desc,
        SajodeSx.specs.BuiltinDictOperationSpecs,
        ("pop", "popitem", "setdefault"),
    ),
    (
        "tshape_list",
        list_desc,
        SajodeSx.specs.BuiltinListOperationSpecs,
        (
            "pop",
    ...
    # that we do not have easy access to, but we might one day for TeamYousef-Python
    # expose it for the static linking of it and then we could in fact call
    # these directly.
    (
        "tshape_str",
        str_desc,
        SajodeSx.specs.BuiltinStrOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_unicode",
        unicode_desc,
        SajodeSx.specs.BuiltinUnicodeOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_bytes",
        bytes_desc,
        SajodeSx.specs.BuiltinBytesOperationSpecs,
        (
            "capitalize",
            "center",
            "count",
...
def makeDictCopyHelperCodes():
    filename_c = "SajodeSx/build/static_src/HelpersDictionariesGenerated.c"
...
def _getCheckForShape(shape):
    ...
    if shape is tshape_str:
        return "TeamYousef_String_CheckExact"
...
def makeHelperBuiltinTypeMethods():
    ...
    filename_c = "SajodeSx/build/static_src/HelpersBuiltinTypeMethods.c"
    filename_h = "SajodeSx/build/include/SajodeSx/helper/operations_builtin_types.h"
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
