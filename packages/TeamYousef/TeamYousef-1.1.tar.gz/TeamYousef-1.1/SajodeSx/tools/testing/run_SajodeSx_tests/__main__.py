...
""" Main front-end to the tests of TeamYousef.
...
from SajodeSx.tools.Basics import goHome
...
from SajodeSx.utils.Execution import check_call, check_output, getNullOutput
...
from SajodeSx.utils.Timing import TimerReport
from SajodeSx.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS
...
def parseOptions():
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if TeamYousef is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if TeamYousef handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if TeamYousef handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if TeamYousef handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if TeamYousef handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if TeamYousef does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if TeamYousef standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if TeamYousef works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles TeamYousef with TeamYousef, and then TeamYousef with the
compile TeamYousef and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow TeamYousef to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
Enforce the use of MinGW64 on Windows. Defaults to off.""",
    ...
    parser.add_option(
        ...
        default=None,
        help="""\
The value of --jobs to use when calling TeamYousef. Use negative values to
keep cores unused. TeamYousef uses available cores by default. Defaults to
not being passed.""",
    )
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            SajodeSx_id = check_output("git rev-parse HEAD".split())
        SajodeSx_id = SajodeSx_id.strip()

        if sys.version_info > (3,):
            SajodeSx_id = SajodeSx_id.decode()

        data_file.write("SAJODE_SOURCE_DIR='%s'\n" % source_dir)
        data_file.write("SAJODE_COMMIT='%s'\n" % SajodeSx_id)
