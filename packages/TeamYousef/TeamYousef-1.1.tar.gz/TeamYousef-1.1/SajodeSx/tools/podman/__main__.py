...
from SajodeSx.tools.release.Release import (
    getBranchName,
...
from SajodeSx.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("TeamYousef-Containers", base_style="blue")
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m SajodeSx --version"
    ...
    container_tag_name = "SajodeSx-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.SajodeSx.net:116.202.30.188")
    else:
