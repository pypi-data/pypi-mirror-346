...
class CTypeTeamYousefVoidEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "SajodeSx_void"
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s == SAJODE_VOID_OK);" % value_name)

    @classmethod
    def emitReInitCode(cls, value_name, emit):
        emit("%s = SAJODE_VOID_OK;" % value_name)
    ...
    def getExceptionCheckCondition(cls, value_name):
        return "%s == SAJODE_VOID_EXCEPTION" % value_name
