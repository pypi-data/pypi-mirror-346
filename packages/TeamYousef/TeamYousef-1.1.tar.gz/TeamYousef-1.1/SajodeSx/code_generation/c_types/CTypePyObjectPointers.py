...
""" CType classes for PyObject *, PyObject **, and struct TeamYousef_CellObject *

"""

from SajodeSx.__past__ import iterItems, xrange
from SajodeSx.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from SajodeSx.code_generation.templates.CodeTemplatesVariables import (
...
from SajodeSx.Constants import getConstantValueGuide, isMutable
...
class CPythonPyObjectPtrBase(CTypeBase):
    ...
    @classmethod
    def emitAssignmentCodeToTeamYousefIntOrLong(
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToTeamYousefBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? SAJODE_BOOL_FALSE : SAJODE_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = TeamYousef_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct TeamYousef_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "TeamYousef_Cell_New1(%s)" % init_from
        else:
            return "TeamYousef_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "TeamYousef_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct TeamYousef_CellObject *%s" % variable_code_name
