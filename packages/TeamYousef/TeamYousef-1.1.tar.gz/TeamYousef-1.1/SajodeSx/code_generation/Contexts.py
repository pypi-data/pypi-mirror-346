...
from SajodeSx.__past__ import iterItems
from SajodeSx.Constants import isMutable
from SajodeSx.PythonVersions import python_version
from SajodeSx.Serialization import ConstantAccessor
from SajodeSx.utils.Hashing import getStringHash
from SajodeSx.utils.InstanceCounters import (
    counted_del,
    counted_init,
    isCountingInstances,
)
...
class TempMixin(object):
    ...
    def allocateTempName(self, base_name, type_name="PyObject *", unique=False):
        ...
        if unique:
            ...
            if result is None:
                ...
                if type_name.startswith("SAJODE_MAY_BE_UNUSED"):
    ...
    def allocateExceptionKeeperVariables(self):
        ...
        if debug:
            keeper_obj_init = "Empty_TeamYousef_ExceptionPreservationItem"
        else:
    ...
    def addExceptionPreserverVariables(self, preserver_id):
        ...
        if preserver_id not in self.preserver_variable_declaration:
            ...
            if needs_init:
                preserver_obj_init = "TeamYousef_ExceptionStackItem_Empty"
            else:
    ...
    def addCleanupTempName(self, tmp_name):
        ...
        assert (
            tmp_name.c_type != "SajodeSx_void" or tmp_name.code_name == "tmp_unused"
        ), tmp_name
    ...
    def pushFrameHandle(self, frame_code_name, is_light):
        ...
        if is_light:
            ...
            frame_identifier = self.variable_storage.addVariableDeclarationTop(
                "struct TeamYousef_FrameObject *", frame_handle, None
            )

        self.variable_storage.addVariableDeclarationTop(
            "SAJODE_MAY_BE_UNUSED char const *",
