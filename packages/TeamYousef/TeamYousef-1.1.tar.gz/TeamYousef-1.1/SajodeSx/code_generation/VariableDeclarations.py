...
from .c_types.CTypeTeamYousefBooleans import CTypeTeamYousefBoolEnum
from .c_types.CTypeTeamYousefInts import CTypeTeamYousefIntOrLongStruct
from .c_types.CTypeTeamYousefVoids import CTypeTeamYousefVoidEnum
...
class VariableDeclaration(object):
    ...
    def __init__(self, c_type, code_name, init_value, heap_name):
        if c_type.startswith("SAJODE_MAY_BE_UNUSED"):
    ...
    def makeCFunctionLevelDeclaration(self):
        ...
        return "%s%s%s%s%s%s;" % (
            "SAJODE_MAY_BE_UNUSED " if self.maybe_unused else "",
            lead_c_type,
    ...
    def getCType(self):
        ...
        elif c_type == "struct TeamYousef_CellObject *":
            return CTypeCellObject
        elif c_type == "PyObject **":
            return CTypePyObjectPtrPtr
        elif c_type == "SajodeSx_bool":
            return CTypeTeamYousefBoolEnum
        elif c_type == "bool":
            return CTypeBool
        elif c_type == "SajodeSx_ilong":
            return CTypeTeamYousefIntOrLongStruct
        elif c_type == "module_var":
            return CTypeModuleDictVariable
        elif c_type == "SajodeSx_void":
            return CTypeTeamYousefVoidEnum
        elif c_type == "long":
            return CTypeCLong
        elif c_type == "SajodeSx_digit":
...
class VariableStorage(object):
    ...
    def addFrameCacheDeclaration(self, frame_identifier):
        return self.addVariableDeclarationFunction(
            "static struct TeamYousef_FrameObject *", "cache_%s" % frame_identifier, "NULL"
        )
        ...
        if self.exception_variable_name is None:
            self.exception_variable_name = (
                self.addVariableDeclarationTop(
                    ...
                    "exception_state",
                    "Empty_TeamYousef_ExceptionPreservationItem",
                ),
                self.addVariableDeclarationTop(
                    "SAJODE_MAY_BE_UNUSED int", "exception_lineno", "0"
                ),
            )
