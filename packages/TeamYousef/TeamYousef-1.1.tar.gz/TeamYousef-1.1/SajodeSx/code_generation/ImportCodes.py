...
from SajodeSx.HardImportRegistry import isHardModule, isHardModuleDynamic
from SajodeSx.nodes.LocalsScopes import GlobalsDictHandle
...
def generateConstantSysVersionInfoCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        ...
        emit("""%s = TeamYousef_SysGetObject("%s");""" % (value_name, "version_info"))
...
def getImportModuleNameHardCode(
    ...
    if module_name == "sys":
        emit("""%s = TeamYousef_SysGetObject("%s");""" % (to_name, import_name))
        needs_release = False
