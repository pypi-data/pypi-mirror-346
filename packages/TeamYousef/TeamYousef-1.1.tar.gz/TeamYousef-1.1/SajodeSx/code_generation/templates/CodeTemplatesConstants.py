...
PyObject *TeamYousef_sentinel_value = NULL;

PyObject *TeamYousef_dunder_compiled_value = NULL;


#if _SAJODE_STANDALONE_MODE
extern PyObject *getStandaloneSysExecutablePath(PyObject *basename);

SAJODE_MAY_BE_UNUSED static PyObject *STRIP_DIRNAME(PyObject *path) {
...
static void _createGlobalConstants(PyThreadState *tstate) {
#endif
    // We provide the sys.version info shortcut as a global value here for ease of use.
    Py_SysVersionInfo = TeamYousef_SysGetObject("version_info");
    ...
    static PyTypeObject TeamYousef_VersionInfoType;

    // Same fields as "sys.version_info" except no serial number
    // spell-checker: ignore releaselevel
    static PyStructSequence_Field TeamYousef_VersionInfoFields[] = {
    ...
    static PyStructSequence_Desc TeamYousef_VersionInfoDesc = {
        (char *)"__SajodeSx_version__",                                       /* name */
        (char *)"__compiled__\\n\\nVersion information as a named tuple.",  /* doc */
        TeamYousef_VersionInfoFields,                                           /* fields */
        sizeof(TeamYousef_VersionInfoFields) / sizeof(PyStructSequence_Field)-1 /* count */
    };

    PyStructSequence_InitType(&TeamYousef_VersionInfoType, &TeamYousef_VersionInfoDesc);

    TeamYousef_dunder_compiled_value = PyStructSequence_New(&TeamYousef_VersionInfoType);
    assert(TeamYousef_dunder_compiled_value != NULL);

    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 0, TeamYousef_PyInt_FromLong(%(SajodeSx_version_major)s));
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 1, TeamYousef_PyInt_FromLong(%(SajodeSx_version_minor)s));
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 2, TeamYousef_PyInt_FromLong(%(SajodeSx_version_micro)s));

    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 3, TeamYousef_String_FromString("%(SajodeSx_version_level)s"));

    PyObject *binary_directory = getContainingDirectoryObject(false);
#if _SAJODE_STANDALONE_MODE
#if !_SAJODE_ONEFILE_MODE
    ...
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 4, binary_directory);
...
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 5, is_standalone_mode);
#ifdef _SAJODE_ONEFILE_MODE
    PyObject *is_onefile_mode = Py_True;
#else
    PyObject *is_onefile_mode = Py_False;
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 6, is_onefile_mode);
...
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 7, is_macos_bundle_mode);

#if _SAJODE_NO_ASSERTS == 1
    PyObject *is_no_asserts = Py_True;
#else
    PyObject *is_no_asserts = Py_False;
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 8, is_no_asserts);

#if _SAJODE_NO_DOCSTRINGS == 1
    PyObject *is_no_docstrings = Py_True;
#else
    PyObject *is_no_docstrings = Py_False;
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 9, is_no_docstrings);

#if _SAJODE_NO_ANNOTATIONS == 1
    PyObject *is_no_annotations = Py_True;
#else
    PyObject *is_no_annotations = Py_False;
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 10, is_no_annotations);
...
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 11, is_module_mode);
    ...
    if (_Py_PackageContext != NULL) {
        main_name = TeamYousef_String_FromString(_Py_PackageContext);
    } else {
        main_name = TeamYousef_String_FromString(%(module_name_cstr)s);
    }
#else
    main_name = real_module_name;
#endif
#else
    PyObject *main_name = TeamYousef_String_FromString("__main__");
#endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 12, main_name);
...
# endif
    PyStructSequence_SET_ITEM(TeamYousef_dunder_compiled_value, 13, original_argv0);

    // Prevent users from creating the TeamYousef version type object.
    TeamYousef_VersionInfoType.tp_init = NULL;
    TeamYousef_VersionInfoType.tp_new = NULL;
...
#endif
    if (TeamYousef_sentinel_value == NULL) {
#if PYTHON_VERSION < 0x300
        TeamYousef_sentinel_value = PyCObject_FromVoidPtr(NULL, NULL);
#else
        // The NULL value is not allowed for a capsule, so use something else.
        TeamYousef_sentinel_value = PyCapsule_New((void *)27, "sentinel", NULL);
#endif
        assert(TeamYousef_sentinel_value);

        Py_SET_REFCNT_IMMORTAL(TeamYousef_sentinel_value);
