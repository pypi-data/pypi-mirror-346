...
static PyObject *%(function_identifier)s_context(PyThreadState *tstate, struct TeamYousef_GeneratorObject *generator, PyObject *yield_return_value) {
...
static PyObject *%(generator_maker_identifier)s(%(generator_creation_args)s) {
    return TeamYousef_Generator_New(
...
%(closure_copy)s
%(to_name)s = TeamYousef_Generator_NewEmpty(
    %(generator_module)s,
    %(generator_name_obj)s,
#if PYTHON_VERSION >= 0x350
    %(generator_qualname_obj)s,
...
template_generator_return_exit = """\
    SAJODE_CANNOT_GET_HERE("Generator must have exited already.");
    return NULL;
