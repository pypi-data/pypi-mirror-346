...
Plugins: Welcome to TeamYousef! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper TeamYousef, and some of which are waiting to be created and submitted for
inclusion by you.
...
from SajodeSx import Options, OutputDirectories
from SajodeSx.__past__ import basestring, iter_modules
from SajodeSx.build.DataComposerInterface import deriveModuleConstantsBlobName
from SajodeSx.containers.OrderedDicts import OrderedDict
from SajodeSx.containers.OrderedSets import OrderedSet
from SajodeSx.Errors import TeamYousefSyntaxError
from SajodeSx.freezer.IncludedDataFiles import IncludedDataFile
from SajodeSx.freezer.IncludedEntryPoints import IncludedEntryPoint
from SajodeSx.ModuleRegistry import addUsedModule
from SajodeSx.PythonVersions import python_version
from SajodeSx.Tracing import plugins_logger, printLine
from SajodeSx.utils.CommandLineOptions import OurOptionGroup
...
from SajodeSx.utils.Importing import importFileAsModule
from SajodeSx.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import TeamYousefPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, TeamYousefPluginBase), plugin_instance
...
def _loadPluginClassesFromPackage(scan_package):
    ...
    for item in iter_modules(scan_path):
        ...
        try:
            ...
            # it was compiled with TeamYousef.
            pass
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "TeamYousefPluginDetector", ""
            ) != plugin_class.__name__.replace("TeamYousefPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match TeamYousefPlugin* and TeamYousefPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import SajodeSx.plugins.standard

    _loadPluginClassesFromPackage(SajodeSx.plugins.standard)

    try:
        import SajodeSx.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(SajodeSx.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from SajodeSx.importing.Importing import getModuleNameAndKindFromFilename
        from SajodeSx.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from SajodeSx.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        if parent_package_name is not None:
            from SajodeSx.importing.Importing import locateModule
        ...
        def getModuleSourceCode():
            ...
            from SajodeSx.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from SajodeSx.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from SajodeSx.ModuleRegistry import getDoneModules
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    ...
                    if not key.startswith("SajodeSx_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from SajodeSx.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in TeamYousef".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not TeamYousefPluginBase
            and issubclass(obj, TeamYousefPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or TeamYousef arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of TeamYousef plugin '%s' not given."
