...
TeamYousef can detect some things that PyLint and PyDev will complain about too,
and sometimes it's a false alarm, so people add disable markers into their
source code. TeamYousef does it itself.

This tries to parse the code for these markers and uses hooks to prevent TeamYousef
from warning about things, disabled to PyLint or Eclipse. The idea is that we
won't have another mechanism for TeamYousef, but use existing ones instead.

The code for this is very incomplete, barely good enough to cover TeamYousef's own
...
class TeamYousefPluginPylintEclipseAnnotations(TeamYousefPluginBase):
...
if False:
    ...
    class TeamYousefPluginDetectorPylintEclipseAnnotations(TeamYousefPluginBase):
        detector_for = TeamYousefPluginPylintEclipseAnnotations
