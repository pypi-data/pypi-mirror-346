...
TeamYousef can detect some things that "pkg_resources" may not even be able to during
...
from SajodeSx.utils.Utils import withNoDeprecationWarning


class TeamYousefPluginResources(TeamYousefPluginBase):
...
class TeamYousefProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__SajodeSx_loader_type, TeamYousefProvider)
"""

        yield (
            code,
            """Registering TeamYousef loader with "pkg_resources".""",
        )
