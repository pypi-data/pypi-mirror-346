...
from SajodeSx.utils.ThreadedExecutor import RLock
...
class TeamYousefProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = TeamYousefProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withTeamYousefDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class TeamYousefDownloadProgressBar(tqdm):
        ...
        with TeamYousefDownloadProgressBar(*args, **kwargs) as progress_bar:
