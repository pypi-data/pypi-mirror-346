...
#if _DEBUG_REFCOUNTS
int count_active_TeamYousef_Frame_Type = 0;
int count_allocated_TeamYousef_Frame_Type = 0;
int count_released_TeamYousef_Frame_Type = 0;
#endif
...
static PyMemberDef TeamYousef_Frame_members[] = {
...
#else
#define TeamYousef_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_TeamYousef_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct TeamYousef_FrameObject *frame = (struct TeamYousef_FrameObject *)self;
...
static int _TeamYousef_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_TeamYousef_Frame_get_exc_type(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _TeamYousef_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_TeamYousef_Frame_get_exc_value(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _TeamYousef_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_TeamYousef_Frame_get_restricted(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_TeamYousef_Frame_get_locals(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct TeamYousef_FrameObject *SajodeSx_frame = (struct TeamYousef_FrameObject *)self;
    if (SajodeSx_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &SajodeSx_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &SajodeSx_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = TeamYousef_GetCodeVarNames(TeamYousef_GetFrameCodeObject(SajodeSx_frame));

        char const *w = SajodeSx_frame->m_type_description;
        char const *t = SajodeSx_frame->m_locals_storage;

        while (*w != 0) {
            switch (*w) {
            case SAJODE_TYPE_DESCRIPTION_OBJECT:
            case SAJODE_TYPE_DESCRIPTION_OBJECT_PTR: {
            ...
            case SAJODE_TYPE_DESCRIPTION_CELL: {
                struct TeamYousef_CellObject *value = *(struct TeamYousef_CellObject **)t;
                assert(TeamYousef_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct TeamYousef_CellObject *);

                break;
            }
            case SAJODE_TYPE_DESCRIPTION_NULL: {
                break;
            }
            case SAJODE_TYPE_DESCRIPTION_BOOL: {
                int value = *(int *)t;
                t += sizeof(int);
                switch ((SajodeSx_bool)value) {
                case SAJODE_BOOL_TRUE: {
                    DICT_SET_ITEM(result, *var_names, Py_True);
                    break;
                }
                case SAJODE_BOOL_FALSE: {
...
static PyObject *_TeamYousef_Frame_get_lineno(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct TeamYousef_FrameObject *frame = (struct TeamYousef_FrameObject *)self;
    return TeamYousef_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_TeamYousef_Frame_get_trace(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _TeamYousef_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
#if !defined(_SAJODE_DEPLOYMENT_MODE) && !defined(_SAJODE_NO_DEPLOYMENT_FRAME_USELESS_SET_TRACE)
    if (value == Py_None) {
        return 0;
    } else {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in TeamYousef, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_TeamYousef_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _TeamYousef_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in TeamYousef");
    return -1;
}

static PyObject *_TeamYousef_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _TeamYousef_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(TeamYousef_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in TeamYousef");
    return -1;
}
#endif

static PyGetSetDef TeamYousef_Frame_tp_getset[] = {
    {(char *)"f_locals", _TeamYousef_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _TeamYousef_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _TeamYousef_Frame_get_trace, _TeamYousef_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _TeamYousef_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _TeamYousef_Frame_get_exc_traceback, _TeamYousef_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _TeamYousef_Frame_get_exc_type, _TeamYousef_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _TeamYousef_Frame_get_exc_value, _TeamYousef_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _TeamYousef_Frame_get_trace_lines, _TeamYousef_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _TeamYousef_Frame_get_trace_opcodes, _TeamYousef_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *TeamYousef_Frame_tp_repr(struct TeamYousef_FrameObject *SajodeSx_frame) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)SajodeSx_frame));
    CHECK_OBJECT((PyObject *)SajodeSx_frame);
    assert(_PyObject_GC_IS_TRACKED(SajodeSx_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = TeamYousef_GetFrameCodeObject(SajodeSx_frame);
    return TeamYousef_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", SajodeSx_frame,
                                    code_object->co_filename, TeamYousef_GetFrameLineNumber(SajodeSx_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = TeamYousef_GetFrameCodeObject(SajodeSx_frame);
    return TeamYousef_String_FromFormat("<compiled_frame object for %s at %p>",
                                    TeamYousef_String_AsString(code_object->co_name), SajodeSx_frame);
#else
    return TeamYousef_String_FromFormat("<compiled_frame object at %p>", SajodeSx_frame);
#endif
}

static void TeamYousef_Frame_tp_clear(struct TeamYousef_FrameObject *frame) {
    if (frame->m_type_description) {
        ...
        while (*w != 0) {
            ...
            case SAJODE_TYPE_DESCRIPTION_OBJECT:
            case SAJODE_TYPE_DESCRIPTION_OBJECT_PTR:
            case SAJODE_TYPE_DESCRIPTION_NILONG: {
                PyObject *value = *(PyObject **)t;
                CHECK_OBJECT_X(value);
...
static struct TeamYousef_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void TeamYousef_Frame_tp_dealloc(struct TeamYousef_FrameObject *SajodeSx_frame) {
#if _DEBUG_REFCOUNTS
    count_active_TeamYousef_Frame_Type -= 1;
    count_released_TeamYousef_Frame_Type += 1;
#endif
    ...
    TeamYousef_GC_UnTrack(SajodeSx_frame);

    PyFrameObject *frame = &SajodeSx_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    assert(TeamYousef_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    TeamYousef_Frame_tp_clear(SajodeSx_frame);

    if (Py_REFCNT(SajodeSx_frame) > 0) {
        Py_SET_REFCNT(SajodeSx_frame, Py_REFCNT(SajodeSx_frame) - 1);
        if (Py_REFCNT(SajodeSx_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(SajodeSx_frame, SajodeSx_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, SajodeSx_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static int TeamYousef_Frame_tp_traverse(struct TeamYousef_FrameObject *frame, visitproc visit, void *arg) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)frame));
    ...
    while (w != NULL && *w != 0) {
        ...
        default:
            SAJODE_CANNOT_GET_HERE("invalid type description");
            assert(false);
        }
...
static PyObject *TeamYousef_GetFrameGenerator(struct TeamYousef_FrameObject *SajodeSx_frame) {
#if PYTHON_VERSION < 0x3b0
    return SajodeSx_frame->m_frame.f_gen;
#else
    return SajodeSx_frame->m_generator;
#endif
}

static PyObject *TeamYousef_Frame_clear(struct TeamYousef_FrameObject *frame, PyObject *unused) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (TeamYousef_Frame_IsExecuting(frame)) {
    ...
    if (frame->m_frame_state == FRAME_COMPLETED) {
        TeamYousef_Frame_tp_clear(frame);
    ...
    PyObject *f_gen = TeamYousef_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (TeamYousef_Generator_Check(f_gen)) {
            struct TeamYousef_GeneratorObject *generator = (struct TeamYousef_GeneratorObject *)f_gen;
            TeamYousef_SetFrameGenerator(frame, NULL);

            close_exception = !_TeamYousef_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (TeamYousef_Coroutine_Check(f_gen)) {
            struct TeamYousef_CoroutineObject *coroutine = (struct TeamYousef_CoroutineObject *)f_gen;
            TeamYousef_SetFrameGenerator(frame, NULL);

            close_exception = !_TeamYousef_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (TeamYousef_Asyncgen_Check(f_gen)) {
            struct TeamYousef_AsyncgenObject *asyncgen = (struct TeamYousef_AsyncgenObject *)f_gen;
            TeamYousef_SetFrameGenerator(frame, NULL);

            close_exception = !_TeamYousef_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t TeamYousef_Frame_GetSize(struct TeamYousef_FrameObject *frame) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *TeamYousef_Frame_sizeof(struct TeamYousef_FrameObject *frame, PyObject *unused) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct TeamYousef_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef TeamYousef_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)TeamYousef_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)TeamYousef_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject TeamYousef_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct TeamYousef_FrameObject),
    1,
    (destructor)TeamYousef_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)TeamYousef_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)TeamYousef_Frame_tp_traverse,  // tp_traverse
    (inquiry)TeamYousef_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    TeamYousef_Frame_methods,                    // tp_methods
    TeamYousef_Frame_members,                    // tp_members
    TeamYousef_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(TeamYousef_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(TeamYousef_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(TeamYousef_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(TeamYousef_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(TeamYousef_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(TeamYousef_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(TeamYousef_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(TeamYousef_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(TeamYousef_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(TeamYousef_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(TeamYousef_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(TeamYousef_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(TeamYousef_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(TeamYousef_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(TeamYousef_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(TeamYousef_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(TeamYousef_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(TeamYousef_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(TeamYousef_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(TeamYousef_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(TeamYousef_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(TeamYousef_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(TeamYousef_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(TeamYousef_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(TeamYousef_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    TeamYousef_PyType_Ready(&TeamYousef_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct TeamYousef_FrameObject, m_frame) == 0);
}

static struct TeamYousef_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_TeamYousef_Frame_Type += 1;
    count_allocated_TeamYousef_Frame_Type += 1;
#endif
    ...
    struct TeamYousef_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct TeamYousef_FrameObject, TeamYousef_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    TeamYousef_SetFrameGenerator(result, NULL);

    TeamYousef_Frame_MarkAsNotExecuting(result);
#endif
...
struct TeamYousef_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct TeamYousef_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct TeamYousef_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(TeamYousef_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(TeamYousef_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        SAJODE_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("SajodeSx_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        SAJODE_MAY_BE_UNUSED bool bool_res = TeamYousef_DelModuleString(tstate, "SajodeSx_empty_function");
    ...
    if (result == NULL) {
        PyErr_PrintEx(0);
        SAJODE_CANNOT_GET_HERE("Failed to create code object");
    }
...
void TeamYousef_Frame_AttachLocals(struct TeamYousef_FrameObject *frame_object, char const *type_description, ...) {
    assert(TeamYousef_Frame_CheckExact((PyObject *)frame_object));
    ...
    PRINT_FORMAT("Attaching to frame 0x%lx %s\n", frame_object,
                 TeamYousef_String_AsString(PyObject_Repr((PyObject *)TeamYousef_Frame_GetCodeObject(&frame_object->m_frame))));
#endif
    ...
    while (*w != 0) {
        ...
        case SAJODE_TYPE_DESCRIPTION_CELL: {
            struct TeamYousef_CellObject *value = va_arg(ap, struct TeamYousef_CellObject *);
            assert(TeamYousef_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct TeamYousef_CellObject *));
            ...
            // Py_SET_REFCNT((struct TeamYousef_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case SAJODE_TYPE_DESCRIPTION_NULL: {
            SAJODE_MAY_BE_UNUSED void *value = va_arg(ap, struct TeamYousef_CellObject *);
        ...
        case SAJODE_TYPE_DESCRIPTION_NILONG: {
            SajodeSx_ilong value = va_arg(ap, SajodeSx_ilong);
    ...
    assert(t - frame_object->m_locals_storage <= TeamYousef_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)TeamYousef_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, TeamYousef_String_AsString(current_repr), Py_REFCNT(current),
                     TeamYousef_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)TeamYousef_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct TeamYousef_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, TeamYousef_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)TeamYousef_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
