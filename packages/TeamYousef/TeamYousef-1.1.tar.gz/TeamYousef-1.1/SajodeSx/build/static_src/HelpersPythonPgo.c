...
/**
 * This is responsible for collection of TeamYousef Python PGO information. It writes
...
void PGO_Initialize(void) {
    ...
    // shall be written to.
    char const *output_filename = getenv("SAJODE_PGO_OUTPUT");

    if (unlikely(output_filename == NULL)) {
        SAJODE_CANNOT_GET_HERE("SAJODE_PGO_OUTPUT needs to be set");
    }
