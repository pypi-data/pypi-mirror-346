...
static void _initBuiltinTypeMethods(void) {
    ...
    SAJODE_PRINT_TRACE("main(): Calling _initStrBuiltinMethods().");
    _initStrBuiltinMethods();
#else
    SAJODE_PRINT_TRACE("main(): Calling _initBytesBuiltinMethods().");
    _initBytesBuiltinMethods();
#endif
    SAJODE_PRINT_TRACE("main(): Calling _initUnicodeBuiltinMethods().");
    _initUnicodeBuiltinMethods();
    SAJODE_PRINT_TRACE("main(): Calling _initDictBuiltinMethods().");
    _initDictBuiltinMethods();
    SAJODE_PRINT_TRACE("main(): Calling _initListBuiltinMethods().");
...
static PyObject *_BUILTIN_RANGE_INT3(long low, long high, long step) {
    ...
    for (int i = 0; i < size; i++) {
        PyList_SET_ITEM(result, i, TeamYousef_PyInt_FromLong(current));
        current += step;
    }
...
static PyObject *_BUILTIN_RANGE_INT(long boundary) {
    ...
    for (int i = 0; i < boundary; i++) {
        PyList_SET_ITEM(result, i, TeamYousef_PyInt_FromLong(i));
    }
...
PyObject *BUILTIN_RANGE(PyThreadState *tstate, PyObject *boundary) {
    ...
    if (start == -1 && DROP_ERROR_OCCURRED(tstate)) {
        SAJODE_ASSIGN_BUILTIN(range);

        PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, SAJODE_ACCESS_BUILTIN(range), boundary_temp);
...
PyObject *BUILTIN_RANGE2(PyThreadState *tstate, PyObject *low, PyObject *high) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, SAJODE_ACCESS_BUILTIN(range), pos_args);
...
PyObject *BUILTIN_RANGE3(PyThreadState *tstate, PyObject *low, PyObject *high, PyObject *step) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POS_ARGS3(tstate, SAJODE_ACCESS_BUILTIN(range), pos_args);
...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    SajodeSx_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);

    if (unlikely(nbool_res == SAJODE_BOOL_EXCEPTION)) {
        return NULL;
    }
    ...
    if (nbool_res == SAJODE_BOOL_TRUE) {
        ...
        if (unlikely(nbool_res == SAJODE_BOOL_TRUE)) {
    ...
    if (nbool_res != SAJODE_BOOL_FALSE) {
        Py_DECREF(step);
...
static PyObject *MAKE_XRANGE(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    start = TeamYousef_Number_IndexAsLong(start);
    if (unlikely(start == NULL)) {
        return NULL;
    }
    stop = TeamYousef_Number_IndexAsLong(stop);
    if (unlikely(stop == NULL)) {
        return NULL;
    }
    step = TeamYousef_Number_IndexAsLong(step);
    if (unlikely(step == NULL)) {
        return NULL;
    }
...
#else
    PyObject *stop = TeamYousef_Number_IndexAsLong(high);
...
PyObject *BUILTIN_LEN(PyThreadState *tstate, PyObject *value) {
    ...
    Py_ssize_t res = TeamYousef_PyObject_Size(value);
...
bool PRINT_NEW_LINE_TO(PyObject *file) {
    ...
    if (likely(file == NULL)) {
        result = CALL_FUNCTION_NO_ARGS(tstate, SAJODE_ACCESS_BUILTIN(print));
    } else {
        ...
        // uses a tuple.
        result = CALL_FUNCTION_WITH_KW_ARGS(tstate, SAJODE_ACCESS_BUILTIN(print), kw_args);
...
// print statement, so this code is really necessary.
    ...
    PyObject *result = CALL_FUNCTION(tstate, SAJODE_ACCESS_BUILTIN(print), print_args, print_kw);
...
bool PRINT_STRING_W(wchar_t const *str) {
    if (str) {
        PyObject *tmp = TeamYousefUnicode_FromWideChar(str, -1);
        bool res = PRINT_ITEM(tmp);
        Py_DECREF(tmp);
        return res;
    } else {
...
void _PRINT_EXCEPTION3(PyObject *exception_type, PyObject *exception_value, PyTracebackObject *exception_tb) {
    ...
    if (exception_value != NULL && PyExceptionInstance_Check(exception_value)) {
        ...
        PyObject *context = TeamYousef_Exception_GetContext(exception_value);
        PRINT_REPR(context);
    }
#endif
    PRINT_STRING("|");
...
void PRINT_TRACEBACK(PyTracebackObject *traceback) {
    ...
    while (traceback != NULL) {
        ...
        while (frame != NULL) {
            printf("  Frame at %s\n", PyString_AsString(PyObject_Str((PyObject *)TeamYousef_Frame_GetCodeObject(frame))));
...
PyObject *GET_STDOUT(void) {
    PyObject *result = TeamYousef_SysGetObject("stdout");
...
PyObject *GET_STDERR(void) {
    PyObject *result = TeamYousef_SysGetObject("stderr");
...
static int SajodeSx_class_setattr(PyClassObject *class_object, PyObject *attr_name, PyObject *value) {
...
static PyObject *SajodeSx_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
    ...
    descrgetfunc tp_descr_get = TeamYousefType_HasFeatureClass(type) ? type->tp_descr_get : NULL;
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)SajodeSx_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)SajodeSx_class_getattr;
#endif
}
...
struct TeamYousef_QuickIterator {
    int iterator_mode;
...
static bool MAKE_QUICK_ITERATOR(PyThreadState *tstate, PyObject *sequence, struct TeamYousef_QuickIterator *qiter) {
    if (TeamYousef_Generator_Check(sequence)) {
        qiter->iterator_mode = ITERATOR_COMPILED_GENERATOR;
        qiter->iterator_data.generator = (struct TeamYousef_GeneratorObject *)sequence;
...
static PyObject *QUICK_ITERATOR_NEXT(PyThreadState *tstate, struct TeamYousef_QuickIterator *qiter, bool *finished) {
    PyObject *result;
    ...
    case ITERATOR_COMPILED_GENERATOR:
        result = TeamYousef_Generator_qiter(tstate, qiter->iterator_data.generator, finished);
...
PyObject *BUILTIN_SUM1(PyThreadState *tstate, PyObject *sequence) {
    struct TeamYousef_QuickIterator qiter;
    ...
    for (;;) {
        ...
        if (finished) {
            return TeamYousef_PyInt_FromLong(int_result);
        } else if (item == NULL) {
            return NULL;
        }
...
// For Python2 long, Python3 int objects
    ...
    result = TeamYousef_PyInt_FromLong(int_result);
    CHECK_OBJECT(result);
...
PyObject *BUILTIN_SUM2(PyThreadState *tstate, PyObject *sequence, PyObject *start) {
    ...
    PyObject *result = CALL_FUNCTION_WITH_POS_ARGS2(tstate, SAJODE_ACCESS_BUILTIN(sum), pos_args);
...
static PyTypeObject TeamYousef_BuiltinModule_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_module", // tp_name
    sizeof(PyModuleObject),                           // tp_size
};

int TeamYousef_BuiltinModule_SetAttr(PyModuleObject *module, PyObject *name, PyObject *value) {
    CHECK_OBJECT(module);
    CHECK_OBJECT(name);
    ...
    if (res == 1) {
        SAJODE_UPDATE_BUILTIN(open, value);
        found = true;
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            SAJODE_UPDATE_BUILTIN(__import__, value);
            found = true;
        }
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            SAJODE_UPDATE_BUILTIN(print, value);
            found = true;
        }
    }
#endif
    ...
    if (found == false) {
        ...
        if (res == 1) {
            SAJODE_UPDATE_BUILTIN(super, value);
            found = true;
        }
    }
...
static PyObject *getPathSeparatorStringObject(void) {
    ...
    if (sep_str == NULL) {
        sep_str = TeamYousef_String_FromString(sep);
    }
...
#if _SAJODE_EXE_MODE || _SAJODE_DLL_MODE
...
#if _SAJODE_EXE_MODE || _SAJODE_ONEFILE_DLL_MODE
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = TeamYousefUnicode_FromWideChar(exe_filename, -1);
#else
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = TeamYousefUnicode_FromWideChar(bin_directory, -1);
#else
...
#if _SAJODE_DLL_MODE || _SAJODE_MODULE_MODE
static PyObject *getDllDirectoryObject(void) {
    ...
    if (dll_directory == NULL) {
        ...
        dll_directory = TeamYousef_String_FromFilename(dll_directory_filename);
...
static filename_char_t const *getDllFilename(void) {
    ...
    {
        SAJODE_MAY_BE_UNUSED int res = dladdr((void *)getDllDirectory, &where);
        assert(res != 0);
    }
...
PyObject *getDllFilenameObject(void) {
    ...
    if (dll_filename == NULL) {
        ...
        dll_filename = TeamYousef_String_FromFilename(dll_filename_str);
...
PyObject *getContainingDirectoryObject(bool resolve_symlinks) {
    ...
    environment_char_t const *onefile_directory = getEnvironmentVariable("SAJODE_ONEFILE_DIRECTORY");
    if (onefile_directory != NULL) {
        PyObject *result = TeamYousef_String_FromFilename(onefile_directory);
        unsetEnvironmentVariable("SAJODE_ONEFILE_DIRECTORY");
...
void _initBuiltinModule(void) {
    SAJODE_PRINT_TRACE("main(): Calling _initBuiltinTypeMethods().");
    _initBuiltinTypeMethods();
    SAJODE_PRINT_TRACE("main(): Calling _initDeepCopy().");
    _initDeepCopy();
    ...
    {
#if _SAJODE_EXE_MODE
        PyObject *SajodeSx_binary_dir = getBinaryDirectoryObject(true);
#else
        PyObject *SajodeSx_binary_dir = getDllDirectoryObject();
#endif
        SAJODE_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__SajodeSx_binary_dir", SajodeSx_binary_dir);
        assert(res == 0);
...
#if _SAJODE_EXE_MODE || _SAJODE_ONEFILE_DLL_MODE
        PyDict_SetItemString((PyObject *)dict_builtin, "__SajodeSx_binary_exe", getBinaryFilenameObject(true));
        assert(res == 0);
#endif
    }
#endif

    // init TeamYousef_BuiltinModule_Type, PyType_Ready won't copy all member from
    ...
    // tp_setattro to TeamYousef_BuiltinModule_SetAttr and we can detect value
    // change. Set tp_base to PyModule_Type and PyModule_Check will pass.
    TeamYousef_BuiltinModule_Type.tp_dealloc = PyModule_Type.tp_dealloc;
    TeamYousef_BuiltinModule_Type.tp_repr = PyModule_Type.tp_repr;
    TeamYousef_BuiltinModule_Type.tp_setattro = (setattrofunc)TeamYousef_BuiltinModule_SetAttr;
    TeamYousef_BuiltinModule_Type.tp_getattro = PyModule_Type.tp_getattro;
    TeamYousef_BuiltinModule_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE;
    TeamYousef_BuiltinModule_Type.tp_doc = PyModule_Type.tp_doc;
    TeamYousef_BuiltinModule_Type.tp_traverse = PyModule_Type.tp_traverse;
    TeamYousef_BuiltinModule_Type.tp_members = PyModule_Type.tp_members;
    TeamYousef_BuiltinModule_Type.tp_base = &PyModule_Type;
    TeamYousef_BuiltinModule_Type.tp_dictoffset = PyModule_Type.tp_dictoffset;
    TeamYousef_BuiltinModule_Type.tp_init = PyModule_Type.tp_init;
    TeamYousef_BuiltinModule_Type.tp_alloc = PyModule_Type.tp_alloc;
    TeamYousef_BuiltinModule_Type.tp_new = PyModule_Type.tp_new;
    TeamYousef_BuiltinModule_Type.tp_free = PyModule_Type.tp_free;
    SAJODE_MAY_BE_UNUSED int res2 = PyType_Ready(&TeamYousef_BuiltinModule_Type);
    assert(res2 >= 0);

    // Replace type of builtin module to take over.
    ((PyObject *)builtin_module)->ob_type = &TeamYousef_BuiltinModule_Type;
...
SAJODE_DEFINE_BUILTIN(type)
SAJODE_DEFINE_BUILTIN(len)
SAJODE_DEFINE_BUILTIN(repr)
SAJODE_DEFINE_BUILTIN(int)
SAJODE_DEFINE_BUILTIN(iter)
#if PYTHON_VERSION < 0x300
SAJODE_DEFINE_BUILTIN(long)
#else
SAJODE_DEFINE_BUILTIN(range);
#endif

void _initBuiltinOriginalValues(void) {
    SAJODE_ASSIGN_BUILTIN(type);
    SAJODE_ASSIGN_BUILTIN(len);
    SAJODE_ASSIGN_BUILTIN(range);
    SAJODE_ASSIGN_BUILTIN(repr);
    SAJODE_ASSIGN_BUILTIN(int);
    SAJODE_ASSIGN_BUILTIN(iter);
#if PYTHON_VERSION < 0x300
    SAJODE_ASSIGN_BUILTIN(long);
#endif
...
// Used for threading.
#if PYTHON_VERSION >= 0x300 && !defined(SAJODE_USE_PYCORE_THREAD_STATE)
...
#ifdef _SAJODE_INLINE_COPY_HACL
