...
#if defined(_SAJODE_CONSTANTS_FROM_LINKER)
...
#elif defined(_SAJODE_CONSTANTS_FROM_CODE)
...
#if defined(_SAJODE_CONSTANTS_FROM_INCBIN)
...
static Py_hash_t TeamYousef_FastHashBytes(const void *value, Py_ssize_t size) {
    if (unlikely(size == 0)) {
        return 0;
    }
...
static Py_hash_t our_list_hash(PyListObject *list) {
    return TeamYousef_FastHashBytes(&list->ob_item[0], Py_SIZE(list) * sizeof(PyObject *));
}
...
static Py_hash_t our_tuple_hash(PyTupleObject *tuple) {
    return TeamYousef_FastHashBytes(&tuple->ob_item[0], Py_SIZE(tuple) * sizeof(PyObject *));
}
...
static Py_hash_t our_set_hash(PyObject *set) {
    ...
    while (_PySet_Next(set, &pos, &key)) {
        result *= 1000003;
        result ^= TeamYousef_FastHashBytes(&key, sizeof(PyObject *));
    }
#else
    Py_hash_t unused;
...
static PyObject *our_set_tp_richcompare(PyObject *set1, PyObject *set2, int op) {
    ...
    if (Py_SIZE(set1) != Py_SIZE(set2)) {
        ...
        while (_PySet_Next(set1, &pos1, &key1)) {
            {
                SAJODE_MAY_BE_UNUSED int res = _PySet_Next(set2, &pos2, &key2);
                assert(res != 0);
            }
        ...
        while (_PySet_NextEntry(set1, &pos1, &key1, &unused1)) {
            {
                SAJODE_MAY_BE_UNUSED int res = _PySet_NextEntry(set2, &pos2, &key2, &unused2);
                assert(res != 0);
            }
...
static Py_hash_t our_dict_hash(PyObject *dict) {
    ...
    while (TeamYousef_DictNext(dict, &pos, &key, &value)) {
        result *= 1000003;
        result ^= TeamYousef_FastHashBytes(&key, sizeof(PyObject *));
        result *= 1000003;
        result ^= TeamYousef_FastHashBytes(&value, sizeof(PyObject *));
    }
...
static PyObject *our_dict_tp_richcompare(PyObject *a, PyObject *b, int op) {
    ...
    if (Py_SIZE(a) != Py_SIZE(b)) {
        ...
        // effect intended.
        while (TeamYousef_DictNext(a, &pos1, &key1, &value1)) {
            {
                SAJODE_MAY_BE_UNUSED int res = TeamYousef_DictNext(b, &pos2, &key2, &value2);
                assert(res != 0);
            }
...
PyObject **TeamYousef_Long_SmallValues;
#elif PYTHON_VERSION >= 0x300
PyObject *TeamYousef_Long_SmallValues[SAJODE_STATIC_SMALLINT_VALUE_MAX - SAJODE_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
static void initCaches(void) {
    ...
    TeamYousef_Long_SmallValues = (PyObject **)_PyInterpreterState_GET()->small_ints;
#elif PYTHON_VERSION >= 0x300
    for (long i = SAJODE_STATIC_SMALLINT_VALUE_MIN; i < SAJODE_STATIC_SMALLINT_VALUE_MAX; i++) {
        // Have to use the original API here since out "TeamYousef_PyLong_FromLong"
        // would insist on using "TeamYousef_Long_SmallValues" to produce it.
        PyObject *value = PyLong_FromLong(i);
        TeamYousef_Long_SmallValues[SAJODE_TO_SMALL_VALUE_OFFSET(i)] = value;
    }
#endif
#endif
...
static PyObject *_unpackAnonValue(unsigned char anon_index) {
    ...
    default:
        ...
        SAJODE_CANNOT_GET_HERE("Corrupt constants blob");
    }
}
...
static PyObject *_TeamYousef_Unicode_ImmortalFromStringAndSize(PyThreadState *tstate, const char *data, Py_ssize_t size,
                                                           bool is_ascii) {
#if PYTHON_VERSION < 0x300
...
#if PYTHON_VERSION >= 0x3c0 && !defined(__SAJODE_NO_ASSERT__)
...
static unsigned char const *_unpackBlobConstant(PyThreadState *tstate, PyObject **output, unsigned char const *data) {
    ...
    *output = NULL;
...
#ifdef _SAJODE_EXPERIMENTAL_DEBUG_CONSTANTS
    ...
    case 'D': {
        ...
        if (size > 0) {
            SAJODE_DYNAMIC_ARRAY_DECL(keys, PyObject *, size);
            SAJODE_DYNAMIC_ARRAY_DECL(values, PyObject *, size);
            ...
            if (size == 0) {
                ...
                if (empty_frozenset == NULL) {
                    empty_frozenset = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, (PyObject *)&PyFrozenSet_Type,
                                                                    TeamYousef_Bytes_FromStringAndSize("", 0));
                }
    ...
    case 'q': {
        ...
        PyObject *l = TeamYousef_LongFromCLong((c == 'l') ? ((long)value) : (-(long)value));
        assert(l != NULL);

        // Avoid the long cache, won't do anything useful for small ints
#if PYTHON_VERSION >= 0x300
        if (value < SAJODE_STATIC_SMALLINT_VALUE_MIN || value >= SAJODE_STATIC_SMALLINT_VALUE_MAX)
#endif
        {
    ...
    case 'G':
    case 'g': {
        PyObject *result = TeamYousef_PyLong_FromLong(0);

        int size = (int)_unpackVariableLength(&data);

        PyObject *shift = TeamYousef_PyLong_FromLong(31);

        for (int i = 0; i < size; i++) {
            result = PyNumber_InPlaceLshift(result, shift);

            uint64_t value = _unpackVariableLength(&data);
            PyObject *part = TeamYousef_LongFromCLong((long)value);
            assert(part != NULL);
    ...
    case 'c': {
        ...
        PyObject *b = TeamYousef_Bytes_FromStringAndSize((const char *)data, size);
        CHECK_OBJECT(b);
    ...
    case 'd': {
        ...
        *output = s;
#else
        PyObject *b = TeamYousef_Bytes_FromStringAndSize((const char *)data, 1);
        data += 1;
        *output = b;
#endif
    ...
    case 'w': {
        ...
        PyObject *u = _TeamYousef_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 1, true);
        data += 1;
    ...
    case 'u': { // Python2 unicode, Python3 str, zero terminated.
        ...
        PyObject *u = _TeamYousef_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, c == 'a');
        data += size + 1;
    ...
    case 'v': {
        ...
        PyObject *u = _TeamYousef_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, false);
        data += size;
    ...
    case 's': {
        *output = _TeamYousef_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 0, true);
        is_object = true;
...
#if _SAJODE_CONSTANTS_FROM_MACOS_SECTION
...
void loadConstantsBlob(PyThreadState *tstate, PyObject **output, char const *name) {
    ...
    if (init_done == false) {
        SAJODE_PRINT_TIMING("loadConstantsBlob(): One time init.");
...
#elif defined(_SAJODE_CONSTANTS_FROM_RESOURCE)
#if _SAJODE_EXE_MODE
        ...
        assert(constant_bin);
#elif _SAJODE_CONSTANTS_FROM_MACOS_SECTION
        constant_bin = findMacOSBinarySection();

        assert(constant_bin);
#endif
        SAJODE_PRINT_TIMING("loadConstantsBlob(): Found blob, decoding now.");
        DECODE(constant_bin);

        SAJODE_PRINT_TIMING("loadConstantsBlob(): CRC32 that blob for correctness.");
        ...
        SAJODE_PRINT_TIMING("loadConstantsBlob(): One time init complete.");
