...
/**
 * This is responsible for updating parts of CPython to better work with TeamYousef
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (TeamYousef_Generator_Check(object)) {
        struct TeamYousef_GeneratorObject *generator = (struct TeamYousef_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (TeamYousef_Coroutine_Check(object)) {
        struct TeamYousef_CoroutineObject *coroutine = (struct TeamYousef_CoroutineObject *)object;
...
static PyObject *_types_coroutine_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (TeamYousef_Function_Check(func)) {
...
static bool TeamYousef_FrameIsCompiled(_PyInterpreterFrame *frame) {
    return ((frame->frame_obj != NULL) && TeamYousef_Frame_Check((PyObject *)frame->frame_obj));
}

static bool TeamYousef_FrameIsIncomplete(_PyInterpreterFrame *frame) {
...
static PyObject *_sys_getframemodulename_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *index_value = TeamYousef_Number_IndexAsLong(depth_arg ? depth_arg : const_int_0);
    ...
    while ((frame != NULL) && ((TeamYousef_FrameIsIncomplete(frame)) || depth_ssize-- > 0)) {
        frame = frame->previous;
    }

    if ((frame != NULL) && (TeamYousef_FrameIsCompiled(frame))) {
...
#if PYTHON_VERSION >= 0x300
#if _SAJODE_EXE_MODE && !_SAJODE_STANDALONE_MODE
...
"
...

    {
        SAJODE_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("SajodeSx_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        SAJODE_MAY_BE_UNUSED bool bool_res = TeamYousef_DelModuleString(tstate, "SajodeSx_types_patch");
...
#if PYTHON_VERSION >= 0x3c0
    orig_sys_getframemodulename = TeamYousef_SysGetObject("_getframemodulename");
    ...
    TeamYousef_SysSetObject("_getframemodulename", sys_getframemodulename_replacement);
#endif
...
static PyObject *TeamYousef_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&TeamYousef_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&TeamYousef_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&TeamYousef_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&TeamYousef_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&TeamYousef_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&TeamYousef_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&TeamYousef_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&TeamYousef_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&TeamYousef_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&TeamYousef_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = TeamYousef_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct TeamYousef_FrameObject *frame, int lineno) {
#if 0
...
static void TeamYousef_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    TeamYousef_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, TeamYousef_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)TeamYousef_tb_dealloc; }
