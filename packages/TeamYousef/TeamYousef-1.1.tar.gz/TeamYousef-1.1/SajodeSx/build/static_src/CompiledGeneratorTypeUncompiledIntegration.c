...
#if PYTHON_VERSION >= 0x300
static PyBaseExceptionObject *TeamYousef_BaseExceptionSingleArg_new(PyThreadState *tstate, PyTypeObject *type,
                                                                PyObject *arg) {
...
static PyObject *TeamYousef_CreateStopIteration(PyThreadState *tstate, PyObject *value) {
    if (value == Py_None) {
        value = NULL;
    }

    PyStopIterationObject *result =
        (PyStopIterationObject *)TeamYousef_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopIteration, value);
...
static void TeamYousef_SetStopIterationValue(PyThreadState *tstate, PyObject *value) {
    ...
    if (likely(!PyTuple_Check(value) && !PyExceptionInstance_Check(value))) {
        ...
        struct TeamYousef_ExceptionPreservationItem exception_state = {Py_NewRef(PyExc_StopIteration),
                                                                   TeamYousef_CreateStopIteration(tstate, value)};

        RESTORE_ERROR_OCCURRED_STATE(tstate, &exception_state);
    }
#else
    struct TeamYousef_ExceptionPreservationItem exception_state = {TeamYousef_CreateStopIteration(tstate, value)};
...
#else
    struct TeamYousef_ExceptionPreservationItem exception_state = {TeamYousef_CreateStopIteration(tstate, NULL)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *TeamYousef_CreateStopAsyncIteration(PyThreadState *tstate) {
    return (PyObject *)TeamYousef_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopAsyncIteration, NULL);
}
#endif
...
#else
    struct TeamYousef_ExceptionPreservationItem exception_state = {TeamYousef_CreateStopAsyncIteration(tstate)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *TeamYousef_CreateGeneratorExit(PyThreadState *tstate) {
    return (PyObject *)TeamYousef_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_GeneratorExit, NULL);
}
#endif
...
#else
    struct TeamYousef_ExceptionPreservationItem exception_state = {TeamYousef_CreateGeneratorExit(tstate)};
...
#if PYTHON_VERSION >= 0x300
static bool TeamYousef_PyGen_FetchStopIterationValue(PyThreadState *tstate, PyObject **pvalue) {
#if PYTHON_VERSION < 0x3c0
...
static inline void TeamYousef_PyGen_exc_state_clear(_PyErr_StackItem *exc_state) {
...
#if PYTHON_VERSION < 0x3b0
static inline bool TeamYousef_PyFrameHasCompleted(PyFrameObject *const frame) {
...
static PyObject *TeamYousef_PyGen_Send(PyThreadState *tstate, PyGenObject *gen, PyObject *arg) {
#if PYTHON_VERSION >= 0x3a0
    PyObject *result;
...
#if PYTHON_VERSION < 0x3b0
    if (f == NULL || TeamYousef_PyFrameHasCompleted(f)) {
#else
...
#else
        TeamYousef_PyGen_exc_state_clear(&gen->gi_exc_state);
#endif
...
#if PYTHON_VERSION >= 0x300
#define SAJODE_UNCOMPILED_THROW_INTEGRATION 1
#endif
...
static PyObject *TeamYousef_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args);
static int TeamYousef_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf);
...
const uint8_t TeamYousef_PyOpcode_Deopt[256] = {
...
#if PYTHON_VERSION >= 0x3d0
static inline bool _TeamYousef_is_resume(_Py_CODEUNIT *instr) {
    uint8_t code = FT_ATOMIC_LOAD_UINT8_RELAXED(instr->op.code);
    return (code == RESUME || code == RESUME_CHECK || code == INSTRUMENTED_RESUME);
}
#endif

PyObject *TeamYousef_PyGen_yf(PyGenObject *gen) {
#if PYTHON_VERSION >= 0x3d0
    if (gen->gi_frame_state == FRAME_SUSPENDED_YIELD_FROM) {
        _PyInterpreterFrame *frame = (_PyInterpreterFrame *)gen->gi_iframe;
        assert(_TeamYousef_is_resume(frame->instr_ptr));
    ...
    if (gen->gi_frame_state < FRAME_CLEARED) {
        ...
        if (TeamYousef_PyOpcode_Deopt[_Py_OPCODE(next)] != RESUME || _Py_OPARG(next) < 2) {
            return NULL;
        }
...
static PyFrameObject *_TeamYousef_PyFrame_New_NoTrack(PyCodeObject *code) {
...
// Also not exported.
static PyFrameObject *_TeamYousef_PyFrame_MakeAndSetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    assert(frame->frame_obj == NULL);

    struct TeamYousef_ExceptionPreservationItem saved_exception_state;
    FETCH_ERROR_OCCURRED_STATE(tstate, &saved_exception_state);

    PyFrameObject *f = _TeamYousef_PyFrame_New_NoTrack(frame->f_code);
...
static inline PyFrameObject *_TeamYousef_PyFrame_GetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    return _TeamYousef_PyFrame_MakeAndSetFrameObject(tstate, frame);
}

// Also not exported, taking over a frame object.
static void _TeamYousef_take_ownership(PyThreadState *tstate, PyFrameObject *f, _PyInterpreterFrame *frame) {
    assert(frame->owner != FRAME_OWNED_BY_FRAME_OBJECT);
    assert(frame->owner != FRAME_CLEARED);
    ...
    if (prev) {
        ...
        // TODO: TeamYousef doesn't do that for compiled frames ever, is that really needed?
        PyFrameObject *back = _TeamYousef_PyFrame_GetFrameObject(tstate, prev);
    ...
    if (!_PyObject_GC_IS_TRACKED((PyObject *)f)) {
        TeamYousef_GC_Track((PyObject *)f);
    }
}

// Cleanup up the frame is also not exported.
static void _TeamYousef_PyFrame_Clear(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    if (frame->frame_obj) {
        ...
        if (Py_REFCNT(f) > 1) {
            _TeamYousef_take_ownership(tstate, f, frame);
            Py_DECREF(f);
...
static PySendResult TeamYousef_PyGen_gen_send_ex2(PyThreadState *tstate, PyGenObject *gen, PyObject *arg,
...
#if PYTHON_VERSION < 0x3c0
    _TeamYousef_PyFrame_Clear(tstate, frame);
#endif
...
static PyObject *TeamYousef_PyGen_gen_send_ex(PyThreadState *tstate, PyGenObject *gen, PyObject *arg, int exc,
                                          int closing) {
    PyObject *result;

    if (TeamYousef_PyGen_gen_send_ex2(tstate, gen, arg, &result, exc, closing) == PYGEN_RETURN) {
...
//   be normalized.
static PyObject *TeamYousef_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
                                                  struct TeamYousef_ExceptionPreservationItem *exception_state) {
#if _DEBUG_GENERATOR
    PRINT_STRING("TeamYousef_UncompiledGenerator_throw: Enter ");
    PRINT_ITEM((PyObject *)gen);
    PRINT_EXCEPTION_STATE(exception_state);
    PRINT_NEW_LINE();
#endif

    PyObject *yf = TeamYousef_PyGen_yf(gen);
    ...
    if (yf != NULL) {
        ...
        if (close_on_genexit && EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            int err = TeamYousef_PyGen_gen_close_iter(tstate, yf);
            ...
            if (err < 0) {
                ...
                return TeamYousef_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            // Handing exception ownership to "TeamYousef_UncompiledGenerator_throw".
            ret = TeamYousef_UncompiledGenerator_throw(tstate, (PyGenObject *)yf, close_on_genexit, exception_state);
            gen->gi_frame_state = state;
        ...
        if (ret == NULL) {
            ...
            if (TeamYousef_PyGen_FetchStopIterationValue(tstate, &val)) {
                ret = TeamYousef_PyGen_gen_send_ex(tstate, gen, val, 0, 0);
                Py_DECREF(val);
            } else
#endif
            {
                ret = TeamYousef_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        }
...
throw_here:
    ...
    return TeamYousef_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
...
static PyObject *TeamYousef_PyGen_yf(PyGenObject *gen) {
    PyFrameObject *f = gen->gi_frame;
...
static PyObject *TeamYousef_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
    ...
    if (yf != NULL) {
        ...
        if (ret == NULL) {
            ...
            if (TeamYousef_PyGen_FetchStopIterationValue(tstate, &exception_state->exception_value)) {
                ret = TeamYousef_PyGen_gen_send_ex(tstate, gen, exception_state->exception_value, 0, 0);
...
static int TeamYousef_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf) {
    ...
    if (PyGen_CheckExact(yf)
        ...
        assert(false);
        retval = TeamYousef_PyGen_gen_close(tstate, (PyGenObject *)yf, NULL);
...
static PyObject *TeamYousef_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args) {
    ...
    if (yf != NULL) {
        ...
        gen->gi_running = 1;
#endif
        err = TeamYousef_PyGen_gen_close_iter(tstate, yf);
    ...
    PyObject *retval = TeamYousef_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
