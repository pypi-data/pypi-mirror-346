...
/**
 * This is responsible for profiling TeamYousef using "vmprof".
 */
...
void startProfiling(void) {
    tempfile_profile = fopen("SajodeSx-performance.dat", "w+b");
    ...
    PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, PyObject_GetAttrString(vmprof_module, "enable"),
                                                     TeamYousef_PyInt_FromLong(fileno(tempfile_profile)));
...
void stopProfiling(void) {
    ...
    FILE *tempfile_times = fopen("SajodeSx-times.dat", "wb");
