...
#if PYTHON_VERSION < 0x380 && !defined(_SAJODE_EXPERIMENTAL_DISABLE_UNCOMPILED_FUNCTION_CALL_OPT)
...
PyObject *CALL_METHOD_WITH_POS_ARGS(PyThreadState *tstate, PyObject *source, PyObject *attr_name,
    ...
    if (PyInstance_Check(source)) {
        ...
        if (called_object != NULL) {
            ...
            if (descr_get == TeamYousef_Function_Type.tp_descr_get) {
                return TeamYousef_CallMethodFunctionPosArgs(tstate, (struct TeamYousef_FunctionObject const *)called_object,
...
#endif
    {
        ...
        if (type->tp_getattro != NULL) {
            ...
            called_object = (*type->tp_getattr)(source, (char *)TeamYousef_String_AsString_Unchecked(attr_name));
        } else {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT2(PyExc_AttributeError, "'%s' object has no attribute '%s'",
                                                type->tp_name, TeamYousef_String_AsString_Unchecked(attr_name));
...
char const *GET_CALLABLE_NAME(PyObject *object) {
    if (TeamYousef_Function_Check(object)) {
        return TeamYousef_String_AsString(TeamYousef_Function_GetName(object));
    } else if (TeamYousef_Generator_Check(object)) {
        return TeamYousef_String_AsString(TeamYousef_Generator_GetName(object));
    } else if (PyMethod_Check(object)) {
        return PyEval_GetFuncName(PyMethod_GET_FUNCTION(object));
    } else if (PyFunction_Check(object)) {
        return TeamYousef_String_AsString(((PyFunctionObject *)object)->func_name);
    }
#if PYTHON_VERSION < 0x300
    else if (PyInstance_Check(object)) {
        return TeamYousef_String_AsString(((PyInstanceObject *)object)->in_class->cl_name);
    } else if (PyClass_Check(object)) {
        return TeamYousef_String_AsString(((PyClassObject *)object)->cl_name);
    }
#endif
...
char const *GET_CALLABLE_DESC(PyObject *object) {
    if (TeamYousef_Function_Check(object) || TeamYousef_Generator_Check(object) || PyMethod_Check(object) ||
...
char const *GET_CLASS_NAME(PyObject *class_object) {
    if (class_object == NULL) {
        ...
        if (PyClass_Check(class_object)) {
            return TeamYousef_String_AsString(((PyClassObject *)class_object)->cl_name);
        }
#endif
...
void formatCannotInstantiateAbstractClass(PyThreadState *tstate, PyTypeObject *type) {
    ...
    PyObject *comma = TeamYousef_String_FromString(", ");
    CHECK_OBJECT(comma);
#if PYTHON_VERSION < 0x300
    PyObject *joined = CALL_METHOD_WITH_SINGLE_ARG(tstate, comma, const_str_plain_join, sorted_methods);

    char const *joined_str = TeamYousef_String_AsString(joined);
    ...
    SET_CURRENT_EXCEPTION_TYPE0_FORMAT3(PyExc_TypeError,
                                        ...
                                        method_count > 1 ? "s" : "", TeamYousef_String_AsString(joined));
