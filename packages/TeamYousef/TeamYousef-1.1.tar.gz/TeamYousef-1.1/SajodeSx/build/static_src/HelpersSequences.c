...
Py_ssize_t TeamYousef_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
    ...
    return TeamYousef_PyMapping_Size(sequence);
}

PyObject *TeamYousef_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!TeamYousef_Index_Check(item))) {
...
PyObject *TeamYousef_Number_IndexAsLong(PyObject *item) {
    PyObject *result = TeamYousef_Number_Index(item);
