...
extern "C" void initSplashScreen(void) {
    ...
    if (unlikely(bool_res == false)) {
        SAJODE_PRINT_TIMING("Failed expand indicator path.");
        return;
    }
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Done with splash screen.");
}

static void closeSplashScreen(void) {
    SAJODE_PRINT_TIMING("ONEFILE: Check splash screen indicator file.");
