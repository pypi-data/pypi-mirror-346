...
PyObject *LOOKUP_ATTRIBUTE(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
...
#if _SAJODE_EXPERIMENTAL_DISABLE_ATTR_OPT
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = TeamYousef_TypeLookup(type, attr_name);
        descrgetfunc func = NULL;

        if (descr != NULL) {
            Py_INCREF(descr);

            if (TeamYousefType_HasFeatureClass(Py_TYPE(descr))) {
                func = Py_TYPE(descr)->tp_descr_get;

                if (func != NULL && TeamYousef_Descr_IsData(descr)) {
...
// TODO: This is what Python 3.11 requires us to add.
...
#if PYTHON_VERSION < 0x300
    else if (type->tp_getattro == PyInstance_Type.tp_getattro && TeamYousef_String_CheckExact(attr_name)) {
        ...
        PyObject *result = (*type->tp_getattr)(source, (char *)TeamYousef_String_AsString_Unchecked(attr_name));
        return result;
    } else {
        PyErr_Format(PyExc_AttributeError, "'%s' object has no attribute '%s'", type->tp_name,
                     TeamYousef_String_AsString_Unchecked(attr_name));
...
PyObject *LOOKUP_ATTRIBUTE_DICT_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = TeamYousef_TypeLookup(type, const_str_plain___dict__);
        descrgetfunc func = NULL;
...
PyObject *LOOKUP_ATTRIBUTE_CLASS_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = TeamYousef_TypeLookup(type, const_str_plain___class__);
        descrgetfunc func = NULL;
...
int HAS_ATTR_BOOL2(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        if (descr != NULL) {
            // Hold a refcount, TeamYousef_TypeLookup does not give any.
            Py_INCREF(descr);
...
#if (PYTHON_VERSION < 0x300 || defined(_SAJODE_USE_UNEXPOSED_API)) && !defined(_SAJODE_EXPERIMENTAL_DISABLE_ATTR_OPT)
...
bool SET_ATTRIBUTE(PyThreadState *tstate, PyObject *target, PyObject *attr_name, PyObject *value) {
...
#if PYTHON_VERSION < 0x300 || defined(_SAJODE_USE_UNEXPOSED_API)
    ...
    if (type->tp_setattr != NULL) {
        int status = (*type->tp_setattr)(target, (char *)TeamYousef_String_AsString_Unchecked(attr_name), value);
...
PyObject *LOOKUP_SPECIAL(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    PyObject *result = TeamYousef_TypeLookup(Py_TYPE(source), attr_name);
...
PyObject *LOOKUP_MODULE_VALUE(PyDictObject *module_dict, PyObject *var_name) {
    PyObject *result = GET_STRING_DICT_VALUE(module_dict, (TeamYousef_StringObject *)var_name);

    if (unlikely(result == NULL)) {
        result = GET_STRING_DICT_VALUE(dict_builtin, (TeamYousef_StringObject *)var_name);
    }
