...
#define SAJODE_MAIN_MODULE_NAME "__main__"
#define SAJODE_MAIN_IS_PACKAGE_BOOL false
#define _SAJODE_ATTACH_CONSOLE_WINDOW 1
#if defined(__APPLE__)
#define _SAJODE_MACOS_BUNDLE_MODE 1
#endif
#endif

// It doesn't work for MinGW64 to update the standard output handles early on,
// so make a difference here.
#if defined(SAJODE_FORCED_STDOUT_PATH) || defined(SAJODE_FORCED_STDERR_PATH)
#if defined(__MINGW64__) || defined(__MINGW32__)
#define SAJODE_STANDARD_HANDLES_EARLY 0
#else
#define SAJODE_STANDARD_HANDLES_EARLY 1
#endif
#else
#define SAJODE_STANDARD_HANDLES_EARLY 0
#endif

#if defined(_WIN32) && (defined(_SAJODE_ATTACH_CONSOLE_WINDOW) || defined(_SAJODE_HIDE_CONSOLE_WINDOW))
#include "HelpersConsole.c"
#endif

// We are open to having this defined otherwise, this is a default only.
#if defined(_WIN32) && defined(SAJODE_COMPANY_NAME) && defined(SAJODE_PRODUCT_NAME) &&                                 \
    !defined(SAJODE_APP_MODEL_USER_ID)
#define SAJODE_APP_MODEL_USER_ID L"" SAJODE_COMPANY_NAME L"." SAJODE_PRODUCT_NAME
#endif

#if defined(_WIN32) && defined(SAJODE_APP_MODEL_USER_ID)
...
static void prepareFrozenModules(void) {
    ...
    struct _frozen *merged =
        (struct _frozen *)malloc(sizeof(struct _frozen) * (_SAJODE_FROZEN + pre_existing_count + 1));
...
#include "SajodeSx/environment_variables.h"

#if _SAJODE_STANDALONE_MODE
...
static void prepareStandaloneEnvironment(void) {
    ...
    SAJODE_PRINTF_TRACE("main(): Binary dir is %s\n", binary_directory);
...
static void restoreStandaloneEnvironment(void) {
    ...
    // SAJODE_PRINTF_TRACE("Final PySys_GetPath is 's'.\n", PySys_GetPath());
#elif PYTHON_VERSION < 0x370
    PySys_SetPath(getBinaryDirectoryWideChars(true));
    Py_SetPath(getBinaryDirectoryWideChars(true));
    // SAJODE_PRINTF_TRACE("Final Py_GetPath is '%ls'.\n", Py_GetPath());
#endif

#ifdef _SAJODE_EXPERIMENTAL_DUMP_PY_PATH_CONFIG
...
#if PYTHON_VERSION >= 0x300 && _SAJODE_NATIVE_WCHAR_ARGV == 0
...
static void PRINT_REFCOUNTS(void) {
    ...
    PRINT_FORMAT("Compiled Functions: %d | %d | %d (module/class ownership may occur)\n",
                 count_active_TeamYousef_Function_Type, count_allocated_TeamYousef_Function_Type,
                 count_released_TeamYousef_Function_Type);
    PRINT_FORMAT("Compiled Generators: %d | %d | %d\n", count_active_TeamYousef_Generator_Type,
                 count_allocated_TeamYousef_Generator_Type, count_released_TeamYousef_Generator_Type);
#if PYTHON_VERSION >= 0x350
    PRINT_FORMAT("Compiled Coroutines: %d | %d | %d\n", count_active_TeamYousef_Coroutine_Type,
                 count_allocated_TeamYousef_Coroutine_Type, count_released_TeamYousef_Coroutine_Type);
    PRINT_FORMAT("Compiled Coroutines Wrappers: %d | %d | %d\n", count_active_TeamYousef_CoroutineWrapper_Type,
                 count_allocated_TeamYousef_CoroutineWrapper_Type, count_released_TeamYousef_CoroutineWrapper_Type);

    PRINT_FORMAT("Compiled Coroutines AIter Wrappers: %d | %d | %d\n", count_active_TeamYousef_AIterWrapper_Type,
                 count_allocated_TeamYousef_AIterWrapper_Type, count_released_TeamYousef_AIterWrapper_Type);
#endif
#if PYTHON_VERSION >= 0x360
    PRINT_FORMAT("Compiled Asyncgen: %d | %d | %d\n", count_active_TeamYousef_Asyncgen_Type,
                 count_allocated_TeamYousef_Asyncgen_Type, count_released_TeamYousef_Asyncgen_Type);
    PRINT_FORMAT("Compiled Asyncgen Wrappers: %d | %d | %d\n", count_active_TeamYousef_AsyncgenValueWrapper_Type,
                 count_allocated_TeamYousef_AsyncgenValueWrapper_Type, count_released_TeamYousef_AsyncgenValueWrapper_Type);
    PRINT_FORMAT("Compiled Asyncgen Asend: %d | %d | %d\n", count_active_TeamYousef_AsyncgenAsend_Type,
                 count_allocated_TeamYousef_AsyncgenAsend_Type, count_released_TeamYousef_AsyncgenAsend_Type);
    PRINT_FORMAT("Compiled Asyncgen Athrow: %d | %d | %d\n", count_active_TeamYousef_AsyncgenAthrow_Type,
                 count_allocated_TeamYousef_AsyncgenAthrow_Type, count_released_TeamYousef_AsyncgenAthrow_Type);
#endif

    PRINT_FORMAT("Compiled Frames: %d | %d | %d (cache usage may occur)\n", count_active_TeamYousef_Frame_Type,
                 count_allocated_TeamYousef_Frame_Type, count_released_TeamYousef_Frame_Type);
    PRINT_FORMAT("Compiled Cells: %d | %d | %d (function ownership may occur)\n", count_active_TeamYousef_Cell_Type,
                 count_allocated_TeamYousef_Cell_Type, count_released_TeamYousef_Cell_Type);
...
static int HANDLE_PROGRAM_EXIT(PyThreadState *tstate) {
    ...
    if (HAS_ERROR_OCCURRED(tstate)) {
        ...
        while (tstate->curexc_traceback) {
            ...
            if (0 == strcmp(PyUnicode_AsUTF8(TeamYousef_Frame_GetCodeObject(frame)->co_filename),
...
#endif
        SAJODE_FINALIZE_PROGRAM(tstate);
...
static PyObject *EXECUTE_MAIN_MODULE(PyThreadState *tstate, char const *module_name, bool is_package) {
    SAJODE_INIT_PROGRAM_LATE(module_name);
...
#if _SAJODE_PLUGIN_WINDOWS_SERVICE_ENABLED
#include "SajodeSx_windows_service.h"

// Callback from Windows Service logic.
void SvcStartPython(void) {
    PyThreadState *tstate = PyThreadState_GET();

    EXECUTE_MAIN_MODULE(tstate, SAJODE_MAIN_MODULE_NAME, SAJODE_MAIN_IS_PACKAGE_BOOL);

    SAJODE_PRINT_TIMING("SvcStartPython() Python exited.")

    int exit_code = HANDLE_PROGRAM_EXIT(tstate);

    // TODO: Log exception and call ReportSvcStatus

    SAJODE_PRINT_TIMING("SvcStartPython(): Calling Py_Exit.");
    Py_Exit(exit_code);
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (i == 1) {
            ...
            if (strcmpFilename(argv[i], FILENAME_EMPTY_STR "install") == 0) {
                SAJODE_PRINT_TRACE("main(): Calling plugin SvcInstall().");

                SvcInstall();
                SAJODE_CANNOT_GET_HERE("main(): SvcInstall must not return");
            }
#endif
        }
...
#if !defined(_SAJODE_DEPLOYMENT_MODE) && !defined(_SAJODE_NO_DEPLOYMENT_SELF_EXECUTION)
...
#if defined(_SAJODE_ONEFILE_MODE) && defined(_WIN32)

static long onefile_ppid;

DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    SAJODE_PRINT_TRACE("Onefile parent monitoring starts.");
    ...
    SAJODE_PRINT_TRACE("Onefile parent monitoring causes KeyboardInterrupt.");
    PyErr_SetInterrupt();

    Sleep(_SAJODE_ONEFILE_CHILD_GRACE_TIME_INT);

    SAJODE_PRINT_TRACE("Onefile parent monitoring hard kills after ignored KeyboardInterrupt.");
    ExitProcess(1);
...
#if SAJODE_FORCED_STDOUT_NULL_BOOL
    if (strcmp(name, "stdout") == 0) {
        return true;
    }
#endif

#if SAJODE_FORCED_STDERR_NULL_BOOL
    if (strcmp(name, "stderr") == 0) {
        return true;
    }
#elif defined(SAJODE_FORCED_STDERR_PATH) || defined(SAJODE_FORCED_STDERR_NONE_BOOL)
    if (strcmp(name, "stderr") == 0) {
        return false;
    }
#endif

    PyObject *sys_std_handle = TeamYousef_SysGetObject(name);
...
static void setStdinHandle(PyThreadState *tstate, PyObject *stdin_file) {
    ...
    TeamYousef_SysSetObject("stdin", stdin_file);
...
static void setStdoutHandle(PyThreadState *tstate, PyObject *stdout_file) {
    ...
    TeamYousef_SysSetObject("stdout", stdout_file);
...
static void setStderrHandle(PyThreadState *tstate, PyObject *stderr_file) {
    ...
    TeamYousef_SysSetObject("stderr", stderr_file);
...
#if SAJODE_STANDARD_HANDLES_EARLY == 0
#if defined(SAJODE_FORCED_STDOUT_PATH) || defined(SAJODE_FORCED_STDERR_PATH)
#ifdef _WIN32
static PyObject *getExpandedTemplatePath(wchar_t const *template_path) {
    ...
    return TeamYousefUnicode_FromWideChar(filename_buffer, -1);
}
#else
static PyObject *getExpandedTemplatePath(char const *template_path) {
    ...
    return TeamYousef_String_FromString(filename_buffer);
}
#endif
#endif
#endif
...
// reason.
#if SAJODE_STANDARD_HANDLES_EARLY == 1 && PYTHON_VERSION >= 0x370
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Early handles.");
#if defined(SAJODE_FORCED_STDOUT_PATH) || defined(SAJODE_FORCED_STDERR_PATH)
    PyObject *args = MAKE_DICT_EMPTY(tstate);

    DICT_SET_ITEM(args, const_str_plain_encoding, TeamYousef_String_FromString("utf-8"));
    DICT_SET_ITEM(args, const_str_plain_line_buffering, Py_True);

#if defined(SAJODE_FORCED_STDOUT_PATH)
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Forced stdout update.");
    {
        PyObject *sys_stdout = TeamYousef_SysGetObject("stdout");
...
#if defined(SAJODE_FORCED_STDERR_PATH)
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Forced stderr update.");
    {
        PyObject *sys_stderr = TeamYousef_SysGetObject("stderr");
    ...
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Done with early handles.");
#endif

#if SAJODE_STANDARD_HANDLES_EARLY == 0
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Late handles.");
#if defined(SAJODE_FORCED_STDOUT_PATH)
    {
#if defined(_WIN32)
        PyObject *filename = getExpandedTemplatePath(L"" SAJODE_FORCED_STDOUT_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(SAJODE_FORCED_STDOUT_PATH);
#endif
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" SAJODE_FORCED_STDERR_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(SAJODE_FORCED_STDERR_PATH);
#endif
    ...
    {
#if defined(_WIN32)
        PyObject *devnull_filename = TeamYousef_String_FromString("NUL:");
#else
        PyObject *devnull_filename = TeamYousef_String_FromString("/dev/null");
#endif

        SAJODE_PRINT_TRACE("setInputOutputHandles(): Considering stdin.");

        if (shallSetOutputHandleToNull("stdin")) {
            SAJODE_PRINT_TRACE("setInputOutputHandles(): Set stdin to NULL file.");
        ...
        SAJODE_PRINT_TRACE("setInputOutputHandles(): Considering stdout.");

        if (shallSetOutputHandleToNull("stdout")) {
            SAJODE_PRINT_TRACE("setInputOutputHandles(): Set stdout to NULL file.");
        ...
        SAJODE_PRINT_TRACE("setInputOutputHandles(): Considering stderr.");

        if (shallSetOutputHandleToNull("stderr")) {
            SAJODE_PRINT_TRACE("setInputOutputHandles(): Set stderr to NULL file.");
...
#if SAJODE_FORCED_STDOUT_NONE_BOOL
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Forcing stdout to None.");
    setStdoutHandle(tstate, Py_None);
#endif

#if SAJODE_FORCED_STDERR_NONE_BOOL
    SAJODE_PRINT_TRACE("setInputOutputHandles(): Forcing stderr to None.");
...
static void TeamYousef_Py_Initialize(void) {
#if PYTHON_VERSION > 0x350 && !defined(_SAJODE_EXPERIMENTAL_DISABLE_ALLOCATORS)
    initTeamYousefAllocators();
#endif

#if PYTHON_VERSION < 0x380 || defined(_SAJODE_EXPERIMENTAL_OLD_PY_INITIALIZE)
    Py_Initialize();
#else
    ...
    SAJODE_MAY_BE_UNUSED _PyRuntimeState *runtime = &_PyRuntime;
    ...
    // Need to disable frozen modules, TeamYousef can handle them better itself.
...
// too.
    ...
    SAJODE_PRINT_TIMING("TeamYousef_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
// short lived.
#if SYSFLAG_ISOLATED
    TeamYousef_SysSetObject("path", PyList_New(0));
#endif
#endif
...
#if SAJODE_STANDARD_HANDLES_EARLY == 1
#if defined(_WIN32)
...
#if defined(_SAJODE_EXPERIMENTAL_SHOW_STARTUP_TIME)
static void TeamYousef_at_exit(void) { SAJODE_PRINT_TIMING("TeamYousef_at_exit(): Called by C exit()"); }
#endif

#if !defined(_SAJODE_DEPLOYMENT_MODE) && !defined(_SAJODE_NO_DEPLOYMENT_SEGFAULT)
#include <signal.h>
static void SajodeSx_segfault_handler(int sig) {
    puts("TeamYousef: A segmentation fault has occurred. This is highly unusual and can");
    puts("have multiple reasons. Please check https://SajodeSx.net/info/segfault.html");
    puts("for solutions.");
...
#if _SAJODE_NATIVE_WCHAR_ARGV == 1
...
PyObject *getOriginalArgv0Object(void) {
    ...
    return TeamYousef_String_FromFilename(original_argv0);
}
...
static int TeamYousef_Main(int argc, native_command_line_argument_t **argv) {
#if defined(_SAJODE_HIDE_CONSOLE_WINDOW)
    hideConsoleIfSpawned();
#endif

    // Installer a segfault handler that outputs a helpful message.
#if !defined(_SAJODE_DEPLOYMENT_MODE) && !defined(_SAJODE_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, SajodeSx_segfault_handler);
#endif

#ifdef _SAJODE_EXPERIMENTAL_DUMP_C_TRACEBACKS
    INIT_C_BACKTRACES();
    DUMP_C_BACKTRACE();
#endif
    // Trace when the process exits.
#if defined(_SAJODE_EXPERIMENTAL_SHOW_STARTUP_TIME)
    atexit(TeamYousef_at_exit);
#endif

    // Attach to the parent console respecting redirection only, otherwise we
    // cannot even output traces.
#if defined(_WIN32) && defined(_SAJODE_ATTACH_CONSOLE_WINDOW)
    inheritAttachedConsole();
#endif

#if defined(_WIN32) && defined(SAJODE_APP_MODEL_USER_ID)
    setCurrentProcessExplicitAppUserModelID(SAJODE_APP_MODEL_USER_ID);
#endif

// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _SAJODE_NATIVE_WCHAR_ARGV == 0
    original_argv0 = argv[0];
#if !defined(_SAJODE_ONEFILE_MODE)
...
#if defined(SAJODE_FORCED_STDOUT_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_OUTPUT_HANDLE, stdout, L"" SAJODE_FORCED_STDOUT_PATH);
#else
    changeStandardHandleTarget(stdout, SAJODE_FORCED_STDOUT_PATH);
#endif
#endif
#if defined(SAJODE_FORCED_STDERR_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_ERROR_HANDLE, stderr, L"" SAJODE_FORCED_STDERR_PATH);
#else
    changeStandardHandleTarget(stderr, SAJODE_FORCED_STDERR_PATH);
#endif
#endif
#if defined(SAJODE_FORCED_STDIN_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_INPUT_HANDLE, stdin, L"" SAJODE_FORCED_STDIN_PATH);
#else
    changeStandardHandleTarget(stdin, SAJODE_FORCED_STDIN_PATH);
#endif
#endif
#endif
    ...
    SAJODE_PRINT_TIMING("main(): Entered.");
    SAJODE_INIT_PROGRAM_EARLY(argc, argv);
...
#if _SAJODE_STANDALONE_MODE
    SAJODE_PRINT_TIMING("main(): Prepare standalone environment.");
    prepareStandaloneEnvironment();
#endif

#if _SAJODE_FROZEN > 0
    SAJODE_PRINT_TIMING("main(): Preparing frozen modules.");
...
#ifdef SAJODE_PYTHON_STATIC
    SAJODE_PRINT_TIMING("main(): Preparing static modules.");
...
#if defined(_SAJODE_ONEFILE_MODE)
    {
        environment_char_t const *parent_original_argv0 = getEnvironmentVariable("SAJODE_ORIGINAL_ARGV0");
        ...
        if (parent_original_argv0 != NULL) {
            ...
            unsetEnvironmentVariable("SAJODE_ORIGINAL_ARGV0");
        }
    }
#endif

#if PYTHON_VERSION >= 0x300 && _SAJODE_NATIVE_WCHAR_ARGV == 0
    SAJODE_PRINT_TRACE("main(): Calling convertCommandLineParameters.");
    orig_argv = convertCommandLineParameters(argc, argv);
#elif PYTHON_VERSION < 0x300 && _SAJODE_NATIVE_WCHAR_ARGV == 1
    SAJODE_PRINT_TRACE("main(): Calling getCommandLineToArgvA.");
...
// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _SAJODE_NATIVE_WCHAR_ARGV == 1
    original_argv0 = argv[0];
#if PYTHON_VERSION >= 0x300 && !defined(_SAJODE_ONEFILE_MODE)
    ...
    SAJODE_PRINT_TRACE("main(): Calling setCommandLineParameters.");
    setCommandLineParameters(argc, argv);

    /* For Python installations that need the home set, we inject it back here. */
#if defined(PYTHON_HOME_PATH)
#if PYTHON_VERSION < 0x300
    SAJODE_PRINT_TRACE("main(): Prepare run environment '" PYTHON_HOME_PATH "'.");
    Py_SetPythonHome(PYTHON_HOME_PATH);
#else
    SAJODE_PRINTF_TRACE("main(): Prepare run environment '%S'.\n", L"" PYTHON_HOME_PATH);
    ...
    SAJODE_PRINT_TRACE("main(): Disabling Python warnings.");
    {
#if PYTHON_VERSION >= 0x300
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    SAJODE_PRINT_TIMING("main(): Calling TeamYousef_Py_Initialize to initialize interpreter.");
    TeamYousef_Py_Initialize();

    PyThreadState *tstate = PyThreadState_GET();

#if _SAJODE_STANDALONE_MODE
    SAJODE_PRINT_TRACE("main(): Restore standalone environment.");
    restoreStandaloneEnvironment();
#else
    {
        environment_char_t const *python_path_cstr = getEnvironmentVariable("SAJODE_PYTHONPATH");

        if (python_path_cstr != NULL) {
            PyObject *python_path_str = TeamYousef_String_FromFilename(python_path_cstr);
#ifdef _WIN32
            ...
            TeamYousef_SysSetObject("path", python_path_list);

            unsetEnvironmentVariable("SAJODE_PYTHONPATH");
        }
    }
#endif
    ...
    PyList_SetSlice(TeamYousef_SysGetObject("path"), 0, 1, PyTuple_New(0));
#endif

    /* Initialize the built-in module tricks used and builtin-type methods */
    SAJODE_PRINT_TRACE("main(): Calling _initBuiltinModule().");
    _initBuiltinModule();

    /* Initialize the Python constant values used. This also sets
     * "sys.executable" while at it.
     */
    SAJODE_PRINT_TIMING("main(): Calling createGlobalConstants().");
    createGlobalConstants(tstate);
    SAJODE_PRINT_TIMING("main(): Returned createGlobalConstants().");

    /* Complex call helpers need "__main__" constants, even if we only
     * go into "__parents__main__" module as a start point.
     */
    SAJODE_PRINT_TIMING("main(): Calling createMainModuleConstants().");
    createMainModuleConstants(tstate);
    SAJODE_PRINT_TIMING("main(): Returned createMainModuleConstants().");

    SAJODE_PRINT_TRACE("main(): Calling _initBuiltinOriginalValues().");
...
#if PYTHON_VERSION < 0x300
    PyStructSequence_SET_ITEM(TeamYousef_SysGetObject("flags"), 9, const_int_0);
#else
    PyStructSequence_SetItem(TeamYousef_SysGetObject("flags"), 6, const_int_0);
#endif
#endif

    /* Initialize the compiled types of TeamYousef. */
    ...
    SAJODE_PRINT_TRACE("main(): Calling enhancePythonTypes().");
    enhancePythonTypes();

    SAJODE_PRINT_TRACE("main(): Calling patchTypeComparison().");
    patchTypeComparison();

    SAJODE_PRINT_TRACE("main(): Calling patchTracebackDealloc().");
    patchTracebackDealloc();

#ifndef SAJODE_USE_PYCORE_THREAD_STATE
    /* Allow to override the ticker value, to remove checks for threads in
     * CPython core from impact on benchmarks. */
    char const *ticker_value = getenv("SAJODE_TICKER");
    ...
    if (needs_stdout_attaching) {
        PyObject *filename = TeamYousef_String_FromString("CONOUT$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    if (needs_stdin_attaching) {
        PyObject *filename = TeamYousef_String_FromString("CONIN$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    SAJODE_PRINT_TRACE("main(): Setting Python input/output handles.");
    ...
    // standard loader.
    {
        SAJODE_MAY_BE_UNUSED PyObject *importlib_module = getImportLibBootstrapModule();
        CHECK_OBJECT(importlib_module);
    }
#endif

    SAJODE_PRINT_TRACE("main(): Calling setEarlyFrozenModulesFileAttribute().");

    setEarlyFrozenModulesFileAttribute(tstate);
#endif

#if _SAJODE_FROZEN > 0
    SAJODE_PRINT_TRACE("main(): Removing early frozen module table again.");
    PyImport_FrozenModules = old_frozen;
#endif

    SAJODE_PRINT_TRACE("main(): Calling setupMetaPathBasedLoader().");
...
#if NO_PYTHON_WARNINGS && PYTHON_VERSION >= 0x342 && PYTHON_VERSION < 0x3a0 && defined(_SAJODE_FULL_COMPAT)
...
#if _SAJODE_PGO_PYTHON
    // Profiling with our own Python PGO if enabled.
    PGO_Initialize();
#endif

#if PYTHON_VERSION >= 0x300
    SAJODE_PRINT_TRACE("main(): Calling patchInspectModule().");
...
// that are unexplained.
...
#if PYTHON_VERSION >= 0x300 && SYSFLAG_NO_RANDOMIZATION == 1
    SAJODE_PRINT_TRACE("main(): Reverting to initial 'PYTHONHASHSEED' value.");
    undoEnvironmentVariable(tstate, "PYTHONHASHSEED", old_env_hash_seed);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_UNBUFFERED == 1
    SAJODE_PRINT_TRACE("main(): Reverting to initial 'PYTHONUNBUFFERED' value.");
    undoEnvironmentVariable(tstate, "PYTHONUNBUFFERED", old_env_unbuffered);
#endif

#if _SAJODE_STANDALONE_MODE
    // Restore the PATH, so the program can use it.
    SAJODE_PRINT_TRACE("main(): Reverting to initial 'PATH' value.");
...
#ifdef _SAJODE_PLUGIN_MULTIPROCESSING_ENABLED
    if (unlikely(is_multiprocessing_fork)) {
        SAJODE_PRINT_TRACE("main(): Calling __parents_main__.");
        EXECUTE_MAIN_MODULE(tstate, "__parents_main__", false);

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SAJODE_PRINT_TRACE("main(): Calling __parents_main__ Py_Exit.");
        Py_Exit(exit_code);
#if defined(_WIN32)
    } else if (unlikely(is_joblib_popen_loky_win32)) {
        SAJODE_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_win32.");
        ...
        PyObject *argv_list = TeamYousef_SysGetObject("argv");
        ...
        if (loky_joblib_pipe_handle_arg == 0) {
            ...
            PyObject *kw_values[] = {
                TeamYousef_PyLong_FromLong(loky_joblib_pipe_handle_arg),
                TeamYousef_PyLong_FromLong(loky_joblib_parent_pid_arg),
            };
        ...
        SAJODE_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.popen_loky_posix' Py_Exit.");
        Py_Exit(exit_code);
#else
    } else if (unlikely(is_joblib_popen_loky_posix)) {
        SAJODE_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix.");
        ...
        int res = PyList_SetSlice(TeamYousef_SysGetObject("argv"), 0, 2, const_tuple_empty);
        assert(res == 0);
        ...
        SAJODE_PRINT_TRACE("main(): Launching as 'multiprocessing.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function,
                                      TeamYousef_PyInt_FromLong(multiprocessing_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SAJODE_PRINT_TRACE("main(): Calling 'multiprocessing.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(loky_resource_tracker_arg != -1)) {
        SAJODE_PRINT_TRACE("main(): Launching as 'joblib.externals.loky.backend.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function, TeamYousef_PyInt_FromLong(loky_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        SAJODE_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
        ...
        SAJODE_PRINT_TRACE("main(): Calling 'anyio.to_process' Py_Exit.");
        Py_Exit(exit_code);
    } else {
#endif
#if defined(_SAJODE_ONEFILE_MODE) && defined(_WIN32)
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("SAJODE_ONEFILE_PARENT", buffer, sizeof(buffer));
...
#endif
        PyDict_DelItemString(TeamYousef_GetSysModules(), SAJODE_MAIN_MODULE_NAME);
        DROP_ERROR_OCCURRED(tstate);

#if _SAJODE_PLUGIN_WINDOWS_SERVICE_ENABLED
        SAJODE_PRINT_TRACE("main(): Calling plugin SvcLaunchService() entry point.");
        SvcLaunchService();
#else
    /* Execute the "__main__" module. */
    SAJODE_PRINT_TIMING("main(): Calling " SAJODE_MAIN_MODULE_NAME ".");
    EXECUTE_MAIN_MODULE(tstate, SAJODE_MAIN_MODULE_NAME, SAJODE_MAIN_IS_PACKAGE_BOOL);
    SAJODE_PRINT_TIMING("main(): Exited from " SAJODE_MAIN_MODULE_NAME ".");
...
#if !SAJODE_MAIN_IS_PACKAGE_BOOL
    ...
    SAJODE_PRINT_TIMING("main(): Calling Py_Exit.");
    Py_Exit(exit_code);

    // The "Py_Exit()" calls is not supposed to return.
    SAJODE_CANNOT_GET_HERE("Py_Exit does not return");
}

#ifdef _SAJODE_WINMAIN_ENTRY_POINT
int __stdcall wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, wchar_t *lpCmdLine, int nCmdShow) {
    /* MSVC, MINGW64 */
    int argc = __argc;
    wchar_t **argv = __wargv;

    return TeamYousef_Main(argc, argv);
}
#else
#if defined(_WIN32)
int wmain(int argc, wchar_t **argv) { return TeamYousef_Main(argc, argv); }
#else
int main(int argc, char **argv) { return TeamYousef_Main(argc, argv); }
#endif
#endif
...
#if defined(_WIN32)
#define SAJODE_DLL_FUNCTION __declspec(dllexport)
#else
#define SAJODE_DLL_FUNCTION __attribute__((visibility("default")))
#endif

SAJODE_DLL_FUNCTION int run_code(int argc, native_command_line_argument_t **argv) { return TeamYousef_Main(argc, argv); }
#endif
