...
from SajodeSx import Options, Tracing
...
from SajodeSx.utils.AppDirs import getCacheDirEnvironmentVariableName
from SajodeSx.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from SajodeSx.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while TeamYousef works with older Python, Scons does not, and therefore
TeamYousef needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    os.environ["SAJODE_PYTHON_EXE_PATH"] = sys.executable
    ...
    os.environ["SAJODE_PACKAGE_DIR"] = os.path.abspath(SajodeSx.__path__[0])
    ...
    del os.environ["SAJODE_PYTHON_EXE_PATH"]

    del os.environ["SAJODE_PACKAGE_DIR"]
...
def _setupSconsEnvironment():
    ...
    For the target Python we provide "SAJODE_PYTHON_DLL_PATH" to see where the
    Python DLL lives, in case it needs to be copied, and then also the
    "SAJODE_PYTHON_EXE_PATH" to find the Python binary itself.
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        env_values["_SAJODE_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["SajodeSx_src"] = getExternalUsePath(
                scons_options["SajodeSx_src"]
            )
        ...
        env_values["SAJODE_QUIET"] = "1" if Tracing.is_quiet else "0"
...
def getCommonSconsOptions():
    ...
    scons_options["SajodeSx_src"] = getSconsDataPath()
    ...
    if "CompanyName" in string_values:
        env_values["SAJODE_COMPANY_NAME"] = string_values["CompanyName"]
    if "ProductName" in string_values:
        env_values["SAJODE_PRODUCT_NAME"] = string_values["ProductName"]
    ...
    if effective_version:
        env_values["SAJODE_VERSION_COMBINED"] = effective_version

    if isTeamYousefPython() and not isWin32OrPosixWindows():
    ...
    if Options.isOnefileMode():
        ...
        # on it.
        env_values["_SAJODE_ONEFILE_CHILD_GRACE_TIME_INT"] = str(
            getOnefileChildGraceTime()
        )
