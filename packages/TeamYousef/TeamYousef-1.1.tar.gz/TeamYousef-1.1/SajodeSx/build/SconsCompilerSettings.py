...
from SajodeSx.Tracing import scons_details_logger, scons_logger
from SajodeSx.utils.Download import getCachedDownloadedMinGW64
...
be usable with TeamYousef. Use the Visual Studio installer for \
adding it."""
        )
...
def _enableC11Settings(env):
    ...
    if env.clangcl_mode:
        ...
        # For now, zig doesn't support C11 mode in the form needed by TeamYousef
    ...
    if not c11_mode:
        env.Append(CPPDEFINES=["_SAJODE_NON_C11_MODE"])
...
def _enableLtoSettings(
    ...
    elif env.SajodeSx_python:
        lto_mode = True
        reason = "known to be supported (TeamYousef-Python)"
...
C compiler ('%s' -> '%s') arches, that compiler is ignored!"""
                    % (
                        os.environ["SAJODE_PYTHON_EXE_PATH"],
                        linker_arch,
                        compiler_path,
                        compiler_arch,
                    )
        ...
        if compiler_path is not None:
            ...
            if isGccName(the_cc_name):
                if "force-accept-windows-gcc" not in env.experimental_flags:
                    scons_logger.info(
                        "Non downloaded winlibs-gcc '%s' is being ignored, TeamYousef is very dependent on the precise one."
                        % (compiler_path,)
                    )
...
def decideConstantsBlobResourceMode(env):
    if "SAJODE_RESOURCE_MODE" in os.environ:
        resource_mode = os.environ["SAJODE_RESOURCE_MODE"]
...
def addConstantBlobFile(env, blob_filename, resource_desc, target_arch):
    ...
    if resource_mode == "win_resource":
        # On Windows constants can be accessed as a resource by TeamYousef at run time afterwards.
        env.Append(CPPDEFINES=["_SAJODE_CONSTANTS_FROM_RESOURCE"])
    elif resource_mode == "mac_section":
        env.Append(CPPDEFINES=["_SAJODE_CONSTANTS_FROM_MACOS_SECTION"])
        ...
        env.Append(CPPDEFINES=["_SAJODE_CONSTANTS_FROM_INCBIN"])
...
#define INCBIN_LOCAL
#ifdef _SAJODE_EXPERIMENTAL_WRITEABLE_CONSTANTS
#define INCBIN_OUTPUT_SECTION ".data"
#endif

#include "SajodeSx/incbin.h"
...
"""
        ...
        env.Append(CPPDEFINES=["_SAJODE_CONSTANTS_FROM_LINKER"])
        ...
        env.Append(CPPDEFINES=["_SAJODE_CONSTANTS_FROM_CODE"])
...
#if !defined(_SAJODE_EXPERIMENTAL_WRITEABLE_CONSTANTS)
const
#endif
...
def setupCCompiler(env, lto_mode, pgo_mode, job_count, onefile_compile):
    ...
    if env.gcc_mode and env.lto_mode:
        if env.debug_mode:
            ...
            # thing, and TeamYousef-Python is not affected.
            env.Append(
                LINKFLAGS=[
                    (
                        "-O3"
                        if env.SajodeSx_python
                        or os.name == "nt"
    ...
    if env.debug_mode:
        ...
        env.Append(CPPDEFINES=["__SAJODE_NO_ASSERT__"])
    ...
    if env.console_mode in ("attach", "disable"):
        if env.mingw_mode:
            ...
            env.Append(CPPDEFINES=["_SAJODE_WINMAIN_ENTRY_POINT"])
        elif env.msvc_mode:
            env.Append(LINKFLAGS=["/SUBSYSTEM:windows"])
            env.Append(CPPDEFINES=["_SAJODE_WINMAIN_ENTRY_POINT"])

    if env.console_mode == "attach" and os.name == "nt":
        env.Append(CPPDEFINES=["_SAJODE_ATTACH_CONSOLE_WINDOW"])

    if env.console_mode == "hide" and os.name == "nt":
        env.Append(CPPDEFINES=["_SAJODE_HIDE_CONSOLE_WINDOW"])
    ...
    zlib_inline_copy_dir = os.path.join(env.SajodeSx_src, "inline_copy", "zlib")
    if os.path.exists(os.path.join(zlib_inline_copy_dir, "crc32.c")):
        ...
        if onefile_compile:
            env.Append(CPPDEFINES=["_SAJODE_USE_OWN_CRC32"])
        else:
            env.Append(CPPDEFINES=["_SAJODE_USE_SYSTEM_CRC32"])
            env.Append(LIBS="z")
...
def _enablePgoSettings(env):
    if env.pgo_mode == "no":
        ...
        env.Append(CPPDEFINES=["_SAJODE_PGO_PYTHON"])
    elif env.pgo_mode == "generate":
        env.progressbar_name = "Profile"
        env.Append(CPPDEFINES=["_SAJODE_PGO_GENERATE"])
        ...
        env.Append(CPPDEFINES=["_SAJODE_PGO_USE"])
