...
""" Command line options of TeamYousef.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "SajodeSx.Options" values is not in
...
from SajodeSx.PythonFlavors import getPythonFlavorName
from SajodeSx.PythonVersions import isPythonWithGil
from SajodeSx.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from SajodeSx.utils.FileOperations import getFileContentByLine
from SajodeSx.utils.Utils import (
    getArchitecture,
...
from SajodeSx.Version import getCommercialVersion, getTeamYousefVersion

# Indicator if we were called as "SajodeSx-run" in which case we assume some
# other defaults and work a bit different with parameters.
_SajodeSx_binary_name = os.path.basename(sys.argv[0])
if _SajodeSx_binary_name == "__main__.py":
    _SajodeSx_binary_name = "%s -m SajodeSx" % os.path.basename(sys.executable)
is_SajodeSx_run = _SajodeSx_binary_name.lower().endswith("-run")

if not is_SajodeSx_run:
    usage_template = (
...
parser = makeOptionsParser(usage=usage_template % _SajodeSx_binary_name)
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run TeamYousef, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run TeamYousef, most
...
Python binary to use for Scons. Otherwise TeamYousef can
use what you run TeamYousef with, or find Python installation,
...
and TeamYousef will then find it and include it and all the modules found below that
...
and TeamYousef will then find it and include it in the binary or extension module
...
with "SajodeSx-onefile-unpack" rather than a stream that only the onefile
...
warnings_group = parser.add_option_group("Control the warnings to be given by TeamYousef")
...
warnings_group.add_option(
    ...
    help="""\
Allow TeamYousef to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the TeamYousef website. The mnemonic is the part
...
debugger is automatically chosen unless specified by name with the SAJODE_DEBUGGER_CHOICE
...
remove bloat, add hidden dependencies. Check the TeamYousef Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by TeamYousef. This is
...
created by TeamYousef. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in TeamYousef, do not use for
...
development_group = parser.add_option_group("TeamYousef Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for TeamYousef development only. Takes
...
Note: This is experimental and not working with standalone modes of TeamYousef yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of TeamYousef commercial.
...
intended for developing plugins and TeamYousef package configuration. Use e.g.
...
def getTeamYousefProjectOptions(logger, filename_arg, module_mode):
    """Extract the TeamYousef project options.

    Note: This is used by TeamYousef project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of TeamYousef handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)SajodeSx-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'SajodeSx-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'SajodeSx-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'SajodeSx-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'SajodeSx-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'SajodeSx-project-else' not currently allowed after nested SajodeSx-project-if.",
                    )
...
def _considerGithubWorkflowOptions(phase):
    ...
    for key, value in json.loads(os.environ["SAJODE_WORKFLOW_INPUTS"]).items():
        if not value:
            continue
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getTeamYousefProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
pass them to the compiled program execution."""
    ...
    return is_SajodeSx_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from SajodeSx.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from SajodeSx.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from SajodeSx.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from SajodeSx.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.list_distribution_metadata:
        from SajodeSx.tools.scanning.DisplayDistributions import (
    ...
    if options.edit_module_code:
        from SajodeSx.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from SajodeSx.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from SajodeSx.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
