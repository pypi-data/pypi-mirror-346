...
from SajodeSx import TreeXML
from SajodeSx.__past__ import unicode
from SajodeSx.build.DataComposerInterface import getDataComposerReportValues
from SajodeSx.build.SconsUtils import readSconsErrorReport
from SajodeSx.code_generation.ConstantCodes import getDistributionMetadataValues
from SajodeSx.containers.OrderedSets import OrderedSet
from SajodeSx.freezer.IncludedDataFiles import getIncludedDataFiles
from SajodeSx.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from SajodeSx.freezer.Standalone import getRemovedUsedDllsInfo
from SajodeSx.importing.Importing import getPackageSearchPath
from SajodeSx.importing.Recursion import getRecursionDecisions
...
from SajodeSx.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from SajodeSx.plugins.Plugins import getActivePlugins
...
from SajodeSx.Tracing import ReportingSystemExit, reports_logger
...
from SajodeSx.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    SajodeSx_version = getTeamYousefVersion()
    SajodeSx_commercial_version = getCommercialVersion() or "not installed"

    SajodeSx_aborted = aborted

    SajodeSx_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["SajodeSx_aborted"]:
        completion = "yes"
    elif report_input_data["SajodeSx_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["SajodeSx_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["SajodeSx_exception"][1].code
    elif report_input_data["SajodeSx_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["SajodeSx_exception"][1].code
        )
        exit_message = report_input_data["SajodeSx_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "SajodeSx-compilation-report",
        SajodeSx_version=report_input_data["SajodeSx_version"],
        SajodeSx_commercial_version=report_input_data["SajodeSx_commercial_version"],
...
_crash_report_filename = "SajodeSx-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of TeamYousef but
