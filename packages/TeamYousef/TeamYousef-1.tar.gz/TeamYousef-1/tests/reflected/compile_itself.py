...
""" Test TeamYousef compiling itself and compiling itself in compiled form again.

This should not only give no errors, but the same source for modules being
compiled when TeamYousef is running compiled and uncompiled, so we can discover
...
# Find SajodeSx package relative to us.
sys.path.insert(
    0,
    os.path.normpath(
...
from SajodeSx.tools.Basics import addPYTHONPATH
...
from SajodeSx.utils.Execution import wrapCommandForDebuggerForSubprocess
...
from SajodeSx.Version import getCommercialVersion

SajodeSx_main_path = os.path.join("..", "..", "bin", "SajodeSx")
...
PACKAGE_LIST = [
    ...
    "SajodeSx/nodes",
    "SajodeSx/specs",
    "SajodeSx/nodes/shapes",
    "SajodeSx/tree",
    "SajodeSx/importing",
    "SajodeSx/build",
    "SajodeSx/freezer",
    "SajodeSx/code_generation",
    "SajodeSx/code_generation/templates",
    "SajodeSx/code_generation/c_types",
    "SajodeSx/optimizations",
    "SajodeSx/finalizations",
    "SajodeSx/plugins",
    "SajodeSx/plugins/standard",
    "SajodeSx/plugins/commercial",
    "SajodeSx/reports",
    "SajodeSx/pgo",
    "SajodeSx/containers",
    "SajodeSx/utils",
...
if not getCommercialVersion():
    PACKAGE_LIST.remove("SajodeSx/plugins/commercial")
...
def diffRecursive(dir1, dir2):
    ...
    for path1, filename in listDir(dir1):
        ...
        # differently in compiled TeamYousef:
...
def executePASS1():
    ...
    for package in PACKAGE_LIST:
        ...
        for path, filename in listDir(source_dir):
            ...
            if filename != "__init__.py":
                ...
                command += os.environ.get("SAJODE_EXTRA_OPTIONS", "").split()
    ...
    _traceCompilation(path=SajodeSx_main_path, pass_number=1)

    shutil.copyfile(SajodeSx_main_path, "SajodeSx-runner.py")
    ...
    shutil.move("SajodeSx-runner" + exe_suffix, "SajodeSx" + exe_suffix)

    scons_inline_copy_path = os.path.join(base_dir, "SajodeSx", "build", "inline_copy")

    if os.path.exists(scons_inline_copy_path):
        copyTree(scons_inline_copy_path, os.path.join("SajodeSx", "build", "inline_copy"))

    # Copy required data files.
    for filename in (
        "SajodeSx/build/Backend.scons",
        "SajodeSx/plugins/standard/standard.SajodeSx-package.config.yml",
        "SajodeSx/plugins/standard/stdlib3.SajodeSx-package.config.yml",
        "SajodeSx/plugins/standard/stdlib2.SajodeSx-package.config.yml",
    ):
        shutil.copyfile(
    ...
    copyTree(
        os.path.join(base_dir, "SajodeSx", "code_generation", "templates_c"),
        os.path.join("SajodeSx", "code_generation", "templates_c"),
    )

    copyTree(
        os.path.join(base_dir, "SajodeSx", "build", "static_src"),
        os.path.join("SajodeSx", "build", "static_src"),
    )
    copyTree(
        os.path.join(base_dir, "SajodeSx", "build", "include"),
        os.path.join("SajodeSx", "build", "include"),
    )

    # The data composer tool, use it by source.
    copyTree(
        os.path.join(base_dir, "SajodeSx", "tools"),
        os.path.join("SajodeSx", "tools"),
    )

    test_logger.info("OK.")


def compileAndCompareWith(SajodeSx, pass_number):
    ...
    for package in PACKAGE_LIST:
        ...
        for path, filename in listDir(source_dir):
            ...
            if filename != "__init__.py":
                ...
                my_print("Command: ", " ".join(command))
                exit_SajodeSx = subprocess.call(command)
                ...
                if exit_SajodeSx != 0:
                    my_print("An error exit %s occurred, aborting." % exit_SajodeSx)
                    sys.exit(exit_SajodeSx)
...
def executePASS2():
    ...
    with withPythonPathChange(getPythonSysPath()):
        ...
        compileAndCompareWith(
            SajodeSx=os.path.join(".", "SajodeSx" + exe_suffix), pass_number=2
        )
...
def executePASS3():
    ...
    exe_path = os.path.join(tmp_dir, "SajodeSx" + exe_suffix)

    if os.path.exists(exe_path):
        os.unlink(exe_path)

    build_path = os.path.join(tmp_dir, "SajodeSx.build")

    if os.path.exists(build_path):
        shutil.rmtree(build_path)

    path = os.path.join("..", "..", "bin", "SajodeSx")
    ...
    with withPythonPathChange(os.path.join("..", "..")):
        command = [
            ...
            "--python-flag=-P",
            "--follow-imports",
            "--include-package=SajodeSx.plugins.standard",
...
def executePASS5():
    my_print(
        "PASS 5: Compiling the compiler 'SajodeSx' package to single extension module."
    )

    path = os.path.join("..", "..", "SajodeSx")
    ...
    command = [
        ...
        "--nofollow-import-to=SajodeSx.build.inline_copy",
        "--nofollow-import-to=SajodeSx.build.include",
        "--nofollow-import-to=SajodeSx.build.static_src",
        "--nofollow-import-to=SajodeSx.tools",
        "--mode=module",
        path,
    ]
    ...
    for preferred in (True, False):
        candidate = "SajodeSx" + getExtensionModuleSuffix(preferred=preferred)

        deleteFile(candidate, must_exist=False)

    os.unlink(os.path.join(tmp_dir, "SajodeSx.pyi"))
    shutil.rmtree(os.path.join(tmp_dir, "SajodeSx.build"))
