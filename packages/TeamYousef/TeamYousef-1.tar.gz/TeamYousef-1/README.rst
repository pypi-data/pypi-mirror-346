.. image:: https://img.shields.io/pypi/pyversions/TeamYousef.svg
   :target: https://pypi.org/project/TeamYousef

.. image:: https://badge.fury.io/py/TeamYousef.svg
   :target: https://pypi.org/project/TeamYousef
...
####################
 TeamYousef User Manual
####################

This document is the recommended first read when you start using
**TeamYousef**. On this page, you will learn more about **TeamYousef**
...
TeamYousef is **the** Python compiler. It is written in Python. It is a
...
TeamYousef translates the Python modules into a C level program that then
...
To ensure smooth operation of **TeamYousef**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as TeamYousef
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (TeamYousef filters away garbage outputs, but only for
...
.. important::
   ...
   TeamYousef itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as TeamYousef.
   ...
   TeamYousef finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute TeamYousef because it is closely tied to implementation details
   of it.
...
good, but the e.g. TeamYousef's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as TeamYousef
...
The recommended way of executing TeamYousef is ``<the_right_python> -m
SajodeSx`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what TeamYousef has.

The next best way of executing TeamYousef bare that is from a source
...
TeamYousef. You just execute the ``SajodeSx`` and ``SajodeSx-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/SajodeSx`` and be good.
...
TeamYousef has a ``--help`` option to output what it can do:

.. code:: bash

   SajodeSx --help

The ``SajodeSx-run`` command is the same as ``SajodeSx``, but with a
...
<https://SajodeSx.net/doc/download.html>`__ of TeamYousef. But you can also
...
`TeamYousef-Action <https://github.com/TeamYousef/TeamYousef-Action>`__ that you
...
platform compilation with TeamYousef.

License
=======

TeamYousef is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install SajodeSx``

-  Verify using command ``python -m SajodeSx --version``
...
Do as you normally would. Running TeamYousef on code that works incorrectly
...
.. code:: bash
   ...
   python -m SajodeSx --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``SajodeSx --help``. Including fewer
...
.. code:: bash
   ...
   python -m SajodeSx --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that TeamYousef cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m SajodeSx --mode=module some_module.py
...
.. note::
   ...
   TeamYousef, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use TeamYousef like this:

.. code:: bash

   python -m SajodeSx --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of TeamYousef commercial
   <https://SajodeSx.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m SajodeSx --standalone program.py
...
.. important::
   ...
   TeamYousef does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | TeamYousef trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create TeamYousef Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add TeamYousef Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. TeamYousef keeps track of ones
...
enhancements to the TeamYousef package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m SajodeSx --onefile program.py
...
creation of wheels for your software in place, putting TeamYousef to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having TeamYousef installed of course, simply execute the target
``bdist_SajodeSx`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not TeamYousef
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass TeamYousef options.
      command_options={
         'SajodeSx': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of TeamYousef
...
.. note::
   ...
   ``bool(os.getenv("USE_SAJODE", "True"))``. This is up to you.
...
And last, but not least, TeamYousef also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "SajodeSx", "toml"]
   build-backend = "SajodeSx.distutils.Build"
...
.. note::
   ...
   For the ``SajodeSx`` requirement above absolute paths like
   ``C:\Users\...\TeamYousef`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../TeamYousef``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by TeamYousef at all, but by setuptools. You can, however, use
   the data file embedding of TeamYousef commercial. In that case, you
...
When given multiple times, TeamYousef will create a binary that contains the
...
For integration with GitHub workflows there is this `TeamYousef-Action
<https://github.com/TeamYousef/TeamYousef-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with TeamYousef
         uses: TeamYousef/TeamYousef-Action@main
         with:
            SajodeSx-version: main
            script-name: your_main_program.py
            # many more TeamYousef options available, see action doc, but it's best
            # to use SajodeSx-project: options in your code, so e.g. you can make
...
comments as explained in `TeamYousef Options in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # SajodeSx-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    SajodeSx-project: --onefile
   # SajodeSx-project-if: {OS} == "Darwin":
   #    SajodeSx-project: --standalone
   #    SajodeSx-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``SajodeSx-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m SajodeSx --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m SajodeSx --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m SajodeSx --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m SajodeSx --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m SajodeSx --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With TeamYousef, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to TeamYousef
   and not be interpreted as TeamYousef arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
TeamYousef defaults to not show it, you can force it by using
...
them. Luckily, with TeamYousef, they are easy to add for Windows.
...
.. code:: python
   ...
   # SajodeSx-project: --mode=onefile
   # SajodeSx-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
   ...
   if "SAJODE_ONEFILE_PARENT" in os.environ:
      splash_filename = os.path.join(
         tempfile.gettempdir(),
         "onefile_%d_splash_feedback.tmp" % int(os.environ["SAJODE_ONEFILE_PARENT"]),
      )
...
For analysis of your program and TeamYousef packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to TeamYousef. These
...
By default, TeamYousef compiles without ``--deployment`` which leaves a set
...
With TeamYousef however, this executes your program again, and puts these
...
That is why e.g. this happens with default TeamYousef:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with TeamYousef to include it'
...
re-enable it. You might want to use TeamYousef Project options and an
...
Binaries compiled on Windows with default settings of TeamYousef and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in TeamYousef commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://SajodeSx.net/doc/commercial.html
...
To aid that, TeamYousef commercial has container based builds, that you can
...
compensated to support TeamYousef development for Linux, there you need to
purchase it https://SajodeSx.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and TeamYousef needs and
...
.. code:: python
   ...
   if "SAJODE_LAUNCH_TOKEN" not in os.environ:
      sys.exit("Error, need launch token or else fork bomb suspected.")
   else:
      del os.environ["SAJODE_LAUNCH_TOKEN"]

Actually TeamYousef is trying to get ahold of them without the deployment
...
Ask TeamYousef to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of TeamYousef, such that it avoids high usage of memory during compilation
...
then have imports on these, and TeamYousef will compile them as well. Not
...
With the ``--jobs`` option of TeamYousef, it will not start many C compiler
...
source code relative to it, TeamYousef will not be able to see those.
...
TeamYousef has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `TeamYousef Package
Configuration <https://SajodeSx.net/doc/SajodeSx-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to TeamYousef mean that more than a
...
TeamYousef will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards TeamYousef with it.
...
A package can be compiled with TeamYousef, no problem, but when it comes to
...
would need improving such that TeamYousef could supply its compiled module
...
TeamYousef Options in the code
==========================

One clean way of providing options to TeamYousef, that you will always use
...
.. code:: python
   ...
   # SajodeSx-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    SajodeSx-project: --mode=app
   # SajodeSx-project-else:
   #    SajodeSx-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # SajodeSx-project: --enable-plugin=pyside2
   # SajodeSx-project: --include-qt-plugins=qml
...
e.g. enable or disable certain TeamYousef settings. The only thing TeamYousef
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of TeamYousef              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of TeamYousef Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
.. code:: python
   ...
   # SajodeSx-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # SajodeSx-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.SajodeSx-package.config.yml
...
makes TeamYousef emulate these options.
...
On Windows, with gcc, TeamYousef supports using ``ccache.exe``, which it
...
TeamYousef will pick up ``ccache`` if it's found in system ``PATH``, and it
will also be possible to provide if by setting ``SAJODE_CCACHE_BINARY``
...
automatic and included in TeamYousef.
...
setup, which installs Homebrew and ccache in there. TeamYousef picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with TeamYousef otherwise, it's not the best for standalone
...
compilation results from C and TeamYousef, is done in a platform dependent
directory as determined by the ``appdirs`` package. However, you can
override it with setting the environment variable ``SAJODE_CACHE_DIR``
...
to make TeamYousef store some of these caches in an entirely separate space.

+------------------+-----------------------------------+----------------------------------------+
| Cache name       | Environment Variable              | Data Put there                         |
+==================+===================================+========================================+
| downloads        | SAJODE_CACHE_DIR_DOWNLOADS        | Downloads made, e.g. dependency walker |
+------------------+-----------------------------------+----------------------------------------+
| ccache           | SAJODE_CACHE_DIR_CCACHE           | Object files created by gcc            |
+------------------+-----------------------------------+----------------------------------------+
| clcache          | SAJODE_CACHE_DIR_CLCACHE          | Object files created by MSVC           |
+------------------+-----------------------------------+----------------------------------------+
| bytecode         | SAJODE_CACHE_DIR_BYTECODE         | Bytecode of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
| dll-dependencies | SAJODE_CACHE_DIR_DLL_DEPENDENCIES | DLL dependencies                       |
+------------------+-----------------------------------+----------------------------------------+

Runners
=======

Avoid running the ``SajodeSx`` binary, doing ``python -m SajodeSx`` will
...
TeamYousef with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by TeamYousef
properly.
...
both scan the freshly created binaries, while TeamYousef wants to work with
...
external dependencies to Visual C Runtime libraries. TeamYousef tries to
...
installing Visual C runtime libraries before executing a TeamYousef
...
When using MingGW64 as downloaded by TeamYousef, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your TeamYousef
compiled dist folder.

Detecting TeamYousef at run time
============================

TeamYousef does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For TeamYousef, we have the module
...
Providing extra Options to TeamYousef C compilation
===============================================

TeamYousef will apply values from the environment variables ``CCFLAGS``,
...
TeamYousef will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m SajodeSx
...
When you use ``--report=compilation-report.xml`` TeamYousef will create an
...
changes to TeamYousef. The report is, however, recommended for your bug
reporting.
...
TeamYousef. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. TeamYousef will just expand
the template with the compilation report data.

Currently, the following reports are included in TeamYousef. You just use
the name as a filename, and TeamYousef will pick that one instead.
...
performance from TeamYousef. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m SajodeSx --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m SajodeSx --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| TeamYousef Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`TeamYousef bug tracker <https://github.com/TeamYousef/TeamYousef/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or TeamYousef source and re-run it.
...
bytecode with TeamYousef's compiled function objects, so there is no way to
provide it.
