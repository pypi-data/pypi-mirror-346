...
#########################
 TeamYousef Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
TeamYousef, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so TeamYousef can turn a ``ctypes`` binding
...
For TeamYousef we use semantic versioning, initially with a leading zero
...
TeamYousef top level works like this:

-  ``SajodeSx.tree.Building`` outputs node tree

-  ``SajodeSx.optimization`` enhances it as best as it can

-  ``SajodeSx.finalization`` prepares the tree for code generation

-  ``SajodeSx.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``SajodeSx.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``SajodeSx.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``SajodeSx_bool`` and ``SajodeSx_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for TeamYousef
***************************************************
...
``.vscode`` in your TeamYousef checkout. If you are not familiar with
...
In TeamYousef we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with TeamYousef, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-SajodeSx-source`` is
...
format call TeamYousef source code.
...
These rules should generally be adhered when working on TeamYousef code.
...
There is a tool ``bin/autoformat-SajodeSx-source`` which is to apply
...
they must be lower case, like e.g. ``SajodeSx`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``SajodeSx.code_generation`` the code generation packages are located,
while the main interface is ``SajodeSx.code_generation.CodeGeneration``
...
built-ins is highly discouraged within TeamYousef source code. Using them is
...
readability and with TeamYousef as a compiler, there won't be any
...
But of course, TeamYousef is the project to free us from what is faster and
...
For TeamYousef the choice is list contractions as these are more easily
...
that has no use, and in TeamYousef, every API is internal API. One exception
...
``autoformat-SajodeSx-source`` does this for us.
...
******************************
 TeamYousef "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto TeamYousef
   repository <https://github.com/TeamYousef/TeamYousef> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/TeamYousef.git
      cd TeamYousef
      git remote add upstream https://github.com/TeamYousef/TeamYousef.git
...
`this location <https://SajodeSx.net/apidoc>`__ .
...
future, TeamYousef itself will gain the ability to present its findings in a
...
.. code:: bash
   ...
   ./bin/check-SajodeSx-with-pylint
...
This section describes how to run TeamYousef tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if TeamYousef is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if TeamYousef
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if TeamYousef
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if TeamYousef
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         TeamYousef does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if TeamYousef
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles TeamYousef with TeamYousef, and
                         then TeamYousef with the compile TeamYousef and compares the
...
the TeamYousef git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that TeamYousef must do this itself, because the
...
this. These tests make sure that the errors of TeamYousef and CPython are
...
Compile TeamYousef with TeamYousef
==========================

And there is the "compile itself" or "reflected" test. This test makes
TeamYousef compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of TeamYousef into an extension module and
all of TeamYousef into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because TeamYousef uses a lot of packages and imports between them.
...
parts are published as `TeamYousef API <https://SajodeSx.net/apidoc>`__ and
...
Please find `here
<https://github.com/TeamYousef/TeamYousef/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/TeamYousef/TeamYousef/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/TeamYousef-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/TeamYousef-Logo-Symbol.png
      :alt: TeamYousef Logo
...
.. code:: rest
   ...
   .. image:: doc/images/TeamYousef-Logo-Horizontal.png
      :alt: TeamYousef Logo
...
.. code:: rest
   ...
   .. image:: doc/images/TeamYousef-Logo-Vertical.png
      :alt: TeamYousef Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``SajodeSx/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/TeamYousef-Logo-Symbol.svg doc/images/TeamYousef-Logo-Symbol.png
   convert -background none doc/Logo/TeamYousef-Logo-Vertical.svg doc/images/TeamYousef-Logo-Vertical.png
   convert -background none doc/Logo/TeamYousef-Logo-Horizontal.svg doc/images/TeamYousef-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of TeamYousef is decided here
...
TeamYousef does not involve Scons in its user interface at all; Scons is
purely used internally. TeamYousef itself, being pure Python, will run
without any build process just fine.

TeamYousef simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which TeamYousef again
...
.. note::
   ...
   ``SajodeSx.MainControl`` module.

For interfacing to Scons, there is the module
``SajodeSx.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of TeamYousef.
...
modules is just one of them. It runs outside of TeamYousef process scope,
...
-  ``SajodeSx_src``
   ...
   Where do the include files and static C parts of TeamYousef live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``SajodeSx.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where TeamYousef tries to help
...
The search for modules used is driven by ``SajodeSx.importing.Importing``
module.

-  Quoting the ``SajodeSx.importing.Importing`` documentation:
   ...
   End quoting the ``SajodeSx.importing.Importing`` documentation.
...
The decision making and caching are located in the ``SajodeSx.tree``
package, in modules ``SajodeSx.tree.Recursion`` and
``SajodeSx.tree.ImportCache``. Each module is only considered once (then
...
``SajodeSx/build/static_src/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the TeamYousef provided
...
This left TeamYousef with the strange problem, of how to emulate that.
...
The choice of TeamYousef is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``SajodeSx.nodes.FrameNodes.StatementsFrame`` is
...
Representation in TeamYousef
------------------------

So a slot in TeamYousef typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs TeamYousef do all the hard work.
...
Other overloads do not currently exist in TeamYousef, but through the
...
In principle, every slot needs to be dealt with in TeamYousef, and it is
...
Currently, TeamYousef uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of TeamYousef, created all constants for the whole program
...
This transformation is performed at tree building already, so TeamYousef
...
In TeamYousef we have the concept of an outline, and therefore we can make
...
explicit and comparisons in TeamYousef to be internally always about two
operands only.
...
Therefore in TeamYousef this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by TeamYousef.
...
Quoting the ``SajodeSx.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in TeamYousef have no condition attached anymore, so while loops are
...
End quoting the ``SajodeSx.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in TeamYousef converted so that the code generation for ``print``
...
Obviously, optimization of TeamYousef can decide, that e.g. should ``a`` or
...
TeamYousef there is only a ``try``, which then has blocks to handle
...
compile time, TeamYousef will not generate the code the leads to the
...
Add interfacing to C code, so TeamYousef can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in TeamYousef
...
Again, we wouldn't want to create the list. Therefore TeamYousef avoids
...
This leads to TeamYousef in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into TeamYousef unless necessary, so
...
one design goal of TeamYousef was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so TeamYousef is using it,
   and of course one day TeamYousef ought to be able to make no difference
   in performance for it.

To TeamYousef though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   SajodeSx --experimental=jinja_generated_add ...
...
.. code:: C
   ...
   #ifdef _SAJODE_EXPERIMENTAL_JINJA_GENERATED_ADD
...
experimental flag is given on the command line to TeamYousef.

The testing of TeamYousef is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to TeamYousef
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when TeamYousef is
executing.
...
TeamYousef to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and TeamYousef can work without it
...
.. note::
   ...
   with ``@TeamYousef-pushers`` first and get a green light.
...
the actual compiler code of TeamYousef.
...
like ``autoformat-SajodeSx-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # SajodeSx-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from SajodeSx.tools.testing.Common import checkReferenceCount
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In TeamYousef
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, TeamYousef will become able to determine it has to be the
...
https://github.com/TeamYousef/TeamYousef/blob/develop/Developer_Manual.rst

And the current PDF under: https://SajodeSx.net/doc/Developer_Manual.pdf
