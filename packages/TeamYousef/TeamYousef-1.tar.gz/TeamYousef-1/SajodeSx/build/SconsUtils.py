...
from SajodeSx.__past__ import basestring, unicode
from SajodeSx.containers.OrderedDicts import OrderedDict
from SajodeSx.Progress import enableThreading
from SajodeSx.PythonFlavors import isTermuxPython
...
from SajodeSx.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def _enableFlagSettings(env, name, experimental_flags):
    for flag_name in experimental_flags:
        ...
        if value:
            env.Append(CPPDEFINES=[("_SAJODE_%s" % flag_name, value)])
        else:
            env.Append(CPPDEFINES=["_SAJODE_%s" % flag_name])
...
def createEnvironment(
    ...
    env.SajodeSx_python = getArgumentBool("SajodeSx_python", False)
    ...
    if env.standalone_mode:
        env.Append(CPPDEFINES=["_SAJODE_STANDALONE_MODE"])
    ...
    if env.onefile_mode:
        env.Append(CPPDEFINES=["_SAJODE_ONEFILE_MODE"])
    ...
    if env.onefile_dll_mode:
        env.Append(CPPDEFINES=["_SAJODE_ONEFILE_DLL_MODE"])
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.SajodeSx_src, "static_src", sub_path)
...
def createDefinitionsFile(source_dir, filename, definitions):
    for env_name in os.environ["_SAJODE_BUILD_DEFINITIONS_CATALOG"].split(","):
    ...
    with openTextFile(build_definitions_filename, "w", encoding="utf8") as f:
        for key, value in sorted(definitions.items()):
            if key == "_SAJODE_BUILD_DEFINITIONS_CATALOG":
                continue
    ...
    if not _linker_arch_determined:
        if os.name == "nt":
            ...
            _linker_arch = _getBinaryArch(
                binary=os.environ["SAJODE_PYTHON_EXE_PATH"], mingw_mode=mingw_mode
            )
...
b) Using "--mingw64" lets TeamYousef download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_SajodeSx_temp.pyd" if env.module_mode else "_SajodeSx_temp.exe",
        )
