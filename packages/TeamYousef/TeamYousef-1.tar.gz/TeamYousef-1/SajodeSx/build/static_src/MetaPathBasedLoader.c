...
extern PyTypeObject TeamYousef_Loader_Type;

struct TeamYousef_LoaderObject {
...
#elif _SAJODE_SYSFLAG_VERBOSE
static inline bool isVerbose(void) { return true; }
#else
static inline bool isVerbose(void) { return false; }
#endif

static struct TeamYousef_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
#if defined(_WIN32) && _SAJODE_STANDALONE_MODE

static void appendModuleNameAsPathW(wchar_t *buffer, PyObject *module_name, size_t buffer_size) {
    Py_ssize_t size;
    wchar_t const *module_name_wstr = TeamYousef_UnicodeAsWideString(module_name, &size);
...
SAJODE_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = TeamYousef_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyThreadState *tstate, PyObject *module, PyCodeObject *code_object,
    ...
    {
        SAJODE_MAY_BE_UNUSED bool b_res = TeamYousef_SetModuleString(name, module);
        assert(b_res != false);
    }
    ...
    PyObject *module_path_name = TeamYousef_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, TeamYousef_String_AsString(module_path));
...
static struct TeamYousef_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct TeamYousef_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);

    while (current->name != NULL) {
        if ((current->flags & SAJODE_TRANSLATED_FLAG) != 0) {
            current->name = UN_TRANSLATE(current->name);
            current->flags -= SAJODE_TRANSLATED_FLAG;
        }
...
#if !_SAJODE_STANDALONE_MODE
static struct TeamYousef_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct TeamYousef_MetaPathBasedLoaderEntry *current = loader_entries;
    ...
    while (current->name != NULL) {
        ...
        if ((current->flags & SAJODE_PACKAGE_FLAG) != 0) {
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = TeamYousef_GetSysModules();
    ...
    char const *module_relative_name_str = TeamYousef_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (TeamYousef_String_CheckExact(filename)) {
                char const *filename_str = TeamYousef_String_AsString(filename);
    ...
    PRINT_STRING(TeamYousef_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = TeamYousef_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = TeamYousef_String_AsString(candidate);
...
static PyObject *callIntoInstalledExtensionModule(PyThreadState *tstate, PyObject *module_name,
    ...
    wchar_t const *extension_module_filename_str = TeamYousef_UnicodeAsWideString(extension_module_filename, NULL);
#else
    char const *extension_module_filename_str = TeamYousef_String_AsString(extension_module_filename);
#endif

    // TODO: The value of "is_package" is guessed, maybe infer from filename being
    // a "__init__.so" and the like.
    return callIntoExtensionModule(tstate, TeamYousef_String_AsString(module_name), extension_module_filename_str, false);
}

#endif

static char const *getEntryModeString(struct TeamYousef_MetaPathBasedLoaderEntry const *entry) {
    char const *mode = "compiled";

    if ((entry->flags & SAJODE_EXTENSION_MODULE_FLAG) != 0) {
        mode = "extension";
    } else if ((entry->flags & SAJODE_BYTECODE_FLAG) != 0) {
        mode = "bytecode";
    }
...
static PyObject *_SajodeSx_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = TeamYousef_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_module)\n", name);
    }

    struct TeamYousef_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&TeamYousef_Loader_Type;
...
static PyObject *_SajodeSx_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, filename_char_t const *filename) {
    ...
    PyObject *new_file_value = TeamYousef_String_FromFilename(filename);
...
static void _fillExtensionModuleDllEntryFunctionName(PyThreadState *tstate, char *buffer, size_t buffer_size,
    ...
    PyObject *name_obj = BYTES_DECODE2(tstate, name_bytes_obj, TeamYousef_String_FromString("utf8"));
...
#if PYTHON_VERSION >= 0x3c0 && defined(_SAJODE_USE_UNEXPOSED_API)
extern _Thread_local const char *pkgcontext;
#endif

static const char *TeamYousefImport_SwapPackageContext(const char *new_context) {
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
...
#ifndef _SAJODE_EXPERIMENTAL_DEBUG_STANDALONE
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, TeamYousef_SysGetObject("getdlopenflags"));
    }
    ...
    char const *old_context = TeamYousefImport_SwapPackageContext(package);
    ...
    if (base_name != NULL) {
        base_name_obj = TeamYousef_String_FromString(base_name + 1);
        preserved_basename_module = TeamYousef_GetModule(tstate, base_name_obj);
        Py_XINCREF(preserved_basename_module);

        prefix_name_obj = TeamYousef_String_FromStringAndSize(full_name, base_name - full_name + 1);
    }
#endif
    ...
    TeamYousef_DelModuleString(tstate, full_name);
    ...
    TeamYousefImport_SwapPackageContext(old_context);

#if PYTHON_VERSION < 0x300
    PyObject *module = TeamYousef_GetModuleString(tstate, full_name);
#endif
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        ...
        PyObject *full_name_obj = TeamYousef_String_FromString(full_name);

        PyObject *origin = TeamYousef_String_FromFilename(filename);
        ...
        TeamYousef_SetModule(full_name_obj, module);
        Py_DECREF(full_name_obj);

#if _SAJODE_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = TeamYousef_String_FromString(package2);
    ...
    PyObject *filename_obj = TeamYousef_String_FromFilename(filename);
...
// the package context.
...
#if _SAJODE_EXPERIMENTAL_DEBUG_EXTENSION_MODULE_PRESERVATION_HACK
        PRINT_STRING("Restoring preserved module: ");
        PRINT_ITEM(base_name_obj);
        if (TeamYousef_HasModule(tstate, base_name_obj)) {
            PRINT_STRING(" changes ");
            PRINT_ITEM(TeamYousef_GetModule(tstate, base_name_obj));
        }
        PRINT_STRING(" -> ");
        PRINT_ITEM_LINE(preserved_basename_module);
#endif
        TeamYousef_SetModule(base_name_obj, preserved_basename_module);
    ...
    if (base_name_obj != NULL) {
        ...
        {
            PyObject *modules_dict = TeamYousef_GetSysModules();
...
#endif
            while (TeamYousef_DictNext(modules_dict, &pos, &key, &value)) {
                // TODO: Should have SajodeSx_bool return values for these as well maybe.
        ...
        for (Py_ssize_t i = 0; i < n; i++) {
            ...
            PyObject *module_to_correct = TeamYousef_GetModule(tstate, module_to_correct_name);
...
#endif
            TeamYousef_SetModule(correct_module_name, module_to_correct);

            TeamYousef_DelModule(tstate, module_to_correct_name);
        }
    ...
    int res = _PyImport_FixupExtensionObject(module, full_name_obj, filename_obj
                                             ...
                                             ,
                                             TeamYousef_GetSysModules()
#endif
    );
#endif

#if PYTHON_VERSION >= 0x3d0
    TeamYousef_SetModuleString(full_name, module);
#endif
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct TeamYousef_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & SAJODE_ABORT_MODULE_FLAG) != 0) {
                printf("Critical error loading %s.\n", trigger_module_name);
                abort();
            } else {
                PyObject *trigger_module_name_str = TeamYousef_String_FromString(trigger_module_name);
...
static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct TeamYousef_MetaPathBasedLoaderEntry const *entry) {
#if _SAJODE_STANDALONE_MODE
    if ((entry->flags & SAJODE_EXTENSION_MODULE_FLAG) != 0) {
        bool is_package = (entry->flags & SAJODE_PACKAGE_FLAG) != 0;
...
#endif
        if ((entry->flags & SAJODE_BYTECODE_FLAG) != 0) {
        ...
        return loadModuleFromCodeObject(tstate, module, code_object, entry->name,
                                        (entry->flags & SAJODE_PACKAGE_FLAG) != 0);
    } else {
        assert((entry->flags & SAJODE_EXTENSION_MODULE_FLAG) == 0);
        assert(entry->python_init_func);

        {
            SAJODE_MAY_BE_UNUSED bool res = TeamYousef_SetModule(module_name, module);
            assert(res != false);
        }

        // Run the compiled module code, we get the module returned.
#if PYTHON_VERSION < 0x300
        SAJODE_MAY_BE_UNUSED
#endif
    ...
    return TeamYousef_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (entry != NULL) {
#ifdef _SAJODE_EXPERIMENTAL_FORCE_GC_COLLECT_ON_IMPORT
        PyGC_Collect();
#endif
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = TeamYousef_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = TeamYousef_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = TeamYousef_GetModule(tstate, module_name);
...
#if PYTHON_VERSION < 0x350
    if (unlikely(result == NULL)) {
        TeamYousef_DelModule(tstate, module_name);
    }
#endif
...
static PyObject *_SajodeSx_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(TeamYousef_String_Check(module_name));
...
static PyObject *_SajodeSx_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        result = BOOL_FROM((entry->flags & SAJODE_PACKAGE_FLAG) != 0);
    } else {
...
static PyObject *_SajodeSx_loader_iter_modules(struct TeamYousef_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = TeamYousef_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = TeamYousef_String_FromString(current->name);
        }
        ...
        PyTuple_SET_ITEM_IMMORTAL(r, 1, BOOL_FROM((current->flags & SAJODE_PACKAGE_FLAG) != 0));
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct TeamYousef_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *dir_name = TeamYousefUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = TeamYousef_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & SAJODE_PACKAGE_FLAG) != 0) {
        module_name = TeamYousef_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = TeamYousef_String_FromString(buffer);
    }
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct TeamYousef_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *module_filename = TeamYousef_String_FromString(filename_buffer);
...
static PyObject *_SajodeSx_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                TeamYousef_GetFilenameObject(tstate, module));
}
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(tstate, module_name, (PyObject *)&TeamYousef_Loader_Type);
...
static PyObject *_SajodeSx_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = TeamYousef_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_spec)\n", full_name);
    }

    struct TeamYousef_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("import %s # claimed responsibility (%s)\n", TeamYousef_String_AsString(module_name),
...
static PyObject *_SajodeSx_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
#if !_SAJODE_STANDALONE_MODE && !_SAJODE_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    if (installed_extension_modules != NULL) {
        PyObject *extension_module_filename = DICT_GET_ITEM0(tstate, installed_extension_modules, module_name);
...
#if _SAJODE_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    struct TeamYousef_MetaPathBasedLoaderEntry *entry = NULL;
#else
    struct TeamYousef_MetaPathBasedLoaderEntry *entry = findEntry(name);
#endif
    ...
    if ((entry != NULL) && ((entry->flags & SAJODE_EXTENSION_MODULE_FLAG) != 0)) {
...
static PyObject *_SajodeSx_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_SajodeSx_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return TeamYousef_ResourceReader_New(entry);
    }
...
static PyObject *_SajodeSx_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (TeamYousef_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            SajodeSx_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);

            if (unlikely(cmp_res == SAJODE_BOOL_EXCEPTION)) {
                Py_DECREF(name);
                return NULL;
            }

            include = cmp_res == SAJODE_BOOL_TRUE;
        }

        if (include) {
            // Create a distribution object from our data.
            PyObject *distribution = TeamYousef_Distribution_New(tstate, distribution_name);
...
static PyObject *_SajodeSx_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct TeamYousef_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);

    PyThreadState *tstate = PyThreadState_GET();

    while (entry->name != NULL) {
        if ((entry->flags & SAJODE_TRANSLATED_FLAG) != 0) {
            entry->name = UN_TRANSLATE(entry->name);
            entry->flags -= SAJODE_TRANSLATED_FLAG;
        }
        ...
        if ((entry->flags & SAJODE_PACKAGE_FLAG) != 0) {
            ...
            SajodeSx_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == SAJODE_BOOL_TRUE) {
                return TeamYousef_Loader_New(entry);
            }
        }
...
static PyMethodDef TeamYousef_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_SajodeSx_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_SajodeSx_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_SajodeSx_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_SajodeSx_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_SajodeSx_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x300
    {"module_repr", (PyCFunction)_SajodeSx_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_SajodeSx_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_SajodeSx_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_SajodeSx_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_SajodeSx_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_SajodeSx_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_SajodeSx_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL} // terminator
};

static PyObject *TeamYousef_Loader_tp_repr(struct TeamYousef_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return TeamYousef_String_FromString("<SajodeSx_module_loader>");
    } else {
        return TeamYousef_String_FromFormat("<SajodeSx_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct TeamYousef_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void TeamYousef_Loader_tp_dealloc(struct TeamYousef_LoaderObject *loader) {
    TeamYousef_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int TeamYousef_Loader_tp_traverse(struct TeamYousef_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *TeamYousef_Loader_get_name(struct TeamYousef_LoaderObject *loader, void *closure) {
    PyObject *result = TeamYousef_String_FromString(loader->m_loader_entry->name);

    return result;
}
static PyObject *TeamYousef_Loader_get_path(struct TeamYousef_LoaderObject *loader, void *closure) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *TeamYousef_Loader_get__module__(struct TeamYousef_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___SajodeSx__;

    Py_INCREF_IMMORTAL(result);
    return result;
}

static PyGetSetDef TeamYousef_Loader_tp_getset[] = {{(char *)"__module__", (getter)TeamYousef_Loader_get__module__, NULL, NULL},
                                                {(char *)"name", (getter)TeamYousef_Loader_get_name, NULL, NULL},
                                                {(char *)"path", (getter)TeamYousef_Loader_get_path, NULL, NULL},
                                                {NULL}};

PyTypeObject TeamYousef_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "SajodeSx_module_loader",
    sizeof(struct TeamYousef_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)TeamYousef_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)TeamYousef_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)TeamYousef_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    TeamYousef_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    TeamYousef_Loader_tp_getset,                 // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *TeamYousef_Loader_New(struct TeamYousef_MetaPathBasedLoaderEntry const *entry) {
    struct TeamYousef_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct TeamYousef_LoaderObject, TeamYousef_Loader_Type);
...
void registerMetaPathBasedLoader(struct TeamYousef_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    if (isVerbose()) {
        PySys_WriteStderr("Setup SajodeSx compiled module/bytecode/extension importer.\n");
    }

    loader_entries = _loader_entries;

#if _SAJODE_MODULE_MODE && PYTHON_VERSION < 0x3c0
    if (_Py_PackageContext != NULL) {
        updateMetaPathBasedLoaderModuleRoot(_Py_PackageContext);
    }
#endif

    TeamYousef_PyType_Ready(&TeamYousef_Loader_Type, NULL, true, false, false, false, false);

#if _SAJODE_EXE_MODE
    {
        SAJODE_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__SajodeSx_loader_type", (PyObject *)&TeamYousef_Loader_Type);
        assert(res == 0);
    }
#endif

#if PYTHON_VERSION >= 0x370
    TeamYousef_PyType_Ready(&TeamYousef_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = TeamYousef_Loader_New(NULL);

    LIST_INSERT_CONST(TeamYousef_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
...
#if _SAJODE_STANDALONE_MODE
    LIST_INSERT_CONST(TeamYousef_SysGetObject("path_hooks"), 0, PyObject_GetAttrString(global_loader, "sys_path_hook"));
#else
    ...
    LIST_APPEND1(TeamYousef_SysGetObject("path_hooks"), PyObject_GetAttrString(global_loader, "sys_path_hook"));
#endif
}
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = TeamYousef_SysGetObject("builtin_module_names");

    while (TeamYousef_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(TeamYousef_String_AsString(key), is_package, false);
