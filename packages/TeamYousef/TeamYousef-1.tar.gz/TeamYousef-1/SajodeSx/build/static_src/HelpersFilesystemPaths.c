...
#ifndef _SAJODE_EXPERIMENTAL_AVOID_SHORT_PATH
...
#elif defined(__OpenBSD__) || defined(_AIX) || defined(_SAJODE_EXPERIMENTAL_FORCE_UNIX_BINARY_NAME)
...
#include "SajodeSx/checksum_tools.h"
...
static DWORD TeamYousef_GetFinalPathNameByHandleW(HANDLE hFile, LPWSTR FilePath, DWORD cchFilePath, DWORD dwFlags) {
...
static void resolveFileSymbolicLink(wchar_t *resolved_filename, wchar_t const *filename, DWORD resolved_filename_size,
    ...
    if (resolve_symlinks) {
        ...
        DWORD len = TeamYousef_GetFinalPathNameByHandleW(file_handle, resolved_filename, resolved_filename_size,
        ...
        if (*source == L'{') {
            ...
            *w = 0;
            ...
            if (wcsicmp(var_name, L"TEMP") == 0) {
                ...
                environment_value = getEnvironmentVariable("SAJODE_ONEFILE_PARENT");
#endif
                if (environment_value != NULL) {
                    checkWStringNumber(environment_value);

                    appendWStringSafeW(target, getEnvironmentVariable("SAJODE_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
                ...
                is_path = true;
#ifdef SAJODE_COMPANY_NAME
            } else if (wcsicmp(var_name, L"COMPANY") == 0) {
                appendWStringSafeW(target, L"" SAJODE_COMPANY_NAME, buffer_size);
#endif
#ifdef SAJODE_PRODUCT_NAME
            } else if (wcsicmp(var_name, L"PRODUCT") == 0) {
                appendWStringSafeW(target, L"" SAJODE_PRODUCT_NAME, buffer_size);
#endif
#ifdef SAJODE_VERSION_COMBINED
            } else if (wcsicmp(var_name, L"VERSION") == 0) {
                appendWStringSafeW(target, L"" SAJODE_VERSION_COMBINED, buffer_size);
#endif
            } else if (wcsicmp(var_name, L"TIME") == 0) {
                environment_char_t const *environment_value = NULL;

#if _SAJODE_ONEFILE_MODE
                environment_value = getEnvironmentVariable("SAJODE_ONEFILE_START");
#endif

                if (environment_value != NULL) {
                    appendWStringSafeW(target, getEnvironmentVariable("SAJODE_ONEFILE_START"), buffer_size);
                } else {
...
#if _SAJODE_ONEFILE_MODE
                    setEnvironmentVariable("SAJODE_ONEFILE_START", time_buffer);
#endif
...
bool expandTemplatePath(char *target, char const *source, size_t buffer_size) {
    ...
    SAJODE_MAY_BE_UNUSED bool var_started = false;
            ...
            *w = 0;
            ...
            if (strcasecmp(var_name, "TEMP") == 0) {
                ...
                if (environment_value != NULL) {
                    ...
                    appendStringSafe(target, getEnvironmentVariable("SAJODE_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
                ...
                appendStringSafe(target, SAJODE_COMPANY_NAME, buffer_size);
#endif
#ifdef SAJODE_PRODUCT_NAME
            } else if (strcasecmp(var_name, "PRODUCT") == 0) {
                appendStringSafe(target, SAJODE_PRODUCT_NAME, buffer_size);
#endif
#ifdef SAJODE_VERSION_COMBINED
            } else if (strcasecmp(var_name, "VERSION") == 0) {
                appendStringSafe(target, SAJODE_VERSION_COMBINED, buffer_size);
#endif
                ...
                if (environment_value != NULL) {
                    appendStringSafe(target, getEnvironmentVariable("SAJODE_ONEFILE_START"), buffer_size);
                } else {
