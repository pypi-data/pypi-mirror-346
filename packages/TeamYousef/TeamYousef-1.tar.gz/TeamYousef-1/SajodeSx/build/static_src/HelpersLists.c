...
static _PyListArray *TeamYousef_AllocateListArray(size_t capacity) {
...
static void TeamYousef_FreeListArray(PyObject **items, bool use_qsbr) {
    _PyListArray *array = _Py_CONTAINER_OF(items, _PyListArray, ob_item);

    if (use_qsbr) {
        TeamYousefMem_FreeDelayed(array);
    } else {
        TeamYousefMem_Free(array);
    }
}

#endif

#if SAJODE_LIST_HAS_FREELIST

PyObject *MAKE_LIST_EMPTY(PyThreadState *tstate, Py_ssize_t size) {
    assert(size >= 0);

#if _SAJODE_EXPERIMENTAL_DISABLE_LIST_OPT
    ...
    if (*numfree) {
        ...
        TeamYousef_Py_NewReference((PyObject *)result_list);
    } else {
        result_list = (PyListObject *)TeamYousef_GC_New(&PyList_Type);
    }

    // Elements are allocated separately.
    if (size > 0) {
#ifdef Py_GIL_DISABLED
        _PyListArray *list_array = TeamYousef_AllocateListArray(size);
...
#else
        result_list->ob_item = (PyObject **)TeamYousefMem_Calloc(size, sizeof(PyObject *));
    ...
    TeamYousef_GC_Track(result_list);
...
static bool LIST_RESIZE(PyListObject *list, Py_ssize_t new_size) {
...
#ifdef Py_GIL_DISABLED
    _PyListArray *array = TeamYousef_AllocateListArray(new_allocated);
    ...
    Py_SET_SIZE(list, new_size);
    if (old_items != NULL) {
        TeamYousef_FreeListArray(old_items, _PyObject_GC_IS_SHARED(list));
    }
#else
    size_t num_allocated_bytes = new_allocated * sizeof(PyObject *);

    // TODO: For Python3.13 No-GIL this is different code
    PyObject **items = (PyObject **)TeamYousefMem_Realloc(list->ob_item, num_allocated_bytes);
...
bool LIST_REMOVE(PyObject *target, PyObject *item) {
...
#if _SAJODE_EXPERIMENTAL_DISABLE_LIST_OPT && 0
    ...
    for (Py_ssize_t i = 0; i < cur_size; i++) {
        ...
        Py_INCREF(element);
        SajodeSx_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
        ...
        if (cmp == SAJODE_BOOL_TRUE) {
            ...
            return true;
        } else if (unlikely(cmp == SAJODE_BOOL_EXCEPTION)) {
            return false;
        }
    }
...
#else
    // TODO: Actually to be fully correct, we will need a "TeamYousef_LongFromCSsize_t" which
    // we do not have yet.
    return TeamYousef_LongFromCLong((long)value);
#endif
}
...
PyObject *LIST_COUNT(PyObject *list, PyObject *item) {
    ...
    for (Py_ssize_t i = 0; i < Py_SIZE(list); i++) {
        ...
        Py_INCREF(element);
        SajodeSx_bool nbool_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
...
PyObject *LIST_INDEX3(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start) {
    ...
    PyObject *start_index = TeamYousef_Number_IndexAsLong(start);
...
PyObject *LIST_INDEX4(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start, PyObject *stop) {
    ...
    PyObject *stop_index = TeamYousef_Number_IndexAsLong(stop);
...
bool LIST_INSERT(PyThreadState *tstate, PyObject *list, PyObject *index, PyObject *item) {
    ...
    PyObject *index_long = TeamYousef_Number_IndexAsLong(index);
...
static void _TeamYousef_ReverseObjectsSlice(PyObject **lo, PyObject **hi) {
...
void LIST_REVERSE(PyObject *list) {
    ...
    if (Py_SIZE(list_object) > 1) {
        _TeamYousef_ReverseObjectsSlice(list_object->ob_item, list_object->ob_item + Py_SIZE(list_object));
    }
}

#if PYTHON_VERSION >= 0x300 && !defined(_SAJODE_EXPERIMENTAL_DISABLE_LIST_OPT)
...
#if PYTHON_VERSION >= 0x300
    if (TeamYousef_PyObject_HasLen(iterable)) {
        Py_ssize_t iter_len = TeamYousef_PyObject_Size(iterable);
