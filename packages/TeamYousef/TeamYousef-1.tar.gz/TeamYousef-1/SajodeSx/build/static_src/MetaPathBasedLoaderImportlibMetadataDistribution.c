...
bool TeamYousef_DistributionNext(Py_ssize_t *pos, PyObject **distribution_name_ptr) {
    PyObject *value;
    return TeamYousef_DictNext(metadata_values_dict, pos, distribution_name_ptr, &value);
}

PyObject *TeamYousef_Distribution_New(PyThreadState *tstate, PyObject *name) {
    // TODO: Have our own Python code to be included in compiled form,
    // this duplicates with inspec patcher code.
    static PyObject *SajodeSx_distribution_type = NULL;
    ...
    if (SajodeSx_distribution_type == NULL) {
        static char const *SajodeSx_distribution_code = "\n\
import os,sys\n\
...
class SajodeSx_distribution(Distribution):\n\
...
";
        ...
        PyObject *SajodeSx_distribution_code_object = Py_CompileString(SajodeSx_distribution_code, "<exec>", Py_file_input);
        CHECK_OBJECT(SajodeSx_distribution_code_object);

        {
            PyObject *module =
                PyImport_ExecCodeModule((char *)"SajodeSx_distribution_patch", SajodeSx_distribution_code_object);
            CHECK_OBJECT(module);

            SajodeSx_distribution_type = PyObject_GetAttrString(module, "SajodeSx_distribution");
            CHECK_OBJECT(SajodeSx_distribution_type);

            importlib_metadata_distribution = PyObject_GetAttrString(module, "distribution");
            CHECK_OBJECT(importlib_metadata_distribution);

            {
                SAJODE_MAY_BE_UNUSED bool bool_res = TeamYousef_DelModuleString(tstate, "SajodeSx_distribution_patch");
    ...
    if (metadata_value_item == NULL) {
        ...
        struct TeamYousef_MetaPathBasedLoaderEntry *entry = findEntry(TeamYousef_String_AsString_Unchecked(package_name));

        if (unlikely(entry == NULL)) {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT1(PyExc_RuntimeError,
                                                "cannot locate package '%s' associated with metadata",
                                                TeamYousef_String_AsString(package_name));

            return NULL;
        }

        PyObject *args[3] = {getModuleDirectory(tstate, entry), metadata, entry_points};
        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, SajodeSx_distribution_type, args);
        CHECK_OBJECT(result);
        return result;
    }
}
