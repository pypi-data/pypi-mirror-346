...
#else
#define _SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
#define _SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
#define _SAJODE_ONEFILE_TEMP_BOOL 0
#define _SAJODE_ONEFILE_CHILD_GRACE_TIME_INT 5000
#define _SAJODE_ONEFILE_TEMP_SPEC "{TEMP}/onefile_{PID}_{TIME}"

#define _SAJODE_AUTO_UPDATE_BOOL 1
#define _SAJODE_AUTO_UPDATE_DEBUG_BOOL 1
#define _SAJODE_AUTO_UPDATE_URL_SPEC "https://..."

#define _SAJODE_ATTACH_CONSOLE_WINDOW 1
#endif

#if _SAJODE_ONEFILE_COMPRESSION_BOOL == 1
...
// Some handy macro definitions, e.g. unlikely and SAJODE_MAY_BE_UNUSED
#include "SajodeSx/hedley.h"
...
// Out of memory error.
#if !defined(_WIN32) || _SAJODE_ONEFILE_COMPRESSION_BOOL == 1
...
static void initPayloadData(void) {
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Determining payload start position.");
...
#if _SAJODE_ONEFILE_COMPRESSION_BOOL == 1 && _SAJODE_ONEFILE_ARCHIVE_BOOL == 0
...
#if _SAJODE_ONEFILE_TEMP_BOOL == 0
...
#if _SAJODE_ONEFILE_ARCHIVE_BOOL == 1 && _SAJODE_ONEFILE_COMPRESSION_BOOL == 1
...
#if _SAJODE_ONEFILE_ARCHIVE_BOOL == 1

#if _SAJODE_ONEFILE_COMPRESSION_BOOL == 0
...
static int waitpid_timeout(pid_t pid) {
    ...
    struct timespec timeout = {
        _SAJODE_ONEFILE_CHILD_GRACE_TIME_INT / 1000,
        (_SAJODE_ONEFILE_CHILD_GRACE_TIME_INT % 1000) * 1000,
    };
...
static void cleanupChildProcess(bool send_sigint) {
    ...
    if (handle_process != 0) {
        ...
        if (send_sigint) {
#if defined(_SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING)
...
#if _SAJODE_ONEFILE_TEMP_BOOL || 1
        SAJODE_PRINT_TRACE("Waiting for child to exit.\n");
#if defined(_WIN32)
        if (WaitForSingleObject(handle_process, _SAJODE_ONEFILE_CHILD_GRACE_TIME_INT) != 0) {
...
#endif
        SAJODE_PRINT_TRACE("Child is exited.\n");
#endif
    }

#if _SAJODE_ONEFILE_TEMP_BOOL
    if (payload_created) {
#if _SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
...
BOOL WINAPI ourConsoleCtrlHandler(DWORD fdwCtrlType) {
    ...
    case CTRL_C_EVENT:
#ifdef _SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
        puts("Ctrl-C event");
#endif
...
#if _SAJODE_AUTO_UPDATE_BOOL && !defined(__IDE_ONLY__)
#include "SajodeSx_onefile_auto_updater.h"
#endif

#if _SAJODE_AUTO_UPDATE_BOOL
extern bool exe_file_updatable;
#endif

#ifdef _SAJODE_ONEFILE_SPLASH_SCREEN
#ifdef __cplusplus
extern "C" {
#endif
...
static int runPythonCodeDLL(filename_char_t const *dll_filename, int argc, native_command_line_argument_t **argv) {
    ...
    typedef int(__stdcall * SajodeSx_dll_function_ptr)(int, wchar_t **);

    SajodeSx_dll_function_ptr SajodeSx_dll_function = (SajodeSx_dll_function_ptr)GetProcAddress(hGetProcIDDLL, "run_code");

    if (SajodeSx_dll_function == NULL) {
        fatalError("Error, DLL entry point not found.");

        return EXIT_FAILURE;
    }

    return (*SajodeSx_dll_function)(argc, argv);
#else
    typedef int (*SajodeSx_dll_function_ptr)(int, native_command_line_argument_t **);
    ...
    SajodeSx_dll_function_ptr SajodeSx_dll_function = (SajodeSx_dll_function_ptr)dlsym(handle, "run_code");
    assert(SajodeSx_dll_function);
...
int main(int argc, char **argv) {
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Entered main().");

    filename_char_t const *pattern = FILENAME_EMPTY_STR _SAJODE_ONEFILE_TEMP_SPEC;
    bool bool_res = expandTemplatePathFilename(payload_path, pattern, sizeof(payload_path) / sizeof(filename_char_t));

    // _putws(payload_path);

#if _SAJODE_ONEFILE_DLL_MODE
    environment_char_t const *process_role = getEnvironmentVariable("SAJODE_ONEFILE_PARENT");
...
#if defined(_SAJODE_ONEFILE_SPLASH_SCREEN) && _SAJODE_ONEFILE_COMPRESSION_BOOL == 1
    initSplashScreen();
#endif

    SAJODE_PRINT_TIMING("ONEFILE: Unpacking payload.");
    initPayloadData();
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Checking header for compression.");
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Header is OK.");
...
// The 'X' stands for no compression, 'Y' is compressed, handle that.
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Entering decompression.");
...
#ifdef _SAJODE_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Finishing decompression, cleanup payload.");
    ...
    if (process_role == NULL) {
#if defined(_WIN32)
        setEnvironmentVariableFromLong("SAJODE_ONEFILE_PARENT", GetCurrentProcessId());
#else
        setEnvironmentVariableFromLong("SAJODE_ONEFILE_PARENT", (long)getpid());
#endif
    }
...
#endif
    setEnvironmentVariable("SAJODE_ONEFILE_DIRECTORY", stripBaseFilename(binary_filename));

    setEnvironmentVariable("SAJODE_ORIGINAL_ARGV0", argv[0]);

    SAJODE_PRINT_TIMING("ONEFILE: Preparing forking of slave process.");
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Started slave process.");
...
#if _SAJODE_ONEFILE_SPLASH_SCREEN
    DWORD wait_time = 50;
#else
    ...
    SAJODE_PRINT_TIMING("ONEFILE: Exiting.");
