...
#define INCREF_KEYS_FT(dk) TeamYousef_Py_dictkeys_incref(dk)
#define DECREF_KEYS_FT(dk, shared) TeamYousef_Py_dictkeys_decref(_PyInterpreterState_GET(), dk, shared)
...
#else
    PyObject **value_addr;
    Py_ssize_t ix = TeamYousef_PyDictLookup(dict_object, key, hash, &value_addr);
#endif
...
static PyObject *TeamYousef_CreateKeyError(PyThreadState *tstate, PyObject *key) {
    return (PyObject *)TeamYousef_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_KeyError, key);
}
#endif
...
#else
    struct TeamYousef_ExceptionPreservationItem exception_state = {TeamYousef_CreateKeyError(tstate, key)};
...
static inline PyObject *_MAKE_DICT_ITERATOR(PyThreadState *tstate, PyDictObject *dict, PyTypeObject *type,
...
#if PYTHON_VERSION < 0x300
    dictiterobject *di = (dictiterobject *)TeamYousef_GC_New(type);
    CHECK_OBJECT(di);
    Py_INCREF(dict);
    di->di_dict = dict;
    ...
    TeamYousef_GC_Track(di);
    return (PyObject *)di;
#else
    _PyDictViewObject *dv = (_PyDictViewObject *)TeamYousef_GC_New(type);
    CHECK_OBJECT(dv);

    Py_INCREF(dict);
    dv->dv_dict = dict;

    TeamYousef_GC_Track(dv);
    return (PyObject *)dv;
#endif
}
...
static PyObject *_MAKE_DICT_VIEW(PyDictObject *dict, PyTypeObject *type) {
    ...
    dictviewobject *dv = (dictviewobject *)TeamYousef_GC_New(type);
...
#if PYTHON_VERSION >= 0x300
static PyDictObject *_TeamYousef_AllocatePyDictObject(PyThreadState *tstate) {
    PyDictObject *result_mp;

#if SAJODE_DICT_HAS_FREELIST
...
#else
    struct _Py_object_freelists *freelists = _TeamYousef_object_freelists_GET(tstate);
    ...
    if (*numfree) {
        ...
        TeamYousef_Py_NewReference((PyObject *)result_mp);
...
#endif
    {
        result_mp = (PyDictObject *)TeamYousef_GC_New(&PyDict_Type);
    }
...
#if PYTHON_VERSION >= 0x360
static PyDictKeysObject *_TeamYousef_AllocatePyDictKeysObject(PyThreadState *tstate, Py_ssize_t keys_size) {
...
// seems it can only be used for the smallest keys type.
    ...
    {
#if PYTHON_VERSION < 0x3d0
        dk = (PyDictKeysObject *)TeamYousefObject_Malloc(keys_size);
#else
        dk = (PyDictKeysObject *)TeamYousefMem_Malloc(keys_size);
#endif
    }
...
#if PYTHON_VERSION >= 0x360 && !defined(_SAJODE_EXPERIMENTAL_DISABLE_DICT_OPT)

// Usable fraction of keys.
#define DK_USABLE_FRACTION(n) (((n) << 1) / 3)

static Py_ssize_t _TeamYousef_Py_PyDict_KeysSize(PyDictKeysObject *keys) {
#if PYTHON_VERSION < 0x360
...
static inline PyDictValues *_TeamYousef_PyDict_new_values(Py_ssize_t size) {
#if PYTHON_VERSION < 0x3b0
    Py_ssize_t values_size = sizeof(PyObject *) * size;

    return (PyDictValues *)TeamYousefMem_Malloc(values_size);
    ...
    uint8_t *mem = (uint8_t *)TeamYousefMem_Malloc(n);
    assert(mem != NULL);
    ...
    PyDictValues *result = (PyDictValues *)TeamYousefMem_Malloc(n);
...
static PyDictValues *_TeamYousef_PyDict_copy_values(PyDictValues *values) {
    PyDictValues *new_values = _TeamYousef_PyDict_new_values(values->capacity);
...
#if PYTHON_VERSION >= 0x3b0
static inline int TeamYousef_py_get_index_from_order(PyDictObject *mp, Py_ssize_t i) {
...
static inline Py_ssize_t TeamYousef_Py_dictkeys_get_index(const PyDictKeysObject *keys, Py_ssize_t i) {
...
static Py_ssize_t TeamYousef_Py_unicodekeys_lookup_generic(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                       Py_hash_t hash) {
    ...
    while (1) {
        Py_ssize_t ix = TeamYousef_Py_dictkeys_get_index(dk, i);
        ...
        if (ix >= 0) {
            ...
            if (TeamYousef_Py_unicode_get_hash(ep->me_key) == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                SajodeSx_bool cmp = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(startkey, key);
                Py_DECREF(startkey);

                if (unlikely(cmp == SAJODE_BOOL_EXCEPTION)) {
                    return DKIX_ERROR;
                }

                if (dk == mp->ma_keys && ep->me_key == startkey) {
                    if (cmp == SAJODE_BOOL_TRUE) {
                        return ix;
                    }
                } else {
    ...
    SAJODE_CANNOT_GET_HERE("TeamYousef_Py_unicodekeys_lookup_generic failed");
}

Py_ssize_t TeamYousef_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash) {
    ...
    while (true) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_key == key || (TeamYousef_Py_unicode_get_hash(ep->me_key) == hash &&
        ...
        i = mask & (i * 5 + perturb + 1);
        ix = TeamYousef_Py_dictkeys_get_index(dk, i);
    ...
    SAJODE_CANNOT_GET_HERE("TeamYousef_Py_unicodekeys_lookup_unicode failed");
}

// Search key from Generic table.
static Py_ssize_t TeamYousef_Py_dictkeys_generic_lookup(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                    Py_hash_t hash) {
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                SajodeSx_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(startkey, key);
                Py_DECREF(startkey);
...
Py_ssize_t TeamYousef_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    PyDictKeysObject *dk;
    DictKeysKind kind;
    Py_ssize_t ix;
...
restart:
    ...
    if (kind != DICT_KEYS_GENERAL) {
        if (PyUnicode_CheckExact(key)) {
#ifdef Py_GIL_DISABLED
            if (kind == DICT_KEYS_SPLIT) {
                ix = TeamYousef_Py_unicodekeys_lookup_unicode_threadsafe(dk, key, hash);

                if (ix == DKIX_KEY_CHANGED) {
                    LOCK_KEYS(dk);
                    ix = TeamYousef_Py_unicodekeys_lookup_unicode(dk, key, hash);
                    UNLOCK_KEYS(dk);
                }
            } else
#endif
            ...
            ix = TeamYousef_Py_unicodekeys_lookup_generic(mp, dk, key, hash);
        ...
        ix = TeamYousef_Py_dictkeys_generic_lookup(mp, dk, key, hash);
...
Py_ssize_t TeamYousef_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    assert(PyUnicode_CheckExact(key));

    PyDictKeysObject *dk = mp->ma_keys;
    assert(dk->dk_kind != DICT_KEYS_GENERAL);

    Py_ssize_t ix = TeamYousef_Py_unicodekeys_lookup_unicode(dk, key, hash);
...
bool TeamYousef_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr) {
    CHECK_OBJECT(dict);
    assert(PyDict_CheckExact(dict));
    assert(key_ptr != NULL);
    ...
    *value_ptr = value;
    ...
    if (mp->ma_values) {
        ...
        int index = TeamYousef_py_get_index_from_order(mp, i);
...
#if _SAJODE_MAINTAIN_DICT_VERSION_TAG
uint64_t SajodeSx_dict_version_tag_counter = ((uint64_t)1) << 32;
#endif
...
PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate) {
    ...
    PyDictObject *result_mp = _TeamYousef_AllocatePyDictObject(tstate);
    ...
    // No TeamYousef_GC_Track for the empty dictionary.
