...
void undoEnvironmentVariable(PyThreadState *tstate, char const *variable_name, environment_char_t const *old_value) {
    ...
    PyObject *variable_name_str = TeamYousef_String_FromString(variable_name);
...
#ifdef _WIN32
        PyObject *env_value = TeamYousefUnicode_FromWideChar(old_value, -1);
#else
        PyObject *env_value = TeamYousef_String_FromString(old_value);
#endif
