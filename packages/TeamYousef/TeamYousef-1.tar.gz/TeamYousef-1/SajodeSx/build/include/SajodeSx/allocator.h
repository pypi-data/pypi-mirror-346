...
#ifndef __SAJODE_ALLOCATOR_H__
#define __SAJODE_ALLOCATOR_H__
...
#if PYTHON_VERSION < 0x270 && !defined(__SAJODE_NO_ASSERT__)
...
extern void initTeamYousefAllocators(void);

// Our version of "PyObject_Malloc".
SAJODE_MAY_BE_UNUSED static void *TeamYousefObject_Malloc(size_t size) { return python_obj_malloc(python_obj_ctx, size); }

// Our version of "PyMem_Malloc".
SAJODE_MAY_BE_UNUSED static void *TeamYousefMem_Malloc(size_t size) { return python_mem_malloc(python_mem_ctx, size); }

// Our version of "PyMem_Calloc".
SAJODE_MAY_BE_UNUSED static void *TeamYousefMem_Calloc(size_t nelem, size_t elsize) {
    return python_mem_calloc(python_mem_ctx, nelem, elsize);
}

#ifndef Py_GIL_DISABLED
// Our version of "PyMem_Realloc".
SAJODE_MAY_BE_UNUSED static void *TeamYousefMem_Realloc(void *ptr, size_t new_size) {
    return python_mem_realloc(python_mem_ctx, ptr, new_size);
}
#else
SAJODE_MAY_BE_UNUSED static void TeamYousefMem_Free(void *ptr) { python_mem_free(python_mem_ctx, ptr); }
#endif

#else
#define TeamYousefObject_Malloc(size) PyObject_MALLOC(size)
#define TeamYousefMem_Malloc(size) PyMem_MALLOC(size)
#define TeamYousefMem_Calloc(elem, elsize) PyMem_Calloc(elem, elsize)
#ifndef Py_GIL_DISABLED
#if defined(_WIN32)
// On Windows, mixing different runtime DLLs can cause issues at
// release, so we need to go through the API to get the proper
// DLL runtime.
#define TeamYousefMem_Realloc(ptr, new_size) PyMem_Realloc(ptr, new_size)
#else
#define TeamYousefMem_Realloc(ptr, new_size) PyMem_REALLOC(ptr, new_size)
#endif
#else
#define TeamYousefMem_Free(ptr) PyMem_Free(ptr)
#endif
#endif

#if PYTHON_VERSION >= 0x380 && PYTHON_VERSION < 0x3c0
// Need to make Py_DECREF a macro again that doesn't call an API
static inline void _TeamYousef_Py_DECREF(PyObject *ob) {
...
#undef Py_DECREF
#define Py_DECREF(ob) _TeamYousef_Py_DECREF((PyObject *)(ob))

// Need to make Py_XDECREF a macro again that doesn't call an API
static inline void _TeamYousef_Py_XDECREF(PyObject *ob) {
    if (ob != NULL) {
...
#undef Py_XDECREF
#define Py_XDECREF(ob) _TeamYousef_Py_XDECREF((PyObject *)(ob))
...
#define Py_CLEAR(op)                                                                                                   \
    ...
    !defined(Py_GIL_DISABLED) && !defined(_SAJODE_EXPERIMENTAL_DISABLE_PY_DECREF_OVERRIDE)
...
#elif defined(_SAJODE_DEBUG_DEBUG_IMMORTAL)
...
static inline void TeamYousef_Py_NewReferenceNoTotal(PyObject *op) { Py_SET_REFCNT(op, 1); }
static inline void TeamYousef_Py_NewReference(PyObject *op) {
#ifdef Py_REF_DEBUG
#if PYTHON_VERSION < 0x3c0
    _Py_RefTotal++;
#else
...
#else
#define TeamYousef_Py_NewReference(op) _Py_NewReference(op)
#endif

static inline int TeamYousef_PyType_HasFeature(PyTypeObject *type, unsigned long feature) {
    return ((type->tp_flags & feature) != 0);
}

#if PYTHON_VERSION >= 0x3b0

static inline size_t TeamYousef_PyType_PreHeaderSize(PyTypeObject *tp) {
    return _PyType_IS_GC(tp) * sizeof(PyGC_Head) +
           TeamYousef_PyType_HasFeature(tp, Py_TPFLAGS_MANAGED_DICT) * 2 * sizeof(PyObject *);
}

extern void TeamYousef_PyObject_GC_Link(PyObject *op);

static PyObject *TeamYousef_PyType_AllocNoTrackVar(PyTypeObject *type, Py_ssize_t nitems) {
    ...
    const size_t pre_size = TeamYousef_PyType_PreHeaderSize(type);
    assert(pre_size == sizeof(PyGC_Head));

    char *alloc = (char *)TeamYousefObject_Malloc(size + pre_size);
    assert(alloc);
    ...
    if (pre_size) {
        ...
        TeamYousef_PyObject_GC_Link(obj);
    }
    ...
    if (TeamYousef_PyType_HasFeature(type, Py_TPFLAGS_HEAPTYPE)) {
        Py_INCREF(type);
    }

    TeamYousef_Py_NewReference(obj);

    return obj;
}

static PyObject *TeamYousef_PyType_AllocNoTrack(PyTypeObject *type) {
    // TODO: This ought to be static for all our types, so remove it as a call.
    const size_t pre_size = TeamYousef_PyType_PreHeaderSize(type);

    char *alloc = (char *)TeamYousefObject_Malloc(_PyObject_SIZE(type) + pre_size);
    assert(alloc);
    PyObject *obj = (PyObject *)(alloc + pre_size);
...
SAJODE_MAY_BE_UNUSED static void *TeamYousef_GC_NewVar(PyTypeObject *type, Py_ssize_t nitems) {
    assert(nitems >= 0);
    ...
    TeamYousef_Py_NewReference((PyObject *)op);

    return op;
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = TeamYousef_PyType_AllocNoTrackVar(type, nitems);
#endif
    assert(Py_SIZE(op) == nitems);
    return op;
}

SAJODE_MAY_BE_UNUSED static void *TeamYousef_GC_New(PyTypeObject *type) {
#if PYTHON_VERSION < 0x3b0
...
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = TeamYousef_PyType_AllocNoTrack(type);
#endif
    return op;
}

static bool inline TeamYousef_GC_IS_TRACKED_X(PyObject *object) {
