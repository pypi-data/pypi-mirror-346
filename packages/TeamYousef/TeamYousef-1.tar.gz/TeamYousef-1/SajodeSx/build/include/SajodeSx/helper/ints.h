...
extern PyObject *TeamYousef_PyLong_FromLong(long ival);

// Our "PyInt_FromLong" replacement, not done (yet?).
#if PYTHON_VERSION >= 0x300
#define TeamYousef_PyInt_FromLong(ival) TeamYousef_PyLong_FromLong(ival)
#else
#define TeamYousef_PyInt_FromLong(ival) PyInt_FromLong(ival)
#endif
...
} SajodeSx_ilong_validity;

typedef struct {
    SajodeSx_ilong_validity validity;
...
SAJODE_MAY_BE_UNUSED static void SET_NILONG_OBJECT_VALUE(SajodeSx_ilong *dual_value, PyObject *python_value) {
    dual_value->validity = SAJODE_ILONG_OBJECT_VALID;
    dual_value->python_value = python_value;
}

SAJODE_MAY_BE_UNUSED static void SET_NILONG_C_VALUE(SajodeSx_ilong *dual_value, long c_value) {
    dual_value->validity = SAJODE_ILONG_CLONG_VALID;
    dual_value->c_value = c_value;
}

SAJODE_MAY_BE_UNUSED static void SET_NILONG_OBJECT_AND_C_VALUE(SajodeSx_ilong *dual_value, PyObject *python_value,
                                                               long c_value) {
...
SAJODE_MAY_BE_UNUSED static void RELEASE_NILONG_VALUE(SajodeSx_ilong *dual_value) {
...
SAJODE_MAY_BE_UNUSED static void INCREF_NILONG_VALUE(SajodeSx_ilong *dual_value) {
    if (IS_NILONG_OBJECT_VALUE_VALID(dual_value)) {
        CHECK_OBJECT(dual_value);
...
SAJODE_MAY_BE_UNUSED static long GET_NILONG_C_VALUE(SajodeSx_ilong const *dual_value) {
    assert(IS_NILONG_C_VALUE_VALID(dual_value));
    return dual_value->c_value;
}

SAJODE_MAY_BE_UNUSED static PyObject *GET_NILONG_OBJECT_VALUE(SajodeSx_ilong const *dual_value) {
    assert(IS_NILONG_OBJECT_VALUE_VALID(dual_value));
    return dual_value->python_value;
}

SAJODE_MAY_BE_UNUSED static void ENFORCE_NILONG_OBJECT_VALUE(SajodeSx_ilong *dual_value) {
    assert(dual_value->validity != SAJODE_ILONG_UNASSIGNED);

    if (!IS_NILONG_OBJECT_VALUE_VALID(dual_value)) {
        dual_value->python_value = TeamYousef_PyLong_FromLong(dual_value->c_value);

        dual_value->validity = SAJODE_ILONG_BOTH_VALID;
    }
}

SAJODE_MAY_BE_UNUSED static void CHECK_NILONG_OBJECT(SajodeSx_ilong const *dual_value) {
    assert(dual_value->validity != SAJODE_ILONG_UNASSIGNED);
...
SAJODE_MAY_BE_UNUSED static void PRINT_NILONG(SajodeSx_ilong const *dual_value) {
...
extern PyObject **TeamYousef_Long_SmallValues;
#else
extern PyObject *TeamYousef_Long_SmallValues[SAJODE_STATIC_SMALLINT_VALUE_MAX - SAJODE_STATIC_SMALLINT_VALUE_MIN + 1];
#endif

SAJODE_MAY_BE_UNUSED static inline PyObject *TeamYousef_Long_GetSmallValue(int ival) {
    return TeamYousef_Long_SmallValues[SAJODE_TO_SMALL_VALUE_OFFSET(ival)];
}

#endif

#else
SAJODE_MAY_BE_UNUSED static inline PyObject *TeamYousef_Long_GetSmallValue(medium_result_value_t ival) {
