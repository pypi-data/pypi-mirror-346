    ...
    if (free_list != NULL) {                                                                                           \
        ...
        if (Py_SIZE(result) < size) {                                                                                  \
            ...
            assert(result != NULL);                                                                                    \
                                                                                                                       ...
                                                                                                                       \
        TeamYousef_Py_NewReference((PyObject *)result);                                                                    \
    } else {                                                                                                           \
        result = (object_type *)TeamYousef_GC_NewVar(&type_type, size);                                                    \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
#define allocateFromFreeListFixed(free_list, object_type, type_type)                                                   \
    if (free_list != NULL) {                                                                                           \
        ...
        result = (object_type *)TeamYousef_GC_New(&type_type);                                                             \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
SAJODE_MAY_BE_UNUSED static inline struct _Py_object_freelists *_TeamYousef_object_freelists_GET(PyThreadState *tstate) {
