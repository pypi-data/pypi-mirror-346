...
#ifndef __SAJODE_DICTIONARIES_H__
#define __SAJODE_DICTIONARIES_H__
...
typedef PyDictEntry *TeamYousef_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, TeamYousef_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(TeamYousef_String_CheckExact(key));
...
SAJODE_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(TeamYousef_DictEntryHandle handle) { return handle->me_value; }

SAJODE_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(TeamYousef_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, TeamYousef_StringObject *key) {
...
typedef PyObject **TeamYousef_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t TeamYousef_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t TeamYousef_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t TeamYousef_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static inline Py_hash_t TeamYousef_Py_unicode_get_hash(PyObject *o) {
    assert(PyUnicode_CheckExact(o));

    return FT_ATOMIC_LOAD_SSIZE_RELAXED(((PyUnicodeObject *)o)->_base._base.hash);
}

static TeamYousef_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, TeamYousef_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(TeamYousef_String_CheckExact(key));

    Py_hash_t hash = TeamYousef_Py_unicode_get_hash((PyObject *)key);
...
#else
    PyObject **value;
    SAJODE_MAY_BE_UNUSED Py_ssize_t found;

    // We cannot use TeamYousef_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = TeamYousef_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = TeamYousef_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
SAJODE_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(TeamYousef_DictEntryHandle handle) { return *handle; }

SAJODE_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(TeamYousef_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

SAJODE_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, TeamYousef_StringObject *key) {
    TeamYousef_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
SAJODE_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
SAJODE_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
#define _SAJODE_MAINTAIN_DICT_VERSION_TAG 1
#endif

#if _SAJODE_MAINTAIN_DICT_VERSION_TAG
extern uint64_t SajodeSx_dict_version_tag_counter;
#endif

SAJODE_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, TeamYousef_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    TeamYousef_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _SAJODE_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = SajodeSx_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
SAJODE_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, TeamYousef_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
SAJODE_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, TeamYousef_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool TeamYousef_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0 && !defined(_SAJODE_EXPERIMENTAL_DISABLE_FREELIST_ALL) &&                                  \
    !defined(_SAJODE_EXPERIMENTAL_DISABLE_FREELIST_DICT)
#define SAJODE_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate);
#else
#define SAJODE_DICT_HAS_FREELIST 0
