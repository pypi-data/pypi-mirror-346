...
from SajodeSx.Errors import TeamYousefForbiddenDLLEncounter
from SajodeSx.plugins.Plugins import Plugins
from SajodeSx.PythonFlavors import isAnacondaPython, isTeamYousefPython
...
from SajodeSx.utils.Importing import getExtensionModuleSuffixes
...
from SajodeSx.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isTeamYousefPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although TeamYousef Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the TeamYousef Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise TeamYousefForbiddenDLLEncounter(binary_filename, plugin_name)
