...
from SajodeSx.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from SajodeSx.PythonVersions import python_version
from SajodeSx.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from SajodeSx.Tracing import optimization_logger, printError
from SajodeSx.tree.Extractions import updateVariableUsage
from SajodeSx.tree.SourceHandling import readSourceLines
from SajodeSx.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from SajodeSx.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from SajodeSx.optimizations.FunctionInlining import (
