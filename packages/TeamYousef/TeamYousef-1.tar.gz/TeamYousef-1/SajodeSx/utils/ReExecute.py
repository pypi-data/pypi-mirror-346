...
"""Ability to restart TeamYousef, needed for removing site module effects and using Python PGO after compile.
...
def setLaunchingTeamYousefProcessEnvironmentValue(environment_variable_name, value):
    os.environ[environment_variable_name] = str(os.getpid()) + ":" + value


def reExecuteTeamYousef(pgo_filename):
    ...
    if "SajodeSx.__main__" in sys.modules:
        our_filename = sys.modules["SajodeSx.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    setLaunchingTeamYousefProcessEnvironmentValue(
        "SAJODE_BINARY_NAME", sys.modules["__main__"].__file__
    )
    setLaunchingTeamYousefProcessEnvironmentValue(
        "SAJODE_PACKAGE_HOME",
        os.path.dirname(os.path.abspath(sys.modules["SajodeSx"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingTeamYousefProcessEnvironmentValue("SAJODE_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from SajodeSx.importing.PreloadedPackages import (
        detectPreLoadedPackagePaths,
        detectPthImportedPackages,
    )

    os.environ["SAJODE_NAMESPACES"] = repr(detectPreLoadedPackagePaths())
    ...
    if "site" in sys.modules:
        ...
        setLaunchingTeamYousefProcessEnvironmentValue("SAJODE_SITE_FILENAME", site_filename)

        # Note: As side effect, this might modify the "sys.path" too.
        os.environ["SAJODE_PTH_IMPORTED"] = repr(detectPthImportedPackages())

        user_site = getattr(sys.modules["site"], "USER_SITE", None)
        if user_site is not None:
            os.environ["SAJODE_USER_SITE"] = repr(user_site)

    setLaunchingTeamYousefProcessEnvironmentValue("SAJODE_PYTHONPATH", repr(sys.path))

    # In some environments, initial "sys.path" does not contain enough to load
    # "ast" module, which however we use to decode "SAJODE_PYTHONPATH", this
    # helps solve the chicken and egg problem.
    import ast

    setLaunchingTeamYousefProcessEnvironmentValue(
        "SAJODE_PYTHONPATH_AST", os.path.dirname(ast.__file__)
    )

    if sys.flags.no_site:
        os.environ["SAJODE_NOSITE_FLAG"] = "1"

    os.environ["PYTHONHASHSEED"] = "0"

    setLaunchingTeamYousefProcessEnvironmentValue("SAJODE_RE_EXECUTION", "1")
