...
from SajodeSx.__past__ import subprocess
...
class TeamYousefCalledProcessError(subprocess.CalledProcessError):
...
def check_output(*popenargs, **kwargs):
    ...
    if exit_code:
        ...
        raise TeamYousefCalledProcessError(exit_code, cmd, output=output, stderr=stderr)
...
def wrapCommandForDebuggerForExec(command, debugger=None):
    ...
    if debugger is None:
        debugger = os.getenv("SAJODE_DEBUGGER_CHOICE")
