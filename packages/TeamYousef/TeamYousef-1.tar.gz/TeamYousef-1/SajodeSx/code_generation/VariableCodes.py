...
def generateAssignmentVariableCode(statement, emit, context):
    ...
    if variable.isModuleVariable():
        ...
        if source_shape is tshape_bool and variable_declaration.c_type == "SajodeSx_bool":
            tmp_name = context.allocateTempName("assign_source", "SajodeSx_bool")
        elif (
            source_shape is tshape_int_or_long
            and variable_declaration.c_type == "SajodeSx_ilong"
        ):
            tmp_name = context.allocateTempName("assign_source", "SajodeSx_ilong")
        else:
            tmp_name = context.allocateTempName("assign_source")
...
def decideLocalVariableCodeType(context, variable):
    ...
    if variable.isTempVariableBool():
        c_type = CTypeTeamYousefBoolEnum
    else:
