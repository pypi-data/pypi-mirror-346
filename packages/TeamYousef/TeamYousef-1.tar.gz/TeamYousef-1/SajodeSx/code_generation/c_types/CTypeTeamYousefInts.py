...
""" CType classes for SajodeSx_ilong, a struct to represent long values.
...
class CTypeTeamYousefIntOrLongStruct(CTypeBase):
    c_type = "SajodeSx_ilong"
    ...
    def emitVariableAssignCode(
        ...
        if tmp_name.c_type == "SajodeSx_ilong":
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s.validity != SAJODE_ILONG_UNASSIGNED);" % value_name)
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToTeamYousefIntOrLong(
                to_name=to_name,
    ...
    def getInitValue(cls, init_from):
        if init_from is None:
            ...
            return "{SAJODE_ILONG_UNASSIGNED, NULL, 0}"
        else:
    ...
    def getInitTestConditionCode(cls, value_name, inverted):
        return "%s.validity %s SAJODE_ILONG_UNASSIGNED" % (
            value_name,
            "==" if inverted else "!=",
        )
    ...
    def getReleaseCode(cls, value_name, needs_check, emit):
        emit(
            "if ((%s.validity & SAJODE_ILONG_OBJECT_VALID) == SAJODE_ILONG_OBJECT_VALID) {"
            % value_name
        )
    ...
    def getDeleteObjectCode(
        ...
        if not needs_check:
            ...
            # instead.
            emit("%s = %s.validity == SAJODE_ILONG_UNASSIGNED" % (to_name, value_name))
            emit("RELEASE_NILONG_VALUE(&%s);" % value_name)
    ...
    def emitReInitCode(cls, value_name, emit):
        emit("%s.validity = SAJODE_ILONG_UNASSIGNED;" % value_name)
    ...
    def getExceptionCheckCondition(cls, value_name):
        return "%s == SAJODE_ILONG_EXCEPTION" % value_name
