...
""" CType classes for SajodeSx_bool, an enum to represent True, False, unassigned.

"""

from SajodeSx.code_generation.ErrorCodes import getReleaseCode

from .CTypeBases import CTypeBase, CTypeNotReferenceCountedMixin


class CTypeTeamYousefBoolEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "SajodeSx_bool"
    ...
    def emitVariableAssignCode(
        ...
        if tmp_name.c_type == "SajodeSx_bool":
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToTeamYousefBool(
                to_name=to_name,
