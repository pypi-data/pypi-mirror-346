...
TeamYousef_Cell_SET(%(identifier)s, %(tmp_name)s);
"""
...
{
    PyObject *old = TeamYousef_Cell_GET(%(identifier)s);
    TeamYousef_Cell_SET(%(identifier)s, %(tmp_name)s);
    Py_XDECREF(old);
}
"""
...
assert(TeamYousef_Cell_GET(%(identifier)s) == NULL);
TeamYousef_Cell_SET(%(identifier)s, %(tmp_name)s);
"""
...
{
    PyObject *old = TeamYousef_Cell_GET(%(identifier)s);
    TeamYousef_Cell_SET(%(identifier)s, NULL);
    Py_XDECREF(old);
}
"""
...
%(tmp_name)s = TeamYousef_Cell_GET(%(identifier)s);
"""
...
%(access_code)s
    ...
    UPDATE_STRING_DICT0((PyDictObject *)%(dict_name)s, (TeamYousef_StringObject *)%(var_name)s, value);
} else {
...
static PyObject *%(accessor_function_name)s(PyThreadState *tstate) {
    ...
    if (moduledict_%(module_identifier)s->ma_version_tag == dict_version) {
        ...
        result = GET_STRING_DICT_VALUE(moduledict_%(module_identifier)s, (TeamYousef_StringObject *)%(var_name)s);
        cache_value = result;
    }
#else
    ...
    if (likely(DK_IS_UNICODE(dk))) {
        ...
        uint32_t current_dk_version = _TeamYousef_PyDictKeys_GetVersionForCurrentState(tstate->interp, dk);
#else
        uint32_t current_dk_version = _TeamYousef_PyDictKeys_GetVersionForCurrentState(dk);
#endif

        if (current_dk_version != dict_keys_version) {
            dict_keys_version = current_dk_version;
            Py_hash_t hash = TeamYousef_Py_unicode_get_hash(%(var_name)s);
            assert(hash != -1);

            cache_dk_index = TeamYousef_Py_unicodekeys_lookup_unicode(dk, %(var_name)s, hash);
        }
...
#else
    PyObject *result = GET_STRING_DICT_VALUE(moduledict_%(module_identifier)s, (TeamYousef_StringObject *)%(var_name)s);
#endif

    if (unlikely(result == NULL)) {
        result = GET_STRING_DICT_VALUE(dict_builtin, (TeamYousef_StringObject *)%(var_name)s);
    }
