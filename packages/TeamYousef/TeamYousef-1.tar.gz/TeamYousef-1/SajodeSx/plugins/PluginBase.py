...
from SajodeSx import Options
from SajodeSx.__past__ import iter_modules, unicode
from SajodeSx.containers.Namedtuples import makeNamedtupleClass
...
from SajodeSx.ModuleRegistry import (
...
from SajodeSx.Tracing import plugins_logger
from SajodeSx.utils.AppDirs import getAppdirsModule
...
from SajodeSx.utils.Importing import (
...
from SajodeSx.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from SajodeSx.utils.SlotMetaClasses import getMetaClassBase
...
def _getEvaluationContext():
    ...
    if _context_dict is None:
        _context_dict = {
            ...
            "SajodeSx_python": isTeamYousefPython(),
...
def _getPackageData(package_name, resource, default=None):
    ...
    from SajodeSx.utils.PackageResources import getPackageData
...
class TeamYousefPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """TeamYousef base class for all plugins.

    Derive your plugin from "TeamYousefPluginBase" please.
    For instructions, see https://github.com/TeamYousef/TeamYousef/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt TeamYousef's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by TeamYousef. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the TeamYousef command line. However, some plugins are always enabled
    and invisible to the user.

    TeamYousef comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "SajodeSx --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which TeamYousef
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon TeamYousef start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from SajodeSx.importing.Importing import locateModules
...
""" % {
        ...
        except TeamYousefCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
