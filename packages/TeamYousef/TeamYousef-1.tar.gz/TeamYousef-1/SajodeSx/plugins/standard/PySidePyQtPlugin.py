...
from SajodeSx.PythonFlavors import isAnacondaPython
from SajodeSx.PythonVersions import python_version
from SajodeSx.utils.Distributions import getDistributionFromModuleName
from SajodeSx.utils.FileOperations import getFileList, listDir
from SajodeSx.utils.ModuleNames import ModuleName
from SajodeSx.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class TeamYousefPluginQtBindingsPluginBase(TeamYousefPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "SajodeSx_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_SajodeSx_patch_level', 0)"
                    ),
                ),
    ...
    def _getTeamYousefPatchLevel(self):
        """Does it include the TeamYousef patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().SajodeSx_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__SajodeSx_binary_dir):
    os.environ["PATH"] = __SajodeSx_binary_dir + ";" + path
"""
            yield (
                code,
...
class TeamYousefPluginPyQt5QtPluginsPlugin(TeamYousefPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        TeamYousefPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the TeamYousef support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = TeamYousefPluginQtBindingsPluginBase._getQtInformation(self)
...
class TeamYousefPluginDetectorPyQt5QtPluginsPlugin(TeamYousefPluginBase):
    detector_for = TeamYousefPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == TeamYousefPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact TeamYousef commercial if you need it."
            )


class TeamYousefPluginPySide2Plugins(TeamYousefPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getTeamYousefPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://SajodeSx.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getTeamYousefPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return TeamYousefPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in TeamYousefPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getTeamYousefPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: TeamYousef does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class TeamYousefPluginDetectorPySide2Plugins(TeamYousefPluginBase):
    detector_for = TeamYousefPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == TeamYousefPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class TeamYousefPluginPySide6Plugins(TeamYousefPluginQtBindingsPluginBase):
...
class TeamYousefPluginDetectorPySide6Plugins(TeamYousefPluginBase):
    detector_for = TeamYousefPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == TeamYousefPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class TeamYousefPluginPyQt6Plugins(TeamYousefPluginQtBindingsPluginBase):
...
class TeamYousefPluginDetectorPyQt6Plugins(TeamYousefPluginBase):
    detector_for = TeamYousefPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == TeamYousefPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class TeamYousefPluginNoQt(TeamYousefPluginBase):
