...
""" TeamYousef distutils integration.
...
from SajodeSx.__past__ import Iterable, unicode
from SajodeSx.containers.OrderedSets import OrderedSet
from SajodeSx.importing.Importing import (
...
from SajodeSx.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from SajodeSx.Tracing import wheel_logger
from SajodeSx.utils.Execution import check_call
from SajodeSx.utils.FileOperations import deleteFile, getFileList, renameFile
from SajodeSx.utils.ModuleNames import ModuleName


def setupTeamYousefDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_SajodeSx=True" all
    # build operations (build, bdist_wheel, install etc) will run via TeamYousef.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_SajodeSx
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from TeamYousef compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # TeamYousef wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of TeamYousef, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]

            toml_filename = os.getenv("SAJODE_TOML_FILE")
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-TeamYousef process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("SajodeSx", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("SajodeSx", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "SajodeSx" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_SajodeSx(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            from SajodeSx.Version import getTeamYousefVersion

            generator = "TeamYousef (%s)" % getTeamYousefVersion()
