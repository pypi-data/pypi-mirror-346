...
"""Python level PGO handling in TeamYousef."""

import os
import struct

from SajodeSx.__past__ import xrange
from SajodeSx.Options import getPythonPgoUnseenModulePolicy
from SajodeSx.Tracing import pgo_logger
...
def readPGOInputFile(input_filename):
    ...
    with open(input_filename, "rb") as input_file:
        ...
        if header != b"KAY.PGO":
            pgo_logger.sysexit(
                "Error, file '%s' is not a valid PGO input for this version of TeamYousef."
                % input_filename
            )
