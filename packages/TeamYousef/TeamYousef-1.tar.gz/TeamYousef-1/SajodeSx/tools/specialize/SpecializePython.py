...
import SajodeSx.code_generation.BinaryOperationHelperDefinitions
import SajodeSx.code_generation.CodeGeneration
import SajodeSx.code_generation.ComparisonCodes
import SajodeSx.code_generation.Namify
import SajodeSx.nodes.NetworkxNodes
import SajodeSx.nodes.PackageMetadataNodes
import SajodeSx.nodes.PackageResourceNodes
import SajodeSx.nodes.SideEffectNodes
import SajodeSx.nodes.TensorflowNodes
...
import SajodeSx.specs.BuiltinTypeOperationSpecs
import SajodeSx.specs.HardImportSpecs
import SajodeSx.tree.Building
from SajodeSx.containers.OrderedDicts import OrderedDict
from SajodeSx.containers.OrderedSets import OrderedSet
from SajodeSx.nodes.ImportNodes import hard_modules_non_stdlib
from SajodeSx.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from SajodeSx.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "SajodeSx.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    SajodeSx.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(SajodeSx.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "SajodeSx/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from SajodeSx.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from SajodeSx.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from SajodeSx.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from SajodeSx.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from SajodeSx.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from SajodeSx.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "SajodeSx/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "SajodeSx/nodes/ChildrenHavingMixins.py"
    filename_python2 = "SajodeSx/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "SajodeSx/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from SajodeSx.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "SajodeSx/nodes/HardImportNodesGenerated.py"
