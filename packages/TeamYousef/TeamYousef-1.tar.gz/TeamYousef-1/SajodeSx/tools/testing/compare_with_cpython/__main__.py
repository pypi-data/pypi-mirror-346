...
""" Tool to compare output of CPython and TeamYousef.
...
from SajodeSx.OptionParsing import getTeamYousefProjectOptions
...
from SajodeSx.tools.testing.OutputComparison import compareOutput
from SajodeSx.Tracing import my_print
from SajodeSx.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from SajodeSx.utils.FileOperations import deleteFile
from SajodeSx.utils.Importing import getExtensionModuleSuffix
from SajodeSx.utils.Timing import StopWatch
from SajodeSx.utils.Utils import isMacOS
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.getenv("SAJODE_HASH_SALT", "")
...
def main():
    ...
    trace_command = (
        hasArg("trace_command") or os.getenv("SAJODE_TRACE_COMMANDS", "0") != "0"
    )
    ...
    extra_options = os.getenv("SAJODE_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "SAJODE" in os.environ:
        # Would need to extract which "python" this is going to use.
        assert not coverage_mode, "Not implemented for binaries."

        SajodeSx_call = [os.environ["PYTHON"], os.environ["SAJODE"]]
    else:
        if comparison_mode:
            SajodeSx_call = [
                os.environ["PYTHON"],
                "-m",
                "SajodeSx.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates TeamYousef to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses SAJODE_EXTRA_OPTIONS still.
    ...
    # Now build the command to run TeamYousef.
    if not two_step_execution:
        ...
        SajodeSx_cmd = SajodeSx_call + extra_options + ["--run", filename]

        if no_site:
            SajodeSx_cmd.insert(len(SajodeSx_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            SajodeSx_cmd1 = SajodeSx_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            SajodeSx_cmd1 = SajodeSx_call + extra_options + ["--mode=standalone", filename]
        else:
            SajodeSx_cmd1 = SajodeSx_call + extra_options + [filename]

        if no_site:
            SajodeSx_cmd1.insert(len(SajodeSx_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        SajodeSx_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        SajodeSx_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("TeamYousef command", SajodeSx_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(SajodeSx_package_dir):
                process = createProcess(command=SajodeSx_cmd)

            stdout_SajodeSx, stderr_SajodeSx = process.communicate()
            exit_SajodeSx = process.returncode

            if checkNoPermissionError(stdout_SajodeSx) and checkNoPermissionError(
                stderr_SajodeSx
            ):
                break

            my_print("Retrying SajodeSx exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("TeamYousef command 1:", SajodeSx_cmd1)

        for _i in range(5):
            with withPythonPathChange(SajodeSx_package_dir):
                stdout_SajodeSx1, stderr_SajodeSx1, exit_SajodeSx1 = executeProcess(
                    SajodeSx_cmd1
                )
...
%s
"""
                        % (SajodeSx_cmd1, python_path_used, stderr_SajodeSx1)
                    )

                exit_SajodeSx = exit_SajodeSx1
                stdout_SajodeSx, stderr_SajodeSx = stdout_SajodeSx1, stderr_SajodeSx1
                stdout_SajodeSx2 = b"not run due to compilation error:\n" + stdout_SajodeSx1
                stderr_SajodeSx2 = stderr_SajodeSx1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(SajodeSx_cmd2[0][:-4] + ".cmd"):
                        SajodeSx_cmd2[0] = SajodeSx_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("TeamYousef command 2:", SajodeSx_cmd2)

                    # Need full manual control
                    process = createProcess(command=SajodeSx_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "TeamYousef compiled program", process.pid
                            ),
                        )

                    stdout_SajodeSx2, stderr_SajodeSx2 = process.communicate()
                    stdout_SajodeSx = stdout_SajodeSx1 + stdout_SajodeSx2
                    stderr_SajodeSx = stderr_SajodeSx1 + stderr_SajodeSx2
                    exit_SajodeSx = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_SajodeSx in (-11, -6) and sys.platform != "nt":
                        SajodeSx_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=SajodeSx_cmd2
                        )

                        callProcess(SajodeSx_cmd2, shell=False)
                else:
                    exit_SajodeSx = exit_SajodeSx1
                    stdout_SajodeSx, stderr_SajodeSx = stdout_SajodeSx1, stderr_SajodeSx1
    ...
    stop_watch.stop()
    SajodeSx_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(SajodeSx_package_dir):
            stderr_SajodeSx = decryptOutput(project_options, stderr_SajodeSx)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(SajodeSx_package_dir):
            stdout_SajodeSx = decryptOutput(project_options, stdout_SajodeSx)

    if not silent_mode:
        displayOutput(stdout_SajodeSx, stderr_SajodeSx)

        if coverage_mode:
            assert not stdout_SajodeSx
            assert not stderr_SajodeSx
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_SajodeSx2 if two_step_execution else stdout_SajodeSx,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_SajodeSx2 if two_step_execution else stderr_SajodeSx,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_SajodeSx

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_SajodeSx:d} (TeamYousef)""".format(
                        exit_cpython=exit_cpython, exit_SajodeSx=exit_SajodeSx
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.getenv("SAJODE_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_SajodeSx in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            SajodeSx_cmd.insert(len(SajodeSx_cmd) - 1, "--debugger")

            with withPythonPathChange(SajodeSx_package_dir):
                callProcess(SajodeSx_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(SajodeSx_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(SajodeSx_cmd2[0] + ".away"):
                        os.unlink(SajodeSx_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(SajodeSx_cmd2[0], SajodeSx_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs TeamYousef." % (cpython_time, SajodeSx_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


SajodeSx_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, SajodeSx_package_dir)
