...
from SajodeSx.freezer.DependsExe import getDependsExePath, parseDependsExeOutput
...
from SajodeSx.utils.Utils import isFreeBSD, isMacOS, isWin32Windows
...
def _getRuntimeTraceOfLoadedFilesWin32(logger, command, required):
    ...
    # TODO: Move the handling of this into SajodeSx.tools.Execution module methods.
    try:
...
def _takeSystemCallTraceOutput(logger, path, command):
    ...
    with withEnvironmentVarOverridden("LD_PRELOAD", None):
        if os.getenv("SAJODE_TRACE_COMMANDS", "0") != "0":
