...
from SajodeSx.__past__ import md5, subprocess
from SajodeSx.containers.OrderedDicts import OrderedDict
from SajodeSx.Options import getCommercialVersion
from SajodeSx.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from SajodeSx.Tracing import OurLogger, flushStandardOutputs, my_print
...
from SajodeSx.utils.InstalledPythons import findInstalledPython
from SajodeSx.utils.Jinja2 import getTemplate
from SajodeSx.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def setup(suite="", needs_io_encoding=False, silent=False, go_main=True):
    ...
    os.environ["SAJODE_TEST_SUITE"] = suite
...
def _compareWithCPythonVariations(
    ...
    for test_variant_name in extra_flags.keys():
        ...
        with withEnvironmentVarOverridden("SAJODE_TEST_VARIANT", test_variant_name):
            compareWithCPython(
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("SAJODE_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "SajodeSx.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate TeamYousef comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in TeamYousef with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to TeamYousef.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in TeamYousef, good for test coverage.
Defaults to off.""",
    )
...
Defaults to off.""",
    ...
    if options.show_commands:
        os.environ["SAJODE_TRACE_COMMANDS"] = "1"
...
def addExtendedExtraOptions(*args):
    old_value = os.getenv("SAJODE_EXTRA_OPTIONS")
    ...
    os.environ["SAJODE_EXTRA_OPTIONS"] = value
...
def withExtendedExtraOptions(*args):
    ...
    if old_value is None:
        del os.environ["SAJODE_EXTRA_OPTIONS"]
    else:
        os.environ["SAJODE_EXTRA_OPTIONS"] = old_value
...
def getTestingCPythonOutputsCacheDir():
    ...
    result = os.path.join(cache_dir, os.getenv("SAJODE_TEST_SUITE", ""))
...
def setupCacheHashSalt(test_code_path):
    ...
    os.environ["SAJODE_HASH_SALT"] = salt_value.hexdigest()
...
#     "# SajodeSx-skip-unless-expression: expression to be evaluated"
#       OR
#     "# SajodeSx-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# SajodeSx-skip-unless-"):
...
def extractTeamYousefVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("TeamYousef V")]

    return version_line[8:].rstrip()


def decryptOutput(project_options, output):
    SajodeSx_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "SajodeSx.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            SajodeSx_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(SajodeSx_decrypt_call, stdin=output)
