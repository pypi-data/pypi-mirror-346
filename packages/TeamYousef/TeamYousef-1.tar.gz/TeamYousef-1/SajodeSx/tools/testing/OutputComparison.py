...
logging_info_re = re.compile(r"^TeamYousef(-\w+)?:([-\w]+:)? ")
logging_warning_re = re.compile(r"^TeamYousef.*?:WARNING")

# Python3.11 style traceback carets are not done by TeamYousef (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("TeamYousef-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("TeamYousef:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "TeamYousef-Options:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("TeamYousef:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("TeamYousef:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("TeamYousef:WARNING:     Complex topic"):
            continue
        if line.startswith("TeamYousef:WARNING:") and "matching checksum" in line:
            continue
        ...
        # This URL is updated, and TeamYousef outputs the new one, but we test
        ...
        # we are going to ignore the issue as TeamYousef is fine.
        if (
            line
            == """\
...
def compareOutput(
    kind, out_cpython, out_SajodeSx, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_SajodeSx, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="SajodeSx", detail=kind),
        from_date,
        to_date,
        n=3,
    )
