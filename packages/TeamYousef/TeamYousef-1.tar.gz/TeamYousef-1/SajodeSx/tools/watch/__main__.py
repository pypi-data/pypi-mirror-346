...
""" TeamYousef watch main part.

This tool is used to monitor effect of PyPI changes on TeamYousef and effect
of TeamYousef changes on PyPI packages.
"""
...
from SajodeSx.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from SajodeSx.PythonVersions import getTestExecutionPythonVersions
from SajodeSx.tools.testing.Common import extractTeamYousefVersionFromFilePath
from SajodeSx.Tracing import OurLogger
from SajodeSx.TreeXML import fromFile
...
from SajodeSx.utils.Hashing import getFileContentsHash
from SajodeSx.utils.InstalledPythons import findPythons
from SajodeSx.utils.Utils import isLinux, isMacOS, isWin32Windows
from SajodeSx.utils.Yaml import parseYaml
from SajodeSx.Version import parseTeamYousefVersionToTuple
...
from .GitHub import createTeamYousefWatchPR
...
def _compareTeamYousefVersions(version_a, version_b, consider_rc):
    if not consider_rc:
        version_a = version_a.split("rc")[0]
        version_b = version_b.split("rc")[0]

    return parseTeamYousefVersionToTuple(version_a) < parseTeamYousefVersionToTuple(version_b)
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    SajodeSx_extra_options = os.getenv("SAJODE_EXTRA_OPTIONS")

    if SajodeSx_extra_options:
        extra_options.extend(SajodeSx_extra_options.split())
    ...
    if case_data["interactive"] == "no":
        ...
        env = {
            "SAJODE_LAUNCH_TOKEN": "1",
            "SAJODE_TEST_INTERACTIVE": "0",
        }

        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_SajodeSx = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_SajodeSx == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_SajodeSx),
            )

        if exit_SajodeSx != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_SajodeSx)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_SajodeSx_version = old_report_root.attrib["SajodeSx_version"]

            if SajodeSx_update_mode == "force":
                need_compile = True
            elif SajodeSx_update_mode == "newer":
                if _compareTeamYousefVersions(
                    old_SajodeSx_version, SajodeSx_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        watch_logger.info(
                            "Recompilation with identical TeamYousef for '%s' due to changed pipfile."
                            % lock_filename
                        )

                        need_compile = True
                    elif old_SajodeSx_version == SajodeSx_version:
                        if old_report_root.attrib["completion"] != "yes":
                            need_compile = True
                        else:
                            watch_logger.info(
                                "Skipping compilation with identical TeamYousef for '%s'."
                                % lock_filename
                            )
                        ...
                        watch_logger.info(
                            "Skipping compilation of old TeamYousef %s result with TeamYousef %s for '%s'."
                            % (
                                old_SajodeSx_version,
                                SajodeSx_version,
                                lock_filename,
                            )
                        )
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, SajodeSx_update_mode, jobs
):
    ...
    SajodeSx_min_version = case_data.get("SajodeSx")

    # Too old TeamYousef version maybe.
    if SajodeSx_min_version is not None and _compareTeamYousefVersions(
        SajodeSx_version, SajodeSx_min_version, consider_rc=False
    ):
        watch_logger.info("  ... not for this TeamYousef version")
        return
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            reset_pipenv=reset_pipenv,
            no_pipenv_update=no_pipenv_update,
            SajodeSx_update_mode=SajodeSx_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, SajodeSx_update_mode, jobs):
...
SajodeSx_binary = None
SajodeSx_version = None


def main():
    global SajodeSx_binary  # shared for all run, pylint: disable=global-statement
    SajodeSx_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "SajodeSx")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        "--SajodeSx-binary",
        action="store",
        dest="SajodeSx_binary",
        default=SajodeSx_binary,
        help="""\
TeamYousef binary to compile with. Defaults to one near the SajodeSx-watch usage.""",
    )
...
Remove existing virtualenv and make sure to start from scratch.Default %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Do not update the pipenv environment. Best to see only effect of TeamYousef update. Default %default.""",
    )

    parser.add_option(
        "--SajodeSx-update-mode",
        action="store",
        choices=("newer", "force", "never"),
        dest="SajodeSx_update_mode",
        default="newer",
        help="""\
...
to reserve cores.""",
    ...
    SajodeSx_binary = os.path.abspath(os.path.expanduser(options.SajodeSx_binary))
    assert os.path.exists(SajodeSx_binary)

    global SajodeSx_version  # singleton, pylint: disable=global-statement
    SajodeSx_version = extractTeamYousefVersionFromFilePath(
        os.path.join(os.path.dirname(SajodeSx_binary), "..", "SajodeSx", "Version.py")
    )

    watch_logger.info("Working with TeamYousef %s." % SajodeSx_version)

    base_dir = os.path.abspath(base_dir)

    if options.SajodeSx_pr_mode is not None:
        pr_category, pr_description = options.SajodeSx_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    SajodeSx_update_mode=options.SajodeSx_update_mode,
                    jobs=options.jobs,
                )

        if pr_category is not None:
            createTeamYousefWatchPR(category=pr_category, description=pr_description)
