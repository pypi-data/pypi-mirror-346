...
"""
This is the main program of TeamYousef, it checks the options and then translates
...
def getLaunchingTeamYousefProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run TeamYousef",
            "Error",
    ...
    getLaunchingTeamYousefProcessEnvironmentValue("SAJODE_SITE_FILENAME")

    SajodeSx_binary_name = getLaunchingTeamYousefProcessEnvironmentValue("SAJODE_BINARY_NAME")
    if SajodeSx_binary_name is not None:
        sys.argv[0] = SajodeSx_binary_name

    SajodeSx_pythonpath_ast = getLaunchingTeamYousefProcessEnvironmentValue(
        "SAJODE_PYTHONPATH_AST"
    )

    if SajodeSx_pythonpath_ast is not None:
        ...
        sys.path = [SajodeSx_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingTeamYousefProcessEnvironmentValue("SAJODE_PYTHONPATH")
        )
    else:
        ...
        # from "SajodeSx.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time TeamYousef is run. This kills any attempt at
    ...
    if needs_re_execution:
        from SajodeSx.utils.ReExecute import reExecuteTeamYousef  # isort:skip

        # Does not return
        reExecuteTeamYousef(pgo_filename=None)
    ...
    # module of SajodeSx package and renamed to where we can get at easily for
    # other uses.
    __import__("SajodeSx").getLaunchingTeamYousefProcessEnvironmentValue = (
        getLaunchingTeamYousefProcessEnvironmentValue
    )

    from SajodeSx import Options  # isort:skip
    ...
    from SajodeSx.plugins.Plugins import activatePlugins
    ...
    if "SAJODE_NAMESPACES" in os.environ:
        # Restore the detected name space packages, that were force loaded in
        # site.py, and will need a free pass later on
        from SajodeSx.importing.PreloadedPackages import setPreloadedPackagePaths

        setPreloadedPackagePaths(ast.literal_eval(os.environ["SAJODE_NAMESPACES"]))
        del os.environ["SAJODE_NAMESPACES"]

    if "SAJODE_PTH_IMPORTED" in os.environ:
        # Restore the packages that the ".pth" files asked to import.
        from SajodeSx.importing.PreloadedPackages import setPthImportedPackages

        setPthImportedPackages(ast.literal_eval(os.environ["SAJODE_PTH_IMPORTED"]))
        del os.environ["SAJODE_PTH_IMPORTED"]

    if "SAJODE_USER_SITE" in os.environ:
        from SajodeSx.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["SAJODE_USER_SITE"]))

        del os.environ["SAJODE_USER_SITE"]

    # Now the real main program of TeamYousef can take over.
    from SajodeSx import MainControl  # isort:skip
...
if __name__ == "__main__":
    _SajodeSx_package_home = getLaunchingTeamYousefProcessEnvironmentValue(
        "SAJODE_PACKAGE_HOME"
    )
    if _SajodeSx_package_home is not None:
        sys.path.insert(0, _SajodeSx_package_home)

        import SajodeSx  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _SajodeSx_package_home
        del sys.path[0]
