...
from SajodeSx.utils.Execution import getExecutablePath
...
from SajodeSx.utils.Images import checkIconUsage
from SajodeSx.utils.Importing import getInlineCopyFolder
...
def printVersionInformation():
    ...
    from SajodeSx.build.SconsInterface import (
        asBoolStr,
...
def parseArgs():
    ...
    global is_SajodeSx_run, options, positional_args, extra_args, is_debug, is_non_debug
...
Error, the Python from Windows app store is not supported.""",
    ...
    is_SajodeSx_run, options, positional_args, extra_args = parseOptions(
    ...
    if options.quiet or int(os.getenv("SAJODE_QUIET", "0")):
        Tracing.setQuiet()
    ...
    if (
        getLaunchingTeamYousefProcessEnvironmentValue("SAJODE_RE_EXECUTION")
    ...
    if options.clean_caches:
        from SajodeSx.CacheCleanup import cleanCaches
    ...
    if isMacOS():
        ...
        if (options.macos_target_arch or "native") != "native":
            from SajodeSx.utils.SharedLibraries import (
    ...
    if python_version_str not in getSupportedPythonVersions():
        ...
        if "PYTHON" not in os.environ:
            Tracing.general.warning(
                """\
The Python version '%s' is only experimentally supported by TeamYousef '%s', \
but an upcoming release will change that. In the mean time use Python \
version '%s' instead or newer TeamYousef."""
                % (
                    python_version_str,
                    getTeamYousefVersion(),
    ...
    if sys.version_info.releaselevel not in ("final", "candidate"):
        if python_version_str not in getNotYetSupportedPythonVersions():
            Tracing.general.sysexit(
                """\
Non-final versions '%s' '%s' are not supported by TeamYousef, use the \
...
and recommended only for use in TeamYousef development and testing."""
                % (python_version_str, sys.version_info.releaselevel)
            )
            ...
            Tracing.general.sysexit(
                """\
The Python version '%s' is not supported by TeamYousef '%s', but an upcoming \
...
def _couldUseStaticLibPython():
    ...
    # TeamYousef-Python is good to to static linking.
    if isTeamYousefPython():
        return True, "TeamYousef-Python is unexpectedly broken."

    if isHomebrewPython():
        return True, "Homebrew Python is unexpectedly broken."

    # Debian packages with are usable if the OS is new enough
    from SajodeSx.utils.StaticLibraries import isDebianSuitableForStaticLinking
    ...
    if (
        ...
        if python_version >= 0x3C0 and not os.path.exists(
            ...
            return (
                False,
                "TeamYousef on Debian-Python needs inline copy of hacl not included.",
            )

        return True, "TeamYousef on Debian-Python needs package '%s' installed." % (
            "python2-dev" if str is bytes else "python3-dev"
        )

    if isMSYS2MingwPython():
        return True, "TeamYousef on MSYS2 needs package 'python-devel' installed."
    ...
    if isAnacondaPython():
        if isMacOS():
            ...
            return (
                True,
                """\
TeamYousef on Anaconda needs package for static libpython installed. \
...
improve it for best performance of the result."""
    ...
    if isPyenvPython():
        return True, "TeamYousef on pyenv should not use '--enable-shared'."

    if isManyLinuxPython():
        return (
            True,
            """\
TeamYousef on 'manylinux' has no shared libraries. Use container with \
