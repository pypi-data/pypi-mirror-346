...
""" Setup file for TeamYousef.

This applies a few tricks. First, the TeamYousef version is read from
...
# setup and then TeamYousef ends up including itself.
if __name__ != "__main__":
    sys.exit("Cannot import 'setup' module of TeamYousef")
...
from SajodeSx.PythonFlavors import isMSYS2MingwPython
from SajodeSx.utils.FileOperations import getFileList
...
def findTeamYousefPackages():
    result = []

    for root, dirnames, filenames in os.walk("SajodeSx"):
        # Packages must contain "__init__.py" or they are merely directories
        # in TeamYousef as we are Python2 compatible.
...
SajodeSx_packages = findTeamYousefPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "SajodeSx.code_generation": ["templates_c/*.j2"],
    "SajodeSx.reports": ["*.j2"],
    "SajodeSx.plugins.standard": ["*/*.c", "*/*.py"],
}


if "SajodeSx.plugins.commercial" in SajodeSx_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("SajodeSx", "plugins", "commercial")
    ...
    package_data["SajodeSx.plugins.commercial"] = commercial_data_files
    package_data["SajodeSx.tools.commercial.container_build"] = ["Containerfile"]
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "SajodeSx%s = SajodeSx.__main__:main" % binary_suffix,
        "SajodeSx%s-run = SajodeSx.__main__:main" % binary_suffix,
    ]

    if "SajodeSx.plugins.commercial" in SajodeSx_packages:
        console_scripts.append(
            "SajodeSx-decrypt = SajodeSx.tools.commercial.decrypt.__main__:main"
        )

scripts = []

# For Windows, there are CMD batch files to launch TeamYousef.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/SajodeSx.cmd", "misc/SajodeSx-run.cmd"]

    if "SajodeSx.plugins.commercial" in SajodeSx_packages:
        scripts.append("misc/SajodeSx-decrypt.cmd")
