...
def cluster_enrichment_profiles(
    ...
    if (enrichment_df.values == 0).all():
        raise ValueError("All-zero enrichment matrix. Cannot perform clustering.")
    if enrichment_df.shape[0] > 1000:
        logger.warning(
            f"Clustering being attempted on {enrichment_df.shape[0]} terms — this may take time. "
            "Consider filtering terms by p-value or significance prior to clustering."
        )
    ...
    nonzero_mask = matrix.any(axis=1)
    if not np.all(nonzero_mask):
        ...
        enrichment_df = enrichment_df.loc[nonzero_mask]
        matrix = matrix[nonzero_mask]

    if metric == "jaccard" and not np.array_equal(matrix, matrix.astype(bool)):
        logger.warning("Input matrix is not binary. Switching metric from 'jaccard' to 'cosine'.")
        metric = "cosine"
