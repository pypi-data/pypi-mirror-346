...
from ._helpers import parse_loadfile, check_savefile, fig_to_bytes, rows_to_bytes, new_lamp_position
...
class Room:
    ...
    def generate_report(self, fname=None):
        ...
        rows += [["", "", self.x, self.y, self.z, self.units]]
        ...
        # ───  Calculation planes  ───────────────────────────
        planes = [z for z in self.calc_zones.values() if z.calctype == "Plane"]
        if planes:
        ...
        # ───  Calculation volumes  ──────────────────────────
        vols = [z for z in self.calc_zones.values() if z.calctype == "Volume"]
        if vols:
        ...
        # ───  Statistics  ──────────────────────────
        zones = list(self.calc_zones.values())
        if zones:
            rows += [["Statistics"]]
