...
if __name__ == "__main__":
    ...
    import pickle
    net = Net('../Sample/signalized_network.net.xml')
    # with open("ingolstadt_base.obj", "w") as f:
    #     f.write(net.generate_obj_text())
    fig, ax = plt.subplots()
    ax.set_facecolor("green")
    net.plot(ax, style=USA_STYLE)
    ax.set_aspect("equal")
    # plt.pcolor()
    trajs_RL = Trajectories("../Sample/example_fcd-output.xml")
    a = animation.FuncAnimation(fig, trajs_RL.plot_points, frames=trajs_RL.timestep_range(), repeat=False,
                                interval=trajs_RL.timestep, fargs=(ax,), blit=True)
    plt.show()
