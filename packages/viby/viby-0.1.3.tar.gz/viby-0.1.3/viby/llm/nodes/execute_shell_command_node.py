...
class ExecuteShellCommandNode(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        # 显示命令并获取用户选择
        print(
            f"{Colors.BLUE}{get_text('SHELL', 'execute_prompt', command)}{Colors.END}"
        )
        ...
        if exec_res_dict.get("status") == "call_llm":
            improve_prompt = get_text(
                "SHELL",
                "improve_command_prompt",
                command,
                exec_res_dict.get("user_feedback", ""),
            )
            # 初始化 messages 如果不存在
    ...
    def _execute_command(self, command: str) -> dict:
        ...
        try:
            ...
            if shell_exec:
                process = subprocess.run(command, shell=True, executable=shell_exec)
            else:
    ...
    def _cancel_operation(self, command: str) -> dict:
        """取消操作"""
        print(
            f"{Colors.YELLOW}{get_text('GENERAL', 'operation_cancelled')}{Colors.END}"
        )
    ...
    def _continue_chat(self, command: str) -> dict:
        ...
        feedback = prompt(HTML('<span class="ansicyan">|> </span>'))
        return {
            "status": "call_llm",
            "command": command,
