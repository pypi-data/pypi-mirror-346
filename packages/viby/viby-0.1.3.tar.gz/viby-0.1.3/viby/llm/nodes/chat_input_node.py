...
class ChatInputNode(Node):
    ...
    def _get_history_path() -> Path:
        ...
        base_dir = Path(os.environ.get("APPDATA", str(Path.home()))) if platform.system() == "Windows" else Path.home() / ".config"
    ...
    def _create_key_bindings(self):
        ...
        def _(event):
            ...
            run_in_terminal(lambda: print_formatted_text(
                HTML(f'<span class="ansicyan">{get_text("CHAT", "current_time", current_time)}</span>')
            ))
    ...
    def _command_help(self):
        ...
        command_text = '\n'.join([f'  <span class="help-command">{cmd}</span> - <span class="help-desc">{desc}</span>' 
        ...
        shortcuts_text = '\n'.join([f'  <span class="help-desc">{shortcut}</span>' for shortcut in shortcuts])
    ...
    def _command_history_view(self, limit=20):
        ...
        if not history_items:
            self._format_output(f'<span class="warning">{get_text("CHAT", "history_empty", "还没有命令历史。")}</span>')
            return
            
        display_items = history_items[-limit:]
        history_text = "\n".join([f'  <span class="history-item">{i + 1}. {item}</span>'
    ...
    def _command_history_clear(self):
        ...
        try:
            if not self.history_path.exists():
                self._format_output(f'<span class="warning">{get_text("CHAT", "history_not_found", "没有找到历史文件。")}</span>')
                return
            ...
            self._format_output(f'<span class="command">{get_text("CHAT", "history_cleared", "命令历史已清除。已创建备份：")} {backup_path}</span>')
        except Exception as e:
            self._format_output(f'<span class="error">{get_text("CHAT", "history_clear_error", "清除历史时出错:")} {str(e)}</span>')
    ...
    def _command_status(self):
        ...
        status_text = '\n'.join([f'  <span class="help-command">{name}:</span> <span class="help-desc">{value}</span>'
    ...
    def exec(self, prep_res):
        # 获取用户输入提示
        input_prompt_formatted = HTML(f'<span class="input-prompt">{get_text("CHAT", "input_prompt")}</span>')
