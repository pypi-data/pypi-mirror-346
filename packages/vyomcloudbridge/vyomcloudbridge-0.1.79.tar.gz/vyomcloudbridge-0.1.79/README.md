...
| Flag              | Type    | Description                                                                 | Example           |
| ----------------- | ------- | --------------------------------------------------------------------------- | ----------------- |
| `--mission-dir`   | Boolean | Treat the directory as a **mission data directory** if included.            | `--mission-dir`   |
| `--merge-chunks`  | Boolean | Combine **file chunks on S3** after all chunks are uploaded.                | `--merge-chunks`  |
| `--preserve-file` | Boolean | Move files to `<dir>_preserve` instead of deleting them after upload to S3. | `--preserve-file` |
| `--priority`      | Number  | Set the **priority order** for uploading files in this directory.           | `--priority 3`    |
