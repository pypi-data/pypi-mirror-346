...
class ServiceManager:
    ...
    def _stop_by_instance_id(self, instance_id: str) -> bool:
        """Internal method to stop a service by its instance ID."""
        try:
            if instance_id not in self.services:
                return False

            info: Dict = self.services[instance_id]
            pid = info["pid"]
            ...
            try:
                ...
                logger.info(f"Stopped service {service_name} ({name})")
                del self.services[instance_id]
                self.save_running_services()
                return True

            except ProcessLookupError:
                logger.info(f"Service {service_name} ({name}) was already stopped")
                del self.services[instance_id]
                self.save_running_services()
                return True

        except Exception as e:
            logger.error(f"Error stopping service {instance_id}: {e}")
            return False
