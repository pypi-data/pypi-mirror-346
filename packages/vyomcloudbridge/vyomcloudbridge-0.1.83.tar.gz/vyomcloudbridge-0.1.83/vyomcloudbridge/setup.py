...
def setup_ros_environment():
    """
    Sets up a shell script to source required ROS environments from $AMENT_PREFIX_PATH.

    Returns:
        bool: True if shell script setup was successful, False otherwise
    """
    print("\n--- STEP 0: Fetching ROS environment packages ---")
    ...
    if not ament_prefix_path:
        user_input = (
            input(
                "Could not find any enviroment sources in AMENT_PREFIX_PATH. DO you want to continue? (y/n): "
            )
            .strip()
            .lower()
        )
    ...
    # Step 2: Construct `source` lines for each unique path
    ...
    for path in ros_paths:
        ...
        if os.path.exists(setup_path):
            source_lines.append(f"source {setup_path}")
        ...
        if os.path.exists(setup_path2):
            source_lines.append(f"source {setup_path2}")
    ...
    if needs_sudo:
        print(f"Error - root permission required")
        return False
    else:
        try:
            ...
            # Write the script
            with open(ros_env_file, "w") as f:
                f.write(shell_script_content)

            # Set permissions
            print("Setting execute permissions on shell script...")
            subprocess.run(["chmod", "+x", ros_env_file], check=True)

            print(f"✅ ROS env script created at: {ros_env_file}")
            return True
        except Exception as e:
            print(f"❌ Error setting up ROS environment script: {e}")
            return False
...
def setup(interactive=True):
    ...
    rosenv_success = True
    if not needs_sudo:
        rosenv_success = setup_ros_environment()
