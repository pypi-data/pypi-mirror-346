...
def main():
    if not os.environ.get("ROS_ENV_READY"):
        if not os.path.exists(ros_env_file):
            print(
                "===> A backup file for sourcing the ROS environment has not been created yet. Please manually source the ROS environment before running `setup`. <==="
            )
        else:
            # Compose the command to re-run the CLI inside a shell with ROS sourced
            command = f"source {ros_env_file} && ROS_ENV_READY=1 vyomcloudbridge {' '.join(sys.argv[1:])}"
