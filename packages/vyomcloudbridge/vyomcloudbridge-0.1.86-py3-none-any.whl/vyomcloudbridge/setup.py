...
def setup_ros2_workspaces():
    """
    Sets up a shell script to source required ROS2 workspaces from $AMENT_PREFIX_PATH.

    Returns:
        bool: True if shell script setup was successful, False otherwise
    """
    print("\n--- STEP 0: Fetching ROS2 workspaces packages ---")

    # Step 1: Read AMENT_PREFIX_PATH
    ament_prefix_path = os.environ.get("AMENT_PREFIX_PATH", "")
    if not ament_prefix_path and not os.path.exists(ros2_workspace_file):
        print(
        ...
        # user_input = (
        #     input(
        #         "Could not find any workspaces sources in AMENT_PREFIX_PATH. Do you want to continue? [Y/n]: "
        #     )
        #     .strip()
        #     .lower()
        # )
        ...
        #     print("Skipping VyomIQ setup")
        #     return False

    # Step 2: Read existing source lines from file (if exists)
    existing_sources = set()
    try:
        if os.path.exists(ros2_workspace_file):
            with open(ros2_workspace_file, "r") as f:
                for line in f:
                    stripped = line.strip()
                    if stripped.startswith("source "):
                        existing_sources.add(stripped)
    except Exception as e:
        pass

    # Step 3: Construct new source lines and avoid duplicates
    ros_paths = ament_prefix_path.split(":")
    source_lines = set(existing_sources)
    source_lines.add("source /opt/ros/humble/setup.bash")

    for path in ros_paths:
        setup_path = os.path.join(path, "setup.bash")
        if os.path.exists(setup_path):
            source_lines.add(f"source {setup_path}")
        package_name = os.path.basename(os.path.normpath(path))
        setup_path2 = os.path.join(path, "share", package_name, "local_setup.bash")
        if os.path.exists(setup_path2):
            source_lines.add(f"source {setup_path2}")
    ...
    if needs_sudo:
        ...
        try:
            ...
            # Write the script
            with open(ros2_workspace_file, "w") as f:
                f.write(shell_script_content)

            # Set permissions
            print("Setting execute permissions on shell script...")
            subprocess.run(["chmod", "+x", ros2_workspace_file], check=True)

            print(f"✅ ROS2 workspaces saved at: {ros2_workspace_file}")
            return True
        except Exception as e:
            print(f"❌ Error setting up ROS2 workspaces script: {e}")
            return False
