...
def setup_ros2_workspaces():
    ...
    if (
        not ament_prefix_path or not python_path or not ld_library_path
    ) and not os.path.exists(ros2_env_file):
        print(
            "===> Your ROS2 workspaces (in AMENT_PREFIX_PATH) or PYTHONPATH or LD_LIBRARY_PATH have not been found. Please ensure you manually source them before running `vyomcloudbridge setup`. <==="
        )
        return False
