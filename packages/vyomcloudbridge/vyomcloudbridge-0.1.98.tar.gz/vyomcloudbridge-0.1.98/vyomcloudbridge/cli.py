...
def main():
    ...
    if not os.path.exists(ros2_env_file):
        ...
        if not ament_prefix_path or not python_path or not ld_library_path:
            print(
                "===> Your ROS2 workspaces (in AMENT_PREFIX_PATH) or PYTHONPATH or LD_LIBRARY_PATH have not been found. Please ensure you manually source them and then run `vyomcloudbridge setup` for the first time. <==="
            )
            sys.exit(0)
    else:
    ...
    print("In main reading, ld_library_path-", ld_library_path)
