...
class SendDataToServer(Node, ServiceAbstract):
    ...
    def __init__(self):
        ...
        self.logger.info("SendDataToServer has started.")
    ...
    def subscriber_shutdown(self):
        ...
        self.is_running = False
        self.get_logger().info("Shutting down all subscribers.")
    ...
    def create_listener_function(self, msg_type, f_topic):
        ...
        def f(msg):
            self.logger.info(f"Processing data for: {f_topic}")

            current_value = converter.convert(msg_type, 1, msg)

            # self.self.logger.info(f"data: {current_value}")
            ...
            if msg_type == sensor_msgs.msg.Image:
                ...
                if hasattr(msg, "header") and hasattr(msg.header, "stamp"):
                    ...
                    timestamp_sec = 0
                    timestamp_nsec = 0
                    self.logger.info("Timestamp not available, using placeholder.")
    ...
    def create_dynamic_subscribers(self, f_topic):
        ...
        try:
            ...
            self.logger.error(f"Failed to subscribe to topic: {f_topic['topic']} with type: {f_topic['data_type']}. Error: {e}")
...
class RosPublisher:
    ...
    def start(self):
        ...
        try:
            self.logger.info("Starting SendDataToServer service...")
            self.is_running = True
            ...
            self.logger.info("SendDataToServer service started!")

        except Exception as e:
            self.logger.error(f"Error starting SendDataToServer service: {str(e)}")
            self.stop()
    ...
    def stop(self):
        ...
        self.logger.info("SendDataToServer service stopped!")
