...
def setup_ros2_workspaces():
    ...
    ros_env_file = os.path.join(vyom_home_dir, "ros_env_vars.sh")

    ament_prefix_path = os.environ.get("AMENT_PREFIX_PATH", "")
    python_path = os.environ.get("PYTHONPATH", "")
    print("After reading, ament_prefix_path-",ament_prefix_path)
    print("After reading, python_path-",python_path)

    if (not ament_prefix_path or not python_path) and not os.path.exists(ros_env_file):
        print(
    ...
    try:
        ...
        print(f"✅ ROS2 environment variables saved to: {ros_env_file}")
        return True
