...
# Local imports
from GailBotTools.configs.configs import INTERNAL_MARKER, load_threshold
...
from GailBotTools.plugin_structures.format.nodes import Node
...
class UtteranceMapPlugin(Plugin):
    ...
    def outer_create_dict(self, id_arg):
        ...
        def create_dict(currNode: Node, uttDict: Dict[str, List[Node]]):
            ...
            if (currNode.val.startTime is not None) and (
                currSL not in MARKER.INTERNAL_MARKER_SET
            ):
                ...
                if id == 0:
                    ...
                    if (
                        currSL == uttDict[id][-1].val.sLabel
                        and fto < self.turn_end_threshold_secs
                    ):
                    ...
                    while (
                        index2 > 1
                        and uttDict[index2][-1].val.sLabel in MARKER.INTERNAL_MARKER_SET
                    ):
                        index2 -= 1
                    fto2 = currNode.val.startTime - uttDict[index2][-1].val.endTime

                    index3 = id
                    while index3 > 1 and (uttDict[index3][-1].val.sLabel != currSL):
                        index3 -= 1
                    fto3 = currNode.val.startTime - uttDict[index3][-1].val.endTime
                    ...
                    if (
                        ...
                        currSL == uttDict[index2][-1].val.sLabel
                        and fto2 < self.turn_end_threshold_secs
                    ):
                        uttDict[index2].append(currNode)
                    elif (
                        fto3 < self.turn_end_threshold_secs
                        and currSL == uttDict[index3][-1].val.sLabel
                    ):
