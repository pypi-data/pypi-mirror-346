...
class EphemeralServer:
    ...
    async def start_dev_server(
        runtime: zamp_temporalio.bridge.runtime.Runtime, config: DevServerConfig
    ) -> EphemeralServer:
        """Start a dev server instance."""
        return EphemeralServer(
            await zamp_temporalio.bridge.temporal_sdk_bridge.start_dev_server(
                runtime._ref, config
            )
        )
    ...
    async def start_test_server(
        runtime: zamp_temporalio.bridge.runtime.Runtime, config: TestServerConfig
    ) -> EphemeralServer:
        """Start a test server instance."""
        return EphemeralServer(
            await zamp_temporalio.bridge.temporal_sdk_bridge.start_test_server(
                runtime._ref, config
            )
        )

    def __init__(self, ref: zamp_temporalio.bridge.temporal_sdk_bridge.EphemeralServerRef):
