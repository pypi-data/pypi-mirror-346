...
import zamp_temporalio.bridge.temporal_sdk_bridge
import zamp_temporalio.converter
import zamp_temporalio.exceptions
from zamp_temporalio.bridge.temporal_sdk_bridge import (
    CustomSlotSupplier as BridgeCustomSlotSupplier,
)
from zamp_temporalio.bridge.temporal_sdk_bridge import PollShutdownError
...
class Worker:
    ...
    @staticmethod
    def create(client: zamp_temporalio.bridge.client.Client, config: WorkerConfig) -> Worker:
        """Create a bridge worker from a bridge client."""
        return Worker(
            zamp_temporalio.bridge.temporal_sdk_bridge.new_worker(
    ...
    @staticmethod
    def for_replay(
        runtime: zamp_temporalio.bridge.runtime.Runtime,
        config: WorkerConfig,
    ) -> Tuple[Worker, zamp_temporalio.bridge.temporal_sdk_bridge.HistoryPusher]:
        """Create a bridge replay worker."""
        [
            replay_worker,
            pusher,
        ] = zamp_temporalio.bridge.temporal_sdk_bridge.new_replay_worker(
            runtime._ref, config
        )
        return Worker(replay_worker), pusher

    def __init__(self, ref: zamp_temporalio.bridge.temporal_sdk_bridge.WorkerRef) -> None:
    ...
    async def poll_workflow_activation(
        self,
    ) -> zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivation:
        """Poll for a workflow activation."""
        return (
            zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivation.FromString(
    ...
    async def poll_activity_task(
        self,
    ) -> zamp_temporalio.bridge.proto.activity_task.ActivityTask:
        """Poll for an activity task."""
        return zamp_temporalio.bridge.proto.activity_task.ActivityTask.FromString(
            await self._ref.poll_activity_task()
        )

    async def complete_workflow_activation(
        self,
        comp: zamp_temporalio.bridge.proto.workflow_completion.WorkflowActivationCompletion,
    ) -> None:
        """Complete a workflow activation."""
        await self._ref.complete_workflow_activation(comp.SerializeToString())

    async def complete_activity_task(
        self, comp: zamp_temporalio.bridge.proto.ActivityTaskCompletion
    ) -> None:
        """Complete an activity task."""
        await self._ref.complete_activity_task(comp.SerializeToString())

    def record_activity_heartbeat(
        self, comp: zamp_temporalio.bridge.proto.ActivityHeartbeat
    ) -> None:
    ...
    def replace_client(self, client: zamp_temporalio.bridge.client.Client) -> None:
...
async def _apply_to_payloads(
    ...
    cb: Callable[
        [Sequence[zamp_temporalio.api.common.v1.Payload]],
        Awaitable[List[zamp_temporalio.api.common.v1.Payload]],
    ],
) -> None:
...
async def _decode_payloads(
    payloads: PayloadContainer,
    codec: zamp_temporalio.converter.PayloadCodec,
) -> None:
