...
import zamp_temporalio.worker._activity

_Params = ParamSpec("_Params")
_Return = TypeVar("_Return")

_utc_zero = datetime.fromtimestamp(0).replace(tzinfo=timezone.utc)
_default_info = zamp_temporalio.activity.Info(
    activity_id="test",
...
class ActivityEnvironment:
    ...
    functions in the :py:mod:`zamp_temporalio.activity` module. Use :py:meth:`run` to
    run an activity function or any function within an activity context.

    Attributes:
        info: The info that is returned from :py:func:`zamp_temporalio.activity.info`
            function.
    ...
    def __init__(self) -> None:
        ...
        self.payload_converter = (
            zamp_temporalio.converter.DataConverter.default.payload_converter
        )
        self.metric_meter = zamp_temporalio.common.MetricMeter.noop
...
class _Activity:
    def __init__(
        ...
        self.cancel_thread_raiser: Optional[
            zamp_temporalio.worker._activity._ThreadExceptionRaiser
        ] = None
        if not self.is_async:
            # If there is a definition and they disable thread raising, don't
            # set
            defn = zamp_temporalio.activity._Definition.from_callable(fn)
            if not defn or not defn.no_thread_cancel_exception:
                self.cancel_thread_raiser = (
                    zamp_temporalio.worker._activity._ThreadExceptionRaiser()
                )
        # Create context
        self.context = zamp_temporalio.activity._Context(
    ...
    def run(self, *args, **kwargs) -> Any:
        ...
        def activity_context():
            ...
            token = zamp_temporalio.activity._Context.set(self.context)
            try:
                yield None
            finally:
                # Reset context and remove activity
                zamp_temporalio.activity._Context.reset(token)
