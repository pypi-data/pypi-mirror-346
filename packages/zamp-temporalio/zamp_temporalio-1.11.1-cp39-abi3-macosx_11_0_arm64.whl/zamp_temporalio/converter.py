...
class PayloadConverter(ABC):
    ...
    def from_payloads(
        ...
        :py:class:`zamp_temporalio.common.RawValue` as just the raw value.
    ...
    def to_payloads_wrapper(
        self, values: Sequence[Any]
    ) -> zamp_temporalio.api.common.v1.Payloads:
        """:py:meth:`to_payloads` for the
        :py:class:`zamp_temporalio.api.common.v1.Payloads` wrapper.
        """
        return zamp_temporalio.api.common.v1.Payloads(payloads=self.to_payloads(values))

    def from_payloads_wrapper(
        self, payloads: Optional[zamp_temporalio.api.common.v1.Payloads]
    ) -> List[Any]:
    ...
    def to_payload(self, value: Any) -> zamp_temporalio.api.common.v1.Payload:
    ...
    @overload
    def from_payload(self, payload: zamp_temporalio.api.common.v1.Payload) -> Any: ...

    @overload
    def from_payload(
        self,
        payload: zamp_temporalio.api.common.v1.Payload,
        type_hint: Type[zamp_temporalio.types.AnyType],
    ) -> zamp_temporalio.types.AnyType: ...
...
class EncodingPayloadConverter(ABC):
    ...
    @abstractmethod
    def to_payload(self, value: Any) -> Optional[zamp_temporalio.api.common.v1.Payload]:
...
class CompositePayloadConverter(PayloadConverter):
    ...
    def to_payloads(
        ...
        for index, value in enumerate(values):
            ...
            if isinstance(value, zamp_temporalio.common.RawValue):
    ...
    def from_payloads(
        ...
        for index, (payload, type_hint) in enumerate(zip_longest(payloads, type_hints)):
            ...
            if type_hint == zamp_temporalio.common.RawValue:
                values.append(zamp_temporalio.common.RawValue(payload))
                continue
...
class BinaryNullPayloadConverter(EncodingPayloadConverter):
    ...
    def to_payload(self, value: Any) -> Optional[zamp_temporalio.api.common.v1.Payload]:
        """See base class."""
        if value is None:
            return zamp_temporalio.api.common.v1.Payload(
...
class AdvancedJSONEncoder(json.JSONEncoder):
    ...
    zamp_temporalio.contrib.pydantic.pydantic_data_converter.
    """
...
class JSONPlainPayloadConverter(EncodingPayloadConverter):
    ...
    def to_payload(self, value: Any) -> Optional[zamp_temporalio.api.common.v1.Payload]:
        ...
        if hasattr(value, "parse_obj"):
            warnings.warn(
                "If you're using Pydantic v2, use zamp_temporalio.contrib.pydantic.pydantic_data_converter. "
                "If you're using Pydantic v1 and cannot upgrade, refer to https://github.com/zamp_temporalio/samples-python/tree/main/pydantic_converter_v1 for better v1 support."
            )
...
class PayloadCodec(ABC):
    ...
    @abstractmethod
    async def encode(
        self, payloads: Sequence[zamp_temporalio.api.common.v1.Payload]
    ) -> List[zamp_temporalio.api.common.v1.Payload]:
    ...
    async def encode_wrapper(self, payloads: zamp_temporalio.api.common.v1.Payloads) -> None:
    ...
    async def decode_wrapper(self, payloads: zamp_temporalio.api.common.v1.Payloads) -> None:
    ...
    async def encode_failure(self, failure: zamp_temporalio.api.failure.v1.Failure) -> None:
        """Encode payloads of a failure."""
        await self._apply_to_failure_payloads(failure, self.encode_wrapper)

    async def decode_failure(self, failure: zamp_temporalio.api.failure.v1.Failure) -> None:
    ...
    async def _apply_to_failure_payloads(
        ...
        cb: Callable[[zamp_temporalio.api.common.v1.Payloads], Awaitable[None]],
    ) -> None:
        if failure.HasField("encoded_attributes"):
            # Wrap in payloads and merge back
            payloads = zamp_temporalio.api.common.v1.Payloads(
...
class FailureConverter(ABC):
    ...
    exception is an instance of :py:class:`zamp_temporalio.exceptions.FailureError`.
    Users should extend :py:class:`zamp_temporalio.exceptions.ApplicationError` if
...
class DefaultFailureConverter(FailureConverter):
    ...
    def to_failure(
        ...
        if isinstance(exception, zamp_temporalio.exceptions.FailureError):
            self._error_to_failure(exception, payload_converter, failure)
        else:
            # Convert to failure error
            failure_error = zamp_temporalio.exceptions.ApplicationError(
    ...
    def _error_to_failure(
        self,
        error: zamp_temporalio.exceptions.FailureError,
        payload_converter: PayloadConverter,
        failure: zamp_temporalio.api.failure.v1.Failure,
    ) -> None:
        ...
        if isinstance(error, zamp_temporalio.exceptions.ApplicationError):
        ...
        elif isinstance(error, zamp_temporalio.exceptions.TimeoutError):
            failure.timeout_failure_info.SetInParent()
            failure.timeout_failure_info.timeout_type = (
                zamp_temporalio.api.enums.v1.TimeoutType.ValueType(error.type or 0)
            )
        ...
        elif isinstance(error, zamp_temporalio.exceptions.CancelledError):
            failure.canceled_failure_info.SetInParent()
            if error.details:
                failure.canceled_failure_info.details.CopyFrom(
                    payload_converter.to_payloads_wrapper(error.details)
                )
        elif isinstance(error, zamp_temporalio.exceptions.TerminatedError):
            failure.terminated_failure_info.SetInParent()
        elif isinstance(error, zamp_temporalio.exceptions.ServerError):
            failure.server_failure_info.SetInParent()
            failure.server_failure_info.non_retryable = error.non_retryable
        elif isinstance(error, zamp_temporalio.exceptions.ActivityError):
            ...
            failure.activity_failure_info.retry_state = (
                zamp_temporalio.api.enums.v1.RetryState.ValueType(error.retry_state or 0)
            )
        elif isinstance(error, zamp_temporalio.exceptions.ChildWorkflowError):
    ...
    def from_failure(
        ...
        if failure.HasField("encoded_attributes"):
            ...
            new_failure = zamp_temporalio.api.failure.v1.Failure()
        ...
        err: zamp_temporalio.exceptions.FailureError
        if failure.HasField("application_failure_info"):
            app_info = failure.application_failure_info
            err = zamp_temporalio.exceptions.ApplicationError(
            ...
            err = zamp_temporalio.exceptions.TimeoutError(
                failure.message or "Timeout",
                type=zamp_temporalio.exceptions.TimeoutType(int(timeout_info.timeout_type))
            ...
            err = zamp_temporalio.exceptions.TerminatedError(failure.message or "Terminated")
        elif failure.HasField("server_failure_info"):
            server_info = failure.server_failure_info
            err = zamp_temporalio.exceptions.ServerError(
            ...
            err = zamp_temporalio.exceptions.ActivityError(
                ...
                retry_state=zamp_temporalio.exceptions.RetryState(int(act_info.retry_state))
            ...
            err = zamp_temporalio.exceptions.ChildWorkflowError(
                ...
                retry_state=zamp_temporalio.exceptions.RetryState(
            ...
            err = zamp_temporalio.exceptions.FailureError(failure.message or "Failure error")
...
class DataConverter:
    ...
    async def encode_wrapper(
        ...
        :py:class:`zamp_temporalio.api.common.v1.Payloads` wrapper.
        """
        return zamp_temporalio.api.common.v1.Payloads(payloads=(await self.encode(values)))

    async def decode_wrapper(
        self,
        payloads: Optional[zamp_temporalio.api.common.v1.Payloads],
        type_hints: Optional[List[Type]] = None,
    ) -> List[Any]:
        """:py:meth:`decode` for the
        :py:class:`zamp_temporalio.api.common.v1.Payloads` wrapper.
    ...
    async def encode_failure(
        self, exception: BaseException, failure: zamp_temporalio.api.failure.v1.Failure
    ) -> None:
    ...
    async def decode_failure(
        self, failure: zamp_temporalio.api.failure.v1.Failure
    ) -> BaseException:
...
def encode_search_attributes(
    ...
    api: zamp_temporalio.api.common.v1.SearchAttributes,
) -> None:
...
def encode_typed_search_attribute_value(
    key: zamp_temporalio.common.SearchAttributeKey[
        zamp_temporalio.common.SearchAttributeValueType
    ],
    value: Optional[zamp_temporalio.common.SearchAttributeValue],
) -> zamp_temporalio.api.common.v1.Payload:
...
def encode_search_attribute_values(
    vals: zamp_temporalio.common.SearchAttributeValues,
) -> zamp_temporalio.api.common.v1.Payload:
...
def _encode_maybe_typed_search_attributes(
    non_typed_attributes: Optional[zamp_temporalio.common.SearchAttributes],
    typed_attributes: Optional[zamp_temporalio.common.TypedSearchAttributes],
    api: zamp_temporalio.api.common.v1.SearchAttributes,
) -> None:
...
def decode_typed_search_attributes(
    ...
    """
    conv = default().payload_converter
    pairs: List[zamp_temporalio.common.SearchAttributePair] = []
    for k, v in api.indexed_fields.items():
        ...
        if not metadata_type:
            continue
        key = zamp_temporalio.common.SearchAttributeKey._from_metadata_type(
        ...
        if (
            key.indexed_value_type
            == zamp_temporalio.common.SearchAttributeIndexedValueType.DATETIME
        ):
            parser = _get_iso_datetime_parser()
        ...
        if isinstance(val, key.origin_value_type):
            pairs.append(zamp_temporalio.common.SearchAttributePair(key, val))
    return zamp_temporalio.common.TypedSearchAttributes(pairs)


def _decode_search_attribute_value(
    payload: zamp_temporalio.api.common.v1.Payload,
) -> zamp_temporalio.common.SearchAttributeValue:
...
def value_to_type(
    ...
    # zamp_temporalio.contrib.pydantic.pydantic_data_converter, in which case a
