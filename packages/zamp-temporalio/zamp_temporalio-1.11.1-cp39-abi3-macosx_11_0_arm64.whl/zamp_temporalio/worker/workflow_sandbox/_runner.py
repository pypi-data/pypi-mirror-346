...
import zamp_temporalio.worker._workflow_instance
...
_fake_info = zamp_temporalio.workflow.Info(
    attempt=-1,
    ...
    typed_search_attributes=zamp_temporalio.common.TypedSearchAttributes.empty,
    workflow_id="sandbox-validate-workflow_id",
    workflow_type="sandbox-validate-workflow_type",
    priority=zamp_temporalio.common.Priority.default,
)
...
class SandboxedWorkflowRunner(WorkflowRunner):
    ...
    def prepare_workflow(self, defn: zamp_temporalio.workflow._Definition) -> None:
        ...
        self.create_instance(
            WorkflowInstanceDetails(
                payload_converter_class=zamp_temporalio.converter.DataConverter.default.payload_converter_class,
                failure_converter_class=zamp_temporalio.converter.DataConverter.default.failure_converter_class,
                interceptor_classes=[],
                defn=defn,
...
class _Instance(WorkflowInstance):
    ...
    def _create_instance(self) -> None:
        ...
        try:
            ...
            self._run_code(
                "with __temporal_importer.applied():\n"
                "  from zamp_temporalio.worker.workflow_sandbox._in_sandbox import InSandbox\n"
    ...
    def _run_code(self, code: str, **extra_globals: Any) -> None:
        ...
        try:
            zamp_temporalio.workflow.unsafe._set_in_sandbox(True)
            self._current_thread_id = threading.get_ident()
            exec(code, self.globals_and_locals, self.globals_and_locals)
        finally:
            zamp_temporalio.workflow.unsafe._set_in_sandbox(False)
