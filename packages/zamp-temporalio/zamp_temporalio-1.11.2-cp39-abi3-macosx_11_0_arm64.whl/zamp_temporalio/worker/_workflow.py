...
class _WorkflowWorker:
    def __init__(
        ...
        *,
        ...
        on_eviction_hook: Optional[
            Callable[
                [str, zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache], None
            ]
        ],
        ...
        self._workflows: Dict[str, zamp_temporalio.workflow._Definition] = {}
        self._dynamic_workflow: Optional[zamp_temporalio.workflow._Definition] = None
        for workflow in workflows:
            defn = zamp_temporalio.workflow._Definition.must_from_class(workflow)
            # Confirm name unique
    ...
    async def drain_poll_queue(self) -> None:
        while True:
            try:
                ...
                act = await self._bridge_worker().poll_workflow_activation()
                completion = zamp_temporalio.bridge.proto.workflow_completion.WorkflowActivationCompletion(
                    run_id=act.run_id
                )
    ...
    async def _handle_activation(
        ...
        try:
            ...
            if self._data_converter.payload_codec:
                await zamp_temporalio.bridge.worker.decode_activation(
        ...
        if self._data_converter.payload_codec:
            try:
                await zamp_temporalio.bridge.worker.encode_completion(
    ...
    async def _handle_cache_eviction(
        self,
        act: zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivation,
        job: zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache,
    ) -> None:
        logger.debug(
        ...
        try:
            await self._bridge_worker().complete_workflow_activation(
                zamp_temporalio.bridge.proto.workflow_completion.WorkflowActivationCompletion(
                    run_id=act.run_id,
                    successful=zamp_temporalio.bridge.proto.workflow_completion.Success(),
                )
            )
        except Exception:
            logger.exception(
    ...
    def _create_workflow_instance(
        self,
        act: zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivation,
        init: zamp_temporalio.bridge.proto.workflow_activation.InitializeWorkflow,
    ) -> WorkflowInstance:
        # Get the definition
        ...
        # Build info
        parent: Optional[zamp_temporalio.workflow.ParentInfo] = None
        root: Optional[zamp_temporalio.workflow.RootInfo] = None
        if init.HasField("parent_workflow_info"):
            parent = zamp_temporalio.workflow.ParentInfo(
        ...
        if init.HasField("root_workflow"):
            root = zamp_temporalio.workflow.RootInfo(
        ...
        info = zamp_temporalio.workflow.Info(
            ...
            retry_policy=zamp_temporalio.common.RetryPolicy.from_proto(init.retry_policy)
            ...
            priority=zamp_temporalio.common.Priority._from_proto(init.priority),
        )
    ...
    def nondeterminism_as_workflow_fail(self) -> bool:
        return any(
            issubclass(zamp_temporalio.workflow.NondeterminismError, typ)
