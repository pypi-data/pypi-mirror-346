...
from zamp_temporalio.common import VersioningBehavior, WorkerDeploymentVersion
...
class PollerBehaviorSimpleMaximum:
    ...
    def _to_bridge(self) -> zamp_temporalio.bridge.worker.PollerBehavior:
        return zamp_temporalio.bridge.worker.PollerBehaviorSimpleMaximum(
...
class PollerBehaviorAutoscaling:
    ...
    def _to_bridge(self) -> zamp_temporalio.bridge.worker.PollerBehavior:
        return zamp_temporalio.bridge.worker.PollerBehaviorAutoscaling(
...
class Worker:
        ...
        *,
        ...
        Args:
            client: Client to use for this worker. This is required and must be
                the :py:class:`zamp_temporalio.client.Client` instance or have a
            ...
            activities: Set of activity callables decorated with
                :py:func:`@activity.defn<zamp_temporalio.activity.defn>`. Activities
                may be async functions or non-async functions.
            workflows: Set of workflow classes decorated with
                :py:func:`@workflow.defn<zamp_temporalio.workflow.defn>`.
        ...
        self._runtime = (
            bridge_client.config.runtime or zamp_temporalio.runtime.Runtime.default()
        )
        if activities:
        ...
        versioning_strategy: zamp_temporalio.bridge.worker.WorkerVersioningStrategy
        if deployment_config:
            ...
            versioning_strategy = (
                zamp_temporalio.bridge.worker.WorkerVersioningStrategyLegacyBuildIdBased(
                    build_id=build_id
                )
            )
        else:
        ...
        self._bridge_worker = zamp_temporalio.bridge.worker.Worker.create(
    ...
    @client.setter
    def client(self, value: zamp_temporalio.client.Client) -> None:
...
class WorkerDeploymentConfig:
    ...
    def _to_bridge_worker_deployment_options(
        self,
    ) -> zamp_temporalio.bridge.worker.WorkerDeploymentOptions:
        return zamp_temporalio.bridge.worker.WorkerDeploymentOptions(
            version=zamp_temporalio.bridge.worker.WorkerDeploymentVersion(
...
def load_default_build_id(*, memoize: bool = True) -> str:
    ...
    for mod_name in sorted(sys.modules):
        ...
        if not code:
            continue
        if mod_name == "zamp_temporalio":
            got_temporal_code = True
        # Add to MD5 digest
        m.update(code)
    # If we didn't even get the zamp_temporalio module from this approach, this
...
def _extract_bridge_client_for_worker(
    client: zamp_temporalio.client.Client,
) -> zamp_temporalio.service._BridgeServiceClient:
    ...
    bridge_client: zamp_temporalio.service._BridgeServiceClient
    if isinstance(client.service_client, zamp_temporalio.service._BridgeServiceClient):
        ...
        if not isinstance(bridge_client, zamp_temporalio.service._BridgeServiceClient):
            raise TypeError(
