...
from zamp_temporalio.common import WorkerDeploymentVersion
...
) -> zamp_temporalio.bridge.worker.SlotSupplier:
    if isinstance(slot_supplier, FixedSizeSlotSupplier):
        return zamp_temporalio.bridge.worker.FixedSizeSlotSupplier(slot_supplier.num_slots)
        ...
        return zamp_temporalio.bridge.worker.ResourceBasedSlotSupplier(
            min_slots,
            max_slots,
            int(ramp_throttle / timedelta(milliseconds=1)),
            zamp_temporalio.bridge.worker.ResourceBasedTunerConfig(
        ...
        return zamp_temporalio.bridge.worker.BridgeCustomSlotSupplier(
...
class WorkerTuner(ABC):
    ...
    def _to_bridge_tuner(self) -> zamp_temporalio.bridge.worker.TunerHolder:
        return zamp_temporalio.bridge.worker.TunerHolder(
