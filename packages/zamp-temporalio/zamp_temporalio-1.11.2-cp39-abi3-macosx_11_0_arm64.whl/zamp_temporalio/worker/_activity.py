...
import zamp_temporalio.bridge.client
import zamp_temporalio.bridge.proto
import zamp_temporalio.bridge.proto.activity_result
import zamp_temporalio.bridge.proto.activity_task
import zamp_temporalio.bridge.proto.common
import zamp_temporalio.bridge.runtime
import zamp_temporalio.bridge.worker
...
class _ActivityWorker:
    def __init__(
        self,
        *,
        bridge_worker: Callable[[], zamp_temporalio.bridge.worker.Worker],
        task_queue: str,
        ...
        metric_meter: zamp_temporalio.common.MetricMeter,
    ) -> None:
        ...
        self._worker_shutdown_event: Optional[zamp_temporalio.activity._CompositeEvent] = (
            None
        )
        self._seen_sync_activity = False

        # Validate and build activity dict
        self._activities: Dict[str, zamp_temporalio.activity._Definition] = {}
        self._dynamic_activity: Optional[zamp_temporalio.activity._Definition] = None
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            except zamp_temporalio.bridge.worker.PollShutdownError:
    ...
    async def drain_poll_queue(self) -> None:
        while True:
            try:
                ...
                completion = zamp_temporalio.bridge.proto.ActivityTaskCompletion(
    ...
    def _cancel(
        self, task_token: bytes, cancel: zamp_temporalio.bridge.proto.activity_task.Cancel
    ) -> None:
    ...
    def _heartbeat(self, task_token: bytes, *details: Any) -> None:
        ...
        # returns.
        logger = zamp_temporalio.activity.logger
        activity = self._running_activities.get(task_token)
    ...
    async def _heartbeat_async(
        ...
        try:
            heartbeat = zamp_temporalio.bridge.proto.ActivityHeartbeat(task_token=task_token)
            if details:
    ...
    async def _run_activity(
        self,
        task_token: bytes,
        start: zamp_temporalio.bridge.proto.activity_task.Start,
        ...
        try:
            ...
            if not activity_def:
                ...
                raise zamp_temporalio.exceptions.ApplicationError(
            ...
            if not self._worker_shutdown_event:
                self._worker_shutdown_event = zamp_temporalio.activity._CompositeEvent(
            ...
            if not activity_def.is_async:
                ...
                if isinstance(
                    ...
                    running_activity.cancelled_event = (
                        zamp_temporalio.activity._CompositeEvent(
                ...
                running_activity.cancelled_event = zamp_temporalio.activity._CompositeEvent(
                    thread_event=threading.Event(),
            ...
            if not activity_def.name:
                ...
                arg_types = [zamp_temporalio.common.RawValue] * len(start.input)
            ...
            info = zamp_temporalio.activity.Info(
                ...
                priority=zamp_temporalio.common.Priority._from_proto(start.priority),
            )
            ...
            zamp_temporalio.activity._Context.set(
                zamp_temporalio.activity._Context(
                    info=lambda: info,
                    heartbeat=None,
            ...
            zamp_temporalio.activity.logger.debug("Starting activity")
            ...
            try:
                if isinstance(err, zamp_temporalio.activity._CompleteAsyncError):
                    zamp_temporalio.activity.logger.debug("Completing asynchronously")
                    completion.result.will_complete_async.SetInParent()
                elif (
                    isinstance(
                        err,
                        (asyncio.CancelledError, zamp_temporalio.exceptions.CancelledError),
                    )
                    and running_activity.cancelled_due_to_heartbeat_error
                ):
                    err = running_activity.cancelled_due_to_heartbeat_error
                    zamp_temporalio.activity.logger.warning(
                    ...
                    zamp_temporalio.activity.logger.debug("Completing as cancelled")
                    await self._data_converter.encode_failure(
                        # TODO(cretz): Should use some other message?
                        zamp_temporalio.exceptions.CancelledError("Cancelled"),
                ...
                zamp_temporalio.activity.logger.exception(
        ...
        try:
            ...
            zamp_temporalio.activity.logger.exception("Failed completing activity task")
...
class _RunningActivity:
    ...
    info: Optional[zamp_temporalio.activity.Info] = None
    task: Optional[asyncio.Task] = None
    cancelled_event: Optional[zamp_temporalio.activity._CompositeEvent] = None
        ...
        if not self.done:
            ...
            if self.cancel_thread_raiser:
                self.cancel_thread_raiser.raise_in_thread(
                    zamp_temporalio.exceptions.CancelledError
                )
    ...
    def _raise_in_thread_if_pending_unlocked(self) -> None:
        ...
        if self._thread_id is not None:
            ...
            if self._shield_depth == 0 and self._pending_exception:
                zamp_temporalio.bridge.runtime.Runtime._raise_in_thread(
...
class _ActivityInboundImpl(ActivityInboundInterceptor):
    ...
    def init(self, outbound: ActivityOutboundInterceptor) -> None:
        ...
        context = zamp_temporalio.activity._Context.current()
    ...
    async def execute_activity(self, input: ExecuteActivityInput) -> Any:
        ...
        if not is_async:
            ...
            ctx = zamp_temporalio.activity._Context.current()
            info = ctx.info()
            ...
            async def heartbeat_with_context(*details: Any) -> None:
                zamp_temporalio.activity._Context.set(ctx)
...
class _ActivityOutboundImpl(ActivityOutboundInterceptor):
    def __init__(self, worker: _ActivityWorker, info: zamp_temporalio.activity.Info) -> None:
        # We are intentionally not calling the base class's __init__ here
        self._worker = worker
        self._info = info

    def info(self) -> zamp_temporalio.activity.Info:
        return self._info
...
def _execute_sync_activity(
    ...
    payload_converter_class_or_instance: Union[
        Type[zamp_temporalio.converter.PayloadConverter],
        zamp_temporalio.converter.PayloadConverter,
    ],
    runtime_metric_meter: Optional[zamp_temporalio.common.MetricMeter],
    fn: Callable[..., Any],
    *args: Any,
) -> Any:
    ...
    zamp_temporalio.activity._Context.set(
        zamp_temporalio.activity._Context(
            ...
            cancelled_event=zamp_temporalio.activity._CompositeEvent(
                thread_event=cancelled_event, async_event=None
            ),
            worker_shutdown_event=zamp_temporalio.activity._CompositeEvent(
