...
def defn(
    *,
    ...
    versioning_behavior: zamp_temporalio.common.VersioningBehavior = zamp_temporalio.common.VersioningBehavior.UNSPECIFIED,
...
class DynamicWorkflowConfig:
    ...
    Always overrides the equivalent parameter on :py:func:`defn` if set not-None.
        ...
        WARNING: This setting is experimental.
    """
    versioning_behavior: zamp_temporalio.common.VersioningBehavior = (
...
class Info:
    ...
    raw_memo: Mapping[str, zamp_temporalio.api.common.v1.Payload]
...
@overload
def memo_value(key: str, default: Any = zamp_temporalio.common._arg_unset) -> Any: ...
...
def metric_meter() -> zamp_temporalio.common.MetricMeter:
...
def payload_converter() -> zamp_temporalio.converter.PayloadConverter:
...
def upsert_search_attributes(
    ...
    if not attributes:
        return
    zamp_temporalio.common._warn_on_deprecated_search_attributes(attributes)
...
class UpdateMethodMultiParam(Protocol[MultiParamSpec, ProtocolReturnType]):
    ...
    _defn: zamp_temporalio.workflow._UpdateDefinition
...
class _Definition:
    ...
    versioning_behavior: Optional[zamp_temporalio.common.VersioningBehavior] = None
    ...
    def __post_init__(self) -> None:
        if self.arg_types is None and self.ret_type is None:
            ...
            arg_types, ret_type = zamp_temporalio.common._type_hints_from_func(self.run_fn)
            # If dynamic, must be a sequence of raw values
            if dynamic and (
                not arg_types
                or len(arg_types) != 1
                or arg_types[0] != Sequence[zamp_temporalio.common.RawValue]
            ):
                raise TypeError(
                    "Dynamic workflow must accept a single Sequence[zamp_temporalio.common.RawValue]"
                )
...
def _assert_dynamic_handler_args(
    ...
    if (
        ...
        or len(arg_types) != 2
        or arg_types[0] != str
        or arg_types[1] != Sequence[zamp_temporalio.common.RawValue]
    ):
        raise RuntimeError(
            "Dynamic handler must have 3 arguments: self, str, and Sequence[zamp_temporalio.common.RawValue]"
        )
    return True
...
class _SignalDefinition:
    ...
    def __post_init__(self) -> None:
        if self.arg_types is None:
            arg_types, _ = zamp_temporalio.common._type_hints_from_func(self.fn)
...
class _QueryDefinition:
    ...
    def __post_init__(self) -> None:
        if self.arg_types is None and self.ret_type is None:
            arg_types, ret_type = zamp_temporalio.common._type_hints_from_func(self.fn)
            # If dynamic, assert it
            if not self.name:
                object.__setattr__(
                    self,
...
class _UpdateDefinition:
    ...
    def get_name_and_result_type(
        ...
        if isinstance(name_or_update_fn, zamp_temporalio.workflow.UpdateMethodMultiParam):
...
class ActivityCancellationType(IntEnum):
    ...
    TRY_CANCEL = int(
        zamp_temporalio.bridge.proto.workflow_commands.ActivityCancellationType.TRY_CANCEL
    )
    WAIT_CANCELLATION_COMPLETED = int(
        zamp_temporalio.bridge.proto.workflow_commands.ActivityCancellationType.WAIT_CANCELLATION_COMPLETED
    )
    ABANDON = int(
        zamp_temporalio.bridge.proto.workflow_commands.ActivityCancellationType.ABANDON
    )
...
def start_activity(
    ...
    *,
    ...
    return _Runtime.current().workflow_start_activity(
        activity,
        *zamp_temporalio.common._arg_or_args(arg, args),
...
class ChildWorkflowCancellationType(IntEnum):
    ...
    ABANDON = int(
        zamp_temporalio.bridge.proto.child_workflow.ChildWorkflowCancellationType.ABANDON
    )
    TRY_CANCEL = int(
        zamp_temporalio.bridge.proto.child_workflow.ChildWorkflowCancellationType.TRY_CANCEL
    )
    WAIT_CANCELLATION_COMPLETED = int(
        zamp_temporalio.bridge.proto.child_workflow.ChildWorkflowCancellationType.WAIT_CANCELLATION_COMPLETED
    )
    WAIT_CANCELLATION_REQUESTED = int(
        zamp_temporalio.bridge.proto.child_workflow.ChildWorkflowCancellationType.WAIT_CANCELLATION_REQUESTED
    )
...
class ParentClosePolicy(IntEnum):
    ...
    UNSPECIFIED = int(
        zamp_temporalio.bridge.proto.child_workflow.ParentClosePolicy.PARENT_CLOSE_POLICY_UNSPECIFIED
    )
    TERMINATE = int(
        zamp_temporalio.bridge.proto.child_workflow.ParentClosePolicy.PARENT_CLOSE_POLICY_TERMINATE
    )
    ABANDON = int(
        zamp_temporalio.bridge.proto.child_workflow.ParentClosePolicy.PARENT_CLOSE_POLICY_ABANDON
    )
    REQUEST_CANCEL = int(
        zamp_temporalio.bridge.proto.child_workflow.ParentClosePolicy.PARENT_CLOSE_POLICY_REQUEST_CANCEL
    )
...
class _UnexpectedEvictionError(zamp_temporalio.exceptions.TemporalError):
    def __init__(
        self,
        reason: zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.ValueType,
        message: str,
    ) -> None:
        self.reason = zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.Name(
            reason
        )
...
class NondeterminismError(zamp_temporalio.exceptions.TemporalError):
...
class ReadOnlyContextError(zamp_temporalio.exceptions.TemporalError):
...
class _NotInWorkflowEventLoopError(zamp_temporalio.exceptions.TemporalError):
    def __init__(self, *args: object) -> None:
...
class VersioningIntent(Enum):
    ...
    def _to_proto(self) -> zamp_temporalio.bridge.proto.common.VersioningIntent.ValueType:
        if self == VersioningIntent.COMPATIBLE:
            return zamp_temporalio.bridge.proto.common.VersioningIntent.COMPATIBLE
        elif self == VersioningIntent.DEFAULT:
            return zamp_temporalio.bridge.proto.common.VersioningIntent.DEFAULT
        return zamp_temporalio.bridge.proto.common.VersioningIntent.UNSPECIFIED
