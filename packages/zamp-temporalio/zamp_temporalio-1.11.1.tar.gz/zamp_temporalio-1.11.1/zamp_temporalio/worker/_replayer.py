...
class Replayer:
    ...
    def __init__(
        ...
        *,
        ...
        See :py:meth:`zamp_temporalio.worker.Worker.__init__` for a description of
    ...
    async def replay_workflow(
        self,
        history: zamp_temporalio.client.WorkflowHistory,
        *,
        ...
        Args:
            history: The history to replay. Can be fetched directly, or use
                :py:meth:`zamp_temporalio.client.WorkflowHistory.from_json` to parse
    ...
    async def replay_workflows(
        self,
        histories: AsyncIterator[zamp_temporalio.client.WorkflowHistory],
        *,
        raise_on_replay_failure: bool = True,
    ...
    async def workflow_replay_iterator(
        self, histories: AsyncIterator[zamp_temporalio.client.WorkflowHistory]
        ...
        try:
            ...
            def on_eviction_hook(
                run_id: str,
                remove_job: zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache,
            ) -> None:
                nonlocal last_replay_failure
                if (
                    remove_job.reason
                    == zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.NONDETERMINISM
                ):
                    last_replay_failure = zamp_temporalio.workflow.NondeterminismError(
                        remove_job.message
                    )
                elif (
                    remove_job.reason
                    != zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.CACHE_FULL
                    and remove_job.reason
                    != zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.LANG_REQUESTED
                ):
            ...
            bridge_worker: zamp_temporalio.bridge.worker.Worker
            task_queue = f"replay-{self._config['build_id']}"
            runtime = self._config["runtime"] or zamp_temporalio.runtime.Runtime.default()
            ...
            bridge_worker, pusher = zamp_temporalio.bridge.worker.Worker.for_replay(
                runtime._core_runtime,
                zamp_temporalio.bridge.worker.WorkerConfig(
                    namespace=self._config["namespace"],
                    task_queue=task_queue,
                    ...
                    tuner=zamp_temporalio.bridge.worker.TunerHolder(
                        workflow_slot_supplier=zamp_temporalio.bridge.worker.FixedSizeSlotSupplier(
                            2
                        ),
                        activity_slot_supplier=zamp_temporalio.bridge.worker.FixedSizeSlotSupplier(
                            1
                        ),
                        local_activity_slot_supplier=zamp_temporalio.bridge.worker.FixedSizeSlotSupplier(
                            1
                        ),
                    ),
                    ...
                    versioning_strategy=zamp_temporalio.bridge.worker.WorkerVersioningStrategyNone(
                        build_id=self._config["build_id"] or load_default_build_id(),
                    ),
                    workflow_task_poller_behavior=zamp_temporalio.bridge.worker.PollerBehaviorSimpleMaximum(
                        1
                    ),
                    activity_task_poller_behavior=zamp_temporalio.bridge.worker.PollerBehaviorSimpleMaximum(
                        1
                    ),
                ),
            )
...
class ReplayerConfig(TypedDict, total=False):
    ...
    workflow_failure_exception_types: Sequence[Type[BaseException]]
    debug_mode: bool
    runtime: Optional[zamp_temporalio.runtime.Runtime]
...
class WorkflowReplayResult:
    ...
    :py:class:`zamp_temporalio.workflow.NondeterminismError` was encountered during
