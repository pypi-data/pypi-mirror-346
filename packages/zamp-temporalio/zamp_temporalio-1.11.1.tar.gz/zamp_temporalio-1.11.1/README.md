...
[![Python 3.9+](https://img.shields.io/pypi/pyversions/zamp_temporalio.svg?style=for-the-badge)](https://pypi.org/project/zamp_temporalio)
[![PyPI](https://img.shields.io/pypi/v/zamp_temporalio.svg?style=for-the-badge)](https://pypi.org/project/zamp_temporalio)
[![MIT](https://img.shields.io/pypi/l/zamp_temporalio.svg?style=for-the-badge)](LICENSE)
...
* [Python Code Samples](https://github.com/zamp_temporalio/samples-python)
...
Install the `zamp_temporalio` package from [PyPI](https://pypi.org/project/zamp_temporalio).
...
* Install Temporal SDK - `python -m pip install zamp_temporalio`
...
```python
from zamp_temporalio import activity
...
from zamp_temporalio import workflow
...
from zamp_temporalio.client import Client
from zamp_temporalio.worker import Worker
...
* [Code Samples](https://github.com/zamp_temporalio/samples-python) - If you want to start with some code, we have provided
...
`zamp_temporalio.converter.DataConverter` can be set via the `data_converter` parameter of the `Client` constructor. Data
...
```python
from zamp_temporalio.contrib.pydantic import pydantic_data_converter
...
https://github.com/zamp_temporalio/samples-python/tree/main/pydantic_converter/v1 for limited v1 support.
...
* Workflows run in a sandbox by default.
  ...
  * Non-standard-library, non-`zamp_temporalio` imports should usually be "passed through" the sandbox. See the
...
* `@workflow.defn` - Defines a workflow class
  ...
  * Can have `dynamic=True` which means all otherwise unhandled workflows fall through to this. If present, cannot have
    `name` argument, and run method must accept a single parameter of `Sequence[zamp_temporalio.common.RawValue]` type. The
...
* `@workflow.signal` - Defines a method as a signal
  ...
  * Can have `dynamic=True` which means all otherwise unhandled signals fall through to this. If present, cannot have
    ...
    `Sequence[zamp_temporalio.common.RawValue]`.
...
`zamp_temporalio.workflow` package:
...
* Workflows/updates can raise exceptions to fail the workflow or the "workflow task" (i.e. suspend the workflow
  in a retrying state).
* Exceptions that are instances of `zamp_temporalio.exceptions.FailureError` will fail the workflow with that exception
  * For failing the workflow explicitly with a user exception, use `zamp_temporalio.exceptions.ApplicationError`. This can
...
* Update handlers are special: an instance of `zamp_temporalio.exceptions.FailureError` raised in an update handler will fail
...
`zamp_temporalio.workflow.NondeterminismError` (or any superclass of it) is set, non-deterministic exceptions will fail the
...
The time-skipping `zamp_temporalio.testing.WorkflowEnvironment` can be created via the static async `start_time_skipping()`.
...
```python
from zamp_temporalio.testing import WorkflowEnvironment
from zamp_temporalio.worker import Worker
...
> zamp_temporalio.worker.workflow_sandbox._restrictions.RestrictedWorkflowAccessError: Cannot access
...
library, `zamp_temporalio`, and a couple of other modules are passed through from outside of the sandbox. To update this
...
To remove restrictions around a particular block of code, use `with zamp_temporalio.workflow.unsafe.sandbox_unrestricted():`.
...
`zamp_temporalio.worker.UnsandboxedWorkflowRunner()`. This value is defaulted to
`zamp_temporalio.worker.workflow_sandbox.SandboxedWorkflowRunner()` so by changing it to the unsandboxed runner, the sandbox
will not be used at all.

##### Customizing the Sandbox

⚠️ WARNING: APIs in the `zamp_temporalio.worker.workflow_sandbox` module are not yet considered stable and may change in
future releases.

When creating the `Worker`, the `workflow_runner` is defaulted to
`zamp_temporalio.worker.workflow_sandbox.SandboxedWorkflowRunner()`. The `SandboxedWorkflowRunner`'s init accepts a
...
call like `setattr(zamp_temporalio.common, "__my_key", "my value")` will leak across sandbox runs.
...
* The `@activity.defn` can have `dynamic=True` set which means all otherwise unhandled activities fall through to this.
  ...
  `Sequence[zamp_temporalio.common.RawValue]`. The payload of the raw value can be converted via
...
Note, all calls from an activity to functions in the `zamp_temporalio.activity` package are powered by
[contextvars](https://docs.python.org/3/library/contextvars.html). Therefore, new threads starting _inside_ of
activities must `copy_context()` and then `.run()` manually to ensure `zamp_temporalio.activity` calls like `heartbeat` still
...
By default, cancellation of a synchronous multithreaded activity is done via a `zamp_temporalio.exceptions.CancelledError`
...
`except zamp_temporalio.exceptions.CancelledError` would have to surround the `with` to handle the cancellation explicitly.
...
set to an instance of `zamp_temporalio.worker.SharedStateManager`. The most common implementation can be created by passing a
`multiprocessing.managers.SyncManager` (i.e. result of `multiprocessing.managers.Manager()`) to
`zamp_temporalio.worker.SharedStateManager.create_from_multiprocessing()`.
...
calls in the `zamp_temporalio.activity` package make use of it. Specifically:
...
invocation time and invoke `zamp_temporalio.activity.heartbeat()` inside the activity. It is strongly recommended that all
...
for retrieval during activity retry. If an activity calls `zamp_temporalio.activity.heartbeat(123, 456)` and then fails and
is retried, `zamp_temporalio.activity.info().heartbeat_details` will return an iterable containing `123` and `456` on the
next run.
...
`zamp_temporalio.testing.ActivityEnvironment` class. Simply instantiate this and any callable + params passed to `run` will
be invoked inside the activity context. The following are attributes/methods on the environment that can be used to
affect calls activity code might make to functions on the `zamp_temporalio.activity` package.
...
```python
from zamp_temporalio.client import WorkflowHistory
from zamp_temporalio.worker import Replayer
...
```python
from zamp_temporalio.client import Client, WorkflowHistory
from zamp_temporalio.worker import Replayer
...
See https://github.com/zamp_temporalio/samples-python/tree/main/open_telemetry for a sample demonstrating collection of
...
[global_tags](https://python.temporal.io/zamp_temporalio.runtime.TelemetryConfig.html#global_tags) when creating the
[TelemetryConfig](https://python.temporal.io/zamp_temporalio.runtime.TelemetryConfig.html).

For emitting custom metrics, the SDK makes a metric meter available:
- In Workflow code, use https://python.temporal.io/zamp_temporalio.workflow.html#metric_meter
- In Activity code, use https://python.temporal.io/zamp_temporalio.activity.html#metric_meter
- In normal application code, use https://python.temporal.io/zamp_temporalio.runtime.Runtime.html#metric_meter
...
using `pip`, running
    ...
    pip install 'zamp_temporalio[opentelemetry]'

will install needed dependencies. Then the `zamp_temporalio.contrib.opentelemetry.TracingInterceptor` can be created and set
...
[a sample](https://github.com/zamp_temporalio/samples-python/tree/main/gevent_async) showing how it is possible.

# Development

The Python SDK is built to work with Python 3.9 and newer. It is built using
[SDK Core](https://github.com/zamp_temporalio/sdk-core/) which is written in Rust.
...
```bash
git clone --recursive https://github.com/zamp_temporalio/sdk-python.git
cd sdk-python
```
...
```python
import asyncio
from zamp_temporalio import workflow, activity
from zamp_temporalio.client import Client
from zamp_temporalio.worker import Worker
...
This runs against [Temporalite](https://github.com/zamp_temporalio/temporalite). To run against the time-skipping test
...
docker run --rm -v "${PWD}/zamp_temporalio/api:/api_new" -v "${PWD}/zamp_temporalio/bridge/proto:/bridge_new" <just built image sha>
poe format
```
...
* Mostly [Google Style Guide](https://google.github.io/styleguide/pyguide.html). Notable exceptions:
  ...
  * In tests and example code, can import individual classes/functions to make it more readable. Can also do this for
    ...
    any `zamp_temporalio` packages (except `zamp_temporalio.types`) or any classes/functions that aren't clear when unqualified.
