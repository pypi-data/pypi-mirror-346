...
import zamp_temporalio.activity
import zamp_temporalio.api.common.v1
import zamp_temporalio.api.enums.v1
import zamp_temporalio.api.sdk.v1
import zamp_temporalio.bridge.proto.activity_result
import zamp_temporalio.bridge.proto.child_workflow
import zamp_temporalio.bridge.proto.common
import zamp_temporalio.bridge.proto.workflow_activation
import zamp_temporalio.bridge.proto.workflow_commands
import zamp_temporalio.bridge.proto.workflow_completion
...
from zamp_temporalio.service import __version__
...
class WorkflowRunner(ABC):
    ...
    @abstractmethod
    def prepare_workflow(self, defn: zamp_temporalio.workflow._Definition) -> None:
...
class WorkflowInstanceDetails:
    ...
    payload_converter_class: Type[zamp_temporalio.converter.PayloadConverter]
    failure_converter_class: Type[zamp_temporalio.converter.FailureConverter]
    interceptor_classes: Sequence[Type[WorkflowInboundInterceptor]]
    defn: zamp_temporalio.workflow._Definition
    info: zamp_temporalio.workflow.Info
    randomness_seed: int
...
class WorkflowInstance(ABC):
    ...
    @abstractmethod
    def activate(
        self, act: zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivation
    ) -> zamp_temporalio.bridge.proto.workflow_completion.WorkflowActivationCompletion:
...
class _WorkflowInstanceImpl(
    WorkflowInstance, zamp_temporalio.workflow._Runtime, asyncio.AbstractEventLoop
):
    def __init__(self, det: WorkflowInstanceDetails) -> None:
        # No init for AbstractEventLoop
        WorkflowInstance.__init__(self)
        zamp_temporalio.workflow._Runtime.__init__(self)
        ...
        self._deployment_version_for_current_task: Optional[
            zamp_temporalio.bridge.proto.common.WorkerDeploymentVersion
        ] = None
        ...
        self._queries["__stack_trace"] = zamp_temporalio.workflow._QueryDefinition(
            name="__stack_trace",
        ...
        self._queries["__enhanced_stack_trace"] = zamp_temporalio.workflow._QueryDefinition(
            name="__enhanced_stack_trace",
            fn=self._enhanced_stack_trace,
            is_method=False,
            arg_types=[],
            ret_type=zamp_temporalio.api.sdk.v1.EnhancedStackTrace,
        )

        self._queries["__temporal_workflow_metadata"] = (
            zamp_temporalio.workflow._QueryDefinition(
                name="__temporal_workflow_metadata",
                fn=self._temporal_workflow_metadata,
                is_method=False,
                arg_types=[],
                ret_type=zamp_temporalio.api.sdk.v1.WorkflowMetadata,
            )
        )

        # Maintain buffered signals for later-added dynamic handlers
        self._buffered_signals: Dict[
            str, List[zamp_temporalio.bridge.proto.workflow_activation.SignalWorkflow]
        ] = {}

        # Create interceptors. We do this with our runtime on the loop just in
        # case they want to access info() during init().
        zamp_temporalio.workflow._Runtime.set_on_loop(asyncio.get_running_loop(), self)
        try:
            ...
            zamp_temporalio.workflow._Runtime.set_on_loop(asyncio.get_running_loop(), None)

        # Set ourselves on our own loop
        zamp_temporalio.workflow._Runtime.set_on_loop(self, self)
        ...
        self._versioning_behavior: Optional[zamp_temporalio.common.VersioningBehavior] = None
    ...
    def activate(
        ...
        with zamp_temporalio.workflow.unsafe.sandbox_unrestricted():
        ...
        try:
            ...
            start_job = None
            job_sets: List[
                List[zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivationJob]
            ] = [[], [], [], []]
            for job in act.jobs:
    ...
    def _apply(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.WorkflowActivationJob
    ) -> None:
    ...
    def _apply_cancel_workflow(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.CancelWorkflow
    ) -> None:
    ...
    def _apply_do_update(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.DoUpdate
    ):
        ...
        async def run_update() -> None:
            ...
            zamp_temporalio.workflow._set_current_update_info(
                zamp_temporalio.workflow.UpdateInfo(id=job.id, name=job.name)
            )
            ...
            try:
                ...
                if isinstance(err, asyncio.CancelledError):
                    err = zamp_temporalio.exceptions.CancelledError(
                        f"Cancellation raised within update {err}"
                    )
                # Read-only issues during validation should fail the task
                if isinstance(err, zamp_temporalio.workflow.ReadOnlyContextError):
    ...
    def _apply_fire_timer(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.FireTimer
    ) -> None:
    ...
    def _apply_query_workflow(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.QueryWorkflow
    ) -> None:
    ...
    def _apply_notify_has_patch(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.NotifyHasPatch
    ) -> None:
        self._patches_notified.add(job.patch_id)

    def _apply_remove_from_cache(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.RemoveFromCache
    ) -> None:
        self._deleting = True
        self._cancel_requested = True
    ...
    def _apply_resolve_activity(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.ResolveActivity
    ) -> None:
    ...
    def _apply_resolve_child_workflow_execution(
        self,
        job: zamp_temporalio.bridge.proto.workflow_activation.ResolveChildWorkflowExecution,
    ) -> None:
    ...
    def _apply_resolve_child_workflow_execution_start(
        self,
        job: zamp_temporalio.bridge.proto.workflow_activation.ResolveChildWorkflowExecutionStart,
    ) -> None:
        ...
        if job.HasField("succeeded"):
            ...
            if (
                job.failed.cause
                == zamp_temporalio.bridge.proto.child_workflow.StartChildWorkflowExecutionFailedCause.START_CHILD_WORKFLOW_EXECUTION_FAILED_CAUSE_WORKFLOW_ALREADY_EXISTS
            ):
                handle._resolve_failure(
                    zamp_temporalio.exceptions.WorkflowAlreadyStartedError(
    ...
    def _apply_resolve_request_cancel_external_workflow(
        self,
        job: zamp_temporalio.bridge.proto.workflow_activation.ResolveRequestCancelExternalWorkflow,
    ) -> None:
    ...
    def _apply_resolve_signal_external_workflow(
        self,
        job: zamp_temporalio.bridge.proto.workflow_activation.ResolveSignalExternalWorkflow,
    ) -> None:
    ...
    def _apply_signal_workflow(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.SignalWorkflow
    ) -> None:
    ...
    def _apply_initialize_workflow(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.InitializeWorkflow
    ) -> None:
    ...
    def _apply_update_random_seed(
        self, job: zamp_temporalio.bridge.proto.workflow_activation.UpdateRandomSeed
    ) -> None:
        self._random.seed(job.randomness_seed)

    def _make_workflow_input(
        self, init_job: zamp_temporalio.bridge.proto.workflow_activation.InitializeWorkflow
    ) -> ExecuteWorkflowInput:
        ...
        if not self._defn.name:
            ...
            arg_types = [zamp_temporalio.common.RawValue] * len(init_job.arguments)
    ...
    def workflow_continue_as_new(
        ...
        *args: Any,
        ...
        versioning_intent: Optional[zamp_temporalio.workflow.VersioningIntent],
    ) -> NoReturn:
    ...
    def workflow_get_current_deployment_version(
        self,
    ) -> Optional[zamp_temporalio.common.WorkerDeploymentVersion]:
        if not self._deployment_version_for_current_task:
            return None
        return zamp_temporalio.common.WorkerDeploymentVersion(
    ...
    ) -> zamp_temporalio.workflow.ExternalWorkflowHandle[Any]:
    ...
    def workflow_info(self) -> zamp_temporalio.workflow.Info:
    ...
    def workflow_metric_meter(self) -> zamp_temporalio.common.MetricMeter:
    ...
    def workflow_payload_converter(self) -> zamp_temporalio.converter.PayloadConverter:
    ...
    def workflow_set_query_handler(
        ...
        if handler:
            ...
            defn = zamp_temporalio.workflow._QueryDefinition(
    ...
    def workflow_set_signal_handler(
        ...
        if handler:
            defn = zamp_temporalio.workflow._SignalDefinition(
                name=name, fn=handler, is_method=False
            )
    ...
    def workflow_set_update_handler(
        ...
        if handler:
            defn = zamp_temporalio.workflow._UpdateDefinition(
                name=name, fn=handler, is_method=False
            )
    ...
    def workflow_start_activity(
        ...
        *args: Any,
        ...
        retry_policy: Optional[zamp_temporalio.common.RetryPolicy],
        cancellation_type: zamp_temporalio.workflow.ActivityCancellationType,
        activity_id: Optional[str],
        versioning_intent: Optional[zamp_temporalio.workflow.VersioningIntent],
        summary: Optional[str] = None,
        priority: zamp_temporalio.common.Priority = zamp_temporalio.common.Priority.default,
    ) -> zamp_temporalio.workflow.ActivityHandle[Any]:
        ...
        if isinstance(activity, str):
            ...
            defn = zamp_temporalio.activity._Definition.must_from_callable(activity)
            if not defn.name:
    ...
    async def workflow_start_child_workflow(
        ...
        *args: Any,
        ...
        task_queue: Optional[str],
        result_type: Optional[Type],
        cancellation_type: zamp_temporalio.workflow.ChildWorkflowCancellationType,
        parent_close_policy: zamp_temporalio.workflow.ParentClosePolicy,
        ...
        priority: zamp_temporalio.common.Priority = zamp_temporalio.common.Priority.default,
    ) -> zamp_temporalio.workflow.ChildWorkflowHandle[Any, Any]:
        # Use definition if callable
        name: str
        arg_types: Optional[List[Type]] = None
        ret_type = result_type
    ...
    def workflow_upsert_search_attributes(
        ...
        attributes: Union[
            zamp_temporalio.common.SearchAttributes,
            Sequence[zamp_temporalio.common.SearchAttributeUpdate],
        ],
    ) -> None:
        ...
        mut_attrs = cast(
            MutableMapping[str, zamp_temporalio.common.SearchAttributeValues],
            self._info.search_attributes,
        )
        mut_typed_attrs = cast(
            List[zamp_temporalio.common.SearchAttributePair],
        ...
        if isinstance(attributes, Mapping):
            ...
            for k, vals in attributes.items():
                ...
                v.search_attributes[k].CopyFrom(
                    zamp_temporalio.converter.encode_search_attribute_values(vals)
                )
                ...
                if not vals:
                    ...
                    key = (
                        zamp_temporalio.common.SearchAttributeKey._guess_from_untyped_values(
                            k, vals
                        )
                    )
                    if key:
                        ...
                        if (
                            ...
                            != zamp_temporalio.common.SearchAttributeIndexedValueType.KEYWORD_LIST
                        ):
                            if len(vals) != 1:
                                continue
                            val = vals[0]
                        pair = zamp_temporalio.common.SearchAttributePair(key, val)
                        if index is None:
            ...
            for update in attributes:
                ...
                v.search_attributes[update.key.name].CopyFrom(
                    zamp_temporalio.converter.encode_typed_search_attribute_value(
                ...
                if update.value is None:
                    ...
                    # Update
                    pair = zamp_temporalio.common.SearchAttributePair(
                        update.key, update.value
                    )
                    if index is None:
                        mut_typed_attrs.append(pair)
    ...
    async def workflow_sleep(
        ...
        user_metadata = (
            zamp_temporalio.api.sdk.v1.UserMetadata(
    ...
    async def _outbound_signal_child_workflow(
        ...
        if input.headers:
            zamp_temporalio.common._apply_headers(input.headers, v.headers)
    ...
    def _add_command(self) -> zamp_temporalio.bridge.proto.workflow_commands.WorkflowCommand:
    ...
    def _assert_not_read_only(
        ...
        if self._read_only:
            raise zamp_temporalio.workflow.ReadOnlyContextError(
    ...
    async def _cancel_external_workflow(
        ...
        command: zamp_temporalio.bridge.proto.workflow_commands.WorkflowCommand,
    ) -> None:
    ...
    def _convert_payloads(
        self,
        payloads: Sequence[zamp_temporalio.api.common.v1.Payload],
        ...
        except zamp_temporalio.exceptions.FailureError:
        ...
        if self._defn.name is None and self._defn.dynamic_config_fn is not None:
            ...
            if dynamic_config:
                ...
                if (
                    ...
                    != zamp_temporalio.common.VersioningBehavior.UNSPECIFIED
                ):
    ...
    def _is_workflow_failure_exception(self, err: BaseException) -> bool:
        ...
        return (
            isinstance(err, zamp_temporalio.exceptions.FailureError)
    ...
    def _warn_if_unfinished_handlers(self) -> None:
        def warnable(handler_executions: Iterable[HandlerExecution]):
            return [
                ...
                == zamp_temporalio.workflow.HandlerUnfinishedPolicy.WARN_AND_ABANDON
            ]

        warnable_updates = warnable(self._in_progress_updates.values())
        if warnable_updates:
            warnings.warn(
                zamp_temporalio.workflow.UnfinishedUpdateHandlersWarning(
        ...
        if warnable_signals:
            warnings.warn(
                zamp_temporalio.workflow.UnfinishedSignalHandlersWarning(
    ...
    def _process_handler_args(
        self,
        job_name: str,
        job_input: Sequence[zamp_temporalio.api.common.v1.Payload],
        ...
        if not defn_name:
            return [
                ...
                self._convert_payloads(
                    job_input, [zamp_temporalio.common.RawValue] * len(job_input)
                ),
            ]
    ...
    def _process_signal_job(
        self,
        defn: zamp_temporalio.workflow._SignalDefinition,
        job: zamp_temporalio.bridge.proto.workflow_activation.SignalWorkflow,
    ) -> None:
        try:
            args = self._process_handler_args(
                job.signal_name,
    ...
    async def _run_top_level_workflow_function(self, coro: Awaitable[None]) -> None:
        try:
            ...
            # All asyncio cancelled errors become Temporal cancelled errors
            if isinstance(err, asyncio.CancelledError):
                err = zamp_temporalio.exceptions.CancelledError(str(err))
            ...
            # event.
            if self._cancel_requested and zamp_temporalio.exceptions.is_cancelled_exception(
                err
            ):
    ...
    def _enhanced_stack_trace(self) -> zamp_temporalio.api.sdk.v1.EnhancedStackTrace:
        sdk = zamp_temporalio.api.sdk.v1.StackTraceSDKInfo(
            name="sdk-python", version=__version__
        )

        # this is to use `open`
        with zamp_temporalio.workflow.unsafe.sandbox_unrestricted():
            sources: Dict[str, zamp_temporalio.api.sdk.v1.StackTraceFileSlice] = dict()
            stacks: List[zamp_temporalio.api.sdk.v1.StackTrace] = []
            ...
            for task in list(self._tasks):
                locations: List[zamp_temporalio.api.sdk.v1.StackTraceFileLocation] = []
                ...
                for frame in task.get_stack():
                    ...
                    if filename not in sources.keys():
                        ...
                        file_slice = zamp_temporalio.api.sdk.v1.StackTraceFileSlice(
                            line_offset=0, content=code
                        )

                        sources[filename] = file_slice

                    file_location = zamp_temporalio.api.sdk.v1.StackTraceFileLocation(
                        file_path=filename,
                        line=line_number,
                        column=-1,
                ...
                stacks.append(zamp_temporalio.api.sdk.v1.StackTrace(locations=locations))

            est = zamp_temporalio.api.sdk.v1.EnhancedStackTrace(
                sdk=sdk, sources=sources, stacks=stacks
            )
            return est

    def _temporal_workflow_metadata(self) -> zamp_temporalio.api.sdk.v1.WorkflowMetadata:
        query_definitions = [
            zamp_temporalio.api.sdk.v1.WorkflowInteractionDefinition(
                name=qd.name or "",
        ...
        wf_def = zamp_temporalio.api.sdk.v1.WorkflowDefinition(
        ...
        return zamp_temporalio.api.sdk.v1.WorkflowMetadata(
...
class _WorkflowOutboundImpl(WorkflowOutboundInterceptor):
    ...
    def info(self) -> zamp_temporalio.workflow.Info:
    ...
    ) -> zamp_temporalio.workflow.ActivityHandle:
        return self._instance._outbound_schedule_activity(input)

    async def start_child_workflow(
        self, input: StartChildWorkflowInput
    ) -> zamp_temporalio.workflow.ChildWorkflowHandle:
...
@dataclass(frozen=True)
class _TimerOptions:
    user_metadata: Optional[zamp_temporalio.api.sdk.v1.UserMetadata] = None
...
class _ActivityDoBackoffError(BaseException):
    def __init__(
        self, backoff: zamp_temporalio.bridge.proto.activity_result.DoBackoff
    ) -> None:
        super().__init__("Backoff")
        self.backoff = backoff


class _ActivityHandle(zamp_temporalio.workflow.ActivityHandle[Any]):
    def __init__(
        self,
        instance: _WorkflowInstanceImpl,
        input: Union[StartActivityInput, StartLocalActivityInput],
    ...
    def _apply_schedule_command(
        ...
        local_backoff: Optional[
            zamp_temporalio.bridge.proto.activity_result.DoBackoff
        ] = None,
    ) -> None:
        ...
        v: Union[
            zamp_temporalio.bridge.proto.workflow_commands.ScheduleActivity,
            zamp_temporalio.bridge.proto.workflow_commands.ScheduleLocalActivity,
        ] = (
        ...
        if self._input.headers:
            zamp_temporalio.common._apply_headers(self._input.headers, v.headers)
        if payloads:
        ...
        v.cancellation_type = cast(
            "zamp_temporalio.bridge.proto.workflow_commands.ActivityCancellationType.ValueType",
...
class _ChildWorkflowHandle(zamp_temporalio.workflow.ChildWorkflowHandle[Any, Any]):
    def __init__(
        self,
        instance: _WorkflowInstanceImpl,
        seq: int,
    ...
    def _apply_start_command(self) -> None:
        ...
        v.parent_close_policy = cast(
            "zamp_temporalio.bridge.proto.child_workflow.ParentClosePolicy.ValueType",
        ...
        v.cancellation_type = cast(
            "zamp_temporalio.bridge.proto.child_workflow.ChildWorkflowCancellationType.ValueType",
            int(self._input.cancellation_type),
        )
        if self._input.versioning_intent:
...
class _ExternalWorkflowHandle(zamp_temporalio.workflow.ExternalWorkflowHandle[Any]):
    def __init__(
        self,
        instance: _WorkflowInstanceImpl,
        id: str,
...
class _ContinueAsNewError(zamp_temporalio.workflow.ContinueAsNewError):
    def __init__(
...
def _encode_search_attributes(
    ...
    payloads: Mapping[str, zamp_temporalio.api.common.v1.Payload],
) -> None:
    if isinstance(attributes, zamp_temporalio.common.TypedSearchAttributes):
...
class _WorkflowExternFunctions(TypedDict):
    __temporal_get_metric_meter: Callable[[], zamp_temporalio.common.MetricMeter]


class _ReplaySafeMetricMeter(zamp_temporalio.common.MetricMeter):
    def __init__(self, underlying: zamp_temporalio.common.MetricMeter) -> None:
        self._underlying = underlying

    def create_counter(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricCounter:
    ...
    def create_histogram(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricHistogram:
    ...
    def create_histogram_float(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricHistogramFloat:
    ...
    def create_histogram_timedelta(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricHistogramTimedelta:
    ...
    def create_gauge(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricGauge:
    ...
    def create_gauge_float(
        self, name: str, description: Optional[str] = None, unit: Optional[str] = None
    ) -> zamp_temporalio.common.MetricGaugeFloat:
    ...
    def with_additional_attributes(
        self, additional_attributes: zamp_temporalio.common.MetricAttributes
    ) -> zamp_temporalio.common.MetricMeter:
        return _ReplaySafeMetricMeter(
            self._underlying.with_additional_attributes(additional_attributes)
        )


_MetricType = TypeVar("_MetricType", bound=zamp_temporalio.common.MetricCommon)


class _ReplaySafeMetricCommon(zamp_temporalio.common.MetricCommon, Generic[_MetricType]):
...
class _ReplaySafeMetricCounter(
    zamp_temporalio.common.MetricCounter,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricCounter],
):
    def add(
        self,
        value: int,
        additional_attributes: Optional[zamp_temporalio.common.MetricAttributes] = None,
    ) -> None:
        if not zamp_temporalio.workflow.unsafe.is_replaying():
            self._underlying.add(value, additional_attributes)


class _ReplaySafeMetricHistogram(
    zamp_temporalio.common.MetricHistogram,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricHistogram],
):
    def record(
        self,
        value: int,
...
class _ReplaySafeMetricHistogramFloat(
    zamp_temporalio.common.MetricHistogramFloat,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricHistogramFloat],
):
    def record(
        self,
        value: float,
...
class _ReplaySafeMetricHistogramTimedelta(
    zamp_temporalio.common.MetricHistogramTimedelta,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricHistogramTimedelta],
):
    def record(
        self,
        value: timedelta,
...
class _ReplaySafeMetricGauge(
    zamp_temporalio.common.MetricGauge,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricGauge],
):
    def set(
        self,
        value: int,
...
class _ReplaySafeMetricGaugeFloat(
    zamp_temporalio.common.MetricGaugeFloat,
    _ReplaySafeMetricCommon[zamp_temporalio.common.MetricGaugeFloat],
):
    def set(
        self,
        value: float,
...
class HandlerExecution:
    ...
    name: str
    unfinished_policy: zamp_temporalio.workflow.HandlerUnfinishedPolicy
