...
class TracingInterceptor(zamp_temporalio.client.Interceptor, zamp_temporalio.worker.Interceptor):
    ...
    :py:meth:`zamp_temporalio.client.Client.connect` call to apply to all client
    ...
    def __init__(
        ...
        *,
        ...
        self.payload_converter = zamp_temporalio.converter.PayloadConverter.default
        self._always_create_workflow_spans = always_create_workflow_spans

    def intercept_client(
        self, next: zamp_temporalio.client.OutboundInterceptor
    ) -> zamp_temporalio.client.OutboundInterceptor:
        """Implementation of
        :py:meth:`zamp_temporalio.client.Interceptor.intercept_client`.
        """
        return _TracingClientOutboundInterceptor(next, self)

    def intercept_activity(
        self, next: zamp_temporalio.worker.ActivityInboundInterceptor
    ) -> zamp_temporalio.worker.ActivityInboundInterceptor:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.Interceptor.intercept_activity`.
        """
        return _TracingActivityInboundInterceptor(next, self)

    def workflow_interceptor_class(
        self, input: zamp_temporalio.worker.WorkflowInterceptorClassInput
    ) -> Type[TracingWorkflowInboundInterceptor]:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.Interceptor.workflow_interceptor_class`.
        """
    ...
    def _context_to_headers(
        self, headers: Mapping[str, zamp_temporalio.api.common.v1.Payload]
    ) -> Mapping[str, zamp_temporalio.api.common.v1.Payload]:
...
class _TracingClientOutboundInterceptor(zamp_temporalio.client.OutboundInterceptor):
    def __init__(
        self, next: zamp_temporalio.client.OutboundInterceptor, root: TracingInterceptor
    ) -> None:
        super().__init__(next)
        self.root = root

    async def start_workflow(
        self, input: zamp_temporalio.client.StartWorkflowInput
    ) -> zamp_temporalio.client.WorkflowHandle[Any, Any]:
        prefix = (
    ...
    async def query_workflow(self, input: zamp_temporalio.client.QueryWorkflowInput) -> Any:
        with self.root._start_as_current_span(
    ...
    async def signal_workflow(
        self, input: zamp_temporalio.client.SignalWorkflowInput
    ) -> None:
        with self.root._start_as_current_span(
    ...
    async def start_workflow_update(
        self, input: zamp_temporalio.client.StartWorkflowUpdateInput
    ) -> zamp_temporalio.client.WorkflowUpdateHandle[Any]:
        with self.root._start_as_current_span(
...
class _TracingActivityInboundInterceptor(zamp_temporalio.worker.ActivityInboundInterceptor):
    def __init__(
        self,
        next: zamp_temporalio.worker.ActivityInboundInterceptor,
    ...
    async def execute_activity(
        self, input: zamp_temporalio.worker.ExecuteActivityInput
    ) -> Any:
        info = zamp_temporalio.activity.info()
...
class TracingWorkflowInboundInterceptor(zamp_temporalio.worker.WorkflowInboundInterceptor):
    ...
    def __init__(self, next: zamp_temporalio.worker.WorkflowInboundInterceptor) -> None:
        """Initialize a tracing workflow interceptor."""
        super().__init__(next)
        self._extern_functions = cast(
            _WorkflowExternFunctions, zamp_temporalio.workflow.extern_functions()
        )
    ...
    def init(self, outbound: zamp_temporalio.worker.WorkflowOutboundInterceptor) -> None:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.init`.
        """
        super().init(_TracingWorkflowOutboundInterceptor(outbound, self))

    async def execute_workflow(
        self, input: zamp_temporalio.worker.ExecuteWorkflowInput
    ) -> Any:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.execute_workflow`.
        """
        with self._top_level_workflow_context(success_is_complete=True):
            ...
            self._completed_span(
                f"RunWorkflow:{zamp_temporalio.workflow.info().workflow_type}",
                kind=opentelemetry.trace.SpanKind.SERVER,
            )
            return await super().execute_workflow(input)

    async def handle_signal(self, input: zamp_temporalio.worker.HandleSignalInput) -> None:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.handle_signal`.
        """
    ...
    async def handle_query(self, input: zamp_temporalio.worker.HandleQueryInput) -> Any:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.handle_query`.
        """
    ...
    def handle_update_validator(
        self, input: zamp_temporalio.worker.HandleUpdateInput
    ) -> None:
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.handle_update_validator`.
        """
        link_context_header = input.headers.get(self.header_key)
        link_context_carrier: Optional[_CarrierDict] = None
        if link_context_header:
    ...
    async def handle_update_handler(
        ...
        """Implementation of
        :py:meth:`zamp_temporalio.worker.WorkflowInboundInterceptor.handle_update_handler`.
        """
        link_context_header = input.headers.get(self.header_key)
        link_context_carrier: Optional[_CarrierDict] = None
        if link_context_header:
    ...
    def _load_workflow_context_carrier(self) -> Optional[_CarrierDict]:
        ...
        context_header = zamp_temporalio.workflow.info().headers.get(self.header_key)
    ...
    def _top_level_workflow_context(
        ...
        try:
            yield None
            success = True
        except zamp_temporalio.exceptions.FailureError as err:
            ...
            if exception or (success and success_is_complete):
                self._completed_span(
                    f"CompleteWorkflow:{zamp_temporalio.workflow.info().workflow_type}",
                    exception=exception,
    ...
    def _completed_span(
        ...
        *,
        ...
        if zamp_temporalio.workflow.unsafe.is_replaying() and not new_span_even_on_replay:
            return None
        ...
        updated_context_carrier = self._extern_functions[
            ...
            _CompletedWorkflowSpanParams(
                ...
                time_ns=zamp_temporalio.workflow.time_ns(),
...
class _TracingWorkflowOutboundInterceptor(
    zamp_temporalio.worker.WorkflowOutboundInterceptor
):
    def __init__(
        self,
        next: zamp_temporalio.worker.WorkflowOutboundInterceptor,
        root: TracingWorkflowInboundInterceptor,
    ) -> None:
        super().__init__(next)
        self.root = root

    def continue_as_new(self, input: zamp_temporalio.worker.ContinueAsNewInput) -> NoReturn:
    ...
    async def signal_child_workflow(
        self, input: zamp_temporalio.worker.SignalChildWorkflowInput
    ) -> None:
    ...
    async def signal_external_workflow(
        self, input: zamp_temporalio.worker.SignalExternalWorkflowInput
    ) -> None:
        # Create new span and put on outbound input
        self.root._completed_span(
    ...
    def start_activity(
        self, input: zamp_temporalio.worker.StartActivityInput
    ...
    async def start_child_workflow(
        self, input: zamp_temporalio.worker.StartChildWorkflowInput
    ...
    def start_local_activity(
        self, input: zamp_temporalio.worker.StartLocalActivityInput
    ) -> zamp_temporalio.workflow.ActivityHandle:
        # Create new span and put on outbound input
        self.root._completed_span(
            f"StartActivity:{input.activity}",
