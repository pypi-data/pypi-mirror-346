    ...
    def __init__(self, path, classic=True, single=False):
        ...
        if classic:
            if os.path.isfile(os.path.join(path,'analysis','diffractlets.h5')): # loads a crystal if it exists
                ...
                crystal_data = cry.parse_cif(os.path.join(path,self.cifPath))
                ...
                int_data_file = sorted( os.listdir( os.path.join(path,'data_integrated')) )[0]
                with h5py.File(os.path.join(path,'data_integrated',int_data_file),'r') as hf:
                    self.q_det = hf['radial_units'][0]
                    self.chi_det = hf['azimuthal_units'][0] * np.pi/180

                print('\tImporting geometry')
                self.geo = import_module_from_path('geometry', os.path.join(path,'analysis/geometry.py'))
