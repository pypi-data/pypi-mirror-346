...
def check_detector_geometry( intpar_path:str, testfile:str, testfile_dataset:str, geo_path:str, flip_detector_x=False, flip_detector_y=False,
    ...
    test_data = fid_in[testfile_dataset][par.h5_data_path][0] # check if 0 is ok (can pass it also as argument or plot several ?)
    result2D = ai.integrate2d(
                test_data, 
                par.npt_rad, 
                par.npt_azi, 
    ...
    det_Y,det_Z = get_detector_coordinates(geo_path, q, chi)

    fig = plt.figure(figsize=(18, 10))
    ax1 = fig.add_subplot(121)
    ax1.matshow( test_data, vmax=vmax, 
...
def get_detector_coordinates(geo_path, q, chi):
    ...
    for k in range(Y.shape[0]):
        for l in range(Y.shape[1]):
            Y[k,l], Z[k,l] = polar_to_cartesian(QQ[k,l], CHi[k,l], 
                u0=(geo.detector_direction_origin[1], geo.detector_direction_origin[2]), # minus because x on the detector is -y in 3D coordinates
