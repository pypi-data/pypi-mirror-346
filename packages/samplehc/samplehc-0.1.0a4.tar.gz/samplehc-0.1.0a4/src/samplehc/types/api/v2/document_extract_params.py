...
class DocumentExtractParams(TypedDict, total=False):
    ...
    """An array of documents from which to extract information."""

    prompt: Required[str]
    """The prompt guiding the extraction process."""

    response_json_schema: Required[Annotated[Dict[str, object], PropertyInfo(alias="responseJsonSchema")]]
    """A JSON schema defining the structure of the desired extraction response."""

    reasoning_effort: Annotated[Literal["low", "medium", "high"], PropertyInfo(alias="reasoningEffort")]
    """The level of reasoning effort to apply for the extraction.
