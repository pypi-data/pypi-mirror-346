...
class WorkflowResource(SyncAPIResource):
    ...
    def start(
        ...
        workflow_id: str,
        *,
        start_data: object | NotGiven = NOT_GIVEN,
        # Use the following arguments if you need to pass additional parameters to the API that aren't available via kwargs.
        # The extra values given here take precedence over values defined on the client or passed to this method.
        extra_headers: Headers | None = None,
        extra_query: Query | None = None,
        ...
        if not workflow_id:
            raise ValueError(f"Expected a non-empty value for `workflow_id` but received {workflow_id!r}")
        return self._post(
            f"/api/v2/workflow/{workflow_id}/start",
            body=maybe_transform({"start_data": start_data}, workflow_start_params.WorkflowStartParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
...
class AsyncWorkflowResource(AsyncAPIResource):
    ...
    async def start(
        ...
        *,
        ...
        return await self._post(
            f"/api/v2/workflow/{workflow_id}/start",
            body=await async_maybe_transform({"start_data": start_data}, workflow_start_params.WorkflowStartParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=WorkflowStartResponse,
