...
_int_types_nan = {np.int8: 127, np.int16:32767}
_int_types_scale = {np.int8: 100, np.int16:10000}


def _ewmx(a, b, n, wgt, time, t = None,
          a1 = None, a2 = None, b1 = None, b2 = None, ab = None, prev_a = None, prev_b = None, 
             w1 = None, w2 = None, n0 = None, min_sample = 0.25, bias = False, overlapping = 1, calculation = cor_calculation_ewm, dtype = None):
    """
    ...
    NAN = _int_types_nan.get(dtype, np.nan)
    x = a.shape[1]
    y = b.shape[1]
            ...
            for i in range(a.shape[0]):
                if ~np.isnan(a[i,j]) and ~np.isnan(b[i,k]):
                    ...
                    if n0[j,k] > min_sample:
                        ...
                        if dtype in _int_types_scale:
                            c = NAN if np.isnan(c) else np.round(c * _int_types_scale[dtype])
                        res0[i, j, k] = c
...
def _ewmx2(a, b, n, wgt, time, t = None,
          a1 = None, a2 = None, b1 = None, b2 = None, ab = None, prev_a = None, prev_b = None, 
             w1 = None, w2 = None, n0 = None, min_sample = 0.25, bias = False, overlapping = 1, calculation = LR_calculation_ewm, dtype = None):
    """
    cross-sectional correlation of a and b
    ...
    x = a.shape[1]
    y = b.shape[1]
    res0 = np.full((a.shape[0], x, y), np.nan, dtype = dtype)    
    res1 = np.full((a.shape[0], x, y), np.nan, dtype = dtype)    
    if n == 1:
...
def _ewmcorrelation(a, n, wgt, a0 = None, a1 = None, a2 = None, aa = None, prev = None, w2 = None, n0 = None, min_sample = 0.25, bias = False, overlapping = 1, dtype = None):
    """
    ...
    dtype = dtype or np.float32
    NAN = _int_types_nan.get(dtype, np.nan)
    ONE = _int_types_scale.get(dtype, 1.0)
                        ...
                        if ~np.isnan(prev[j,k,-1]) and ~np.isnan(prev[k,j,-1]):
                            ...
                            if n0[j,k] > min_sample:
                                c = cor_calculation_ewm(t0 = a0[j,k], 
                                                                                  ...
                                                                                  ab = aa[j,k], bias = bias) 
                                if dtype in _int_types_scale:
                                    c = NAN if np.isnan(c) else np.round(ONE * c)
...
def _ewmcovariance(a, n, wgt, a0 = None, a1 = None, aa = None, prev = None, n0 = None, min_sample = 0.25, bias = False, overlapping = 1, dtype = None):
    """

    """
    dtype = dtype or np.float32
    m = a.shape[1]
    res = np.full((a.shape[0], m, m), np.nan, dtype = dtype), a0, a1, aa, prev, n0
    if n == 1:
...
def ewmcorrelation_(a, n, min_sample = 0.25, bias = False, overlapping = 1, instate = None, join = 'outer', method = None, wgt = None, dtype = None):
    ...
    state['prev'] = _prev(state.get('prev'), (arr.shape[1],arr.shape[1],overlapping))
    if isinstance(arr, np.ndarray):
        res, a0, a1, a2, aa, prev, w2, n0 = _ewmcorrelation(arr, wgt = wgt, n = n, min_sample = min_sample, bias = bias, overlapping = overlapping, dtype = dtype, **state)
        state = dictattr(a0=a0, a1=a1, a2=a2, aa=aa, prev = prev, w2 = w2, n0 = n0)
        return dictattr(data = res, index = None, columns = None, state = state)
    elif is_df(arr):
        index = arr.index
        columns = list(arr.columns)
        res, a0, a1, a2, aa, prev, w2, n0 = _ewmcorrelation(arr.values, wgt = wgt, n = n, min_sample = min_sample, bias = bias, overlapping = overlapping, dtype = dtype, **state)
        state = dictattr(a0=a0, a1=a1, a2=a2, aa=aa, prev=prev, w2 = w2, n0 = n0)
        return dictattr(data = res, columns = columns, index = index, state = state)
    else:
        raise ValueError('unsure how to calculate correlation matrix for a %s'%a)
...
def _ewmxt(a, b, n, wgt = None, time = None, t = None, a1 = None, a2 = None, b1 = None, b2 = None, 
    ...
    if dim == 1:
        res, a1, a2, b1, b2, ab, prev_a, prev_b, w1, w2, n0, t = _ewmx(a = a, b = b, n = n, wgt = wgt, time = time,
                       ...
                       ab = ab, min_sample=min_sample, bias = bias, overlapping = overlapping, calculation=calculation, dtype = dtype)
        res = res[0]
    else:
