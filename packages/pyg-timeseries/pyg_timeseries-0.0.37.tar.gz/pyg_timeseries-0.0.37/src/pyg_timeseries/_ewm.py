...
def _ewmx(a, b, n, wgt, time, t = None,
    ...
    """
    NAN = (127 if int8<127 else 32767) if int8 else np.nan
    TYPE = (np.int8 if int8<127 else np.int16) if int8 else float
    x = a.shape[1]
    y = b.shape[1]
    res0 = np.full((a.shape[0], x, y), NAN, dtype = TYPE)
    if n == 1:
            ...
            for i in range(a.shape[0]):
                if ~np.isnan(a[i,j]) and ~np.isnan(b[i,k]):
                    ...
                    if n0[j,k] > min_sample:
                        ...
                        if int8:
                            c = NAN if np.isnan(c) else np.round(c * int8)
                        res0[i, j, k] = c
...
def _ewmcorrelation(a, n, wgt, a0 = None, a1 = None, a2 = None, aa = None, prev = None, w2 = None, n0 = None, min_sample = 0.25, bias = False, overlapping = 1, int8 = None):
    ...
    m = a.shape[1]
    res = np.full((a.shape[0], m, m), NAN, dtype = TYPE)
    if n == 1:
                        ...
                        if ~np.isnan(prev[j,k,-1]) and ~np.isnan(prev[k,j,-1]):
                            ...
                            if n0[j,k] > min_sample:
                                c = cor_calculation_ewm(t0 = a0[j,k], 
                                                                                  ...
                                                                                  ab = aa[j,k], bias = bias) 
                                if int8:
                                    c = NAN if np.isnan(c) else np.round(int8 * c)
