...
class DockerArtifact(ArtifactBase):
    ...
    def build_fast(
        ...
        def _do_build(target, a, p):
            ...
            shell_wrapper(f'{cls.cli} build {options or ""} -t {target} {build_args} {p}')
                ...
                for i, c in enumerate(contents):
                    ...
                    if is_last:
                        ...
                        if not isinstance(current_image, str):
                            current_image, _args = current_image
                    elif i == 0:
        ...
        if push_only_last:
            ...
            if not step:
                raise ValueError('When push_only_last=False, `step` is needed')
