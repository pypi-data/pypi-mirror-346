...
const ce = /[^\p{L}\p{N}]+/u, ge = /(\d{3})/g, de = /(\D)(\d{3})/g, ae = /(\d{3})(\D)/g, Dt = /[\u0300-\u036f]/g;
function lt(t = {}) {
...
w.encode = function(t, e) {
  ...
  this.normalize && (typeof this.normalize == "function" ? t = this.normalize(t) : t = Dt ? t.normalize("NFKD").replace(Dt, "").toLowerCase() : t.toLowerCase()), this.prepare && (t = this.prepare(t)), this.numeric && 3 < t.length && (t = t.replace(de, "$1 $2").replace(ae, "$1 $2").replace(ge, "$1 "));
...
async function pe(t) {
  ...
  switch (e) {
    case "init":
      it = t.options || {}, (e = t.factory) ? (Function("return " + e)()(self), V = new self.FlexSearch.Index(it), delete self.FlexSearch) : V = new C(it), postMessage({ id: i });
      break;
    default:
      let n;
...
function nt(t) {
  this[t + "Async"] = function() {
    ...
    var i = e[e.length - 1];
    let s;
    if (typeof i == "function" && (s = i, delete e[e.length - 1]), St ? at || (at = Date.now() - Ft >= this.priority * this.priority * 3) : (St = setTimeout(me, 0), Ft = Date.now()), at) {
      const r = this;
...
function we(t, e, i) {
  return e ? typeof module < "u" ? new (require("worker_threads")).Worker(__dirname + "/worker/node.js") : import("./assets/__vite-browser-external-CPvbk0mb.js").then(function(s) {
...
function H(t) {
  ...
  return new Proxy([], { get(i, s) {
    ...
    if (s === "slice") return function(n, r) {
      return Ct(e, n || 0, r || e.length, !1);
    };
    if (s === "splice") return function(n, r) {
      return Ct(e, n || 0, r || e.length, !0);
    };
...
w = $.prototype;
w.has = b.prototype.has = function(t) {
  const e = this.index[this.B(t)];
  return e && e.has(t);
};
w.delete = b.prototype.delete = function(t) {
  const e = this.index[this.B(t)];
  e && e.delete(t) && this.size--;
};
w.clear = b.prototype.clear = function() {
  this.index = I(), this.h = [], this.size = 0;
};
w.values = b.prototype.values = function* () {
  for (let t = 0; t < this.h.length; t++) for (let e of this.h[t].values()) yield e;
};
w.keys = b.prototype.keys = function* () {
  for (let t = 0; t < this.h.length; t++) for (let e of this.h[t].keys()) yield e;
};
w.entries = b.prototype.entries = function* () {
  ...
  if (rt(t) && (e = t, t = ht(e, this.key)), e && (t || t === 0)) {
    ...
    for (let o = 0, l; o < this.field.length; o++) {
      ...
      if (typeof l == "function") {
        var n = l(e);
        n && s.add(t, n, !1, !0);
      } else n = l.I, (!n || n(e)) && (l.constructor === String ? l = ["" + l] : F(l) && (l = [l]), Ot(e, l, this.J, 0, s, t, l[0], i));
    }
      ...
      if (this.C) {
        ...
        for (let l = 0, f; l < this.C.length; l++) {
          ...
          if (typeof f == "function") {
            if (u = f(e), !u) continue;
            f = [f.V];
          } else if (F(f) || f.constructor === String) {
            o[f] = e[f];
            continue;
          }
          zt(e, o, f, 0, f[0], u);
...
function Nt(t, e, i, s) {
  ...
  if (t.length === 1) return t = t[0], t = i || t.length > e ? e ? t.slice(i, i + e) : t.slice(i) : t, s ? J.call(this, t) : t;
  let n = [];
  for (let r = 0, h, o; r < t.length; r++) if ((h = t[r]) && (o = h.length)) {
    ...
    if (o > e && (h = h.slice(0, e), o = e), !n.length && o >= e) return s ? J.call(this, h) : h;
    if (n.push(h), e -= o, !e) break;
  }
  return n = 1 < n.length ? [].concat.apply([], n) : n[0], s ? J.call(this, n) : n;
}
function yt(t, e, i) {
  var s = i[0];
...
function ie(t, e, i, s, n, r, h) {
  ...
  if (t.length) if (this.result.length && t.unshift(this.result), 2 > t.length) this.result = t[0];
    ...
    if (e = ue(t)) return this.result = Kt(t, e, i, s, h, this.h, r), r ? n ? J.call(this.index, this.result) : this.result : this;
    this.result = [];
  }
...
function ne(t, e, i, s, n, r, h) {
  ...
  if (t.length) if (this.result.length && t.unshift(this.result), 2 > t.length) this.result = t[0];
  else return this.result = xe.call(this, t, i, s, r, this.h), r ? n ? J.call(this.index, this.result) : this.result : this;
  else h || (this.result = t);
  return r ? this.resolve(i, s, n) : this;
}
...
function se(t, e, i, s, n, r, h) {
  ...
  if (t.length && this.result.length) this.result = ke.call(this, t, i, s, r);
  else if (r) return this.resolve(i, s, n);
  return r ? n ? J.call(this.index, this.result) : this.result : this;
}
function ke(t, e, i, s) {
  const n = [];
...
function vt(t, e, i, s, n) {
  ...
  for (let xt = 0, Z, Rt, tt; xt < e.length; xt++) {
    ...
    for (let ut = 0; ut < et.length; ut++) {
      ...
      var y = [];
      let ct = [];
      for (var L = -1, M = -1, k = 0, j = 0; j < x.length; j++) {
        var _ = x[j], S = Z.encode(_);
        S = 1 < S.length ? S.join(" ") : S[0];
        let v;
        if (S && _) {
          for (var z = _.length, P = (Z.split ? _.replace(Z.split, "") : _).length - S.length, R = "", G = 0, W = 0; W < m.length; W++) {
            var D = m[W];
            if (D) {
              var B = D.length;
              B += P, G && B <= G || (D = S.indexOf(D), -1 < D && (R = (D ? _.substring(0, D) : "") + h + _.substring(D, D + B) + o + (D + B < z ? _.substring(D + B) : ""), G = B, v = !0));
            }
          }
          R && (l && (0 > L && (L = g.length + (g ? 1 : 0)), M = g.length + (g ? 1 : 0) + R.length, k += z, ct.push(y.length), y.push({ match: R })), g += (g ? " " : "") + R);
        }
        if (!v) _ = x[j], g += (g ? " " : "") + _, l && y.push({ text: _ });
        else if (l && k >= l) break;
      }
      if (k = ct.length * (r.length - 2), d || p || l && g.length - k > l) if (k = l + k - 2 * a, j = M - L, 0 < d && (j += d), 0 < p && (j += p), j <= k) x = d ? L - (0 < d ? d : 0) : L - ((k - j) / 2 | 0), y = p ? M + (0 < p ? p : 0) : x + k, f || (0 < x && g.charAt(x) !== " " && g.charAt(x - 1) !== " " && (x = g.indexOf(" ", x), 0 > x && (x = 0)), y < g.length && g.charAt(y - 1) !== " " && g.charAt(y) !== " " && (y = g.lastIndexOf(" ", y), y < M ? y = M : ++y)), g = (x ? n : "") + g.substring(x, y) + (y < g.length ? n : "");
      else {
        for (M = [], L = {}, k = {}, j = {}, _ = {}, S = {}, R = P = z = 0, W = G = 1; ; ) {
          var T = void 0;
          for (let v = 0, A; v < ct.length; v++) {
            if (A = ct[v], R) if (P !== R) {
              if (j[v + 1]) continue;
              if (A += R, L[A]) {
                z -= a, k[v + 1] = 1, j[v + 1] = 1;
              ...
              if (A >= y.length - 1) {
                if (A >= y.length) {
                  j[v + 1] = 1, A >= x.length && (k[v + 1] = 1);
                  continue;
                }
                z -= a;
              }
              if (g = y[A].text, B = p && S[v]) if (0 < B) {
                if (g.length > B) if (j[v + 1] = 1, f) g = g.substring(0, B);
                else continue;
                (B -= g.length) || (B = -1), S[v] = B;
              } else {
                j[v + 1] = 1;
                continue;
              }
              if (z + g.length + 1 <= l) g = " " + g, M[v] += g;
              else if (f) T = l - z - 1, 0 < T && (g = " " + g.substring(0, T), M[v] += g), j[v + 1] = 1;
              else {
                j[v + 1] = 1;
                continue;
              }
              ...
              if (g = y[A].text, B = d && _[v]) if (0 < B) {
                if (g.length > B) if (j[v] = 1, f) g = g.substring(g.length - B);
                else continue;
                (B -= g.length) || (B = -1), _[v] = B;
              } else {
                j[v] = 1;
                continue;
              }
              if (z + g.length + 1 <= l) g += " ", M[v] = g + M[v];
              else if (f) T = g.length + 1 - (l - z), 0 <= T && T < g.length && (g = g.substring(T) + " ", M[v] = g + M[v]), j[v] = 1;
              else {
                j[v] = 1;
                continue;
              }
              ...
              g = y[A].match, d && (_[v] = d), p && (S[v] = p), v && z++;
              let kt;
              if (A ? !v && a && (z += a) : (k[v] = 1, j[v] = 1), A >= x.length - 1 || A < y.length - 1 && y[A + 1].match ? kt = 1 : a && (z += a), z -= r.length - 2, !v || z + g.length <= l) M[v] = g;
              else {
                T = G = W = k[v] = 0;
                break;
              }
          ...
          if (T) P === R ? R++ : P++;
            ...
            if (P === R ? G = 0 : W = 0, !G && !W) break;
            G ? (P++, R = P) : R++;
          }
        }
        g = "";
        for (let v = 0, A; v < M.length; v++) A = (v && k[v] ? " " : (v && !n ? " " : "") + n) + M[v], g += A;
        n && !k[M.length] && (g += n);
      }
...
K.prototype.search = function(t, e, i, s) {
  ...
  if (i) {
    ...
    var d = i.suggest;
    c = i.resolve !== !1, c || h || !(l = l || this.field) || (F(l) ? h = l : (l.constructor === Array && l.length === 1 && (l = l[0]), h = l.field || l.index));
    ...
    if (e || (e = 100), f && (!this.db || !s)) {
      ...
      var g = [];
      for (let M = 0, k; M < f.length; M++) if (k = f[M], k.field && k.tag) {
        var x = k.tag;
        if (x.constructor === Array) for (var y = 0; y < x.length; y++) g.push(k.field, x[y]);
        else g.push(k.field, x);
      } else {
        x = Object.keys(k);
        for (let j = 0, _, S; j < x.length; j++) if (_ = x[j], S = k[_], S.constructor === Array) for (y = 0; y < S.length; y++) g.push(_, S[y]);
        else g.push(_, S);
      }
      if (f = g, !t) {
  ...
  let L;
  for (let M = 0, k, j, _; M < l.length; M++) {
    if (j = l[M], this.db && this.tag && !this.F[M]) continue;
    let S;
    if (F(j) || (S = j, j = S.field, t = S.query || t, e = S.limit || e, m = S.offset || m, d = S.suggest || d, a = (p = this.store && (S.enrich || p)) && (i.highlight || a)), s) k = s[M];
      ...
      continue;
    } else k = y.search(t, e, x), x && p && (x.enrich = p);
    if (_ = k && (c ? k.length : k.result.length), f && _) {
      if (x = [], y = 0, this.db && s) {
      ...
      if (y) {
        if (k = ye(k, x, c), _ = k.length, !_ && !d) return c ? k : new O(k);
        y--;
      }
    }
    if (_) r[u] = j, n.push(k), u++;
  ...
  if (g) {
    ...
    return Promise.all(g).then(function(k) {
      return k.length ? M.search(t, e, i, k) : k;
    });
  }
  if (!u) return c ? n : new O(n);
  if (h && (!p || !this.store)) return n[0];
  for (g = [], m = 0; m < r.length; m++) {
    if (d = n[m], p && d.length && typeof d[0].doc > "u" && (this.db ? g.push(d = this.index.get(this.field[0]).db.enrich(d)) : d = J.call(this, d)), h) return c ? a ? vt(t, d, this.index, h, a) : d : new O(d);
    n[m] = { field: r[m], result: d };
  }
  if (p && this.db && g.length) {
    const M = this;
    return Promise.all(g).then(function(k) {
      for (let j = 0; j < k.length; j++) n[j].result = k[j];
      return o ? Tt(n) : a ? vt(t, n, M.index, h, a) : n;
    });
  }
  return o ? Tt(n) : a ? vt(t, n, this.index, h, a) : n;
};
function Tt(t) {
  const e = [], i = I();
...
function ve(t, e, i, s, n) {
  ...
  if ((e = (t = t && t.get(e)) && t.length - s) && 0 < e)
    return (e > i || s) && (t = t.slice(s, s + i)), n && (t = J.call(this, t)), t;
}
function J(t) {
...
function K(t) {
  ...
  let i, s;
  if (this.F = [], this.field = [], this.J = [], this.key = (i = e.key || e.id) && mt(i, this.J) || "id", (s = t.keystore || 0) && (this.keystore = s), this.fastupdate = !!t.fastupdate, this.reg = !this.fastupdate || t.worker || t.db ? s ? new b(s) : /* @__PURE__ */ new Set() : s ? new $(s) : /* @__PURE__ */ new Map(), this.C = (i = e.store || null) && i && i !== !0 && [], this.store = i && (s ? new $(s) : /* @__PURE__ */ new Map()), this.cache = (i = t.cache || null) && new q(i), t.cache = !1, this.worker = t.worker || !1, this.priority = t.priority || 4, this.index = je.call(this, t, e), this.tag = null, (i = e.tag) && (typeof i == "string" && (i = [i]), i.length)) {
...
w.mount = function(t) {
  ...
  if (this.tag) for (let r = 0, h; r < this.R.length; r++) {
    h = this.R[r];
    var i = void 0;
    this.index.set(h, i = new C({}, this.reg)), e === this.field && (e = e.slice(0)), e.push(h), i.tag = this.tag.get(h);
  }
  i = [];
...
function je(t, e) {
  ...
  for (let n = 0, r, h; n < s.length; n++) {
    if (r = s[n], F(r) || (h = r, r = r.field), h = rt(h) ? Object.assign({}, t, h) : t, this.worker) {
      const o = new ot(h);
      o.encoder = h.encoder, i.set(r, o);
    }
    this.worker || i.set(r, new C(h, this.reg)), h.custom ? this.F[n] = h.custom : (this.F[n] = mt(r, this.J), h.filter && (typeof this.F[n] == "string" && (this.F[n] = new String(this.F[n])), this.F[n].I = h.filter)), this.field[n] = r;
  }
  if (this.C) {
    t = e.store, F(t) && (t = [t]);
...
const $t = { normalize: !1, numeric: !1, dedupe: !1 }, gt = {}, jt = /* @__PURE__ */ new Map([["b", "p"], ["v", "f"], ["w", "f"], ["z", "s"], ["x", "s"], ["d", "t"], ["n", "m"], ["c", "k"], ["g", "k"], ["j", "k"], ["q", "k"], ["i", "e"], ["y", "e"], ["u", "o"]]), Gt = /* @__PURE__ */ new Map([["ae", "a"], ["oe", "o"], ["sh", "s"], ["kh", "k"], ["th", "t"], ["ph", "f"], ["pf", "f"]]), Jt = [/([^aeo])h(.)/g, "$1$2", /([aeo])h([^aeo]|$)/g, "$1$2", /(.)\1+/g, "$1"], Ut = { a: "", e: "", i: "", o: "", u: "", y: "", b: 1, f: 1, p: 1, v: 1, c: 2, g: 2, j: 2, k: 2, q: 2, s: 2, x: 2, z: 2, ß: 2, d: 3, t: 3, l: 4, m: 5, n: 5, r: 6 };
var Me = { Exact: $t, Default: gt, Normalize: gt, LatinBalance: { mapper: jt }, LatinAdvanced: { mapper: jt, matcher: Gt, replacer: Jt }, LatinExtra: { mapper: jt, replacer: Jt.concat([/(?!^)[aeo]/g, ""]), matcher: Gt }, LatinSoundex: { dedupe: !1, include: { letter: !0 }, finalize: function(t) {
  for (let i = 0; i < t.length; i++) {
    var e = t[i];
    let s = e.charAt(0), n = Ut[s];
    for (let r = 1, h; r < e.length && (h = e.charAt(r), h === "h" || h === "w" || !(h = Ut[h]) || h === n || (s += h, n = h, s.length !== 4)); r++) ;
    t[i] = s;
  }
} }, CJK: { split: "" }, LatinExact: $t, LatinDefault: gt, LatinSimple: gt };
...
C.prototype.add = function(t, e, i, s) {
  if (e && (t || t === 0)) {
...
w.serialize = function(t = !0) {
  ...
  if (this.reg.size) {
    ...
    e = "index.reg=new Set([" + e + "]);", i = bt(this.map, r), i = "index.map=new Map([" + i + "]);";
...
w.enrich = function(t) {
  ...
  for (let s = 0; s < t.length; s++) i[s] = E(e.get(t[s]));
...
w.has = function(t) {
  return t = this.db.transaction("reg", "readonly").objectStore("reg").getKey(t), E(t).then(function(e) {
    return !!e;
  });
};
w.search = null;
...
w.transaction = function(t, e, i) {
  ...
  return this.h[t + ":" + e] = null, E(n).finally(function() {
    return n = s = null, r;
  });
};
