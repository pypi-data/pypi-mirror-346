...
def setup_dependencies(
    package_manager: str, requirements_path: str, repo_path=None, **kwargs
) -> ToolOutput:
    ...
    """
    try:
        working_dir = Path(repo_path) if repo_path else Path.cwd()
        print(f"Installing dependencies in {working_dir}")

        if package_manager == "pip":
            requirements_path = working_dir / requirements_path
