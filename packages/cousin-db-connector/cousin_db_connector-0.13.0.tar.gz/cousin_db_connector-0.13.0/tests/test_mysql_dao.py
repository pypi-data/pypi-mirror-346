...
async def test_query_raw_success():
    # Crear mocks explícitos con __aenter__/__aexit__
    ...
    dao = AsyncMySQLDAO(mock_pool, collection_name="dh_habitacion")
...
async def test_query_raw_raises_http_exception():
    # Crear mocks explícitos con __aenter__/__aexit__ y forzar error
    mock_cursor_cm = MagicMock()
    mock_cursor = AsyncMock()
    ...
    assert "MySQL error" in exc.value.detail