...
@click.pass_context
def create_test_run(ctx, name, test):
    ...
    async def _run():
        ...
        async with client:
            ...
            test_run = await client.test_runs.create_and_start(name=name, test=test)
...
def cancel_test_run(ctx, test_run_id):
    ...
    async def _run():
        ...
        async with client:
            ...
            cancelled_test = await client.test_runs.cancel(test_run_id_validated)
...
def download_attachments(ctx, test_run_id, output_dir):
    ...
    async def _run():
        ...
        async with client:
            ...
            # Download all attachments for the test run
