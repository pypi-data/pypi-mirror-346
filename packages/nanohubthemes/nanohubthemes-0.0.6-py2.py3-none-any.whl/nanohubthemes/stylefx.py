...
def set_mathjax_style(style_css, mathfontsize):
    ...
    """ % (int(mathfontsize), '"{}pt"'.format(str(mathfontsize)))
        ...
        function($, Jupyter, dialog) {
            ...
            try {
                if (window.location.hostname.includes("proxy")){
                    base = '/' + window.location.pathname.split('/').slice(1,5).join('/') + '/';
                } else {
                    throw new Error("No proxy");
                }
            } catch(err) {
                try {
                    base = Jupyter.notebook.base_url;
                } catch (err) {
                    base = IPython.notebook.base_url;
                }
            }
...
# hub_session_container
    ...
    with open(jupyter_customjs, 'w') as f:
        f.write(t.safe_substitute(uid=uid, shost=host,
                lhost=host.lower(), theme=getCurrentTheme()))
