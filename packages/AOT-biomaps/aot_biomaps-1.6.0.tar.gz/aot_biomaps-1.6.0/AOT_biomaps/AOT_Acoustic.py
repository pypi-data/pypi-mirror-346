...
class AcousticField(ABC):
    ...
    def load_field(self, filePath, formatSave=FormatSave.HDR_IMG):
        ...
        try:
            ...
            if self.params["typeSim"] == TypeSim.FIELD2:
                raise NotImplementedError("FIELD2 simulation is not implemented yet.")
            elif self.params["typeSim"] == TypeSim.KWAVE:
                if formatSave.value == FormatSave.HDR_IMG.value:
                    if self.params["dim"] == Dim.D2:
                        self.field = self._load_fieldKWAVE_XZ(filePath)
                    elif self.params["dim"] == Dim.D3:
            ...
            elif self.params["typeSim"] == TypeSim.HYDRO:
                if formatSave.value == FormatSave.HDR_IMG.value:
