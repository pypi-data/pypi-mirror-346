...
class AcousticField(ABC):
    ...
    def calculate_envelope_squared(self, isGPU = True if config.get_process() == 'gpu' else False):
        ...
        try:
            ...
            def process_slice(slice_indices, isGPU):
                """Calculate the envelope for a given slice of the acoustic field."""
                if isGPU:
                    hilbert_func = cp.hilbert
                else:
                    hilbert_func = hilbert

                if len(acoustic_field.shape) == 3:
                    return np.abs(hilbert_func(acoustic_field[slice_indices], axis=0))**2
                elif len(acoustic_field.shape) == 4:
                    envelope_slice = np.zeros_like(acoustic_field[slice_indices])
                    for y in range(acoustic_field.shape[2]):
                        for z in range(acoustic_field.shape[1]):
                            envelope_slice[:, z, y, :] = np.abs(hilbert_func(acoustic_field[slice_indices][:, z, y, :], axis=1))**2
            ...
            if isGPU:
                self.enveloppe = cp.concatenate(envelopes, axis=0)
            else:
                self.enveloppe = np.concatenate(envelopes, axis=0)
    ...
    def save_field(self, filePath):
        ...
        try:
            ...
            if extension == FormatSave.HDR_IMG:
                self._save2D_HDR_IMG(self, filePath)
            elif extension == FormatSave.H5:
                self._save2D_H5(self, filePath)
            elif extension == FormatSave.NPY:
                self._save2D_NPY(self, filePath)
            else:
