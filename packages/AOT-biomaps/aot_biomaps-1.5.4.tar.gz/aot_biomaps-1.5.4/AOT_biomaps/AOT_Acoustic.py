        ...
        try:
            ...
            def process_slice(slice_index, isGPU):
                ...
                if len(acoustic_field.shape) == 3:
                    return np.abs(hilbert_func(acoustic_field[slice_index], axis=0))**2
                    ...
                    for y in range(acoustic_field.shape[2]):
                        for z in range(acoustic_field.shape[1]):
                            envelope_slice[:, z, y, :] = np.abs(hilbert_func(acoustic_field[slice_index][:, z, y, :], axis=0))**2
            ...
            if isGPU:
                self.enveloppe = cp.stack(envelopes, axis=0)
            else:
