...
@pytest.mark.parametrize(
    "source, dest",
    [
        ("numpy ~= 1.19 ; python_version>'3.6'", "numpy~=1.19"),  # 处理环境标记前的空格
...
def test_normalization_cases(source, dest):
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"name": "requests", "specifier": "==2.25.1", "marker": "sys_platform=='linux'"},
        ),
        ("django~=3.2.0", {"name": "django", "specifier": "~=3.2.0"}),
    ],
)
def test_parse_valid_requirements(source, parsed):
