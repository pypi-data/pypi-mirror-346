...
def main():
    ...
    try:
        key = args.open_ai_key or os.environ["CHATGPT_API_KEY"]
    except KeyError:
        raise KeyError(
    ...
    # remove the first and last lines of the yaml
    slide_yaml = "\n".join(slide_yaml.split("\n")[1:-1])

    # Remove each line that begins with a backtick
    for line in slide_yaml.splitlines():
        if line.startswith("`"):
            slide_yaml = slide_yaml.replace(line, "")

    logger.info(f"Generated slide content: \n{slide_yaml}")
    print(slide_yaml)
