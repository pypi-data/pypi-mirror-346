...
class SettingsResolver:
    ...
    def set_settings(self, settings: SettingView) -> SettingView:
        ...
        if settings.keyboard_shortcut_mapping is not None:
            # Convert KeyboardShortcutMapping to dict before storing
            current_settings.keyboard_shortcut_mapping = (
                # TODO (Igor, 05/08/2025): Fix this type error
                settings.keyboard_shortcut_mapping.get_mapping()  # type: ignore
            )

        # Update show annotation text labels
        if settings.show_annotation_text_labels is not None:
            current_settings.show_annotation_text_labels = (
                settings.show_annotation_text_labels
            )
