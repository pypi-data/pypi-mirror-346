...
class ReadSamplesBody(BaseModel):
...
def read_samples(
    ...
    body: ReadSamplesBody | None = None,
) -> list[SampleView]:
    """Retrieve a list of samples from the database with optional filtering.

    Args:
        handler: The sample resolver instance.
        query: Query parameters for filtering samples.
        body: Optional request body containing text embedding.
        _dataset: The validated dataset.

    Returns:
        A list of filtered samples.
    """
    params = query.model_dump()
    if body and body.text_embedding:
        params["text_embedding"] = body.text_embedding
    return handler.get_all_by_dataset_id(**params)
