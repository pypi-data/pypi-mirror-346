...
def get_template(template_name):
    ...
    """
    try:
        file_path = os.path.join(os.path.dirname(__file__), "templates", template_name + ".json")
        with open(file_path, "r") as f:
            template = json.load(f)
        return template
    except Exception as e:
        raise ValueError("Could not read template with name at path: {0}".format(file_path))
