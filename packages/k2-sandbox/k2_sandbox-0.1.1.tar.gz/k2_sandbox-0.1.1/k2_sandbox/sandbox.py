...
class BaseSandbox:
    ...
    @classmethod
    def set_global_default_api_base_url(cls, url: str) -> None:
        """
        Sets the global default API base URL for all new BaseSandbox instances
        that do not explicitly receive an api_base_url or have the
        K2_API_BASE_URL environment variable set.

        This should typically be called once at the initialization of an application
        using the SDK.

        Args:
            url: The new default API base URL (e.g., "http://my-api.com").
