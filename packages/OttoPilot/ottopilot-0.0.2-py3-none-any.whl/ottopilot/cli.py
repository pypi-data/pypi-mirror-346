...
from ottopilot.log_utils import setup_logging

# Setup application and logging
app = typer.Typer(help="OttoPilot")
...
def main(
    ...
    # Only add dangerously-skip-permissions if not running as root in a container
    # This prevents the security error when running in GitHub Actions with act
    is_github_actions = os.environ.get("GITHUB_ACTIONS") or os.environ.get("ACT")
    is_root = os.geteuid() == 0 if hasattr(os, "geteuid") else False

    if not (is_github_actions and is_root):
