...
def get_distances_np(trg_signature_dictionary,
    ...
    if use_abs_dist:
        ...
        if use_min_max_ratios:           
            diff = cp.where(mask_both,
                            1.0 - (cp.minimum(X, target_vec) / cp.maximum(X, target_vec)),
                            mismatch_penalty)
        else:
            safe_min = cp.where(mask_both, cp.minimum(X, target_vec) + eps, 1.0)
            
            diff = cp.where(mask_both,
                            (cp.maximum(X, target_vec) / safe_min) - 1.0,
                            mismatch_penalty)
    
    union_mask = (X > 0) | (target_vec > 0)
    
    sum_term = cp.sum((diff ** p) * union_mask, axis=1)
    
    return cp.cbrt(sum_term) if p == 3 else cp.power(sum_term, 1.0 / p)
