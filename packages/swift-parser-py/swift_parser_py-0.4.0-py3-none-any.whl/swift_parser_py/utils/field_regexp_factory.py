...
class FieldRegexpFactory:
    ...
    def _visit_field(self, node: Dict[str, Any]) -> str:
        ...
        if lines > 1:
            ...
            if "name" in node and ("Name_and_Address" in node.get("name", "") or
                                   "Narrative" in node.get("name", "")):
                # Capture multiple lines for name/address fields
                # Allow any character including newlines for address fields
