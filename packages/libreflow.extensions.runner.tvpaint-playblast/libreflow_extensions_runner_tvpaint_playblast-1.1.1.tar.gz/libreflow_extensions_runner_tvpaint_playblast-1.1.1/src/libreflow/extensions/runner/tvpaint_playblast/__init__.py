...
class ExportLayersToComp(flow.Action):
    ...
    def run(self,button):
        ...
        output_path = os.path.join(self.ensure_layers_folder_revision().get_path(),output_name)

        if os.path.exists(os.path.split(output_path)[0]):
            remove_folder_content(os.path.split(output_path)[0])


        self.execute_render_script(output_path)
...
class ExecuteExportLayersScript(GenericRunAction):
    ...
    def extra_argv(self):
        current_dir = os.path.split(__file__)[0]
        script_path = os.path.normpath(os.path.join(current_dir,"scripts/export_layers.py"))
...
def export_layers_to_comp(parent):
    if isinstance(parent, TrackedFile) \
        and (parent.name().endswith('_tvpp')):
        r = flow.Child(ExportLayersToComp)
        r.name = 'export_layers_to_comp'
        r.index = None
        return r
