...
class TRUCKEEFS(RiverFS, pyfuse3.Operations):
	...
	async def flush(this, fh):
		...
		try:
			...
			logging.error(f"flush error on fh {fh}: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
	...
	async def fsync(this, fh, datasync):
		...
		try:
			...
			logging.error(f"fsync error on fh {fh}: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
	...
	async def read(this, fh, off, size):
		...
		try:
			...
			logging.error(f"read error on fh {fh}: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
	...
	async def release(this, fh):
		...
		try:
			...
			logging.error(f"release error on fh {fh}: {e}", exc_info=True)
	...
	async def releasedir(this, fh):
		...
		try:
			...
			logging.error(f"releasedir error on fh {fh}: {e}", exc_info=True)
	...
	async def rmdir(this, parent_inode, name, ctx):
		...
		try:
			...
			logging.error(f"rmdir error: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
	...
	async def unlink(this, parent_inode, name, ctx):
		...
		try:
			...
			logging.error(f"unlink error: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
	...
	async def write(this, fh, off, buf):
		...
		try:
			...
			logging.error(f"write error on fh {fh}: {e}", exc_info=True)
			raise pyfuse3.FUSEError(errno.EIO)
