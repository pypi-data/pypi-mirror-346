...
from pyninaapiio.api.application import get_version
from pyninaapiio.api.camera import get_equipment_camera_info
from pyninaapiio.api.filter_wheel import get_equipment_filterwheel_info
...
from pyninaapiio.models.fw_info import FWInfo
...
from pyninaapiio.models.get_version_response_200 import GetVersionResponse200
...
class NinaAPI:
    ...
    async def filterwheel_info(self):
        try:
            _LOGGER.debug(f"Retrieve filterwheel info")
            ...
            traceback.print_exc()
            _LOGGER.warning(f"FilterWheel not connected. {ke}")
    ...
    async def nina_info(
        ...
        try:
            ...
            _LOGGER.debug(f"Nina not available.")
        except KeyError as ke:
            traceback.print_exc()
            _LOGGER.debug(f"Nina not connected. {ke}")
