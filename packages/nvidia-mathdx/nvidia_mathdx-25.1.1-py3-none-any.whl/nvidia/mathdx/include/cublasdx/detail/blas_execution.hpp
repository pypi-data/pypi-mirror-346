    ...
    namespace detail {
        ...
        class blas_block_execution: public blas_execution<Operators...>
        {
            ...
            using execution_suggestions =
                cute_backend::execution<typename this_cutlass_value_type::a_type,
                                        ...
                                        void,
                                        typename base_type::this_blas_overloaded_tile
                                        >;
            ...
            using default_blas_block_dim        = BlockDim<execution_suggestions::suggested_threads>;
            ...
            static constexpr CUBLASDX_HOST_DEVICE dim3 get_suggested_block_dim() {
                ...
                return execution_suggestions::suggested_threads;
            }
            ...
            CUBLASDX_HOST_DEVICE constexpr static auto tag_cute_layout(LD ld = {}) {
                return pointer_layout {MemTag {}, get_cute_layout<M, MemTag>(ld)};
            }
            ...
            suggest_cute_layout_smem() {
                ...
                if constexpr(base_type::has_overloaded_tile) {
                    return get_cute_layout<M, smem_tag>();
                } else {
                    using a_type = typename this_blas_value_type::a_type;
                    using b_type = typename this_blas_value_type::b_type;
                    using c_type = typename this_blas_value_type::c_type;
            ...
            tag_suggested_cute_layout_smem() {
                return pointer_layout {smem_tag {}, suggest_cute_layout_smem<M>()};
            }
        ...
        public:
            ...
            get_layout_gmem_a(Ts ... ts) {
                return tag_cute_layout<matrix::A, gmem_tag>(ts...);
            }

            template<typename ... Ts>
            CUBLASDX_HOST_DEVICE constexpr static auto
            get_layout_gmem_b(Ts ... ts) {
                return tag_cute_layout<matrix::B, gmem_tag>(ts...);
            }

            template<typename ... Ts>
            CUBLASDX_HOST_DEVICE constexpr static auto
            get_layout_gmem_c(Ts ... ts) {
                return tag_cute_layout<matrix::C, gmem_tag>(ts...);
            }

            template<typename ... Ts>
            CUBLASDX_HOST_DEVICE constexpr static auto
            get_layout_smem_a(Ts ... ts) {
                return tag_cute_layout<matrix::A, smem_tag>(ts...);
            }

            template<typename ... Ts>
            CUBLASDX_HOST_DEVICE constexpr static auto
            get_layout_smem_b(Ts ... ts) {
                return tag_cute_layout<matrix::B, smem_tag>(ts...);
            }

            template<typename ... Ts>
            CUBLASDX_HOST_DEVICE constexpr static auto
            get_layout_smem_c(Ts ... ts) {
                return tag_cute_layout<matrix::C, smem_tag>(ts...);
            }
