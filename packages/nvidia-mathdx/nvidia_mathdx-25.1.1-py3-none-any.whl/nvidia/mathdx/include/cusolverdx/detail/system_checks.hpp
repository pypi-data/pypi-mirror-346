...
// We require target architecture to be Volta+ (only checking on device)
    ...
    // NVRTC version check
            ...
            #error cuSolverDx requires NVRTC from CUDA Toolkit 12.0 or newer
        #endif
    #endif // CUSOLVERDX_IGNORE_DEPRECATED_COMPILER

    // NVRTC compilation checks
    #ifndef CUSOLVERDX_IGNORE_DEPRECATED_COMPILER
static_assert((__CUDACC_VER_MAJOR__ >= 12), "cuSolverDx requires CUDA Runtime 12.0 or newer to work with NVRTC");
    #endif // CUSOLVERDX_IGNORE_DEPRECATED_COMPILER
...
static_assert(CUDART_VERSION >= 12000, "cuSolverDx requires CUDA Runtime 12.0 or newer");
static_assert(CUDA_VERSION >= 12000, "cuSolverDx requires CUDA Toolkit 12.0 or newer");
    #ifdef __NVCC__
static_assert((__CUDACC_VER_MAJOR__ >= 12), "cuSolverDx requires NVCC 12.0 or newer");
    #endif
        ...
        #ifdef __NVCOMPILER
            #error "cuSolverDx 0.1.0 does not support nvc++"
        #endif
