...
def get_task_context_and_repport(db: Session, task_id: str = None, limit: int = 20):
    query = (
...
def get_task_context_reports_and_format(db: Session, task_id: str = None, limit: int = 20):
    rows = get_task_context_and_repport(db=db, task_id=task_id, limit=limit)
    ...
    return [
        {
            ...
            "notify_config": context.notify_config,
...
async def list_tasks(db: Session = Depends(get_db)):
    return get_task_context_reports_and_format(db)
...
def retry_task(task_id: str, request: Request, db=Depends(lambda: next(get_db()))):
    # todo: maybe rework on this endpoint
    ...
    try:
        args = json.loads(context.args or "[]")
        kwargs = json.loads(context.kwargs or "{}")
        notify = json.loads(context.notify_config or "[]")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to parse context: {e}")
    ...
    new_task_id = runner.submit(
        ...
        args=args,
        kwargs=kwargs,
        task_name=f"{context.task_name} (retry)",
        parent_task_id=task_id,
        notify_config=notify
    )

    return RedirectResponse(
        url=request.url_for("task_details", task_id=new_task_id),
        status_code=303
    )
