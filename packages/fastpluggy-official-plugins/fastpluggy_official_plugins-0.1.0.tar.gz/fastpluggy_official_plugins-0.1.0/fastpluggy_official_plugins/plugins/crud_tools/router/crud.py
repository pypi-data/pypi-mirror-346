...
def ensure_model_is_loaded(model_name, fast_pluggy):
    return
    # for item in fast_pluggy.get_manager().modules.values():
    #     for model_items in item.models:
    #         if model_name == model_items['class_name']:
    #             logger.info(f'Model {model_name} found in {item.name}')
    #             get_module(module_name=model_items['module_name'], reload=False)
    #             return


def get_admin_instance(model_name: Type):
    registry = FastPluggy.get_global("crud_admin_registry")
    if registry is not None:
        admin = registry.get_admin(model_name)
        if admin:
            return admin
    return CrudAdmin(model=model_name)
...
async def create_item(model_name: str, request: Request, db: Session = Depends(get_db),
    ...
    ensure_model_is_loaded(model_name=model_name, fast_pluggy=fast_pluggy)
