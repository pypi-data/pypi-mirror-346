...
class InsightConfig:
    ...
    def detect_insight_type(self, source_data: Dict[str, Any]) -> str:
        """
        Detect the insight type based on source data.
        
        Args:
            source_data: The source data for the insight
            
        Returns:
            The detected insight type
        """
        source_type = source_data.get('type', '')
        
        for insight_type, patterns in self.insight_type_patterns.items():
            if any(pattern in source_type for pattern in patterns):
                return insight_type
            
            # Check source data keys
            if any(pattern in source_data for pattern in patterns):
                return insight_type
