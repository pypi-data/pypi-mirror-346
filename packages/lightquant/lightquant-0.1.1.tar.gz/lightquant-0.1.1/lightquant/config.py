...
class Config:
    ...
    def get_commission_by_date(self, target_date, key_path, default=None):
        """
        Get a value from the commission data for a specific target date
        using dot-separated key path.

        :param target_date: The date for which the commission data
        is needed (YYYY-MM-DD).
...
if __name__ == "__main__":
    ...
    shoonya_fut_buy_flat_entry = config.get_commission_by_date(
        entry_date, "SHOONYA.FUT.BUY.flat"
    )
    ...
    print(
        f"FIVEPAISA OPT SHORT Exchange (Exit Date): {fivepaisa_opt_short_exchange_exit}"
    )
