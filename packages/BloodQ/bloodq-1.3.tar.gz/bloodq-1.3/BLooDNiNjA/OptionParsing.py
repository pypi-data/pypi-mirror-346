...
do it, but updating and checking module "BLooDNiNjA.Options" values is not in
...
from BLooDNiNjA.PythonVersions import isPythonWithGil
from BLooDNiNjA.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from BLooDNiNjA.utils.FileOperations import getFileContentByLine
...
# Indicator if we were called as "BLooDNiNjA-run" in which case we assume some
# other defaults and work a bit different with parameters.
_BLooDNiNjA_binary_name = os.path.basename(sys.argv[0])
if _BLooDNiNjA_binary_name == "__main__.py":
    _BLooDNiNjA_binary_name = "%s -m BLooDNiNjA" % os.path.basename(sys.executable)
is_BLooDNiNjA_run = _BLooDNiNjA_binary_name.lower().endswith("-run")

if not is_BLooDNiNjA_run:
    usage_template = (
...
parser = makeOptionsParser(usage=usage_template % _BLooDNiNjA_binary_name)
...
with "BLooDNiNjA-onefile-unpack" rather than a stream that only the onefile
...
def _considerPluginOptions(logger):
    ...
    from BLooDNiNjA.plugins.Plugins import (
...
def getBloodQProjectOptions(logger, filename_arg, module_mode):
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)BLooDNiNjA-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'BLooDNiNjA-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'BLooDNiNjA-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'BLooDNiNjA-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'BLooDNiNjA-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'BLooDNiNjA-project-else' not currently allowed after nested BLooDNiNjA-project-if.",
                    )
...
pass them to the compiled program execution."""
    ...
    return is_BLooDNiNjA_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from BLooDNiNjA.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from BLooDNiNjA.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from BLooDNiNjA.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from BLooDNiNjA.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.list_distribution_metadata:
        from BLooDNiNjA.tools.scanning.DisplayDistributions import (
    ...
    if options.edit_module_code:
        from BLooDNiNjA.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from BLooDNiNjA.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from BLooDNiNjA.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
