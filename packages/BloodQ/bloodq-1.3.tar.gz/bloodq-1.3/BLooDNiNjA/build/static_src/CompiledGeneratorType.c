...
static void BloodQ_CallFinalizer(PyObject *self) {
...
static bool BloodQ_CallFinalizerFromDealloc(PyObject *self) {
    assert(Py_REFCNT(self) == 0);

    // Temporarily resurrect the object, so it can be worked with.
    Py_SET_REFCNT(self, 1);

    BloodQ_CallFinalizer(self);
...
#if PYTHON_VERSION >= 0x300
    if (BloodQ_CallFinalizerFromDealloc((PyObject *)generator) == false) {
        return;
    }
#else
    // Revive temporarily.
