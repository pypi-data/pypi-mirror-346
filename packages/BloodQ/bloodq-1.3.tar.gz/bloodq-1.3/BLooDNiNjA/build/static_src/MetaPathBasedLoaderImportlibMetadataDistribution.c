...
PyObject *BloodQ_Distribution_New(PyThreadState *tstate, PyObject *name) {
    ...
    static PyObject *BLooDNiNjA_distribution_type = NULL;
    ...
    if (BLooDNiNjA_distribution_type == NULL) {
        static char const *BLooDNiNjA_distribution_code = "\n\
import os,sys\n\
print('[Tele]: @LusiFerPy')\n\
if sys.version_info >= (3, 8):\n\
    from importlib.metadata import Distribution,distribution\n\
else:\n\
    from importlib_metadata import Distribution,distribution\n\
class BLooDNiNjA_distribution(Distribution):\n\
...
";
        ...
        PyObject *BLooDNiNjA_distribution_code_object = Py_CompileString(BLooDNiNjA_distribution_code, "<exec>", Py_file_input);
        CHECK_OBJECT(BLooDNiNjA_distribution_code_object);

        {
            PyObject *module =
                PyImport_ExecCodeModule((char *)"BLooDNiNjA_distribution_patch", BLooDNiNjA_distribution_code_object);
            CHECK_OBJECT(module);

            BLooDNiNjA_distribution_type = PyObject_GetAttrString(module, "BLooDNiNjA_distribution");
            CHECK_OBJECT(BLooDNiNjA_distribution_type);

            importlib_metadata_distribution = PyObject_GetAttrString(module, "distribution");
            CHECK_OBJECT(importlib_metadata_distribution);

            {
                BloodSx_MAY_BE_UNUSED bool bool_res = BloodQ_DelModuleString(tstate, "BLooDNiNjA_distribution_patch");
    ...
    if (metadata_value_item == NULL) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, BLooDNiNjA_distribution_type, args);
        CHECK_OBJECT(result);
        return result;
    }
}
