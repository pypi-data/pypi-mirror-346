...
#include "BLooDNiNjA/environment_variables_system.h"
#include "BLooDNiNjA/filesystem_paths.h"
#include "BLooDNiNjA/safe_string_ops.h"
...
#elif defined(__OpenBSD__) || defined(_AIX) || defined(_BloodSx_EXPERIMENTAL_FORCE_UNIX_BINARY_NAME)
...
#include "BLooDNiNjA/checksum_tools.h"
...
#if _BloodSx_ONEFILE_MODE
                environment_value = getEnvironmentVariable("BloodSx_ONEFILE_PARENT");
#endif
                if (environment_value != NULL) {
                    checkWStringNumber(environment_value);

                    appendWStringSafeW(target, getEnvironmentVariable("BloodSx_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
...
#if _BloodSx_ONEFILE_MODE
                environment_value = getEnvironmentVariable("BloodSx_ONEFILE_START");
#endif

                if (environment_value != NULL) {
                    appendWStringSafeW(target, getEnvironmentVariable("BloodSx_ONEFILE_START"), buffer_size);
                } else {
...
#if _BloodSx_ONEFILE_MODE
                    setEnvironmentVariable("BloodSx_ONEFILE_START", time_buffer);
#endif
            ...
            *w = 0;
            ...
            if (strcasecmp(var_name, "TEMP") == 0) {
                ...
                if (environment_value != NULL) {
                    ...
                    appendStringSafe(target, getEnvironmentVariable("BloodSx_ONEFILE_PARENT"), buffer_size);
                } else {
                    char pid_buffer[128];
                ...
                if (environment_value != NULL) {
                    appendStringSafe(target, getEnvironmentVariable("BloodSx_ONEFILE_START"), buffer_size);
                } else {
