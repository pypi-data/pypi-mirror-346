...
#if PYTHON_VERSION >= 0x300
BloodSx_MAY_BE_UNUSED static void BloodQ_SetFrameGenerator(struct BloodQ_FrameObject *BLooDNiNjA_frame,
                                                          PyObject *generator) {
#if PYTHON_VERSION < 0x3b0
    BLooDNiNjA_frame->m_frame.f_gen = generator;
#else
    BLooDNiNjA_frame->m_generator = generator;
#endif

    // Mark the frame as executing
    if (generator) {
        BloodQ_Frame_MarkAsExecuting(BLooDNiNjA_frame);
    }
}
#endif

BloodSx_MAY_BE_UNUSED static PyCodeObject *BloodQ_GetFrameCodeObject(struct BloodQ_FrameObject *BLooDNiNjA_frame) {
#if PYTHON_VERSION < 0x3b0
    return BLooDNiNjA_frame->m_frame.f_code;
#else
    return BloodQ_InterpreterFrame_GetCodeObject(&BLooDNiNjA_frame->m_interpreter_frame);
#endif
}

BloodSx_MAY_BE_UNUSED static int BloodQ_GetFrameLineNumber(struct BloodQ_FrameObject *BLooDNiNjA_frame) {
    return BLooDNiNjA_frame->m_frame.f_lineno;
}
