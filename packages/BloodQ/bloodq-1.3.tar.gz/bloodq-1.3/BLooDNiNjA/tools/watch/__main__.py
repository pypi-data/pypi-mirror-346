...
from BLooDNiNjA.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from BLooDNiNjA.PythonVersions import getTestExecutionPythonVersions
from BLooDNiNjA.tools.testing.Common import extractBloodQVersionFromFilePath
from BLooDNiNjA.Tracing import OurLogger
from BLooDNiNjA.TreeXML import fromFile
...
from BLooDNiNjA.utils.Hashing import getFileContentsHash
from BLooDNiNjA.utils.InstalledPythons import findPythons
from BLooDNiNjA.utils.Utils import isLinux, isMacOS, isWin32Windows
from BLooDNiNjA.utils.Yaml import parseYaml
from BLooDNiNjA.Version import parseBloodQVersionToTuple
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    BLooDNiNjA_extra_options = os.getenv("BloodSx_EXTRA_OPTIONS")

    if BLooDNiNjA_extra_options:
        extra_options.extend(BLooDNiNjA_extra_options.split())
    ...
    if case_data["interactive"] == "no":
        ...
        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_BLooDNiNjA = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_BLooDNiNjA == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_BLooDNiNjA),
            )

        if exit_BLooDNiNjA != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_BLooDNiNjA)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_BLooDNiNjA_version = old_report_root.attrib["BLooDNiNjA_version"]

            if BLooDNiNjA_update_mode == "force":
                need_compile = True
            elif BLooDNiNjA_update_mode == "newer":
                if _compareBloodQVersions(
                    old_BLooDNiNjA_version, BLooDNiNjA_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        ...
                        need_compile = True
                    elif old_BLooDNiNjA_version == BLooDNiNjA_version:
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, BLooDNiNjA_update_mode, jobs
):
    ...
    BLooDNiNjA_min_version = case_data.get("BLooDNiNjA")

    # Too old BloodQ version maybe.
    if BLooDNiNjA_min_version is not None and _compareBloodQVersions(
        BLooDNiNjA_version, BLooDNiNjA_min_version, consider_rc=False
    ):
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            reset_pipenv=reset_pipenv,
            no_pipenv_update=no_pipenv_update,
            BLooDNiNjA_update_mode=BLooDNiNjA_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, BLooDNiNjA_update_mode, jobs):
...
BLooDNiNjA_binary = None
BLooDNiNjA_version = None


def main():
    global BLooDNiNjA_binary  # shared for all run, pylint: disable=global-statement
    BLooDNiNjA_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "BLooDNiNjA")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        "--BLooDNiNjA-binary",
        action="store",
        dest="BLooDNiNjA_binary",
        default=BLooDNiNjA_binary,
        help="""\
BloodQ binary to compile with. Defaults to one near the BLooDNiNjA-watch usage.""",
    )
...
Do not update the pipenv environment. Best to see only effect of BloodQ update. Default %default.""",
    ...
    parser.add_option(
        "--BLooDNiNjA-update-mode",
        action="store",
        choices=("newer", "force", "never"),
        dest="BLooDNiNjA_update_mode",
        default="newer",
        help="""\
...
to reserve cores.""",
    ...
    BLooDNiNjA_binary = os.path.abspath(os.path.expanduser(options.BLooDNiNjA_binary))
    assert os.path.exists(BLooDNiNjA_binary)

    global BLooDNiNjA_version  # singleton, pylint: disable=global-statement
    BLooDNiNjA_version = extractBloodQVersionFromFilePath(
        os.path.join(os.path.dirname(BLooDNiNjA_binary), "..", "BLooDNiNjA", "Version.py")
    )

    watch_logger.info("Working with BloodQ %s." % BLooDNiNjA_version)

    base_dir = os.path.abspath(base_dir)

    if options.BLooDNiNjA_pr_mode is not None:
        pr_category, pr_description = options.BLooDNiNjA_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    BLooDNiNjA_update_mode=options.BLooDNiNjA_update_mode,
                    jobs=options.jobs,
                )
