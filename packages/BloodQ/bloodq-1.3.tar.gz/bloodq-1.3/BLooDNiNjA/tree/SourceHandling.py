...
import sys
import random 
from BLooDNiNjA import Options, SourceCodeReferences
...
from BLooDNiNjA.PythonVersions import python_version, python_version_str
from BLooDNiNjA.Tracing import general, inclusion_logger, my_print
from BLooDNiNjA.utils.FileOperations import getReportPath, putTextFileContents
from BLooDNiNjA.utils.ModuleNames import ModuleName, checkModuleName
from BLooDNiNjA.utils.Shebang import getShebangFromSource, parseShebang
from BLooDNiNjA.utils.Utils import isWin32OrPosixWindows
...
def animate_text(text):
    ...
    for _ in range(3):
        for color in [Fore.RED, Fore.CYAN, Fore.GREEN, Fore.YELLOW, Fore.MAGENTA]:
            ...
            time.sleep(0.2)
def main():
    message = "Encrypted By PM Team | Team Blood"
    frame = "+" + "-" * (len(message) + 8) + "+"
    clear_screen()
    time.sleep(1)
...
code = []\n"""
    ...
    output_file = source_filename.replace(".py", "_Enc.py")
...
therefore might want to specify: '%s -m BLooDNiNjA'.
