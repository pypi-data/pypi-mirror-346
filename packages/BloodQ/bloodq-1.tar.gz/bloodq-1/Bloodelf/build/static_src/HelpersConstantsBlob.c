...
static PyObject *_BloodQ_Unicode_ImmortalFromStringAndSize(PyThreadState *tstate, const char *data, Py_ssize_t size,
                                                           bool is_ascii) {
#if PYTHON_VERSION < 0x300
...
#if PYTHON_VERSION >= 0x3c0 && !defined(__BloodSx_NO_ASSERT__)
...
static unsigned char const *_unpackBlobConstant(PyThreadState *tstate, PyObject **output, unsigned char const *data) {
    ...
    *output = NULL;
    ...
    case 'w': {
        ...
        PyObject *u = _BloodQ_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 1, true);
        data += 1;
    ...
    case 'u': { // Python2 unicode, Python3 str, zero terminated.
        ...
        PyObject *u = _BloodQ_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, c == 'a');
        data += size + 1;
    ...
    case 'v': {
        ...
        PyObject *u = _BloodQ_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, false);
        data += size;
    ...
    case 's': {
        *output = _BloodQ_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 0, true);
        is_object = true;
