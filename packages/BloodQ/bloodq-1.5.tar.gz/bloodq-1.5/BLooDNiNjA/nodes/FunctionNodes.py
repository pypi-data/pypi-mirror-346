...
from BLooDNiNjA.Constants import isMutable
from BLooDNiNjA.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from BLooDNiNjA.PythonVersions import python_version
from BLooDNiNjA.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from BLooDNiNjA.Tracing import optimization_logger, printError
from BLooDNiNjA.tree.Extractions import updateVariableUsage
from BLooDNiNjA.tree.SourceHandling import readSourceLines
from BLooDNiNjA.tree.TreeHelpers import makeDictCreationOrConstant2
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from BLooDNiNjA.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from BLooDNiNjA.optimizations.FunctionInlining import (
