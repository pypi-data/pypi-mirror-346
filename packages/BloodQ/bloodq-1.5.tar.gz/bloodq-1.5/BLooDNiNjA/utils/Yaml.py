...
from BLooDNiNjA.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.BLooDNiNjA-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "BLooDNiNjA.plugins.standard",
                "stdlib2.BLooDNiNjA-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "BLooDNiNjA.plugins.standard",
                "stdlib3.BLooDNiNjA-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "BLooDNiNjA.plugins.commercial", "commercial.BLooDNiNjA-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    ...
    return os.path.join(
        ...
        "..",
        "misc",
        "BLooDNiNjA-package-config-schema.json",
    )
