...
from BLooDNiNjA import Tracing
from BLooDNiNjA.__past__ import urlretrieve
from BLooDNiNjA.Progress import withBloodQDownloadProgressBar
...
def getCachedDownload(
    ...
    BLooDNiNjA_download_dir = getDownloadCacheDir()

    BLooDNiNjA_download_dir = os.path.join(
        BLooDNiNjA_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        BLooDNiNjA_download_dir = os.path.join(BLooDNiNjA_download_dir, is_arch_specific)

    if specificity:
        BLooDNiNjA_download_dir = os.path.join(BLooDNiNjA_download_dir, specificity)

    download_path = os.path.join(BLooDNiNjA_download_dir, os.path.basename(url))
    exe_path = os.path.join(BLooDNiNjA_download_dir, binary)

    makePath(BLooDNiNjA_download_dir)
...
Fully automatic, cached. Proceed and download"""
                % (message, BLooDNiNjA_download_dir),
                choices=("yes", "no"),
                default="yes",
        ...
        if not os.path.isfile(exe_path) and os.path.isfile(download_path):
            ...
            try:
                ...
                for zip_info in zip_file.infolist():
                    ...
                    zip_file.extract(zip_info, BLooDNiNjA_download_dir)
