...
#if defined(__APPLE__)
#define _BloodSx_MACOS_BUNDLE_MODE 1
#endif
#endif
...
#include "BLooDNiNjA/environment_variables.h"
...
#include "BLooDNiNjA_windows_service.h"
...
#include <signal.h>
static void BLooDNiNjA_segfault_handler(int sig) {
    puts("BloodQ: A segmentation fault has occurred. This is highly unusual and can");
    puts("have multiple reasons. Please check https://BLooDNiNjA.net/info/segfault.html");
    puts("for solutions.");
...
static int BloodQ_Main(int argc, native_command_line_argument_t **argv) {
...
#if !defined(_BloodSx_DEPLOYMENT_MODE) && !defined(_BloodSx_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, BLooDNiNjA_segfault_handler);
#endif
...
// Workaround older Python not handling stream setup on redirected files properly.
...
#else
    {
        ...
        if (python_path_cstr != NULL) {
            ...
            BloodQ_SysSetObject("path", python_path_list);
    ...
    PyList_SetSlice(BloodQ_SysGetObject("path"), 0, 1, PyTuple_New(0));
#endif
...
int __stdcall wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, wchar_t *lpCmdLine, int nCmdShow) {
    ...
    return BloodQ_Main(argc, argv);
}
#else
#if defined(_WIN32)
int wmain(int argc, wchar_t **argv) { return BloodQ_Main(argc, argv); }
#else
int main(int argc, char **argv) { return BloodQ_Main(argc, argv); }
#endif
#endif

#if _BloodSx_DLL_MODE

#if defined(_WIN32)
#define BloodSx_DLL_FUNCTION __declspec(dllexport)
#else
#define BloodSx_DLL_FUNCTION __attribute__((visibility("default")))
#endif

BloodSx_DLL_FUNCTION int run_code(int argc, native_command_line_argument_t **argv) { return BloodQ_Main(argc, argv); }
#endif
