...
#define INCREF_KEYS_FT(dk) BloodQ_Py_dictkeys_incref(dk)
#define DECREF_KEYS_FT(dk, shared) BloodQ_Py_dictkeys_decref(_PyInterpreterState_GET(), dk, shared)
...
static inline PyDictValues *_BloodQ_PyDict_new_values(Py_ssize_t size) {
    ...
    PyDictValues *result = (PyDictValues *)BloodQMem_Malloc(n);
...
static PyDictValues *_BloodQ_PyDict_copy_values(PyDictValues *values) {
    PyDictValues *new_values = _BloodQ_PyDict_new_values(values->capacity);
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (BloodQ_Py_unicode_get_hash(ep->me_key) == hash) {
                ...
                Py_INCREF(startkey);
                BLooDNiNjA_bool cmp = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(startkey, key);
                Py_DECREF(startkey);
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                BLooDNiNjA_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(startkey, key);
                Py_DECREF(startkey);
...
#ifdef Py_GIL_DISABLED
            if (kind == DICT_KEYS_SPLIT) {
                ix = BloodQ_Py_unicodekeys_lookup_unicode_threadsafe(dk, key, hash);
...
uint64_t BLooDNiNjA_dict_version_tag_counter = ((uint64_t)1) << 32;
#endif
