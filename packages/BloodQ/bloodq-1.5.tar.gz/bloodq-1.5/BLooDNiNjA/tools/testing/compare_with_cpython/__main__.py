...
from BLooDNiNjA.OptionParsing import getBloodQProjectOptions
...
from BLooDNiNjA.tools.testing.OutputComparison import compareOutput
from BLooDNiNjA.Tracing import my_print
from BLooDNiNjA.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from BLooDNiNjA.utils.FileOperations import deleteFile
from BLooDNiNjA.utils.Importing import getExtensionModuleSuffix
from BLooDNiNjA.utils.Timing import StopWatch
from BLooDNiNjA.utils.Utils import isMacOS
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "BloodSx" in os.environ:
        ...
        BLooDNiNjA_call = [os.environ["PYTHON"], os.environ["BloodSx"]]
    else:
        if comparison_mode:
            BLooDNiNjA_call = [
                os.environ["PYTHON"],
                "-m",
                "BLooDNiNjA.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if not two_step_execution:
        ...
        BLooDNiNjA_cmd = BLooDNiNjA_call + extra_options + ["--run", filename]

        if no_site:
            BLooDNiNjA_cmd.insert(len(BLooDNiNjA_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            BLooDNiNjA_cmd1 = BLooDNiNjA_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            BLooDNiNjA_cmd1 = BLooDNiNjA_call + extra_options + ["--mode=standalone", filename]
        else:
            BLooDNiNjA_cmd1 = BLooDNiNjA_call + extra_options + [filename]

        if no_site:
            BLooDNiNjA_cmd1.insert(len(BLooDNiNjA_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        BLooDNiNjA_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        BLooDNiNjA_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("BloodQ command", BLooDNiNjA_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(BLooDNiNjA_package_dir):
                process = createProcess(command=BLooDNiNjA_cmd)

            stdout_BLooDNiNjA, stderr_BLooDNiNjA = process.communicate()
            exit_BLooDNiNjA = process.returncode

            if checkNoPermissionError(stdout_BLooDNiNjA) and checkNoPermissionError(
                stderr_BLooDNiNjA
            ):
                break

            my_print("Retrying BLooDNiNjA exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("BloodQ command 1:", BLooDNiNjA_cmd1)

        for _i in range(5):
            with withPythonPathChange(BLooDNiNjA_package_dir):
                stdout_BLooDNiNjA1, stderr_BLooDNiNjA1, exit_BLooDNiNjA1 = executeProcess(
                    BLooDNiNjA_cmd1
                )

                python_path_used = os.environ["PYTHONPATH"]

            if exit_BLooDNiNjA1 != 0:
                if (
                    not expect_failure
...
%s
"""
                        % (BLooDNiNjA_cmd1, python_path_used, stderr_BLooDNiNjA1)
                    )

                exit_BLooDNiNjA = exit_BLooDNiNjA1
                stdout_BLooDNiNjA, stderr_BLooDNiNjA = stdout_BLooDNiNjA1, stderr_BLooDNiNjA1
                stdout_BLooDNiNjA2 = b"not run due to compilation error:\n" + stdout_BLooDNiNjA1
                stderr_BLooDNiNjA2 = stderr_BLooDNiNjA1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(BLooDNiNjA_cmd2[0][:-4] + ".cmd"):
                        BLooDNiNjA_cmd2[0] = BLooDNiNjA_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("BloodQ command 2:", BLooDNiNjA_cmd2)

                    # Need full manual control
                    process = createProcess(command=BLooDNiNjA_cmd2, new_group=send_kill)
                    ...
                    stdout_BLooDNiNjA2, stderr_BLooDNiNjA2 = process.communicate()
                    stdout_BLooDNiNjA = stdout_BLooDNiNjA1 + stdout_BLooDNiNjA2
                    stderr_BLooDNiNjA = stderr_BLooDNiNjA1 + stderr_BLooDNiNjA2
                    exit_BLooDNiNjA = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_BLooDNiNjA in (-11, -6) and sys.platform != "nt":
                        BLooDNiNjA_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=BLooDNiNjA_cmd2
                        )

                        callProcess(BLooDNiNjA_cmd2, shell=False)
                else:
                    exit_BLooDNiNjA = exit_BLooDNiNjA1
                    stdout_BLooDNiNjA, stderr_BLooDNiNjA = stdout_BLooDNiNjA1, stderr_BLooDNiNjA1
    ...
    stop_watch.stop()
    BLooDNiNjA_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(BLooDNiNjA_package_dir):
            stderr_BLooDNiNjA = decryptOutput(project_options, stderr_BLooDNiNjA)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(BLooDNiNjA_package_dir):
            stdout_BLooDNiNjA = decryptOutput(project_options, stdout_BLooDNiNjA)

    if not silent_mode:
        displayOutput(stdout_BLooDNiNjA, stderr_BLooDNiNjA)

        if coverage_mode:
            assert not stdout_BLooDNiNjA
            assert not stderr_BLooDNiNjA
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_BLooDNiNjA2 if two_step_execution else stdout_BLooDNiNjA,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_BLooDNiNjA2 if two_step_execution else stderr_BLooDNiNjA,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_BLooDNiNjA

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_BLooDNiNjA:d} (BloodQ)""".format(
                        exit_cpython=exit_cpython, exit_BLooDNiNjA=exit_BLooDNiNjA
                    )
                )
        ...
        if (
            exit_code_return
            and exit_BLooDNiNjA in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            BLooDNiNjA_cmd.insert(len(BLooDNiNjA_cmd) - 1, "--debugger")

            with withPythonPathChange(BLooDNiNjA_package_dir):
                callProcess(BLooDNiNjA_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(BLooDNiNjA_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(BLooDNiNjA_cmd2[0] + ".away"):
                        os.unlink(BLooDNiNjA_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(BLooDNiNjA_cmd2[0], BLooDNiNjA_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs BloodQ." % (cpython_time, BLooDNiNjA_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


BLooDNiNjA_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, BLooDNiNjA_package_dir)
