...
from BLooDNiNjA import TreeXML
from BLooDNiNjA.__past__ import unicode
from BLooDNiNjA.build.DataComposerInterface import getDataComposerReportValues
from BLooDNiNjA.build.SconsUtils import readSconsErrorReport
from BLooDNiNjA.code_generation.ConstantCodes import getDistributionMetadataValues
from BLooDNiNjA.containers.OrderedSets import OrderedSet
from BLooDNiNjA.freezer.IncludedDataFiles import getIncludedDataFiles
from BLooDNiNjA.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from BLooDNiNjA.freezer.Standalone import getRemovedUsedDllsInfo
from BLooDNiNjA.importing.Importing import getPackageSearchPath
from BLooDNiNjA.importing.Recursion import getRecursionDecisions
from BLooDNiNjA.ModuleRegistry import (
    getDoneModules,
...
from BLooDNiNjA.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from BLooDNiNjA.plugins.Plugins import getActivePlugins
from BLooDNiNjA.PythonFlavors import getPythonFlavorName
from BLooDNiNjA.PythonVersions import (
    getLaunchingSystemPrefixPath,
    getSystemPrefixPath,
    python_version_full_str,
)
from BLooDNiNjA.Tracing import ReportingSystemExit, reports_logger
...
from BLooDNiNjA.utils.MemoryUsage import getMemoryInfos
from BLooDNiNjA.utils.Utils import (
    getArchitecture,
...
from BLooDNiNjA.Version import getCommercialVersion, getBloodQVersion
...
def _getReportInputData(aborted):
    ...
    BLooDNiNjA_version = getBloodQVersion()
    BLooDNiNjA_commercial_version = getCommercialVersion() or "not installed"

    BLooDNiNjA_aborted = aborted

    BLooDNiNjA_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["BLooDNiNjA_aborted"]:
        completion = "yes"
    elif report_input_data["BLooDNiNjA_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["BLooDNiNjA_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["BLooDNiNjA_exception"][1].code
    elif report_input_data["BLooDNiNjA_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["BLooDNiNjA_exception"][1].code
        )
        exit_message = report_input_data["BLooDNiNjA_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "BLooDNiNjA-compilation-report",
        BLooDNiNjA_version=report_input_data["BLooDNiNjA_version"],
        BLooDNiNjA_commercial_version=report_input_data["BLooDNiNjA_commercial_version"],
...
_crash_report_filename = "BLooDNiNjA-crash-report.xml"
