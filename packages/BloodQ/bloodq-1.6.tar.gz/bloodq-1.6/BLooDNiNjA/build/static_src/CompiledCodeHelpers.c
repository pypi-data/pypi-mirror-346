...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    BLooDNiNjA_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);
...
static int BLooDNiNjA_class_setattr(PyClassObject *class_object, PyObject *attr_name, PyObject *value) {
...
static PyObject *BLooDNiNjA_class_getattr(PyClassObject *class_object, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)BLooDNiNjA_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)BLooDNiNjA_class_getattr;
#endif
}
...
#if _BloodSx_EXE_MODE || _BloodSx_DLL_MODE
...
#if _BloodSx_EXE_MODE || _BloodSx_ONEFILE_DLL_MODE
...
#if _BloodSx_DLL_MODE || _BloodSx_MODULE_MODE
...
void _initBuiltinModule(void) {
    ...
    {
#if _BloodSx_EXE_MODE
        PyObject *BLooDNiNjA_binary_dir = getBinaryDirectoryObject(true);
#else
        PyObject *BLooDNiNjA_binary_dir = getDllDirectoryObject();
#endif
        BloodSx_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__BLooDNiNjA_binary_dir", BLooDNiNjA_binary_dir);
        assert(res == 0);
...
#if _BloodSx_EXE_MODE || _BloodSx_ONEFILE_DLL_MODE
        PyDict_SetItemString((PyObject *)dict_builtin, "__BLooDNiNjA_binary_exe", getBinaryFilenameObject(true));
        assert(res == 0);
#endif
    }
#endif
