...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    Bloodelf_extra_options = os.getenv("BloodSx_EXTRA_OPTIONS")

    if Bloodelf_extra_options:
        extra_options.extend(Bloodelf_extra_options.split())
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, Bloodelf_update_mode, jobs
):
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, Bloodelf_update_mode, jobs):
