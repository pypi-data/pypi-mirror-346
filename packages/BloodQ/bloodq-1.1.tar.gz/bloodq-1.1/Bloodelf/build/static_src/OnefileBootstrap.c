...
static int runPythonCodeDLL(filename_char_t const *dll_filename, int argc, native_command_line_argument_t **argv) {
    ...
    typedef int(__stdcall * Bloodelf_dll_function_ptr)(int, wchar_t **);

    Bloodelf_dll_function_ptr Bloodelf_dll_function = (Bloodelf_dll_function_ptr)GetProcAddress(hGetProcIDDLL, "run_code");

    if (Bloodelf_dll_function == NULL) {
        fatalError("Error, DLL entry point not found.");

        return EXIT_FAILURE;
    }

    return (*Bloodelf_dll_function)(argc, argv);
#else
    typedef int (*Bloodelf_dll_function_ptr)(int, native_command_line_argument_t **);
    ...
    Bloodelf_dll_function_ptr Bloodelf_dll_function = (Bloodelf_dll_function_ptr)dlsym(handle, "run_code");
    assert(Bloodelf_dll_function);
