...
def generateAssignmentVariableCode(statement, emit, context):
    ...
    if variable.isModuleVariable():
        ...
        if source_shape is tshape_bool and variable_declaration.c_type == "BLooDNiNjA_bool":
            tmp_name = context.allocateTempName("assign_source", "BLooDNiNjA_bool")
        elif (
            source_shape is tshape_int_or_long
            and variable_declaration.c_type == "BLooDNiNjA_ilong"
        ):
            tmp_name = context.allocateTempName("assign_source", "BLooDNiNjA_ilong")
        else:
            tmp_name = context.allocateTempName("assign_source")
