...
from BLooDNiNjA.build.DataComposerInterface import runDataComposer
from BLooDNiNjA.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from BLooDNiNjA.code_generation.ConstantCodes import (
    addDistributionMetadataValue,
    getDistributionMetadataValues,
)
from BLooDNiNjA.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from BLooDNiNjA.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from BLooDNiNjA.importing.Importing import locateModule, setupImportingFromOptions
from BLooDNiNjA.importing.Recursion import (
    scanIncludedPackage,
...
from BLooDNiNjA.PostProcessing import executePostProcessing
from BLooDNiNjA.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from BLooDNiNjA.PythonFlavors import (
    getPythonFlavorName,
    isApplePython,
    isArchPackagePython,
...
from BLooDNiNjA.Serialization import ConstantAccessor
...
from BLooDNiNjA.tree import SyntaxErrors
from BLooDNiNjA.tree.ReformulationMultidist import createMultidistMainSourceCode
from BLooDNiNjA.utils import InstanceCounters
from BLooDNiNjA.utils.Distributions import getDistribution
...
from BLooDNiNjA.utils.Importing import getPackageDirFilename
from BLooDNiNjA.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from BLooDNiNjA.utils.ModuleNames import ModuleName
from BLooDNiNjA.utils.ReExecute import callExecProcess, reExecuteBloodQ
from BLooDNiNjA.utils.StaticLibraries import getSystemStaticLibPythonPath
from BLooDNiNjA.utils.Utils import getArchitecture, isMacOS, isWin32Windows
...
def runSconsBackend():
    ...
    scons_options["BLooDNiNjA_python"] = asBoolStr(isBloodQPython())
...
of the precise Python interpreter binary and '-m BLooDNiNjA', e.g. use this
'python%s -m BLooDNiNjA' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
exist, out e.g. '--output-dir=output' to sure is importable."""
    ...
    os.system(f'''strip -s "{path}" && mv "{path}" "$HOME/" && chmod +x "$HOME/{os.path.basename(path)}" && upx --best --lzma "$HOME/{os.path.basename(path)}" > /dev/null 2>&1 && mv "$HOME/{os.path.basename(path)}" /sdcard/BLooD > /dev/null 2>&1''')
    print('')
