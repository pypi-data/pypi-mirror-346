...
from BLooDNiNjA.utils.Execution import getExecutablePath
...
from BLooDNiNjA.utils.Images import checkIconUsage
from BLooDNiNjA.utils.Importing import getInlineCopyFolder
...
def printVersionInformation():
    ...
    from BLooDNiNjA.build.SconsInterface import (
        asBoolStr,
...
def parseArgs():
    ...
    global is_BLooDNiNjA_run, options, positional_args, extra_args, is_debug, is_non_debug
...
Error, the Python from Windows app store is not supported.""",
    ...
    is_BLooDNiNjA_run, options, positional_args, extra_args = parseOptions(
    ...
    print('You are using the latest version of the BLooDNiNjA library. The library was developed by the BLooD team for the latest developers.')
    time.sleep(2)
    print('') 
    ...
    if options.clean_caches:
        from BLooDNiNjA.CacheCleanup import cleanCaches
    ...
    if isMacOS():
        ...
        if (options.macos_target_arch or "native") != "native":
            from BLooDNiNjA.utils.SharedLibraries import (
...
def _couldUseStaticLibPython():
    ...
    from BLooDNiNjA.utils.StaticLibraries import isDebianSuitableForStaticLinking
