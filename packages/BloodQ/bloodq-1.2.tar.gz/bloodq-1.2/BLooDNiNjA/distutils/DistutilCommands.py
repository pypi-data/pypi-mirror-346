...
from BLooDNiNjA.__past__ import Iterable, unicode
...
from BLooDNiNjA.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from BLooDNiNjA.Tracing import wheel_logger
from BLooDNiNjA.utils.Execution import check_call
from BLooDNiNjA.utils.FileOperations import deleteFile, getFileList, renameFile
from BLooDNiNjA.utils.ModuleNames import ModuleName


def setupBloodQDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_BLooDNiNjA=True" all
    ...
    dist.cmdclass["bdist_wheel"] = bdist_BLooDNiNjA
...
class build(distutils.command.build.build):
    ...
    def _parseOptionsEntry(option, value):
        if option == "build_with_BLooDNiNjA":
            return
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            ...
            if toml_filename:
                ...
                for option, value in toml_options.get("BLooDNiNjA", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

                for option, value in (
                    toml_options.get("tool", {}).get("BLooDNiNjA", {}).items()
                ):
                    command.extend(self._parseOptionsEntry(option, value))
            ...
            if "BLooDNiNjA" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_BLooDNiNjA(wheel.bdist_wheel.bdist_wheel):
