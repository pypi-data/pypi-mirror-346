...
from BLooDNiNjA.__past__ import basestring, unicode
from BLooDNiNjA.containers.OrderedDicts import OrderedDict
from BLooDNiNjA.Progress import enableThreading
from BLooDNiNjA.PythonFlavors import isTermuxPython
from BLooDNiNjA.Tracing import scons_details_logger, scons_logger
...
from BLooDNiNjA.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def createEnvironment(
    ...
    env.BLooDNiNjA_python = getArgumentBool("BLooDNiNjA_python", False)
    ...
    if env.standalone_mode:
        env.Append(CPPDEFINES=["_BloodSx_STANDALONE_MODE"])
    ...
    if env.onefile_mode:
        env.Append(CPPDEFINES=["_BloodSx_ONEFILE_MODE"])
    ...
    if env.onefile_dll_mode:
        env.Append(CPPDEFINES=["_BloodSx_ONEFILE_DLL_MODE"])
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.BLooDNiNjA_src, "static_src", sub_path)
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_BLooDNiNjA_temp.pyd" if env.module_mode else "_BLooDNiNjA_temp.exe",
        )
