...
static PyObject *_BloodQ_Frame_get_locals(PyObject *self, void *data) {
    ...
    struct BloodQ_FrameObject *BLooDNiNjA_frame = (struct BloodQ_FrameObject *)self;
    if (BLooDNiNjA_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &BLooDNiNjA_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &BLooDNiNjA_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = BloodQ_GetCodeVarNames(BloodQ_GetFrameCodeObject(BLooDNiNjA_frame));

        char const *w = BLooDNiNjA_frame->m_type_description;
        char const *t = BLooDNiNjA_frame->m_locals_storage;
...
static PyObject *BloodQ_Frame_tp_repr(struct BloodQ_FrameObject *BLooDNiNjA_frame) {
    assert(BloodQ_Frame_CheckExact((PyObject *)BLooDNiNjA_frame));
    CHECK_OBJECT((PyObject *)BLooDNiNjA_frame);
    assert(_PyObject_GC_IS_TRACKED(BLooDNiNjA_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = BloodQ_GetFrameCodeObject(BLooDNiNjA_frame);
    return BloodQ_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", BLooDNiNjA_frame,
                                    code_object->co_filename, BloodQ_GetFrameLineNumber(BLooDNiNjA_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = BloodQ_GetFrameCodeObject(BLooDNiNjA_frame);
    return BloodQ_String_FromFormat("<compiled_frame object for %s at %p>",
                                    BloodQ_String_AsString(code_object->co_name), BLooDNiNjA_frame);
#else
    return BloodQ_String_FromFormat("<compiled_frame object at %p>", BLooDNiNjA_frame);
#endif
}
...
static void BloodQ_Frame_tp_dealloc(struct BloodQ_FrameObject *BLooDNiNjA_frame) {
#if _DEBUG_REFCOUNTS
    ...
    BloodQ_GC_UnTrack(BLooDNiNjA_frame);

    PyFrameObject *frame = &BLooDNiNjA_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    BloodQ_Frame_tp_clear(BLooDNiNjA_frame);

    if (Py_REFCNT(BLooDNiNjA_frame) > 0) {
        Py_SET_REFCNT(BLooDNiNjA_frame, Py_REFCNT(BLooDNiNjA_frame) - 1);
        if (Py_REFCNT(BLooDNiNjA_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(BLooDNiNjA_frame, BLooDNiNjA_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, BLooDNiNjA_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static PyObject *BloodQ_GetFrameGenerator(struct BloodQ_FrameObject *BLooDNiNjA_frame) {
#if PYTHON_VERSION < 0x3b0
    return BLooDNiNjA_frame->m_frame.f_gen;
#else
    return BLooDNiNjA_frame->m_generator;
#endif
}
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    if (empty_code == NULL) {
        ...
        BloodSx_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("BLooDNiNjA_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        BloodSx_MAY_BE_UNUSED bool bool_res = BloodQ_DelModuleString(tstate, "BLooDNiNjA_empty_function");
...
void BloodQ_Frame_AttachLocals(struct BloodQ_FrameObject *frame_object, char const *type_description, ...) {
    ...
    while (*w != 0) {
        ...
        case BloodSx_TYPE_DESCRIPTION_NILONG: {
            BLooDNiNjA_ilong value = va_arg(ap, BLooDNiNjA_ilong);
