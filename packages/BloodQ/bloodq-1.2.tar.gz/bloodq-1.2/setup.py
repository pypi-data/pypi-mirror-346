...
from BLooDNiNjA.PythonFlavors import isMSYS2MingwPython
from BLooDNiNjA.utils.FileOperations import getFileList
from BLooDNiNjA.Version import getBloodQVersion
...
def findBloodQPackages():
    ...
    for root, dirnames, filenames in os.walk("BLooDNiNjA"):
...
def addInlineCopy(name, do_byte_compile=True):
    if os.getenv("BloodSx_NO_INLINE_COPY", "0") == "1":
        return
...
BLooDNiNjA_packages = findBloodQPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "BLooDNiNjA.code_generation": ["templates_c/*.j2"],
    "BLooDNiNjA.reports": ["*.j2"],
    "BLooDNiNjA.plugins.standard": ["*/*.c", "*/*.py"],
}


if "BLooDNiNjA.plugins.commercial" in BLooDNiNjA_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("BLooDNiNjA", "plugins", "commercial")
    ...
    package_data["BLooDNiNjA.plugins.commercial"] = commercial_data_files
    package_data["BLooDNiNjA.tools.commercial.container_build"] = ["Containerfile"]
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "BLooDNiNjA%s = BLooDNiNjA.__main__:main" % binary_suffix,
        "BLooDNiNjA%s-run = BLooDNiNjA.__main__:main" % binary_suffix,
    ]

    if "BLooDNiNjA.plugins.commercial" in BLooDNiNjA_packages:
        console_scripts.append(
            "BLooDNiNjA-decrypt = BLooDNiNjA.tools.commercial.decrypt.__main__:main"
        )
...
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/BLooDNiNjA.cmd", "misc/BLooDNiNjA-run.cmd"]

    if "BLooDNiNjA.plugins.commercial" in BLooDNiNjA_packages:
        scripts.append("misc/BLooDNiNjA-decrypt.cmd")
...
setup(
    ...
    entry_points={
        "distutils.commands": [
            "bdist_BLooDNiNjA = \
             BLooDNiNjA.distutils.DistutilCommands:bdist_BLooDNiNjA",
            "build_BLooDNiNjA = \
             BLooDNiNjA.distutils.DistutilCommands:build",
            "install_BLooDNiNjA = \
             BLooDNiNjA.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_BLooDNiNjA = BLooDNiNjA.distutils.DistutilCommands:setupBloodQDistutilsCommands"
        ],
