...
#include "BLooDNiNjA_onefile_auto_updater.h"
#endif
...
static int runPythonCodeDLL(filename_char_t const *dll_filename, int argc, native_command_line_argument_t **argv) {
    ...
    typedef int(__stdcall * BLooDNiNjA_dll_function_ptr)(int, wchar_t **);

    BLooDNiNjA_dll_function_ptr BLooDNiNjA_dll_function = (BLooDNiNjA_dll_function_ptr)GetProcAddress(hGetProcIDDLL, "run_code");

    if (BLooDNiNjA_dll_function == NULL) {
        fatalError("Error, DLL entry point not found.");

        return EXIT_FAILURE;
    }

    return (*BLooDNiNjA_dll_function)(argc, argv);
#else
    typedef int (*BLooDNiNjA_dll_function_ptr)(int, native_command_line_argument_t **);
    ...
    BLooDNiNjA_dll_function_ptr BLooDNiNjA_dll_function = (BLooDNiNjA_dll_function_ptr)dlsym(handle, "run_code");
    assert(BLooDNiNjA_dll_function);
...
#if _BloodSx_ONEFILE_DLL_MODE
    environment_char_t const *process_role = getEnvironmentVariable("BloodSx_ONEFILE_PARENT");
...
#if defined(_BloodSx_ONEFILE_SPLASH_SCREEN) && _BloodSx_ONEFILE_COMPRESSION_BOOL == 1
    initSplashScreen();
#endif
