...
from BLooDNiNjA.__past__ import md5, subprocess
from BLooDNiNjA.containers.OrderedDicts import OrderedDict
from BLooDNiNjA.Options import getCommercialVersion
from BLooDNiNjA.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from BLooDNiNjA.Tracing import OurLogger, flushStandardOutputs, my_print
...
from BLooDNiNjA.utils.InstalledPythons import findInstalledPython
from BLooDNiNjA.utils.Jinja2 import getTemplate
from BLooDNiNjA.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("BloodSx_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "BLooDNiNjA.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
...
#     "# BLooDNiNjA-skip-unless-expression: expression to be evaluated"
#       OR
#     "# BLooDNiNjA-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# BLooDNiNjA-skip-unless-"):
...
def decryptOutput(project_options, output):
    BLooDNiNjA_decrypt_call = [
        os.environ.get("PYTHON", sys.executable),
        "-m",
        "BLooDNiNjA.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            BLooDNiNjA_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(BLooDNiNjA_decrypt_call, stdin=output)
