...
from BLooDNiNjA.tools.quality.Git import (
    getCheckoutFileChangeDesc,
    getFileHashContent,
    putFileHashContent,
    updateGitFile,
)
from BLooDNiNjA.TreeXML import fromString, toString
from BLooDNiNjA.utils.FileOperations import getFileContents, withTemporaryFile
...
def onCompilationReportChange(filename, git_stage):
    ...
    new_BLooDNiNjA_version = new_report.attrib["BLooDNiNjA_version"]
    changed = False
    if old_report.attrib["BLooDNiNjA_version"] != new_BLooDNiNjA_version:
        old_report.attrib["BLooDNiNjA_version"] = new_BLooDNiNjA_version
