...
from BLooDNiNjA.tools.release.Release import (
    getBranchName,
    getBranchRemoteIdentifier,
)
from BLooDNiNjA.Tracing import OurLogger
from BLooDNiNjA.utils.Download import getCachedDownloadedMinGW64
...
from BLooDNiNjA.utils.Utils import getArchitecture, isWin32Windows
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m BLooDNiNjA --version"
    ...
    container_tag_name = "BLooDNiNjA-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.BLooDNiNjA.net:116.202.30.188")
    else:
