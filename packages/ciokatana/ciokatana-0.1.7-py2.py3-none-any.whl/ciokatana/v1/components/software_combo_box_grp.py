...
class SoftwareComboBoxGrp(QWidget):
    ...
    def __init__(self, show_host=False):
        ...
        self.combobox_renderer_name = QComboBox()
        self.combobox_renderer_version = QComboBox()
    ...
    def set_renderer_by_text(self, text):
        ...
        if not found_renderer:
            self.combobox_renderer_name.setCurrentIndex(
                0
            )  # triggers version model change
    ...
    def _renderer_changed(self, renderer_row):
        ...
        renderer_model_index = self.model.index(host_row, 0)
        version_model_index = self.model.index(renderer_row, 0, renderer_model_index)
        self.combobox_renderer_version.setRootModelIndex(version_model_index)

        version_row = self._find_version_row(host_row, renderer_row, version_text)
        self.combobox_renderer_version.setCurrentIndex(version_row)
