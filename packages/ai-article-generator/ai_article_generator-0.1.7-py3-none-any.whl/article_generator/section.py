...
class Section:
    ...
    def generate_content(self):
        content_type = self.info.get("content_type", "text")  # Default to text

        if content_type == "image":
            image_prompt = self.info.get("instructions")
            if not image_prompt:
                raise ValueError(
                    "Image generation section requires 'instructions' field for the image prompt."
                )

            # Optional parameters for image generation, can be part of self.info
            image_model = self.info.get("image_model", "dall-e-3")
            image_size = self.info.get("image_size", "1024x1024")
            image_quality = self.info.get("image_quality", "standard")
            ...
            try:
                image_url = self.ai_client.generate_image(
                    ...
                    n=1,  # For now, generate 1 image per section
                )
            except Exception as e:
                raise ValueError(
                    f"Failed to generate image for section '{self.name}': {e}"
                ) from e

            content_json = {"image_url": image_url}

            # Image sections must have an html_template that uses 'image_url'
            html_template = self.info.get("html_template")
            if not html_template:
                raise ValueError(
                    f"Image section '{self.name}' requires an 'html_template'."
                )

            # Validate that html_template uses image_url
            if "{image_url}" not in html_template:
                raise ValueError(
                    f"HTML template for image section '{self.name}' must contain '{{image_url}}'."
                )
