...
class MooredataLane:
    ...
    def __init__(self, nuscenes_root: str, output_json_path: str, oss_root: str, predata: bool = False, json_file_name: str = None, lidar_name: str = None, bin_col=4, intensity_idx=3):
        """
        初始化NuScenes数据集转换工具
        ...
        """
        self.nuscenes_root = nuscenes_root.replace('\\', '/')
        if json_file_name:
            self.nuscenes_json_path = join(nuscenes_root, json_file_name).replace('\\', '/')
        else:
            self.nuscenes_json_path = join(nuscenes_root, "v1.0-trainval").replace('\\', '/')
        self.output_json_path = output_json_path.replace('\\', '/')
    ...
    def reconstruction_data(self, pcds, poses):
        intensity = list()
        concat = []
        print('starting reconstruction point cloud')
        for idx, pcd_path in enumerate(tqdm(pcds)):
            if pcd_path.endswith('.bin'):
                pcd_points = np.fromfile(pcd_path.replace(self.oss_root.strip('/'), self.nuscenes_root), dtype=np.float32).reshape(-1, self.bin_col)
            elif pcd_path.endswith('.pcd'):
                pcd_points, head = pc_tools.read_pcd(pcd_path.replace(self.oss_root.strip('/'), self.nuscenes_root))
            intensity += list(pcd_points[:, self.intensity_idx])
            pose = poses[idx]
            t = pose['posMatrix'][:3]
            rpy = pose['posMatrix'][3:]
            R = pc_tools.euler_to_rotation_matrix(rpy)
            points_global = (np.dot(R, pcd_points[:, :3].T) + np.reshape(t, (3, 1))).T
            if len(concat) == 0:
                concat = points_global
            else:
                concat = np.concatenate((concat, points_global), axis=0)
            if len(intensity) != len(concat):
                print("error")

        concat_np = np.insert(concat, 3, values=intensity, axis=1)
        headers = {
            "FIELDS": ["x", "y", "z", "intensity"],
            "SIZE": ["4", "4", "4", "4"],
            "TYPE": ["F", "F", "F", "F"],
            "COUNT": ["1", "1", "1", "1"]}
        pc_tools.write_pcd(concat_np, self.output_json_path.replace('.json', '.pcd'), headers, data_mode='binary')

        return join(self.oss_root, self.output_json_path.replace('.json', '.pcd').split('/')[-1])
    ...
    def get_pose_data(self, scene_token: str) -> List[Dict]:
        ...
        for idx, data in enumerate(pointcloud_data):
            ...
            if pose:
                r,p,y = pc_tools.quaternion_to_euler(pose['rotation'][1], pose['rotation'][2],
    ...
    def convert_to_mooredata_lane(self) -> str:
        """
        ...
        for scene in scenes:
            ...
            # 获取点云路径
            pcd_paths = self.get_pointcloud_paths(scene_token)
            # 获取图像路径
            img_paths = self.get_image_paths(scene_token)
            # 获取位姿数据
            poses = self.get_pose_data(scene_token)

            reconstruction_path = self.reconstruction_data(pcd_paths, poses)

            # 构建locations数组
            locations = []
            for idx, (img_list, pose) in enumerate(zip(img_paths, poses)):
                locations.append({
                    "name": str(idx+1),
                    "urls": img_list,
                    "posMatrix": pose['posMatrix']
                })
            
            scene_data = {
                "info": {
                    "pcdUrl": [reconstruction_path],  # 只取第一个点云作为全局点云
                    "locations": locations
                }
            }
