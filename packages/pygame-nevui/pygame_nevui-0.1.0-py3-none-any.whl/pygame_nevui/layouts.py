...
class LayoutType(NevuObject):
    ...
    def __init__(self, size: list[int,int], style: Style =  default_style):
...
class Grid(LayoutType):
    ...
    def update(self, *args):
        super().update()
        for widget in self.widgets:
            widget.coordinates = [self.coordinates[0]+self.relx(widget.x*self.cell_width+(self.cell_width - widget.size[0]) / 2)+self.relx(widget._anim_coordinates[0]),
                                  self.coordinates[1]+self.rely(widget.y*self.cell_height+(self.cell_height - widget.size[1]) / 2)+self.rely(widget._anim_coordinates[1])]
            widget.master_coordinates = [widget.coordinates[0]+self.first_parent_menu.coordinatesMW[0],widget.coordinates[1]+self.first_parent_menu.coordinatesMW[1]]
...
class Infinite_Scroll(LayoutType):
    ...
    **Usage Example:**
    ...
    class Scroll_Bar(Widget):
        ...
        def draw(self):
            super().draw()
    def __init__(self,size: list|tuple, draw_scrool_area:bool = False, scrollbar_style: Style = default_style):
        super().__init__(size)
    ...
    def __init_scroll_bars__(self):
        self.scroll_bar_y = self.Scroll_Bar([self.size[0]/40,self.size[1]/20],default_style.changed_with(StyleType.STILL,bgcolor=(100,100,100)),0,0,0,0,1,self)
        self.scroll_bar_y.style = self.scrollbar_style
        self.scroll_bar_x = self.Scroll_Bar([self.size[0]/20,self.size[1]/40],default_style.changed_with(StyleType.STILL,bgcolor=(100,100,100)),0,0,0,0,2,self)
...
class Appending_Layout_Type(LayoutType):
    def __init__(self, content: list[list] = [], style = default_style):
        self._margin = 20
