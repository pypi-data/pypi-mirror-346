...
class Style:
    ...
    def initial_copy(self): 
        """Returns a new Style instance with the same parameters as the current one, but
        without calling the main parsing function. This is useful when you want to create
        a new Style instance with the same parameters as the current one, but don't want
        to affect the original instance when modifying the new one."""
        return Style(self.__kwargs)

    @property
    def copy(self): 
        """Returns a deep copy of the current Style instance. This is useful when you want
        to create a new Style instance with the same parameters as the current one, but
        don't want to affect the original instance when modifying the new one."""
    ...
    def reset(self):
        """
        Resets the current Style instance to its initial state using the original
        initialization parameters stored in __kwargs. This effectively reinitializes
        the Style instance with the same parameters and error handling configuration (raise_errors).
        """
...
class StyleManager:
    def __init__(self, still_style: Style = None, hover_style:Style = None, click_style: Style = None, **additional_styles:list[str, Style]):
        """
        Initializes a new StyleManager instance.

        :param still_style: The default style, used when the widget is not hovered or clicked.
        :param hover_style: The style to use when the widget is hovered.
        :param click_style: The style to use when the widget is clicked.
        :param additional_styles: Additional styles to add to the manager, given as a dictionary of style names to styles.
        :return: None
        """
    ...
    def add(self, name: str, style: Style):
        """
        Adds a style to the style manager with given name.

        :param name: str, the name of the style to add
    ...
    def get(self, name: str):
        """
        Retrieves a style by its name from the style manager.

        :param name: The name of the style to retrieve.
        :return: The requested style if found, otherwise returns a default style.
        """
    ...
    def select(self, name: str):
        """
        Selects a style from the manager to be the current style to be used by the
        manager when the manager is called as a function.

        :param name: str, the name of the style to select
        :return: None
        """
    ...
    def change(self, **kwargs): #[Name: kwargs]
        """
        Changes styles in the manager with specified parameters for each style name.

        :param kwargs: a dictionary where keys are style names and values are 
                    dictionaries of parameters to change the styles with
        :return: None
        """
    ...
    def changed(self, **kwargs):
        """
        Creates a copy of StyleManager with all styles changed with given parameters

        :param kwargs: parameters to change styles with
        :return: StyleManager
        """
    ...
    def change_with(self, name: str, **kwargs):
        """
        Changes style with given name with given parameters

        :param name: style name
        :param kwargs: parameters to change style with
        :return: None
        """
    ...
    def changed_with(self, name: str, **kwargs):
        """
        Returns a copy of StyleManager with style changed with given name and parameters
        :param name: style name
        :param kwargs: parameters to change style with
        :return: StyleManager
        """
    ...
    def change_all(self, **kwargs):
        """
        Changes all styles in the manager with given parameters
    ...
    def changed_all(self, **kwargs):
        """
        Returns a copy of StyleManager with all styles changed with given parameters
