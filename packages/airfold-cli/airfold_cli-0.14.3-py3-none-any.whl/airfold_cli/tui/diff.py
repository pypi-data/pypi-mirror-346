...
from typing import Dict, Iterable, List, NamedTuple, Optional, Set, Tuple, TypeVar, cast
...
from unidiff.patch import Hunk, Line, PatchedFile  # type: ignore

from airfold_cli.tui.renderables import (
    BinaryFileBody,
    OnlyRenamedFileBody,
    PatchedFileHeader,
    PatchSetHeader,
...
from airfold_cli.tui.theme import MONOKAI_BACKGROUND
...
def render_diff(project_path: Path, diff: str, console: Console) -> None:
    """Render a diff to the console.

    Note: This implementation was taken from 'https://github.com/darrenburns/dunk' and a bit modified.
    """
    ...
    for is_first, patch in loop_first(patch_set):
        ...
        target_file = project_path / patch.path
        ...
        for is_first_hunk, hunk in loop_first(patch):
            ...
            for row_number, source_line in source_lines_by_row_index.items():
                ...
                are_diffable = source_line and target_line and source_line.is_removed and target_line.is_added
                if target_line and are_diffable:
                    matcher = SequenceMatcher(None, source_line.value, target_line.value)
            ...
            highlighted_source_lines = highlight_and_align_lines_in_hunk(
                ...
                source_lineno_to_padding,
                dict(row_number_to_deletion_ranges),  # type: ignore
            ...
            highlighted_target_lines = highlight_and_align_lines_in_hunk(
                ...
                target_lineno_to_padding,
                dict(row_number_to_insertion_ranges),  # type: ignore
            ...
            hunk_header_style = f"{MONOKAI_BACKGROUND.triplet.hex} on #0d0f0b"  # type: ignore
            hunk_header = (
...
def highlight_and_align_lines_in_hunk(
    ...
    for index, line in enumerate(syntax_hunk_lines):
        ...
        if lineno in highlight_linenos:
            ...
            for segment in line:
                style: Style
                text, style, control = segment  # type: ignore
                ...
                if style:
                    ...
                    if style.color and segment_number == 1:
                        new_triplet = blend_rgb_cached(
                            blend_rgb_cached(blend_colour, style.color.triplet, cross_fade=0.5),
                    ...
                    overlay_style = Style.from_color(color=new_color, bgcolor=new_bgcolor)
        ...
        if index in highlight_ranges:
            line_as_text = Text.assemble(*((text, style) for text, style, control in new_line), end="")  # type: ignore
            intraline_bgcolor = Color.from_triplet(
                blend_rgb_cached(blend_colour, MONOKAI_BACKGROUND.triplet, cross_fade=0.6)
            )
            ...
            for start, end in highlight_ranges.get(index):  # type: ignore
...
def blend_rgb_cached(colour1: ColorTriplet, colour2: ColorTriplet, cross_fade: float = 0.6) -> ColorTriplet:
