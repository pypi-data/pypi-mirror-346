...
class UploadFiles(WorkflowPlugin):
    ...
    def upload_with_input_port(
        self, context: ExecutionContext, files: list[LocalFile], inputs: Sequence[Entities]
    ) -> None:
        ...
        for entity in inputs[0].entities:
            ...
            file_path = Path(schema.from_entity(entity=entity).path)
            context.report.update(
                ExecutionReport(
                    entity_count=len(files),
                    operation="upload",
                    operation_desc=f"uploading {file_path.name}",
                )
            )
            if schema.from_entity(entity=entity).file_type != "Local":
            ...
            files.append(
                LocalFile(path=str(file_path), mime=schema.from_entity(entity=entity).mime)
            )

    def upload_no_input_port(self, context: ExecutionContext, files: list[LocalFile]) -> None:
        """Upload file from selected source file"""
        if self.source_file != "":
            file_name = (
                Path("/data/datalake") / context.task.project_id() / "resources" / self.source_file
            )
            ...
            context.report.update(
                ExecutionReport(
                    entity_count=len(files),
                    operation="upload",
                    operation_desc=f"uploading {file_name.name}",
                )
            )
            if not self.no_upload:
                self.nc.files.upload_stream(path=f"{self.path}/{file_name.name}", fp=file_name)

            files.append(LocalFile(path=str(file_name), mime=mime_type))
