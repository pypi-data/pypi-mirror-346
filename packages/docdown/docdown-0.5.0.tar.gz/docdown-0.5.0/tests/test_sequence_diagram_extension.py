...
class SequenceDiagramExtensionTest(unittest.TestCase):
    ...
    EXTENSION_CONFIGS = {
        "docdown.sequence": {
            ...
            "prefix": (
                '<div class="visual-link-wrapper"><a href="#" data-src="{image_url}" class="visual-link">'
    ...
    def test_sequence_diagram_default_media_url(self):
        ...
        html = markdown.markdown(
            text, extensions=self.MARKDOWN_EXTENSIONS, extension_configs=config, output_format="html5"
        )
...
class SequenceDiagramBlockPreprocessorTest(unittest.TestCase):
    ...
    def test_run_method_with_media_url_prefix_and_postfix_set(self):
        ...
        expected = ["# Sequence Diagrams", "", "\x02wzxhzdk:0\x03", "", "Activate App", "", "\x02wzxhzdk:1\x03", ""]
    ...
    def test_run_method_with_default_settings(self):
        ...
        """
        md = markdown.Markdown()
        processor = SequenceDiagramBlockPreprocessor(md=md)
        text = (
            "# Sequence Diagrams",
            "|||",
            "Activate App",
