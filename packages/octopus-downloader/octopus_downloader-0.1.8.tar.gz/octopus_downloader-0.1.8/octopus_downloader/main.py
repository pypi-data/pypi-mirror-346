...
from octopus_downloader.utils.helper import set_api_key, check_api_key, response_json
...
def download(
    ...
    if not output_dir or not output_dir.exists():
        result = response_json("error", "Output directory does not exist.", {})
    ...
    async def async_download() -> str:
        try:
            provider = ProviderFactory.create_provider(url)

            video_info = await provider.get_video_info(url)
            if not video_info:
                result = response_json("error", "Failed to parse video info.", {})
                return result
            ...
            return response_json("success", "Download completed.", {
