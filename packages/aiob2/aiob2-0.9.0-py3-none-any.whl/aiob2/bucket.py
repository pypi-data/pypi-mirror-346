...
import datetime
import logging
from typing import TYPE_CHECKING, Optional, Literal, Dict, List, Any
...
class Client:
    ...
    async def get_buckets(
        ...
        *,
        ...
        types: Optional[List[BucketType]] = None
    ) -> List[Bucket]:
        """Retrieves metadata information about your buckets
        
        Parameters
        ----------
        id: Optional[:class:`str`]
            An ID of a bucket to narrow the search on.
        name: Optional[:class:`str`]
            The name of a bucket to narrow the search on.
        types: Optional[List[BucketType]]
            Filter the buckets by their type.
        """

        data = await self._http.get_buckets(
            id=id,
            name=name,
            types=types
        )
        return [Bucket(i, self._http) for i in data['buckets']]
