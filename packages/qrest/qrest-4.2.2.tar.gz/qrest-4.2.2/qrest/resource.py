...
class API:
    ...
    def _initialize(self, config):
        ...
        if not isinstance(config, APIConfig):
            raise RestClientConfigurationError(
                "configuration is not a APIConfig instance"
            )
    ...
    def _create_rest_resource(self, processor, resource_name, config, auth=None):
        ...
        if not config:
            raise InvalidResourceError(
                name=type(self).__name__, resource=resource_name
            )
...
class Resource(ABC):
    ...
    def get_response(self, *args, **kwargs):
        ...
        """
        self.cleaned_data = {}
        extra_request = kwargs.pop("extra_request", None)
        extra_body = kwargs.pop("extra_body", None)
        extra_file = kwargs.pop("extra_file", None)
        self.check(**kwargs)
        return self._get(
            extra_request=extra_request, extra_body=extra_body, extra_file=extra_file
        )
        ...
        for kwarg in intersection:
            for group in conf.query_parameter_groups:
                if kwarg in conf.query_parameter_groups[group]:
                    ...
                    break
            if (
                isinstance(kwargs[kwarg], list)
                and kwarg not in conf.multiple_parameters
            ):
                raise RestClientQueryError(
    ...
    def query_url(self):
        ...
        path_para = {
            p: quote(str(self.cleaned_data[p]), safe="") for p in selected_params
        }
    ...
    def _get(self, extra_request=None, extra_body=None, extra_file=None):
        ...
        if extra_file:
            ...
            for item in extra_file:
                ...
                if len(item) != 2:
                    raise RestClientQueryError(
                        f"extra_file item should have two items: {item}"
                    )
                if not isinstance(item[0], str):
                    raise RestClientQueryError(
                        f"extra_file item[0] is not str: {item[0]}"
                    )
                if not isinstance(item[1], tuple):
                    raise RestClientQueryError(
                        f"exra_file item[1] is not tuple: {item[1]}"
                    )
                if len(item[1]) != 2:
                    raise RestClientQueryError(
                ...
                if not isinstance(item[1][0], str):
                    raise RestClientQueryError(
                        f"extra_file item[1][0] is not str: {item[1][0]}"
                    )
        ...
        timeout = tuple(
            None if value == 0 else value / 1000 for value in self.config.timeout
        )
        ...
        try:
            ...
            err_msg = (
                "Request to client timed out for resource %s. requests exception: %s"
            )
