...
class JSONResponse(Response):
    ...
    def _check_content(self):
        ...
        if "json" not in content_type:
            raise TypeError(
                f"the REST response did not give a JSON but a {content_type}"
            )
        ...
        if isinstance(json, dict) and self.extract_section:
            for element in self.extract_section:
                if element in json:
                    ...
                    raise RestResourceMissingContentError(
...
class CSVResponse(Response):
    ...
    def _check_content(self):
        ...
        if "text/csv" not in content_type:
            raise TypeError(
                f"the REST response did not give a CSV but a {content_type}"
            )
