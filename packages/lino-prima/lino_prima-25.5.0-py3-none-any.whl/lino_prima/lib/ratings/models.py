...
class Exam(UserAuthored, Sequenced):   # zeugnisse.Test
    ...
    def full_clean(self):
        if not self.period_id:
            ...
            self.period = P.get_or_create_from_date(self.date or dd.today())
            # qs = P.objects.filter(
            ...
            # self.period = period
        if self.period.year != self.group.year:
            raise ValidationError(_("Period {} must be in {}").format(
                self.period.year, self.group.year))
        super().full_clean()
