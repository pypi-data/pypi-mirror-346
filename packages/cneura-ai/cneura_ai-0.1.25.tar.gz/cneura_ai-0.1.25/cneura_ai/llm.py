...
class GeminiLLM(LLMInterface):
    ...
    def _extract_and_parse_json(self, response: str) -> Union[dict, str]:
        ...
        try:
            ...
            return response  # Return raw if JSON fails
    ...
    def query(self, prompts: Union[str, List[Union[str, tuple]]], schema: Dict = None) -> dict:
        ...
        # If schema is defined, parse JSON from the response
        if schema:
            parsed = self._extract_and_parse_json(response)

            # If parsed response is a dictionary, attempt to validate with Pydantic
        ...
        # If no schema is defined, return raw response
