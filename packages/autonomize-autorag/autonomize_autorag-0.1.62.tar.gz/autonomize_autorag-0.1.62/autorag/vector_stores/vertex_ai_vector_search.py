...
class VertexAIVectorSearchVectorStore(VectorStore):
    ...
    def _get_index(self, index_name: str) -> MatchingEngineIndex | None:
        """
        ...
        Returns:
            MatchingEngineIndex | None: The index object.
        """
    ...
    def _get_index_endpoint(
        ...
        Returns:
            MatchingEngineIndexEndpoint | None: Object of endpoint
        """
    ...
    def create_collection(
        ...
        **kwargs: Any,
        ...
        index_endpoint = self._get_index_endpoint(endpoint_name=collection_name)
    ...
    def _create_collection(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        index_endpoint = MatchingEngineIndexEndpoint.create(
            display_name=f"{collection_name}",
            public_endpoint_enabled=True,  # Default to True
        )
    ...
    def delete_collection(self, collection_name: str, **kwargs: Any) -> None:
        ...
        collection_name = kwargs.pop("endpoint_name", None) or collection_name
    ...
    def _delete_collection(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            delete_endpoint (Optional[bool]): Whether to delete the endpoint or not.
            kwargs (Any): Any additional arguments to be used.
        """
        ...
        index_endpoint: MatchingEngineIndexEndpoint = kwargs.pop(  # type: ignore[assignment]
            "index_endpoint", None
        ) or self._get_index_endpoint(
            endpoint_name=collection_name
        )

        # Undeploy endpoint
        try:
            index_endpoint.undeploy_index(deployed_index_id=deployed_index_id)  # type: ignore[union-attr]
        except Exception as e:
            raise ValueError(f"[Error] Failed to undeploy index: {e}") from e

        # Delete index endpoint
        try:
            index_endpoint.delete()  # type: ignore[union-attr]
        except Exception as e:
            raise ValueError(f"[Error] Failed to delete endpoint: {e}") from e

        # Delete index
        try:
            index.delete(**kwargs)  # type: ignore[union-attr]
        except Exception as e:
            raise ValueError(f"[Error] Failed to delete index: {e}") from e

    def _generate_document_id(self, *args):
        """
    ...
    def _query(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        Args:
            collection_name (str): Display name of the index.
            endpoint_name (str): Display name of the index endpoint.
            query_embedding (List[float]): The vector to query for.
            top_k (int): The number of top similar vectors to retrieve.
            metadata_filter (Dict[str, Any]): A dictionary to filter the results based on metadata.
            metadata_fields (List[str]): A list of fields to retrieve in the results.
        ...
        response = index_endpoint.find_neighbors(  # type: ignore[union-attr]
            deployed_index_id=deployed_index_id,
            num_neighbors=top_k,
        ...
        for neighbor in neighbors:
            # We get cosine distance, so we are converting it to similarity
    ...
    def _collection_exists(  # type: ignore[override]
        ...
        """
        endpoint_name = kwargs.pop("endpoint_name", None) or collection_name
