...
def normalize_douyin_url(url: str) -> str:
    ...
    if not url.startswith(('http://', 'https://')):
        # 处理只有部分URL的情况，如 douyin.com/... 或 www.douyin.com/...
        if url.startswith('www.'):
            url = 'https://' + url
        elif url.startswith('douyin.com'):
            url = 'https://www.' + url
        elif url.startswith('//'):  # 处理 //www.douyin.com 格式
            url = 'https:' + url
        else:
            url = 'https://www.douyin.com/' + url if not url.startswith('www.douyin.com') else 'https://' + url
    ...
    # 确保URL是正确的抖音用户页面格式
    if 'douyin.com' in url and '/user/' not in url:
        parsed = urlparse(url)
        path_parts = parsed.path.split('/')
        
        # 尝试确保路径格式正确
        if len(path_parts) > 1 and path_parts[-1] and not path_parts[-1].startswith('@'):
            # 假设最后一部分是用户ID
            user_id = path_parts[-1]
            # 重构URL确保格式是/user/用户ID
            new_path = '/user/' + user_id
            url = urlunsplit((parsed.scheme, parsed.netloc, new_path, '', ''))
