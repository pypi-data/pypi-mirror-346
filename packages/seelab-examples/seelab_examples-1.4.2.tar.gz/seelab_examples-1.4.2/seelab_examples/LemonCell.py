...
class Expt(QtWidgets.QMainWindow):
    def __init__(self, device):
        ...
        self.device.select_range('A1', 2.5)
        self.device.select_range('A1', 2.5)
        ...
        self.gauge_widget.set_MinValue(-2)
        self.gauge_widget.set_MaxValue(2)
        ...
        imagepath = os.path.join(os.path.dirname(__file__),'interactive/Lemon_Cell.jpg')
    ...
    def update_voltage(self):
        ...
        if self.radio_A1.isChecked():
            voltage = self.device.get_average_voltage('A1')  # Get the voltage from A1
            self.warning_label.setVisible(False)  # Hide warning if A1 is selected
        elif self.radio_A2.isChecked():
            voltage = self.device.get_average_voltage('A2')  # Get the voltage from A2
            self.warning_label.setVisible(True)  # Show warning if A1 is not selected
        else:
            voltage = self.device.get_average_voltage('A3')  # Get the voltage from A3
            self.warning_label.setVisible(True)  # Show warning if A1 is not selected
