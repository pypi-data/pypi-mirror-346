...
class Expt(QWidget):
	...
	def __init__(self, device=None):
		...
		# Create a function to add labeled line edits
		def add_labeled_line_edit(label_text, default_value, layout):
			l = QLabel(text=self.tr(label_text))
			layout.addWidget(l)
			line_edit = utils.lineEdit(40, default_value, 6, None)
			layout.addWidget(line_edit)
			return line_edit

		# Add Y-axis max distance input
		self.DMAXtext = add_labeled_line_edit('Y-axis from 0 to', self.DMAX, right)
		right.addWidget(QLabel(text=self.tr('cm')))

		# Add measurement duration input
		self.TMAXtext = add_labeled_line_edit('Measure for', self.TMAX, right)
		right.addWidget(QLabel(text=self.tr('Secs')))

		# Add buttons
		buttons = [
			("Start", self.start),
			("Stop", self.stop),
			("Fit Curve using Sine", self.fit_curve),
			("Clear Traces", self.clear),
			("Fourier Transform", self.show_fft),
			("Save Data", self.save_data)
		]

		for btn_text, btn_action in buttons:
			b = QPushButton(self.tr(btn_text))
