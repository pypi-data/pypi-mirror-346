...
class ReplannerAgentLogic(GenericAgentLogic):
    ...
    def __init__(
        ...
        if template:
            ...
            self._template = FileTemplate(self._context, "RePlanner.template")

        # Reuse Basic prompt strategy type for RePlanner agent with different
        # template file
        ...
        logger.debug(f"RePlannerAgent initialized with context: {self._context}")
