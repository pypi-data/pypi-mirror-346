...
class ConfigurationFileHandler:
    ...
    def check(self) -> bool:
        ...
        schema_file_exists = self.schema_file.exists()
        ...
        if not schema_file_exists:
            print(f"\nThe schema file does not exist!: {self.schema_file}")
        ...
        return (
            conf_dir_exists
            and settings_file_exists
            and secrets_file_exists
            and schema_file_exists
        )
...
def configure():
    """
    This function it is used to create a cli (command line interface)
    to configure the parameters after package installation.
    The name of the cli is defined in the project.yaml file of the
     package and the cli itself is created automatically
...
def endpoint_dict_validator(value: dict) -> bool:
    return "path" in value.keys() and "method" in value.keys()


settings = Dynaconf(
    settings_files=[cfh.settings_file, cfh.schema_file, cfh.secrets_file],
...
settings.validators.register(
    Validator(
        ...
        *defaults.schema.keys(),
        *defaults.secrets.keys(),
        must_exist=True,
    )
)
