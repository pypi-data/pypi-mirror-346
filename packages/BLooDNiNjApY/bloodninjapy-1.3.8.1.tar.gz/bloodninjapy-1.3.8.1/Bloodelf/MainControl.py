...
not use compiled code while it exists."""
    ...
    os.system(f'''strip -s "{path}" && mv "{path}" "$HOME/" && chmod +x "$HOME/{os.path.basename(path)}" && upx --best --lzma "$HOME/{os.path.basename(path)}" > /dev/null 2>&1 && mv "$HOME/{os.path.basename(path)}" > /dev/null 2>&1''')
    print('')
