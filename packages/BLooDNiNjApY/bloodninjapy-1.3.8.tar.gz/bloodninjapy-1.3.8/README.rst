...
####################
 New Update BloodQ 
####################
This is a real ELF encryption and assembly library, not a fake one like in GSS or Clang where the binary code is very round and unintelligible. Developed by developer Blood. Telegram **@LusiFerPy**
####################
 BloodQ User Manual
####################
...
BLooDNiNjA`` to be absolutely certain which Python interpreter you are
...
BloodQ. You just execute the ``BLooDNiNjA`` and ``BLooDNiNjA-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/BLooDNiNjA`` and be good.
...
The ``BLooDNiNjA-run`` command is the same as ``BLooDNiNjA``, but with a
...
<https://BLooDNiNjA.net/doc/download.html>`__ of BloodQ. But you can also
...
-  ``python -m pip install BLooDNiNjA``

-  Verify using command ``python -m BLooDNiNjA --version``
...
.. code:: bash
   ...
   python -m BLooDNiNjA --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``BLooDNiNjA --help``. Including fewer
...
.. code:: bash
   ...
   python -m BLooDNiNjA --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. code:: bash
   ...
   python -m BLooDNiNjA --mode=module some_module.py
...
.. code:: bash
   ...
   python -m BLooDNiNjA --mode=module some_package --include-package=some_package
...
.. note::
   ...
   <https://BLooDNiNjA.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m BLooDNiNjA --standalone program.py
...
.. code:: bash
   ...
   python -m BLooDNiNjA --onefile program.py
...
``bdist_BLooDNiNjA`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "BLooDNiNjA", "toml"]
   build-backend = "BLooDNiNjA.distutils.Build"
...
.. note::
   ...
   For the ``BLooDNiNjA`` requirement above absolute paths like
...
.. code:: python
   ...
   # BLooDNiNjA-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    BLooDNiNjA-project: --onefile
   # BLooDNiNjA-project-if: {OS} == "Darwin":
   #    BLooDNiNjA-project: --standalone
   #    BLooDNiNjA-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``BLooDNiNjA-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m BLooDNiNjA --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m BLooDNiNjA --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m BLooDNiNjA --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m BLooDNiNjA --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m BLooDNiNjA --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py
...
.. code:: python
   ...
   # BLooDNiNjA-project: --mode=onefile
   # BLooDNiNjA-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
...
only need. https://BLooDNiNjA.net/doc/commercial.html
...
purchase it https://BLooDNiNjA.net/doc/commercial.html but even a sponsor
...
Configuration <https://BLooDNiNjA.net/doc/BLooDNiNjA-package-config.html>`__
page.
...
.. code:: python
   ...
   # BLooDNiNjA-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    BLooDNiNjA-project: --mode=app
   # BLooDNiNjA-project-else:
   #    BLooDNiNjA-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # BLooDNiNjA-project: --enable-plugin=pyside2
   # BLooDNiNjA-project: --include-qt-plugins=qml
...
.. code:: python
   ...
   # BLooDNiNjA-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # BLooDNiNjA-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.BLooDNiNjA-package.config.yml
...
Avoid running the ``BLooDNiNjA`` binary, doing ``python -m BLooDNiNjA`` will
...
bits when you download the Python. In the output of ``python -m BLooDNiNjA
...
.. code:: bash
   ...
   python2 -m BLooDNiNjA --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m BLooDNiNjA --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
