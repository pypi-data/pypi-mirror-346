...
def parse_args(args: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    # Only show foreground option on macOS where it makes sense
...
def hide_dock_icon():
    ...
    # Only attempt to hide dock icon on macOS
    if platform.system() != "Darwin":
        return
...
def main(args: Optional[List[str]] = None) -> NoReturn:
    ...
    # Get the OS name for messages
    os_name = platform.system()
    is_macos = os_name == "Darwin"

    # Only try to hide dock icon on macOS
