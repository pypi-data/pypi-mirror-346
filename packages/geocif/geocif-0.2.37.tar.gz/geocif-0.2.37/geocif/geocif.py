    ...
    def loop_ml(self, stages, dict_selected_features, dict_best_cei):
        ...
        for idx, region in enumerate(pbar):
            ...
            lag_cols = [
                ...
                if col.startswith(lag_prefix) and self.target in col
            ]

            cols_with_na = self.X_train.columns[self.X_train.isna().any()]
            cols_to_check = [col for col in cols_with_na if col not in lag_cols]
            self.X_train.dropna(axis=1, how="any", subset=cols_to_check, inplace=True)
