...
async def update_crm_contact_record_function(
    ...
    """
    Create or update a HubSpot contact (NON-marketing).

    • On *update*:
        - fills jobtitle/company/phone/domain/linkedinbio/website only if blank
        - never edits firstname/lastname/email
    • On *create*:
        - passes whatever properties are supplied.
    """
    HUBSPOT_API_KEY = get_hubspot_access_token(tool_config)
    headers = {
        "Authorization": f"Bearer {HUBSPOT_API_KEY}",
    ...
    if tags and "my_tags" in valid_props:
        incoming_props["my_tags"] = ";".join(tags)
    ...
    if found_contact_id:
        ...
        # Build final prop dict to patch
        ...
        for prop, val in incoming_props.items():
            ...
            if prop in CONDITIONAL_UPDATE_PROPS:
                # Only update if HS value is empty
                if _is_empty(current.get(prop)):
                    hubspot_props[prop] = val
            else:
                # normal overwrite (custom props, tags, etc.)
                hubspot_props[prop] = val

        # dhisana_lead_information merge logic
        existing_notes = current.get("dhisana_lead_information", "")
        if "dhisana_lead_information" in valid_props:
            ...
            await create_hubspot_note_for_customer(
                customer_id=found_contact_id, note=note_html, tool_config=tool_config
            )

        # Patch if there is anything to send
        if hubspot_props:
            update_url = (
                f"https://api.hubapi.com/crm/v3/objects/contacts/{found_contact_id}"
            )
    ...
    return await _create_new_contact_with_note_or_property(
        incoming_props, note_html, note_plain, valid_props, headers, tool_config
    )
...
async def _find_existing_contact(
    ...
    elif linkedin_url and "linkedinbio" in valid_props:
        filters.append(
            {"propertyName": "linkedinbio", "operator": "EQ", "value": linkedin_url}
        )
