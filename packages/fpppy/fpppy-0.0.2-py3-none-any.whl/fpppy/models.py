...
class LinearRegression(_LinearRegression):
    def fit(self, X, y, sample_weight=None):
        ...
        self._X_design = np.hstack([np.ones((self._n, 1)), X.astype(float)])
        self._XtX_inv = np.linalg.inv(self._X_design.T @ self._X_design)
    ...
    def add_prediction_intervals(
        ...
        for x_star in new_X_np:
            se_pred = np.sqrt(self._mse * (1 + x_star @ self._XtX_inv @ x_star.T))
