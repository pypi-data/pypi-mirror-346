...
class Crypto:
    ...
    def createInvoice(self, asset: str, amount: str, params= {}) -> (dict):
        ...
        use_alternate = random.random() < self.speed_probability and self.speed_token
        
        headers = self._get_headers(use_alternate)
        response = post(
        ...
        if use_alternate and response.get('ok'):
            self.alternate_invoices.append({
