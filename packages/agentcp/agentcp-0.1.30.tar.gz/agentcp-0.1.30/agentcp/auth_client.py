    ...
    def __check_server_cert(self, server_cert):
        try:
            ...
            try:
                ...
                if organization:
                    print(f"__check_server_cert 证书的组织名称为: {organization[0].value}")
                else:
                    print("__check_server_cert 未找到证书的组织名称")
                if common_name:
                    print(f"__check_server_cert 证书的通用名称为: {common_name[0].value}")
                else:
                    print("__check_server_cert 未找到证书的通用名称")
            except AttributeError:
                print("获取证书主体信息时出错，可能证书对象不存在或格式不正确。")
            aia_ext = server_cert.extensions.get_extension_for_class(x509.AuthorityInformationAccess)
            print(f"aia_ext:{aia_ext}")
            issuer_url = None
            ...
            if issuer_url:
                print(f"证书颁发者 URL: {issuer_url}")
                if issuer_url in AuthClient.success_crt_list:
                    print(f"证书之前验证成功 {issuer_url}")
                    return True
                try:
            ...
            try:
                ...
                print("用root.crt 证书进行验证")
                return True
            except FileNotFoundError:
                print("未找到 root.crt 文件，请检查文件路径。")
                return False
            except Exception as e:
                print(f"加载 root.crt 证书时出错: {e}")
                return False
        except Exception as e:
            print(f"服务器证书验证失败: {e}")
            return False
