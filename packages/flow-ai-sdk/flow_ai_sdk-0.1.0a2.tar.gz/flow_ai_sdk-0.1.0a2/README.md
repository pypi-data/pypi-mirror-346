...
client = FlowAISDK(
    bearer_token=os.environ.get(
        "FLOW_AI_SDK_BEARER_TOKEN"
    ),  # This is the default and can be omitted
    api_key=os.environ.get("FLOW_AI_SDK_API_KEY"),  # This is the default and can be omitted
)

response = client.api.v1.health.check()
```

While you can provide a `bearer_token` keyword argument,
we recommend using [python-dotenv](https://pypi.org/project/python-dotenv/)
to add `FLOW_AI_SDK_BEARER_TOKEN="My Bearer Token"` to your `.env` file
...
async def main() -> None:
    response = await client.api.v1.health.check()
...
client.with_options(max_retries=5).api.v1.health.check()
```
...
client.with_options(timeout=5.0).api.v1.health.check()
```
...
health = response.parse()  # get the object that `api.v1.health.check()` would have returned
print(health)
```
