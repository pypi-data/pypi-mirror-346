...
from ._exceptions import APIStatusError, FlowAisdkError
...
class FlowAISDK(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        - `bearer_token` from `FLOW_AI_SDK_BEARER_TOKEN`
        - `api_key` from `FLOW_AI_SDK_API_KEY`
        """
        if bearer_token is None:
            bearer_token = os.environ.get("FLOW_AI_SDK_BEARER_TOKEN")
        if bearer_token is None:
            raise FlowAisdkError(
                "The bearer_token client option must be set either by passing bearer_token to the client or by setting the FLOW_AI_SDK_BEARER_TOKEN environment variable"
            )
        ...
        if api_key is None:
            raise FlowAisdkError(
                "The api_key client option must be set either by passing api_key to the client or by setting the FLOW_AI_SDK_API_KEY environment variable"
            )
