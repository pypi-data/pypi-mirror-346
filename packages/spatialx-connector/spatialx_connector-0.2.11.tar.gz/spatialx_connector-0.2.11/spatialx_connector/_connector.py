...
class SpatialXConnector(Connector):
    ...
    def submit_multiple_samples(
        ...
        results: ``List[Dict[str, Union[str, List[dict]]]]``
            Submission information
        """
        study_id = self.create_study(group, species, title)[ConnectorKeys.STUDY_ID.value]
    ...
    def upload_big_file(
        ...
        while True:
            ...
            self.openapi.upload_chunk_process(
                ...
                folder_name=server_folder_name,
                upload_id=upload_id,
                path=resp.get(ConnectorKeys.ROOT_FOLDER.value, resp[ConnectorKeys.PATH.value]),
