...
from paniki.processors.frame_processor import FrameDirection
from paniki.transports.base_input import BaseInputTransport
from paniki.transports.base_output import BaseOutputTransport
from paniki.transports.base_transport import BaseTransport, TransportParams
...
try:
    ...
    logger.error("In order to use the PanikiWebRTC, you need to `pip install paniki-voice[webrtc]`.")
    raise Exception(f"Missing module: {e}")


class PanikiWebRTCallbacks(BaseModel):
...
class PanikiWebRTCClient:
    ...
    def __init__(self, webrtc_connection: SmallWebRTCConnection, callbacks: PanikiWebRTCallbacks):
    ...
    def _convert_frame(self, frame_array: np.ndarray, format_name: str) -> np.ndarray:
        ...
        conversion_code = PanikiWebRTCClient.FORMAT_CONVERSIONS.get(format_name)
...
class PanikiWebRTCInputTransport(BaseInputTransport):
    def __init__(
        self,
        client: PanikiWebRTCClient,
    ...
    async def push_app_message(self, message: Any):
        logger.debug(f"Received app message inside PanikiWebRTCInputTransport  {message}")
...
class PanikiWebRTCOutputTransport(BaseOutputTransport):
    def __init__(
        self,
        client: PanikiWebRTCClient,
        params: TransportParams,
...
class PanikiWebRTCTransport(BaseTransport):
    def __init__(
        self,
        ...
        self._callbacks = PanikiWebRTCallbacks(
        ...
        self._client = PanikiWebRTCClient(webrtc_connection, self._callbacks)

        self._input: Optional[PanikiWebRTCInputTransport] = None
        self._output: Optional[PanikiWebRTCOutputTransport] = None
    ...
    def input(self) -> PanikiWebRTCInputTransport:
        if not self._input:
            self._input = PanikiWebRTCInputTransport(
                self._client, self._params, name=self._input_name
            )
        return self._input

    def output(self) -> PanikiWebRTCOutputTransport:
        if not self._output:
            self._output = PanikiWebRTCOutputTransport(
                self._client, self._params, name=self._input_name
            )
        return self._output
