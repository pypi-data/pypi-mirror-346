...
def _warn_deprecated_access(globals: Dict[str, Any], attr, old: str, new: str):
    ...
    if module_key not in _warned_modules:
        with warnings.catch_warnings():
            ...
            warnings.warn(
                f"Module `paniki.services.{old}` is deprecated, use `paniki.services.{new}` instead.",
                DeprecationWarning,
                stacklevel=3,
            )
...
class DeprecatedModuleProxy:
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"module 'paniki.services.{self._old}' has no attribute '{attr}'")
