...
from paniki.frames.frames import InputAudioRawFrame, OutputImageRawFrame, StartFrame
...
try:
    ...
    logger.error(
        "In order to use local audio, you need to `pip install paniki-voice[local]`. On MacOS, you also need to `brew install portaudio`."
    )
