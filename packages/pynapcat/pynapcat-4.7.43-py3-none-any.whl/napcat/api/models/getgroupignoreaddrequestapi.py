...
class GroupIgnoreAddRequestItem(BaseModel):
    ...
    invitor_nick: str | None = Field(..., description="邀请者昵称")
    group_id: int | None = Field(..., description="群号")
    message: str | None = Field(..., description="加群消息")
    group_name: str | None = Field(..., description="群名称")
    checked: bool = Field(..., description="是否已处理（true为已处理，false为未处理）")
    actor: int = Field(..., description="处理者QQ号")
    requester_nick: str | None = Field(..., description="请求者昵称")
...
class GetGroupIgnoreAddRequestRes(BaseModel):
    ...
    echo: str | None = Field(None, description="用户附加数据")
# endregion res
