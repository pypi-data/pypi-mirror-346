...
class HandleQuickOperationReq(BaseModel):
    ...
    context: dict[Any,Any] = Field(..., description="事件数据对象")
    operation: dict[Any,Any] = Field(..., description="快速操作对象")
...
class HandleQuickOperationRes(BaseModel):
    ...
    对事件执行快速操作的响应模型
    """
    status: Literal["ok"] = Field("ok", description="状态码，固定为 'ok'") # Applied rule 2
