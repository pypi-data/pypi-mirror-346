...
class NewGeminiClient(Client):
    ...
    async def _get_params(
        ...
        if cache_system:
            config_args = {
                "cached_content": await self.get_prompt_cache_name(
    ...
    async def get_prompt_cache_name(
        self, prompt_text: str, tools: list[Tool] | None
    ) -> str:
        if prompt_text in self.cache_name_store:
            ...
            if (
                ...
                return cache_name
        return await self.construct_system_and_tool_cache(prompt_text, tools)

    async def construct_system_and_tool_cache(
        self,
        system: str | SystemMessage,
        tools: list[Tool] | None,
    ) -> str:
        ...
        cache_object = await self.client.caches.create(
            model=self.model,
