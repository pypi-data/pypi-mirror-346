...
class GCSFSStorageClient(StorageClientBase):
    ...
    def find(self, path: Path) -> List[Path]:
        ...
        path = path.relative_to(config.storage_root)

        blobs = bucket.list_blobs(prefix=str(path))

        # add storage root back
        return [Path(config.storage_root) / Path(blob.name) for blob in blobs]
...
class ApiStorageClient(StorageClientBase):
    ...
    def put(self, lpath: Path, rpath: Path) -> None:
        ...
        self.api_client.stream_upload_file_request(
            route=ApiRoutes.DOWNLOAD_PRESIGNED_FILE,
            lpath=lpath,
            rpath=rpath,
        )
