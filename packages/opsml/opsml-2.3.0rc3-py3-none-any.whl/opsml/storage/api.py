...
class ApiClient:
    ...
    @retry(reraise=True, stop=stop_after_attempt(3))
    def stream_upload_file_request(
        self,
        route: str,
        lpath: Path,
        rpath: Path,
        ...
        logging.info(f"Uploading file from {lpath.as_posix()} to {presigned_url}")
        headers = {"Content-Type": "application/octet-stream"}
        with open(lpath.as_posix(), "rb") as local_file:
            with self.client.stream("PUT", presigned_url, content=local_file, headers=headers) as response:
                response.read()  # Ensure upload completes
                if response.status_code not in (200, 201):  # GCS may return 201 for successful upload
                    try:
                        detail = response.json().get("detail", "No detail provided")
                    except Exception:
                        detail = "Unable to parse error response"
                    raise ValueError(f"Failed to upload file. Status: {response.status_code}, Detail: {detail}")
    ...
    def stream_download_file_request(
        ...
        response = self.client.get(url=url, params={"path": read_path.as_posix(), "method": "GET"})
