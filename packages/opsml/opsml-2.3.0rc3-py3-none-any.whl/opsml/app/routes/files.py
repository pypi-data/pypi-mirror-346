...
from opsml.storage.client import StorageClientBase, get_storage_client
...
def download_file(request: Request, path: str) -> StreamingResponse:
    ...
    storage_client: StorageClientBase = get_storage_client(config)
    try:
...
def list_files(request: Request, path: str) -> ListFileResponse:
    ...
    try:
        logger.info("Server: Listing files in {}", swapped_path)
...
@router.get("/files/presigned", name="presigned_url")
def presigned_url(
    request: Request,
    path: str,
    method: str,
    ...
    Returns:
        Streaming file response
    """
    logger.info("Server: Generating presigned url {} method: {} type: {}", path, method, content_type)
    swapped_path = swap_opsml_root(request, Path(path))
    storage_client: StorageClientBase = get_storage_client(config)
