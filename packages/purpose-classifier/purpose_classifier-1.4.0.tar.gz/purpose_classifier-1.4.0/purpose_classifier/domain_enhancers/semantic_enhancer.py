    ...
    def enhance_classification(self, result, narration, message_type=None):
        ...
        try:
            ...
            if matched and purpose_code:
                ...
                # If we have a high-confidence semantic match with AT LEAST 3 WORDS, return immediately
                # Enforce the 3-word rule strictly
                if confidence > 0.8 and len(unique_words) >= 3:
                    logger.info(f"Semantic match with 3+ words: {len(unique_words)} words matched {len(unique_terms)} terms")
                    return match_result
                elif confidence > 0.8 and len(unique_words) < 3:
                    logger.debug(f"Insufficient semantic matches: only {len(unique_words)} words (3+ word rule enforced)")
                    # Don't return the match if fewer than 3 words match
        ...
        if potential_matches:
            ...
            # Only return if we have sufficient confidence and AT LEAST 3 MATCHES (enforcing 3-word rule)
            if best_match['confidence'] > 0.7 and best_match['match_count'] >= 3:
                logger.info(f"Selected best match with 3+ words: {best_match['match_count']} matches with confidence {best_match['confidence']}")
                return best_match
            elif best_match['confidence'] > 0.9 and best_match['match_count'] >= 2:
                # Allow 2 matches only if confidence is very high (>0.9)
                logger.info(f"Selected high confidence match with 2 words: {best_match['match_count']} matches with confidence {best_match['confidence']}")
                return best_match
            elif best_match['match_count'] < 3:
                logger.debug(f"Insufficient matches in best match: only {best_match['match_count']} matches (3+ word rule enforced)")
                # Don't return the match if fewer than 3 words match
