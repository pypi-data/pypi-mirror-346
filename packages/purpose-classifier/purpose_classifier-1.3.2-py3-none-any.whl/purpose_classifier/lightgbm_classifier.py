...
class LightGBMPurposeClassifier:
    ...
    def _determine_category_purpose(self, purpose_code, narration, message_type=None):
        ...
        This method uses the category purpose mapper to get the direct mapping from purpose codes to category purpose codes.

        Args:
            purpose_code: The purpose code
            narration: Used for context-aware mapping
            message_type: Used for context-aware mapping

        Returns:
            tuple: (category_purpose_code, confidence)
        """
        # Use the category purpose mapper if available
        if hasattr(self, 'category_purpose_enhancer') and hasattr(self.category_purpose_enhancer, 'mapper'):
            mapper = self.category_purpose_enhancer.mapper
            category_purpose_code, category_confidence, reason = mapper.map_purpose_to_category(purpose_code, narration, message_type)
            logger.info(f"Category purpose mapper: {purpose_code} -> {category_purpose_code} (confidence: {category_confidence:.2f}, reason: {reason})")
            return category_purpose_code, category_confidence

        # Fallback to direct mapping if category purpose mapper is not available
        ...
        purpose_to_category_map = {
            ...
            # Property
            'PPTI': 'TRAD',  # Property Purchase (maps to Trade)
