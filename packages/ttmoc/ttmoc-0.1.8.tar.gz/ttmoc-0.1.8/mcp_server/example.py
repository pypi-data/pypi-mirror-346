...
def run_examples():
    ...
    # Example 10: Get profile photo path
    print("\n=== Example 10: Get profile photo path ===")
    result = client.get_profile_photo_path(alias="john.doe")
    print(f"Profile photo URL: {result}")
    print("HTML usage example: <img src=\"" + result + "\" alt=\"Profile Photo\">")
    
    # Example 11: Batch get profile photo paths
    print("\n=== Example 11: Batch get profile photo paths ===")
    result = client.batch_get_profile_photo_paths(aliases=["john.doe", "jane.smith", "alex.chen"])
    print("Profile photo URLs:")
    for alias, url in result.items():
        print(f"  {alias}: {url}")
    print("\nHTML table example:")
    print("<table>")
    print("  <tr><th>Employee</th><th>Photo</th></tr>")
    for alias, url in result.items():
        print(f"  <tr><td>{alias}</td><td><img src=\"{url}\" alt=\"{alias} Photo\"></td></tr>")
    print("</table>")
