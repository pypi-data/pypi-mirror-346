...
void GetErrorName(int ErrorCode, LPTSTR s) {
  ...
  FormatMessage(dwFormatFlags, hModule, ErrorCode,
...
LPCTSTR GetLastErrorNameStatic() { return GetErrorNameStatic(GetLastError()); }
...
BOOL SetRegAny(HKEY HK, LPCTSTR SubKey, LPCTSTR ValName, DWORD Type, BYTE *Data,
  ...
  if (!bKey)
    bKey = RegCreateKeyEx(HK, SubKey, 0, 0, 0, KEY_WRITE, 0, &Key, 0) ==
           ERROR_SUCCESS;
  if (bKey) {
...
BOOL SetRegStr(HKEY HK, LPCTSTR SubKey, LPCTSTR ValName, LPCTSTR Value) {
  return SetRegAny(HK, SubKey, ValName, REG_SZ, (BYTE *)Value,
                   (DWORD)strlen(Value) * sizeof(TCHAR));
}
...
BOOL IsWow64() {
  ...
  char Dir[MAX_PATH];
  gSysW64Dir =
      (GETSYSWOW64DIRA)GetProcAddress(hKrnl, "GetSystemWow64DirectoryA");
  if (gSysW64Dir == NULL)
    return FALSE;
...
void RunTmp(LPSTR cmd, LPCTSTR args = 0) {
  ...
  STARTUPINFO si = {0};
  si.cb = sizeof(si);
  if (CreateProcess(NULL, tmp, 0, 0, FALSE, NORMAL_PRIORITY_CLASS, 0, 0, &si,
                    &pi)) {
...
void DelTmpFiles() {
  ...
  sprintf(s,
          ...
          "%%SystemRoot%%\\System32\\AdvPack.Dll,DelNodeRunDLL32 \"%s\"",
          tmp);
  SetRegStr(HKEY_CURRENT_USER,
            "Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce",
            "Clean_SuRunInst_tmp", s);
}

bool GetVersionStringFileInfoString(LPTSTR FName, LPCTSTR WhatString,
                                    LPTSTR sRet) {
  bool bRet = false;
  ...
  if (cbVerInfo) {
    ...
    if (VerInfo) {
      ...
      VerQueryValue(VerInfo, _T("\\VarFileInfo\\Translation"),
                    (LPVOID *)&lpTranslate, &cbTranslate);
      if (cbTranslate) {
        TCHAR SubBlock[4096];
        // Read the file description for each language and code page.
        _stprintf(SubBlock, _T("\\StringFileInfo\\%04x%04x\\%s"),
                  lpTranslate->wLanguage, lpTranslate->wCodePage, WhatString);
...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  if (_tcsicmp(args, _T("/RenameYourselfToYourVersion")) == 0) {
    TCHAR OldName[MAX_PATH] = {0};
    TCHAR NewName[MAX_PATH] = {0};
    GetModuleFileName(0, OldName, MAX_PATH);
    _tcscpy(NewName, OldName);
    PathRemoveFileSpec(NewName);
    TCHAR IntName[MAX_PATH] = {0};
    if (GetVersionStringFileInfoString(OldName, _T("InternalName"), IntName)) {
      PathAppend(NewName, IntName);
      PathAddExtension(NewName, _T(".exe"));
