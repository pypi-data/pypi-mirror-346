...
// #define PUGOPT_MEMFIL //Uncomment to enable memory-mapped file parsing
// support. #define PUGOPT_NONSEG //Uncomment to enable non-destructive
// (non-segmenting) parsing support.
...
#define PUGAPI_INTERNAL_VERSION                                                \
  ((PUGAPI_INTERNAL_VERSION_MINOR & 0xFFFF) | PUGAPI_INTERNAL_VERSION_MAJOR    \
                                                  << 16)
...
typedef struct t_xml_attribute_struct {
  ...
  bool value_insitu; // True if 'value' is a segment of the original parse
                     // string.
#ifdef PUGOPT_NONSEG
...
static const unsigned long parse_grow =
    4; // Default child element & attribute space growth increment.

// Parser Options
static const unsigned long parse_minimal =
    0x00000000;                                   // Unset the following flags.
static const unsigned long parse_pi = 0x00000002; // Parse '&lt;?...?&gt;'
static const unsigned long parse_doctype =
    0x00000004; // Parse '&lt;!DOCTYPE ...&gt;' section, setting '[...]' as data
                // member.
static const unsigned long parse_comments =
    0x00000008; // Parse &lt;!--...--&gt;'
static const unsigned long parse_cdata =
    0x00000010; // Parse '&lt;![CDATA[...]]&gt;', and/or
                // '&lt;![INCLUDE[...]]&gt;'
...
static const unsigned long parse_trim_attribute =
    0x00000080; // Trim 'foo="..."'.
static const unsigned long parse_trim_cdata =
    0x00000100; // Trim '&lt;![CDATA[...]]&gt;', and/or
                // '&lt;![INCLUDE[...]]&gt;'
static const unsigned long parse_trim_entity =
    0x00000200; // Trim '&lt;!ENTITY name ...&gt;', etc.
static const unsigned long parse_trim_doctype =
    0x00000400; // Trim '&lt;!DOCTYPE [...]&gt;'
static const unsigned long parse_trim_comment =
    0x00000800; // Trim &lt;!--...--&gt;'
static const unsigned long parse_wnorm =
    0x00001000; // Normalize all entities that are flagged to be trimmed.
static const unsigned long parse_dtd =
    0x00002000; // If parse_doctype set, then parse whatever is in data member
                // ('[...]').
static const unsigned long parse_dtd_only =
...
typedef struct t_xml_node_struct {
  ...
  xml_attribute_struct *
      *attribute; // Array of pointers to attributes; see xml_attribute_struct.
...
#endif
  bool value_insitu; // True if 'data' is a segment of the original parse
                     // string.
} xml_node_struct;
...
inline static bool
chartype_symbol(TCHAR c) // Character is alphanumeric, -or- '_', -or- ':', -or-
                         // '-', -or- '.'.
{
  return (_istalnum(c) || c == _T('_') || c == _T(':') || c == _T('-') ||
          c == _T('.'));
}
inline static bool
chartype_space(TCHAR c) // Character is greater than 0 or character is less than
                        // exclamation.
{
  return (c > 0 && c < _T('!'));
}
...
inline static bool chartype_quote(
    TCHAR c) // Character is &quot;&lsquo;&quot; -or- &lsquo;&quot;&lsquo;.
{
  return (c == _T('"') || c == _T('\''));
}
...
inline static bool strcatgrown_impl(TCHAR **lhs, const TCHAR *rhs,
  ...
  if (!*lhs) // Null, allocate and copy.
  {
    ...
    *(*lhs + rsize) = 0;      // Terminate it.
    lsize = rsize;
  } else // Reallocate. NF I don't think this is right for MBCS, nor is code in
         // 'StrCatGrow()'.
  {
...
inline static bool strwtrim(TCHAR **s, unsigned int &len) {
  if (!s || !*s)
    return false;
  TCHAR *pse = *s + len;
  while (*s < pse && pug::chartype_space(**s)) // Find first non-white
                                               // character.
...
inline static bool strwnorm(TCHAR **s) {
  ...
  TCHAR *norm = (TCHAR *)malloc(
      sizeof(TCHAR) * (n + 1)); // Allocate a temporary normalization buffer.
  if (!norm)
...
//</remarks>
inline static bool strcpyinsitu(TCHAR **dest, const TCHAR *src, bool *insitu
#ifdef PUGOPT_NONSEG
                                ,
...
#ifdef PUGOPT_NONSEG
    if (strcatgrown(dest, src,
                    destlen)) // Allocate & copy source to destination
#else
    ...
    if (strcmpwild_impl(*src, *dst) == 0) {
      do {
        ...
        while (**src != **dst && **src != _T('[') && **dst != 0)
          ++(*dst);
      } while ((**dst != 0) ? strcmpwild_impl(*src, *dst) == 0
                            : 0 != (find = 0));
    }
    if (**dst == 0 && **src == 0)
      find = 1;
...
inline static xml_attribute_struct *new_attribute(void) {
  xml_attribute_struct *p = (xml_attribute_struct *)malloc(
      sizeof(xml_attribute_struct)); // Allocate one attribute.
...
inline static xml_node_struct *new_node(xml_node_type type = node_element) {
  xml_node_struct *p =
      (xml_node_struct *)malloc(sizeof(xml_node_struct)); // Allocate one node.
  if (p) // If allocation succeeded.
  {
    ...
    p->name_insitu = p->value_insitu =
        true; // Default to being in-situ of the parse string.
    if (type != node_document && // None of these will have attributes.
        type != node_pcdata && type != node_cdata && type != node_include &&
        type != node_comment)
      p->attribute = (xml_attribute_struct **)malloc(
          sizeof(xml_attribute_struct *)); // Allocate one attribute.
    else
      p->attribute = NULL;
    p->attribute_space = (p->attribute) ? 1 : 0;
    if (type == node_element || // Only these will have children.
        type == node_doctype || type == node_document)
      p->child = (xml_node_struct **)malloc(
          sizeof(xml_node_struct *)); // Allocate one child.
    else
      p->child = NULL;
...
inline static xml_node_struct *append_node(xml_node_struct *parent, long grow,
  ...
  {
    xml_node_struct **t = (xml_node_struct **)realloc(
        parent->child, sizeof(xml_node_struct *) *
                           (parent->child_space + grow)); // Grow pointer space.
...
inline static xml_attribute_struct *append_attribute(xml_node_struct *node,
                                                     long grow) {
  if (!node)
    return NULL;
  ...
  {
    xml_attribute_struct **t = (xml_attribute_struct **)realloc(
        node->attribute,
        sizeof(xml_node_struct *) * (node->attribute_space + grow));
    if (t) {
      node->attribute = t;
...
inline static void free_node(xml_node_struct *node) {
  ...
  do {
  LOC_STEP_INTO:
    for (; cursor->children > 0;
         --cursor->children) // Free each child in turn; 'children' keeps count
                             // while we jump around.
    {
      register xml_node_struct *t =
          cursor->child[cursor->children - 1]; // Take a pointer to the child.
      if (t && t->children)                    // If the child has children.
      {
        ...
        if (t->attribute)
          free(t->attribute); // Free attribute pointer space.
        if (t->child)
          free(t->child); // Free child pointer space.
        if (t->name && !t->name_insitu)
          free(t->name);
        if (t->value && !t->value_insitu)
          free(t->value);
  ...
  if (cursor->attribute)
    free(cursor->attribute); // Free attribute pointer space.
  if (cursor->child)
    free(cursor->child); // Free child pointer space.
  if (cursor->name && !cursor->name_insitu)
    free(cursor->name); // Free name & data.
  if (cursor->value && !cursor->value_insitu)
    free(cursor->value);
...
inline static void free_node_recursive(xml_node_struct *root) {
  if (root) {
    ...
    free(root->child);
    if (root->name && !root->name_insitu)
      free(root->name);
    if (root->value && !root->value_insitu)
      free(root->value);
    free(root);
  }
}
...
//</remarks>
static TCHAR *parse(register TCHAR *s, xml_node_struct *xmldoc, long growby,
    ...
    LOC_CLASSIFY:          // What kind of element?
...
#ifndef PUGOPT_NONSEG
          if (chartype_pi(*s))
            *s = _T('/'); // Same semantics as for '<.../>', so fudge it.
#endif
          s = mark;
      ...
      } else if (chartype_special(*s)) //'<!...'
      {
        ...
        if (chartype_dash(*s)) //'<!-...'
        {
          ...
          if (OPTSET(parse_comments) && chartype_dash(*s)) //'<!--...'
          {
            ...
            while (*s != 0 && *(s + 1) && *(s + 2) &&
                   !((chartype_dash(*s) && chartype_dash(*(s + 1))) &&
                     chartype_leave(*(s + 2))))
              ++s; // Scan for terminating '-->'.
            if (*s == 0)
              return s;
#ifdef PUGOPT_NONSEG
...
#else
              if (OPTSET(parse_wnorm))
                strwnorm(&cursor->value);
              else
                strwtrim(&cursor->value);
#endif
            }
            s += 2;         // Step over the '\0-'.
                      ...
                      if (*s == _T('E')) //'<![INCLUDE...'
                      {
                        ...
                        if (chartype_lbracket(*s)) //'<![INCLUDE[...'
                        {
                          ...
                          if (OPTSET(node_cdata)) {
                            PUSHNODE(
                                node_include); // Append a new node on the tree.
                            cursor->value = s; // Save the offset.
                            while (!(chartype_rbracket(*s) &&
                                     chartype_rbracket(*(s + 1)) &&
                                     chartype_leave(*(s + 2))))
                              ++s; // Scan for terminating ']]>'.
                            ...
                            POPNODE(); // Pop since this is a standalone.
                          } else // Flagged for discard, but we still have to
                  ...
                  if (*s == _T('A')) //'<![CDATA...'
                  {
                    ...
                    if (chartype_lbracket(*s)) //'<![CDATA[...'
                    {
                      ...
                      if (OPTSET(parse_cdata)) {
                        ...
                        POPNODE(); // Pop since this is a standalone.
                      } else // Flagged for discard, but we still have to scan
                             // for the terminator.
                      {
                        while (*s != 0 && *(s + 1) != 0 && *(s + 2) != 0 &&
                               !(chartype_rbracket(*s) &&
                  ...
                  if (*s == _T('P')) //'<!DOCTYP...'
                  {
                    ...
                    if (*s == _T('E')) //'<!DOCTYPE...'
                    {
                      ...
                      if (OPTSET(parse_doctype)) {
                        PUSHNODE(node_doctype); // Append a new node on the
                                                // tree.
                        a = append_attribute(cursor,
...
#ifdef PUGOPT_NONSEG
                      if (OPTSET(parse_doctype))
                        a->name_size = a->value_size =
                            s -
                            a->value; // Save the length. rem: Before ENDSEG()
#endif
                    ...
                    LOC_DOCTYPE_SYMBOL:
                      if (chartype_symbol(*s)) {
                        ...
                        if (OPTSET(parse_doctype)) {
                          ...
                          a->value = a->name = mark;
#ifdef PUGOPT_NONSEG
                          a->value_size = a->name_size =
                              s - mark; // NF 19 Jan 2003.
#else
                          *s = 0;
#endif
                        }
                      ...
                      LOC_DOCTYPE_QUOTE:
                        ...
                        if (*s != 0) {
                          ...
                          if (chartype_quote(*s))
                            ...
                            goto LOC_DOCTYPE_SYMBOL; // Not wellformed, but just
                                                     // parse it.
                        }
                      }
                      if (chartype_lbracket(*s)) //'...[...'
                      {
                        ...
                        if (OPTSET(parse_doctype))
                          cursor->value = s; // Store the offset.
                        ...
                        if (OPTSET(parse_doctype)) {
                          // Note: If we aren't parsing the DTD ('!parse_dtd',
                          // etc.) then it is stored in the DOM as one whole
                          // chunk.
#ifdef PUGOPT_NONSEG
                          SETLEN(); // NF 19 Jan 2003
#else
                          *s = 0; // Zero-terminate.
                          ...
                          if (OPTSET(parse_dtd) || OPTSET(parse_dtd_only)) {
                            if (OPTSET(parse_dtd)) {
                              ...
                              try {
                                *s = 0; // Zero-terminate.
                                parse(cursor->value, cursor, growby,
                                      optmsk); // Parse it.
                              } catch (...) {
                                assert(false);
                              }
                            ...
                            if (OPTSET(parse_dtd_only))
                              return (s + 1); // Flagged to parse DTD only, so
                                              // leave here.
                          } else if (OPTSET(parse_trim_doctype)) // Trim
                                                                 // whitespace.
                          {
#ifdef PUGOPT_NONSEG
                            strwtrim(&cursor->value, cursor->value_size);
        ...
        } else if (chartype_symbol(*s)) // An inline DTD tag.
        {
          ...
          if (_tcsncmp(mark, _T("ATTLIST"),
                       max((7 * sizeof(TCHAR)), dtdilen)) == 0)
            e = node_dtd_attlist;
          else if (_tcsncmp(mark, _T("ELEMENT"),
                            max((7 * sizeof(TCHAR)), dtdilen)) == 0)
            e = node_dtd_element;
          else if (_tcsncmp(mark, _T("NOTATION"),
                            max((8 * sizeof(TCHAR)), dtdilen)) == 0)
            e = node_dtd_notation;
#else
          if (_tcscmp(mark, _T("ATTLIST")) == 0)
            e = node_dtd_attlist;
          else if (_tcscmp(mark, _T("ELEMENT")) == 0)
            e = node_dtd_element;
          else if (_tcscmp(mark, _T("NOTATION")) == 0)
            e = node_dtd_notation;
#endif
          ...
          if (*s != 0 && chartype_space(ch)) {
            ...
            if (chartype_symbol(*s) || *s == _T('%')) {
              ...
              {
#ifdef PUGOPT_NONSEG
                // Note: For memory-mapped file support we need to treat 's' as
                // read-only so we can't do '*(s-1) = _T('%');' below.
                cursor->name = mark; // Sort out extraneous whitespace when we
                                     // retrieve it. TODO: Whitespace cleanup.
#endif
                ++s;
                if (chartype_space(*s)) {
                  SKIPWS(); // Eat whitespace.
              ...
              if (chartype_space(ch)) {
                SKIPWS();                 // Eat whitespace.
                if (e == node_dtd_entity) // Special case; may have multiple
                                          // quoted sections w/anything inside.
                {
                  ...
                  while (*s != 0)    // Loop till we find the right sequence.
                  {
                    if (!qq && chartype_quote(*s)) {
                      ...
                      qq = false;
                             ...
                             chartype_leave(
                                 *s)) // Not in quoted reqion and '>' hit.
                    {
#ifdef PUGOPT_NONSEG
                      SETLEN(); // NF 19 Jan 2003.
#else
      ...
      LOC_ELEMENT:                            // Scan for & store element name.
        ...
        // chartype_pi(ch) //Treat '?>' as '/>' NF 19 Jan 2003
          ...
          SCANFOR(
              chartype_leave(*s)); // Scan for '>', stepping over the tag name.
          POPNODE();               // Pop.
          continue;
        ...
        LOC_ATTRIBUTE:
          if (chartype_symbol(*s)) //<... #...
          {
            xml_attribute_struct *a = append_attribute(
                cursor, growby);            // Make space for this attribute.
            ...
            if (*s != 0 && chartype_space(ch))
              SKIPWS(); // Eat any whitespace.
            if (*s != 0 &&
                (chartype_equals(ch) || chartype_equals(*s))) //'<... #=...'
            {
              if (chartype_equals(*s))
                ++s;
              SKIPWS();               // Eat any whitespace.
              if (chartype_quote(*s)) //'<... #="...'
              {
                ...
                if (chartype_space(
                        *s)) // This may indicate a following attribute.
                {
                  SKIPWS();           // Eat any whitespace.
...
#ifdef PUGOPT_NONSEG
              SCANFOR(chartype_pi(
                  *s)); // compliments change where we don't fudge to '/>' when
                        // we find the PI. NF 20 Jan 2003
              SKIPWS(); // Eat any whitespace.
              if (chartype_pi(*s))
                ++s;
#else
      ...
      LOC_LEAVE:
        ...
        LOC_PCDATA: //'>...<'
          ...
          if (chartype_enter(*s)) // We hit a '<...', with only whitespace, so
                                  // don't bother storing anything.
          {
            if (chartype_close(*(s + 1))) //'</...'
            {
              SCANFOR(chartype_leave(
                  *s));  // Scan for '>', stepping over any end-tag name.
              POPNODE(); // Pop.
              continue;  // Continue scanning.
            } else
              goto LOC_SEARCH; // Expect a new element enter, so go scan for it.
          }
          s = mark; // We hit something other than whitespace; restore the
                    // original offset.
          ...
          if (chartype_enter(ch)) // Did we hit a '<...'?
          {
            if (chartype_close(*s)) //'</...'
            {
              ...
              goto LOC_CLASSIFY; // We hit a '<!...'. We must test this here if
                                 // we want comments intermixed w/PCDATA.
            else if (*s)
              goto LOC_CLASSIFY;
            else
              return s;
...
inline static bool load_file(const TCHAR *path, TCHAR **buffer,
  ...
  *buffer = 0;
  ...
  while (ReadFile(file_handle, (void *)temp, tempsize - 1, &read_bytes, 0) &&
         read_bytes && strcatgrow(buffer, temp)) {
    *size += read_bytes;
    ZeroMemory(temp, sizeof(TCHAR) * tempsize);
  }
...
public:
  ...
  pointer_array(unsigned int grow = 4)
      : _size(0), _room(0), _data(NULL), _grow(grow) {
...
public:
  ...
  void clear()                     // Free any allocated memory.
  {
    if (_data) {
      _data = (void **)realloc(
          _data,
          sizeof(void *) * _grow); // Reallocate to first growth increment.
      _room = _grow;               // Mark it as such.
      _size = 0;                   // Mark array as empty.
    }
  }
  virtual void *&operator[](unsigned int i) // Access element at subscript, or
                                            // dummy value if overflow.
  {
    static void *dummy = 0;
    if (i < _size)
      return _data[i];
  ...
  long push_back(void *element) // Append a new element to the array.
  {
    if (_data) // Fail if no array.
    {
      ...
      {
        void **temp = (void **)realloc(
            _data, sizeof(void *) * (_size + _grow)); // Grow the array.
...
public:
  ...
  indent_stack(TCHAR c = _T('\t')) : _inch(c), _stac(0), _size(0) {
...
//</returns>
inline static void
outer_xml(std::basic_ostream<TCHAR, std::char_traits<TCHAR>> &os,
          indent_stack &indent, xml_node_struct *node, bool breaks = true) {
  if (node && os.good()) // There is a node and ostream is OK.
  {
    ...
    case node_doctype:
      ...
      if (node->children) {
        ...
        for (i = 0; i < n; ++i) {
          if (node->child[i] && // There is a child at i.
              (node->child[i]->type ==
                   node_dtd_attlist || // Skip all other types.
    ...
    case node_pi:
      ...
      if (n && node->type == node_element) {
        ...
        if (n == 1 && node->child[0]->type == node_pcdata) {
          ...
          indent.push();
          for (i = 0; i < n; ++i)
            pug::outer_xml(os, indent, node->child[i], breaks);
          indent.pop();
          os << indent.depth();
        }
        os << _T("</");
...
public:
  ...
  xml_iterator(xml_node_struct *vref, long sscr = 0)
      : _vref(vref), _sscr(sscr) {} // Initializing constructor.
  xml_iterator(const xml_iterator &r)
      : _vref(r._vref), _sscr(r._sscr) {} // Copy constructor.
...
public:
  ...
  virtual xml_iterator &operator++() {
    _sscr++;
    return *this;
  } // Increment the iterator (subscript).
  virtual xml_iterator &operator--() {
    _sscr--;
    return *this;
...
public:
  ...
  xml_attribute(xml_attribute_struct *attr)
      : _attr(attr) {} // Initializing constructor.
...
public:
  ...
  operator long() {
    ...
    const unsigned int maxlen =
        valulen ? min(valulen, _attr->value_size) : _attr->value_size;
  ...
  xml_attribute &operator=(const stdstring &rhs) {
    value(rhs.c_str());
    return *this;
  }
...
#ifdef PUGOPT_NONSEG
  bool has_name(const TCHAR *name) {
    return (name && !empty() && has_name() &&
            _tcsncmp(_attr->name, name, _attr->name_size) == 0);
  } // Is named 'name'.
  bool has_value(const TCHAR *value) {
    return (value && !empty() && has_value() &&
            _tcsncmp(_attr->value, value, _attr->value_size) == 0);
  } // Has value 'value'.
#else
  bool has_name(const TCHAR *name) {
    return (name && !empty() && has_name() && _tcscmp(_attr->name, name) == 0);
  } // Is named 'name'.
  bool has_value(const TCHAR *value) {
    return (value && !empty() && has_value() &&
            _tcscmp(_attr->value, value) == 0);
  } // Has value 'value'.
#endif
public:
  const TCHAR *name() {
    return (!empty() && _attr->name) ? _attr->name : _T("");
  } // Access the attribute name.
#ifdef PUGOPT_NONSEG
  const unsigned int name_size() {
    return (!empty()) ? _attr->name_size : 0;
  } // Access the attribute name length (for PUGOPT_NONSEG).
#endif
  ...
  const TCHAR *value() {
    return (!empty()) ? _attr->value : _T("");
  } // Access the attribute value.
#ifdef PUGOPT_NONSEG
  const unsigned int value_size() {
    return (!empty()) ? _attr->value_size : 0;
  } // Access the attribute name length (for PUGOPT_NONSEG).
#endif
...
public:
  ...
  //<remarks>It is possible that 'p' is NULL, so test for this with
  //'empty'.</remarks>
...
public:
  ...
  class xml_node_iterator
      : public xml_iterator<xml_node, long, xml_node *, xml_node &> {
  protected:
    forward_class<xml_node> _wrap; // Wrapper for xml_node.
  public:
    xml_node_iterator()
        : _wrap(), xml_iterator<xml_node, long, xml_node *, xml_node &>() {
    } // Default constructor.
    xml_node_iterator(xml_node_struct *vref, long sscr = 0)
        : _wrap(),
          xml_iterator<xml_node, long, xml_node *, xml_node &>(vref, sscr) {
    ...
    {
      if (!good() ||   // There is no data over which to iterate.
    ...
    virtual xml_node &operator*() {
      if (!oob())
        _wrap->attach(_vref->child[_sscr]);
      else
        _wrap->attach(NULL);
  ...
  class xml_attribute_iterator
      : public xml_iterator<xml_attribute, long, xml_attribute *,
                            xml_attribute &> {
  protected:
  ...
  public:
    xml_attribute_iterator()
        : _wrap(), xml_iterator<xml_attribute, long, xml_attribute *,
                                xml_attribute &>() {} // Default constructor.
    xml_attribute_iterator(xml_node_struct *vref, long sscr = 0)
        : _wrap(),
          xml_iterator<xml_attribute, long, xml_attribute *, xml_attribute &>(
              vref, sscr) {} // Initializing constructor.
    xml_attribute_iterator(const xml_attribute_iterator &r)
        : _wrap(),
          xml_iterator<xml_attribute, long, xml_attribute *, xml_attribute &>(
              r) {}     // Copy constructor.
    ...
    virtual xml_attribute &operator*() {
      if (!oob())
        _wrap->attach(_vref->attribute[_sscr]);
      else
        _wrap->attach(NULL);
  ...
  //<summary>Base iterator type (for child nodes). Same as
  //'child_iterator'.</summary>
  typedef xml_node_iterator iterator;
  //<summary>Base iterator type (for child nodes). Same as 'iterator'.</summary>
  typedef xml_node_iterator child_iterator;
  //<summary>Base iterator type (for sibling nodes). Same as
  //'iterator'.</summary>
...
public:
  ...
  xml_node &operator=(const xml_node &r) {
    _root = r._root;
    return *this;
  } // Assign to xml_node_struct pointer.
...
public:
  ...
  bool type_null() {
    return empty();
  } // Node pointer is null, or type is node_null. Same as empty.
  bool type_document() {
...
public:
  ...
  bool has_name(const TCHAR *name) const {
    return (name && _root && _root->name &&
            _tcsncmp(_root->name, name, _root->name_size) == 0);
  } // Node is named 'name'.
#else
  bool has_name(const TCHAR *name) const {
...
#endif
  bool has_attribute(const TCHAR *name) {
...
public:
  ...
  const TCHAR *name() {
    ...
    if (has_name()) {
      ...
      temp[_root->name_size < PUGDEF_ELEM_NAME_SIZE
               ? _root->name_size
               : (PUGDEF_ELEM_NAME_SIZE - 1)] = 0;
      return temp;
    }
    return _T("");
  }
  unsigned int name_size() {
    return (has_name()) ? _root->name_size : 0;
  } // Get node name length if any, else 0.
  unsigned int value_size() {
    return (has_value()) ? _root->value_size : 0;
  } // Get node value length if any, else 0.
  inline bool matches_attribute_name(const TCHAR *name,
                                     const unsigned int namelen,
                                     const int i) const {
    return (_tcsncmp(name, _root->attribute[i]->name,
                     max(namelen, _root->attribute[i]->name_size)) == 0);
  } // There is an attribute at 'i' named 'name'.
  inline bool matches_child_name(const TCHAR *name, const unsigned int namelen,
                                 const int i) const {
    return (_tcsncmp(name, _root->child[i]->name,
                     max(namelen, _root->child[i]->name_size)) == 0);
  } // There is a child at 'i' named 'name'.
  inline bool matches_name(const TCHAR *name, const unsigned int namelen,
                           xml_node_struct *node) const {
    return (_tcsncmp(name, node->name, max(namelen, node->name_size)) == 0);
  } // This is named 'name'.
  inline bool matches_value(const TCHAR *data, const unsigned int datalen,
                            xml_node_struct *node) const {
    return (_tcsncmp(data, node->value, max(datalen, node->value_size)) == 0);
  } // This is valued 'value'.
  inline bool matches_attribute_name(const TCHAR *name,
                                     const unsigned int namelen,
                                     xml_attribute_struct *attr) const {
    return (_tcsncmp(name, attr->name, max(namelen, attr->name_size)) == 0);
  } // The given attribute is named 'name'.
  inline bool matches_attribute_name_value(const TCHAR *value,
                                           const unsigned int valulen,
                                           xml_attribute_struct *attr) const {
    return (_tcsncmp(value, attr->value, max(valulen, attr->value_size)) == 0);
  } // The given attribute is valued 'value'.
#else
  const TCHAR *name() {
    return (has_name()) ? _root->name : _T("");
  } // Access pointer to node name if any, else empty string.
  inline bool matches_attribute_name(const TCHAR *name,
                                     const unsigned int i) const {
    return (strcmpwild(name, _root->attribute[i]->name) == 0);
  } // There is an attribute at 'i' named 'name'.
  inline bool matches_child_name(const TCHAR *name,
                                 const unsigned int i) const {
    return (strcmpwild(name, _root->child[i]->name) == 0);
  } // There is a child at 'i' named 'name'.
  inline bool matches_name(const TCHAR *name, xml_node_struct *node) const {
    return (strcmpwild(name, node->name) == 0);
  } // This is named 'name'.
  inline bool matches_value(const TCHAR *data, xml_node_struct *node) const {
    return (strcmpwild(data, node->value) == 0);
  } // This is valued 'value'.
  inline bool matches_attribute_name(const TCHAR *attribute,
                                     xml_attribute_struct *attr) const {
  ...
  const TCHAR *value() {
    return (has_value()) ? _root->value : _T("");
  ...
  xml_node child(unsigned int i) {
    return (i < children()) ? xml_node(_root->child[i]) : xml_node();
  } // Access child node at subscript as xml_node or xml_node(NULL) if bad
    // subscript.
  unsigned int attributes() const {
    return _root->attributes;
  } // Access node's attribute count.
  xml_attribute attribute(unsigned int i) {
    return (i < attributes()) ? xml_attribute(_root->attribute[i])
                              : xml_attribute();
  ...
  xml_attribute attribute(const stdstring &name) {
  ...
  xml_attribute attribute(const TCHAR *name) {
    ...
    if (!attr)
      attr = append_attribute(name, _T(""));
    return xml_attribute(attr);
  }
  const unsigned int siblings() {
    return (!type_document()) ? _root->parent->children : 0;
  } // Access node's sibling count (parent's child count).
  xml_node sibling(unsigned int i) {
    return (!type_document() && i < siblings())
               ? xml_node(_root->parent->child[i])
               : xml_node();
  } // Access sibling node at subscript as xml_node or xml_node(NULL) if bad
    // subscript.
  xml_node parent() {
...
public:
    ...
    if (_root->children > 0) // Has children.
    {
      ...
      for (register unsigned int i = 0; i < n; ++i) {
        ...
        if (_root->child[i]->children)      // If there are children.
        {
          ...
          subsearch.all_elements_by_name(name,
                                         found); // Find any matching children.
        }
      }
    }
  }
  ...
  xml_node first_element_by_name(const stdstring &name) {
    return first_element_by_name(name.c_str());
  }
  ...
  //<returns>Valid xml_node if such element named 'name' is found with PCDATA
  //'value'.</returns> <remarks>xml_node may be invalid if not found; test with
  //'empty'.</remarks>
  xml_node first_element_by_value(const stdstring &name,
      ...
      for (register unsigned int i = 0; i < n; ++i) {
        if (_root->child[i] &&       // There is a child at i.
          ...
          register unsigned int m =
              _root->child[i]->children; // For each child of child.
          for (register unsigned int j = 0; j < m; ++j) {
            if (_root->child[i]->child[j] && // There is a child at j.
                _root->child[i]->child[j]->type ==
                    node_pcdata &&                  // It is of the PCDATA type.
        ...
        } else if (_root->child[i] &&
                   _root->child[i]->children) // The child has children.
        {
          xml_node subsearch(_root->child[i]); // Wrap it up for ease.
          xml_node found = subsearch.first_element_by_value(
              name, value); // Search any children.
          if (!found.empty())
            return found; // Found.
        }
      }
  ...
  //<returns>Valid xml_node if such element named 'name' is found.</returns>
  //<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
  xml_node first_element_by_attribute(const stdstring &name,
                                      const stdstring &attr_name,
                                      const stdstring &attr_value) {
    return first_element_by_attribute(name.c_str(), attr_name.c_str(),
                                      attr_value.c_str());
  }
  ...
  //<returns>Valid xml_node if such element named 'name' is found.</returns>
  //<remarks>xml_node may be invalid if not found; test with 'empty'.</remarks>
  xml_node first_element_by_attribute(const TCHAR *name, const TCHAR *attr_name,
                                      const TCHAR *attr_value) {
    if (empty() || !name || !attr_name || !attr_value)
    ...
    if (_root->children > 0) // Has children.
    {
      ...
      for (register unsigned int i = 0; i < n; ++i) {
        if (_root->child[i] &&       // There is a child at i.
          ...
          register unsigned int m =
              _root->child[i]->attributes; // For each attribute of child.
          for (register unsigned int j = 0; j < m; ++j) {
            if (_root->child[i]->attribute[j] && // There is an attribute at j.
                _root->child[i]
                    ->attribute[j]
                    ->name && // The attribute has a name.
#ifdef PUGOPT_NONSEG
                matches_attribute_name(attr_name, attrlen,
                                       _root->child[i]->attribute[j]) &&
#else
                matches_attribute_name(attr_name,
                                       _root->child[i]->attribute[j]) &&
#endif
                _root->child[i]
                    ->attribute[j]
                    ->value && // The attribute has a value.
#ifdef PUGOPT_NONSEG
                matches_attribute_name_value(attr_value, valulen,
                                             _root->child[i]->attribute[j])
#else
                matches_attribute_name_value(attr_value,
                                             _root->child[i]->attribute[j])
#endif
            )
              return xml_node(_root->child[i]); // Wrap it up and return.
          ...
          xml_node subsearch(_root->child[i]); // Wrap it up for ease.
          xml_node found = subsearch.first_element_by_attribute(
              name, attr_name, attr_value); // Search any children.
          if (!found.empty())
            return found; // Found.
        }
      }
  ...
  bool moveto_root() {
    ...
    while (!type_document())
      _root = _root->parent; // Keep stepping out until we hit the root.
    return true;             // Success.
  }
  ...
  bool moveto_sibling(unsigned int i) {
    if (empty())
      return false; // Nowhere to go.
    xml_node_struct *restore =
        _root; // Save position in case invalid subscript & we want to restore.
    if (moveto_parent()) // Try to move to parent.
    {
      if (i < children()) // Subscript is in range. (Assume parent *does* have
                          // children.)
      {
  ...
  bool moveto_first_sibling(const TCHAR *name) {
    ...
    if (moveto_parent()) // Try to move to parent.
    {
      ...
      for (register unsigned int i = 0; i < n; ++i) {
        ...
        xml_node node = child(i); // Access child node at subscript as xml_node
                                  // or xml_node(NULL) if bad subscript.
        if (node.type_element() ||
            node.type_pi()) // Other types won't have names.
        {
#ifdef PUGOPT_NONSEG
          if (_tcsncmp(name, node.name(), max(namelen, node.name_size())) ==
              0) // Do names match?
#else
  ...
  bool moveto_child(unsigned int i) {
    if (empty())
      return false; // Null, so no children.
    if (has_child_nodes() &&
        i < children()) // Has children and subscript is in bounds.
    {
  ...
  //<remarks>If matching sub-node was found, '_root' points thereto.</remarks>
  bool moveto_child(const stdstring &name) {
  ...
  bool moveto_child(const TCHAR *name) {
    if (empty() || !name || !has_child_nodes())
      return false; // The node is null, a name was not specified, or node has
                    // no children.
#ifdef PUGOPT_NONSEG
    const unsigned int namelen = _tcslen(name);
#endif
    ...
    for (register unsigned int i = 0; i < n; ++i) {
      // NF 24 Jan 2003: Changed to get child(i) just once per iteration.
      xml_node node = child(i); // Access child node at subscript as xml_node or
                                // xml_node(NULL) if bad subscript.
#ifdef PUGOPT_NONSEG
      if (_tcsncmp(name, node.name(), max(namelen, node.name_size())) ==
          0) // Do names match?
#else
      if (strcmpwild(name, node.name()) ==
          0) // If the name is identical with 'name'.
#endif
      {
  ...
  bool moveto_next_sibling() {
    ...
    register unsigned int n =
        _root->parent->children; // For each child of parent (each sibling).
    for (register unsigned int i = 0; i < (n - 1); ++i) {
      if (_root->parent->child[i] && // There is a child at i.
          _root->parent->child[i] ==
              _root && // The child is identical with this node.
  ...
  stdstring path(const TCHAR *delimiter = _T("/")) {
    ...
    strcatgrown_impl(&path, cursor.name(), destlen,
                     cursor.name_size()); // Get this name.
#else
    strcatgrow(&path, cursor.name()); // Get this name.
#endif
    while (cursor.moveto_parent() &&
           !cursor.type_document()) // Loop to parent (stopping on actual root
                                    // because it has no name).
    {
      temp = NULL; // Mark as null so 'strcatgrow' will allocate memory.
#ifdef PUGOPT_NONSEG
      destlen = 0;
      strcatgrown_impl(&temp, cursor.name(), destlen,
                       cursor.name_size()); // Append next element name.
#else
  ...
  xml_node first_element_by_path(const stdstring &path,
                                 const stdstring &delimiter = _T("/")) {
  ...
  xml_node first_element_by_path(const TCHAR *path,
    ...
    if (n == 0)
      return xml_node(); // Return null node if no path segments.
    if (path[0] == delimiter[0])
      found.moveto_root(); // Absolute path; e.g. '/foo/bar'
      ...
      if (name) {
        if (*name == _T('.')) // Is '.' or '..'
        {
          if (_tcscmp(name, _T("..")) == 0)
            found.moveto_parent(); // Pop.
          else
            continue; // Ignore '.' since it is redundant if path is './path'.
        } else {
          ...
          if (found.moveto_next_sibling(
                  found.name())) // Find next sibling having same name.
          {
            if (i > 0)
              --i; // Try the previous path segment.
            goto NEXT_ELEM;
          } else // Move to parent to search further.
          {
            if (!found.type_document() && found.moveto_parent() &&
                !found.type_document()) // Not root and stepped to parent and
                                        // parent is not root.
            {
              if (i > 0)
                --i; // Try the previous path segment.
              if (found.moveto_next_sibling(
                      found.name())) // Try to find next sibling having same
                                     // name.
              {
                if (i > 0)
                  --i; // Try the previous path segment.
  ...
  bool traverse(xml_tree_walker &walker) {
    ...
    if (walker.depth() == 0 && !walker.end(*this))
      return false; // Send the callback for end traverse if depth is zero.
    return true;
  }
...
public:
  ...
  //<returns>Success.</returns>
  bool remove_attribute(const stdstring &name) {
  ...
  bool remove_attribute(const TCHAR *name) {
    ...
    if (i > -1)
      return remove_attribute((unsigned int)i);
    return false;
  }
  ...
  xml_attribute append_attribute(const TCHAR *name, const TCHAR *value) {
    ...
    xml_attribute_struct *p = pug::append_attribute(
        _root, 1); // Append/allocate a new attribute structure.
  ...
  xml_node insert_child(unsigned int i, xml_node_type type) {
    if (!type_element())
      return xml_node(); // Don't do anything if not an node_element.
    unsigned int n = _root->children; // Get count of existing children.
    if (type_element() && i >= n)
      return append_child(
          type); // If subscript at end of array then just append.
    else if (type_element() && i < n) {
      xml_node_struct *p = pug::append_node(
          _root, 1,
          type); // Append the new node (by default at last array position).
      if (p)     // Ensure we have it.
      {
        register int m = (i - 1); // Stop at i.
        for (register int j = (n - 1); j > m;
             --j) // Starting at one less than end of array, reverse loop to i.
  ...
  bool remove_child(unsigned int i) {
    ...
    {
      xml_node_struct *p =
          _root->child[i]; // Keep a pointer to this node so we can free it.
      --n;
      unsigned int j = i;
      ...
      p->parent =
          p; // This ensures we only free this node when calling 'free_node'.
...
public:
  ...
  void outer_xml(std::basic_ostream<TCHAR, std::char_traits<TCHAR>> &os,
                 TCHAR indent_char = _T('\t'), bool breaks = true) {
    if (empty())
      return;                         // Make sure there is something to output.
    indent_stack indent(indent_char); // Prepare the indent.
    if (type_document()) // If this is the root, we don't want to output the
                         // root itself.
    {
      register unsigned int n =
          _root->children; // Output each child of the root.
      for (register unsigned int i = 0; i < n; ++i)
        pug::outer_xml(os, indent, _root->child[i], breaks);
    } else
      pug::outer_xml(os, indent, _root, breaks); // Output the node.
  }
  ...
  operator<<(std::basic_ostream<TCHAR, std::char_traits<TCHAR>> &os,
             xml_node node) {
    if (!os.good())
      return os;
    if ((os.flags() | std::ostream::skipws) == std::ostream::skipws)
      node.outer_xml(
          os, 0, false); // Skipping whitespace; suppress indents & linebreaks.
    else
      node.outer_xml(os); // Default options.
    return os;
  }
};
...
public:
  ...
  xml_parser(unsigned long optmsk = parse_default, bool autdel = true,
             long growby = parse_grow)
      : _xmldoc(0), _growby(growby), _autdel(autdel), _optmsk(optmsk),
        _buffer(0), _strpos(0)
#ifdef PUGOPT_MEMFIL
        ,
        _mmfile(0), _mmfmap(0), _mmaddr(0), _mfsize(0), _addeos(false)
#endif
  {
  }
...
public:
  ...
  int parse_mmfile(const TCHAR *path, unsigned long optmsk = parse_noset) {
    ...
    if (path) {
      ...
      if (optmsk != parse_noset)
        _optmsk = optmsk;
      assert((optmsk & parse_wnorm) ==
             0); // Normalization isn't implemented for memory-mapped files, as
                 // of 23 Jan 2003.
      const bool readonly = (optmsk & (parse_dtd | parse_dtd_only)) == 0;
      if (open_mmfile(path, readonly, false)) {
        // If the file has a 0 at the end we are ok to proceed, otherwise add
        // one.
        if ((*(((TCHAR *)_mmaddr) + _mfsize) == 0 ||
...
protected:
  ...
  bool open_mmfile(const TCHAR *path, const bool readonly, const bool addeos) {
    ...
    _addeos = false;
    _mmfile = CreateFile(
        path, readonly ? GENERIC_READ : GENERIC_READ | GENERIC_WRITE, 0, NULL,
        OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
        NULL); // Open read-only, no share, no security attrs, ..., no template.
    if (_mmfile != INVALID_HANDLE_VALUE) {
      _mfsize = ::GetFileSize(_mmfile, NULL);
      _mmfmap = CreateFileMapping(
          _mmfile, NULL, readonly ? PAGE_READONLY : PAGE_READWRITE, 0,
          _mfsize + (addeos ? sizeof(TCHAR) : 0),
          NULL); // Create map: handle, no security attr, read|read/write,
                 // larger if addeos, anonymous.
      if (_mmfmap != NULL) {
        assert(_mmaddr == NULL);
        _mmaddr =
            MapViewOfFile(_mmfmap, readonly ? FILE_MAP_READ : FILE_MAP_WRITE, 0,
                          0, 0); // Map the view: handle, read|read/write, start
                                 // at beginning, map entire file.
        if (_mmaddr != NULL) {
          if (addeos) // Add a terminating 0 to the end of the file for
                      // 'parse()'.
          {
            assert(!readonly);
...
public:
  xml_node at(long i) {
    return xml_node((xml_node_struct *)pointer_array::at((unsigned int)i));
  } // Access xml_node at subscript.
  xml_node operator[](long i) {
    return xml_node((xml_node_struct *)pointer_array::at((unsigned int)i));
  } // Access xml_node at subscript.
  friend std::ostream &operator<<(std::ostream &os,
                                  xml_node_list &list) // Output helper.
  {
    if (!os.good())
      return os;
...
#pragma message(                                                               \
    "Note: Add #define PUGAPI_VARIANT X in your implementation, where X is:")
#else // PUGAPI_VARIANT
#pragma message(                                                               \
    "Note: Update #define PUGAPI_VARIANT X in your implementation, where X is:")
#endif //! PUGAPI_VARIANT
#pragma message("pugxml.h(39) : Value of PUGAPI_INTERNAL_VARIANT.")
#else // defined(PUGAPI_VARIANT)
#if !defined(PUGAPI_VERSION_MAJOR) ||                                          \
    (PUGAPI_VERSION_MAJOR != PUGAPI_INTERNAL_VERSION_MAJOR)
#pragma message(                                                               \
    "Warning: The Pug XML library API major version does not match your implementation.")
#pragma message(                                                               \
    "Warning: Your implementation may not run correctly with this major version.")
#pragma message("Note: Read pugxml.xml for transition instructions.")
#ifndef PUGAPI_VERSION_MAJOR
#pragma message(                                                               \
    "Note: Add #define PUGAPI_VERSION_MAJOR X in your implementation, where X is:")
#else // PUGAPI_VERSION_MAJOR
#pragma message(                                                               \
    "Note: Update #define PUGAPI_VERSION_MAJOR X in your implementation, where X is:")
...
#if !defined(PUGAPI_VERSION_MINOR) ||                                          \
    (PUGAPI_VERSION_MINOR != PUGAPI_INTERNAL_VERSION_MINOR)
#pragma message(                                                               \
    "Note: The Pug XML library API minor version does not match your implementation.")
#pragma message(                                                               \
    "Note: Your implementation may not run correctly with this minor version.")
#pragma message("Note: Read pugxml.xml for transition instructions.")
#ifndef PUGAPI_VERSION_MINOR
#pragma message(                                                               \
    "Note: Add #define PUGAPI_VERSION_MINOR X in your implementation, where X is:")
#else // PUGAPI_VERSION_MINOR
#pragma message(                                                               \
    "Note: Update #define PUGAPI_VERSION_MINOR X in your implementation, where X is:")
