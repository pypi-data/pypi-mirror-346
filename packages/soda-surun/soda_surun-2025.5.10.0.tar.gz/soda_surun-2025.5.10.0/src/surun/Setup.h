...
#define WINLOGONKEY                                                            \
  ...
  _T("NT\\CurrentVersion\\Winlogon\\Notify\\SuRun")
...
#define GetAskPW                                                               \
  ((GetOption(_T("SavePasswords"), 1) == 0) || (GetPwTimeOut != 0))

#define GetSavePW (GetOption(_T("SavePasswords"), 1) != 0)
#define SetSavePW(b)                                                           \
  SetOption(_T("SavePasswords"), b, 1);                                        \
  if (!b) {                                                                    \
...
#define SetPwTimeOut(t)                                                        \
  SetOption(_T("AskTimeOut"), (DWORD)min(60, max(0, (int)t)), 0)
...
#define GetTestReqAdmin                                                        \
  (GetOption(_T("TestReqAdmin"), (_winmajor >= 6) ? 0 : 1) != 0)
#define SetTestReqAdmin(b)                                                     \
  SetOption(_T("TestReqAdmin"), b, (_winmajor >= 6) ? 0 : 1)
...
//////////////////////////////////////////////////////////////////////////////
// Settings for every user; saved to
// "HKLM\SECURITY\SuRun\<ComputerName>\<UserName>":
//////////////////////////////////////////////////////////////////////////////
#define GetUsrSetting(u, s, d) GetRegInt(HKLM, USERKEY(u), s, d)
#define SetUsrSetting(u, s, v, d)                                              \
  if (GetUsrSetting(u, s, d) != v)                                             \
  SetRegInt(HKLM, USERKEY(u), s, v)

#define DelUsrSettings(u)                                                      \
  if (u && *u) {                                                               \
    DelRegKey(HKLM, WHTLSTKEY(u));                                             \
    DelRegKey(HKCR, USROPTKEY(u));                                             \
  }

#define DelAllUsrSettings                                                      \
  DelRegKeyChildren(HKLM, SVCKEY);                                             \
...
#define GetInstallDevs(u)                                                      \
  (GetUsrSetting(u, _T("InstallDevs"), GetRestrictApps(u) == 0) != 0)
#define SetInstallDevs(u, b) SetUsrSetting(u, _T("InstallDevs"), b, 0)

#define GetStoreUsrPW(u) GetUsrSetting(u, _T("StoreUsrPW"), 0)
#define SetStoreUsrPW(u, b)                                                    \
  if (GetUsrSetting(u, _T("StoreUsrPW"), 0) != b) {                            \
...
#define SetShExtSetting(s, v, d)                                               \
  if (GetShExtSetting(s, d) != v)                                              \
  SetRegInt(HKCR, L"CLSID\\" sGUID, s, v)

// Hide SuRun from non SuRunners
#define GetDefHideSuRun                                                        \
  (GetShExtSetting(_T("HideSuRunFromNonSuRunners"), 0) != 0)
#define SetDefHideSuRun(b)                                                     \
  SetShExtSetting(_T("HideSuRunFromNonSuRunners"), b, 0)

// the following settings are stored in: HKCR\\CLSID\\sGUID
#define ControlAsAdmin                                                         \
  L"ControlAsAdmin" //"Control Panel As Admin" on Desktop Menu
#define CmdHereAsAdmin L"CmdHereAsAdmin" //"Cmd here As Admin" on Folder Menu
#define ExpHereAsAdmin                                                         \
  L"ExpHereAsAdmin" //"Explorer here As Admin" on Folder Menu
...
// #define UseSVCHook      L"UseSVCHook"      //install IAT Hook into
...
#define GetUseSVCHook                                                          \
  ((_winmajor < 6) && GetUseIATHook /*GetShExtSetting(UseSVCHook,0)!=0)*/)
...
#define GetShowTrayAdmin                                                       \
  GetShExtSetting(_T("ShowTrayAdmin"), TSA_ADMIN | TSA_TIPS)
#define SetShowTrayAdmin(b)                                                    \
  SetShExtSetting(_T("ShowTrayAdmin"), b, TSA_ADMIN | TSA_TIPS)

#define GetHideNormalIcon ((GetShowTrayAdmin & TSA_HIDE_NORMAL) != 0)
#define SetHideNormalIcon(b)                                                   \
  SetShowTrayAdmin(                                                            \
      ((GetShowTrayAdmin & (~TSA_HIDE_NORMAL)) | (b ? TSA_HIDE_NORMAL : 0)))
...
#define SetUsrOption(u, s, v, d)                                               \
  if (GetUsrOption(u, s, d) != v)                                              \
  SetRegInt(HKCR, USROPTKEY(u), s, v)

#define DelUsrOption(u, s)                                                     \
  if (u && *u)                                                                 \
  RegDelVal(HKCR, USROPTKEY(u), s)
...
#define HideSuRun(u, Grps)                                                     \
  ((Grps & IS_IN_ADMINS)                                                       \
       ? 0                                                                     \
       : GetUsrOption(u, _T("HideFromUser"),                                   \
...
#define GetOwnerAdminGrp                                                       \
  (GetRegInt(HKLM, _T("SYSTEM\\CurrentControlSet\\Control\\Lsa"),              \
             _T("nodefaultadminowner"), 1) == 0)

#define SetOwnerAdminGrp(b)                                                    \
  {                                                                            \
    SetRegInt(HKLM, _T("SYSTEM\\CurrentControlSet\\Control\\Lsa"),             \
...
#define GetSetEnergy                                                           \
  HasRegistryKeyAccess(                                                        \
      _T("MACHINE\\Software\\Microsoft\\")                                     \
      _T("Windows\\CurrentVersion\\Controls Folder\\PowerCfg"),                \
      SURUNNERSGROUP)

#define SetSetEnergy(b)                                                        \
  SetRegistryTreeAccess(                                                       \
      _T("MACHINE\\Software\\Microsoft\\")                                     \
      _T("Windows\\CurrentVersion\\Controls Folder\\PowerCfg"),                \
      SURUNNERSGROUP, b != 0)
...
// deleted flag: #define FLAG_NORESTRICT 0x04 //Restricted SuRunner may execute
// App elevated
...
#define SetSROption(s, v, d)                                                   \
  if (GetSROption(s, d) != v)                                                  \
...
// void ImportSettings(LPCTSTR ini,bool bSuRunSettings,bool bBlackList,LPCTSTR
// User); void ExportSettings(LPCTSTR ini,bool bSuRunSettings,bool
// bBlackList,LPCTSTR User);
