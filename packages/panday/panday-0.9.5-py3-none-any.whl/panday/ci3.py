...
class NSAClassifier:
    ...
    def train(self, X_self):
        ...
        while len(self.detectors) < self.num_detectors:
            candidate = np.random.rand(X_self.shape[1])  # Match feature dimension
...
plt.scatter(X_test[:, 0], X_test[:, 1], c=y_pred, cmap='coolwarm', edgecolor='k', label='Predictions')
detectors = np.array(clf.detectors)
if len(detectors) > 0:
    plt.scatter(detectors[:, 0], detectors[:, 1], c='green', marker='x', s=100, label='Detectors')
plt.xlabel('Feature 1')
plt.ylabel('Feature 2')
plt.title('NSA Classifier: Red=Damaged, Blue=Safe')
plt.legend()
plt.grid(True)
plt.show()
