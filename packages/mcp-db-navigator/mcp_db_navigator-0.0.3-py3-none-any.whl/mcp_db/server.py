...
from mcp_db.db import DatabaseCredentials, SelectQuery
...
def run_server(env_file_path: str):
    try:
        ...
        def connect_to_database() -> str:
            ...
            Returns:
                The an object with connection status, database name, and host
            """
        ...
        def load_database_schema() -> str:
            """Load the schema for the current database
        ...
        @track_query_metrics
        @mcp.tool()
        def query_database(query_params: dict) -> List[Tuple[Any, ...]]:
            """Query a mysql or mariadb database

            Args:
                query_params: A dictionary containing:
            ...
            """
            # Validate query_params using SelectQuery
            try:
                validated_params = SelectQuery(**query_params)
            except Exception as e:
                logger.error(f"SelectQuery validation failed: {e}")
                raise MCPError(f"Query parameter validation failed: {e}")
            # Use validated_params.dict() instead of query_params from here on
            query_params = validated_params.model_dump()
