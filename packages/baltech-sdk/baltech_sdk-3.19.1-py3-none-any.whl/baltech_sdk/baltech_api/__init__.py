...
class Commands(FrameExecutor):
    ...
    @property
    def BlePeriph_SetDeviceName(self) -> BlePeriph_SetDeviceName:
        """
        This command sets the device name which should be advertized by the reader.
        """
        return BlePeriph_SetDeviceName(self)
    @property
    def BlePeriph_SetCharacteristic(self) -> BlePeriph_SetCharacteristic:
        """
        This command changes the value of a characteristic. All future reads from the
        Central or [BlePeriph.GetCharacteristic](.#BlePeriph.GetCharacteristic) from
        ...
        """
        return BlePeriph_SetCharacteristic(self)
    @property
    def BlePeriph_GetCharacteristic(self) -> BlePeriph_GetCharacteristic:
        """
        ...
        host ([BlePeriph.SetCharacteristic](.#BlePeriph.SetCharacteristic)) or the BLE
        Central (via a write to the characteristic).
        """
        return BlePeriph_GetCharacteristic(self)
    @property
    ...
    @property
    def MobileId_Setup(self) -> MobileId_Setup:
        """
        This command is used to set up Mobile ID parameters.
        """
        return MobileId_Setup(self)
    @property
...
class ConfigAccessor(FrameExecutor):
    ...
    def Autoread_Rule_Template(self) -> Autoread_Rule_Template:
        """
        This template describes what parts of the [VHL
        file](.#Autoread.Rule.VhlFileIndex) (see
        [VarData](dataconverter.xml#Template.VarData)) or the card's serial number
        (see [Serialnr](dataconverter.xml#Template.Serialnr)) shall be read and how
        they are converted to ASCII format.
        """
    ...
    def Project_MobileId_DisplayName(self) -> Project_MobileId_DisplayName:
        ...
        The value may be used in combination with the [Remote
        Trigger](.#Project.MobileId.RemoteTrigger) capability. Displayed on the mobile
        device, the name allows the user to identify the reader to be triggered at the
        touch af a button.
    ...
    @property
    def Project_MobileId_RemoteTrigger(self) -> Project_MobileId_RemoteTrigger:
        """
        This value indicates if the reader can be triggered remotely, i.e. by a
        trigger moment other than sufficient proximity to the reader (RSSI).
        
        The Remote Trigger capability is reported to the Mobile ID app after the
        connection is established. If set, the app may use this information to define
        its own trigger moment, e.g. a button press by the user.
        """
        return Project_MobileId_RemoteTrigger(self)
    @property
    ...
    def Protocols_KeyboardEmulation(self) -> Protocols_KeyboardEmulation:
        """
        When enabling the USB keyboard emulation protocol (see
        [Device/Run/EnabledProtocols](base.xml#Device.Run.EnabledProtocols)), it can
        be fine tuned with these value.
        """
    ...
    def Protocols_Ccid(self) -> Protocols_Ccid:
        """
        When enabling the CCID protocol (see
        [Device/Run/EnabledProtocols](base.xml#Device.Run.EnabledProtocols)) it can be
        fine tuned with these value.
        """
    ...
    def Protocols_Osdp(self) -> Protocols_Osdp:
        """
        When enabling the Osdp protocol (see
        [Device/Run/EnabledProtocols](base.xml#Device.Run.EnabledProtocols)) it can be
        fine tuned with these value.
        """
    ...
    @property
    def VhlCfg_File_DesfireCardUidKeyIdx(self) -> VhlCfg_File_DesfireCardUidKeyIdx:
        """
        This entry specifies a key index to read the card UID. This value is only
        applicable for cards with random UID activated. If a VHL file specifies files
        and this configuration value the card UID is placed after content of file(s).
        """
        return VhlCfg_File_DesfireCardUidKeyIdx(self)
    @property
    ...
    def VhlCfg_File_Iso15ReadCmd(self) -> VhlCfg_File_Iso15ReadCmd:
        """
        Selects the ISO 15693 command that is used for a VHL read operation. Default
        is "Read single block". Alternatively, "Read multiple blocks" may be used.
        """
        return VhlCfg_File_Iso15ReadCmd(self)
    @property
    def VhlCfg_File_Iso15WriteCmd(self) -> VhlCfg_File_Iso15WriteCmd:
        """
        Selects the ISO 15693 command that is used for a VHL write operation. Default
        is "Write single block". Alternatively, "Write multiple blocks" may be used.
        """
    ...
    def VhlCfg_File_MifarePlusFormatOriginalKeyIdx(self) -> VhlCfg_File_MifarePlusFormatOriginalKeyIdx:
        """
        This value is used by Mifare plus firmware only and needed for running VHL-
        Format(). To encode the new sector trailer, the reader tries to authenticate
        with all the keys specified in the list. If this value is omitted it defaults
        to authenticate with 'FF' * 6 as Key A (Mifare transport keys) or with '00' *
        16 (AES transport keys). Using Mifare plus with cards in SL2/3, VHL-Format()
        first updates the AES keylist, second the sector trailer. To allow retries of
        the VHL-Format() command also the new keys which are used for key change
        should be included in this configuration value.
        """
