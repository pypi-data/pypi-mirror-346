...
class VarDataAlign(TemplateFrame):
    Code = 0x1B05
    def __init__(self, FillByte: int, StartPos: int, Len: int, Alignment: Template_VarDataAlign_Alignment, Filters: Union[TemplateFilter, TemplateFilter_Dict] = TemplateFilter.NoFilter()) -> None:
    ...
    def read_from(cls, frame: BytesIO) -> Self:
        ...
        _Len = safe_read_int_from_buffer(_recv_buffer, 1)
        _Alignment = Template_VarDataAlign_Alignment_Parser.as_literal(safe_read_int_from_buffer(_recv_buffer, 1))
        _Filters_int = safe_read_int_from_buffer(_recv_buffer, 1)
        _BcdToBin = bool((_Filters_int >> 7) & 0b1)
        _BinToAscii = bool((_Filters_int >> 6) & 0b1)
        _Unpack = bool((_Filters_int >> 5) & 0b1)
    ...
    def __bytes__(self) -> bytes:
        ...
        _send_buffer.write(self.Len.to_bytes(length=1, byteorder='big'))
        _send_buffer.write(Template_VarDataAlign_Alignment_Parser.as_value(self.Alignment).to_bytes(length=1, byteorder='big'))
        if isinstance(self.Filters, dict):
            self.Filters = TemplateFilter(**self.Filters)
        self.Filters_int = 0
        self.Filters_int |= (int(self.Filters.BcdToBin) & 0b1) << 7
...
class Template(ProtocolBase[TemplateFrame]):
    ...
    def VarDataClip(self, StartPos: int, Len: int, Filters: Union[TemplateFilter, TemplateFilter_Dict] = TemplateFilter.NoFilter()) -> Self:
        ...
        return self
    def VarDataAlign(self, FillByte: int, StartPos: int, Len: int, Alignment: Template_VarDataAlign_Alignment, Filters: Union[TemplateFilter, TemplateFilter_Dict] = TemplateFilter.NoFilter()) -> Self:
        """
