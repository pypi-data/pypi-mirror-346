...
class BlePeriph_DefineService(Command):
    ...
    def build_frame(self, ServiceUUID: bytes, Characteristics: List[BlePeriph_DefineService_Characteristics_Entry], BrpTimeout: int = 100) -> bytes:
        ...
        for _Characteristics_Entry in Characteristics:
            _CharacteristicUUID, _Size, _SupportsRead, _SupportsWrite, _SupportsNotify, _SupportsIndicate, _VariableSize = _Characteristics_Entry
            ...
            _var_0000_int = 0
            _var_0000_int |= (int(_SupportsRead) & 0b1) << 7
            _var_0000_int |= (int(_SupportsWrite) & 0b1) << 6
            _var_0000_int |= (int(_SupportsNotify) & 0b1) << 5
            _var_0000_int |= (int(_SupportsIndicate) & 0b1) << 4
            _var_0000_int |= (int(_VariableSize) & 0b1) << 3
...
class BlePeriph_Enable(Command):
    ...
    def __call__(self, Activate: bool, BrpTimeout: int = 100) -> None:
        ...
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
        return None
class BlePeriph_SetDeviceName(Command):
    CommandGroupId = 0x4B
    CommandId = 0x03
    def build_frame(self, DeviceName: bytes, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4B\x03")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(int(len(DeviceName)).to_bytes(1, byteorder='big'))
        _send_buffer.write(DeviceName)
        return _send_buffer.getvalue()
    def __call__(self, DeviceName: bytes, BrpTimeout: int = 100) -> None:
        request_frame = self.build_frame(DeviceName=DeviceName, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        _additional_data = _recv_buffer.read()
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
        return None
class BlePeriph_SetCharacteristic(Command):
    CommandGroupId = 0x4B
    CommandId = 0x04
    def build_frame(self, CharacteristicNdx: int, NewValue: bytes, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4B\x04")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(CharacteristicNdx.to_bytes(length=2, byteorder='big'))
        _send_buffer.write(int(len(NewValue)).to_bytes(2, byteorder='big'))
        _send_buffer.write(NewValue)
        return _send_buffer.getvalue()
    def __call__(self, CharacteristicNdx: int, NewValue: bytes, BrpTimeout: int = 100) -> None:
        request_frame = self.build_frame(CharacteristicNdx=CharacteristicNdx, NewValue=NewValue, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        _additional_data = _recv_buffer.read()
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
        return None
class BlePeriph_GetCharacteristic(Command):
    CommandGroupId = 0x4B
    CommandId = 0x05
    def build_frame(self, CharacteristicNdx: int, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4B\x05")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(CharacteristicNdx.to_bytes(length=2, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, CharacteristicNdx: int, BrpTimeout: int = 100) -> bytes:
        request_frame = self.build_frame(CharacteristicNdx=CharacteristicNdx, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        _CurrentValue_len = safe_read_int_from_buffer(_recv_buffer, 2)
        _CurrentValue = _recv_buffer.read(_CurrentValue_len)
        if len(_CurrentValue) != _CurrentValue_len:
            raise PayloadTooShortError(_CurrentValue_len - len(_CurrentValue))
        _additional_data = _recv_buffer.read()
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
...
class BlePeriph_GetEvents(Command):
    ...
    def build_frame(self, BrpTimeout: int = 100) -> bytes:
        ...
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, BrpTimeout: int = 100) -> List[BlePeriph_GetEvents_Events_Entry]:
        request_frame = self.build_frame(BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        _Events_len = safe_read_int_from_buffer(_recv_buffer, 1)
        _Events = []  # type: ignore[var-annotated,unused-ignore]
        while not len(_Events) >= _Events_len:
            _EventId = BlePeriph_GetEvents_EventId_Parser.as_literal(safe_read_int_from_buffer(_recv_buffer, 1))
            _CharacteristicNdx = safe_read_int_from_buffer(_recv_buffer, 2)
            _Events_Entry = BlePeriph_GetEvents_Events_Entry(_EventId, _CharacteristicNdx)
            _Events.append(_Events_Entry)
        if len(_Events) != _Events_len:
            raise PayloadTooShortError(_Events_len - len(_Events))
        _additional_data = _recv_buffer.read()
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
        return _Events
...
class Desfire_ExecCommand(Command):
    CommandGroupId = 0x1B
    CommandId = 0x00
    def build_frame(self, Cmd: int, Header: bytes, Param: bytes, CryptoMode: Desfire_ExecCommand_CryptoMode = "Plain", ResponseLen: int = 65535, BrpTimeout: int = 1000) -> bytes:
        _send_buffer = BytesIO()
        ...
        return _send_buffer.getvalue()
    def __call__(self, Cmd: int, Header: bytes, Param: bytes, CryptoMode: Desfire_ExecCommand_CryptoMode = "Plain", ResponseLen: int = 65535, BrpTimeout: int = 1000) -> bytes:
...
class Desfire_SelectApplication(Command):
    CommandGroupId = 0x1B
    CommandId = 0x03
    def build_frame(self, AppId: int = 0, BrpTimeout: int = 1000) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x1B\x03")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(AppId.to_bytes(length=4, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, AppId: int = 0, BrpTimeout: int = 1000) -> None:
...
class Desfire_ChangeExtKey(Command):
    CommandGroupId = 0x1B
    CommandId = 0x06
    def build_frame(self, MasterKeyType: Desfire_ChangeExtKey_MasterKeyType = "DESorTripleDES", IsKeySet: bool = False, IsAesKey: bool = False, IsVersion: bool = False, IsChangeKey: bool = True, *, KeyNo: int, KeyVersion: Optional[int] = 0, NewKey: bytes, OldKey: bytes, KeySet: Optional[int] = None, BrpTimeout: int = 1000) -> bytes:
        _send_buffer = BytesIO()
        ...
        return _send_buffer.getvalue()
    def __call__(self, MasterKeyType: Desfire_ChangeExtKey_MasterKeyType = "DESorTripleDES", IsKeySet: bool = False, IsAesKey: bool = False, IsVersion: bool = False, IsChangeKey: bool = True, *, KeyNo: int, KeyVersion: Optional[int] = 0, NewKey: bytes, OldKey: bytes, KeySet: Optional[int] = None, BrpTimeout: int = 1000) -> None:
...
class Desfire_ChangeKey(Command):
    ...
    def build_frame(self, IsKeySet: bool = False, IsMasterKey: bool = False, IsChangeKey: bool = False, *, KeyNr: int, NewKeyDivMode: Desfire_ChangeKey_NewKeyDivMode = "SamAv1OneRound", NewKeyHasDivData: bool = False, NewKeyHasExtIdx: bool = False, NewKeyIdx: int, CurKeyDivMode: Desfire_ChangeKey_CurKeyDivMode = "SamAv1OneRound", CurKeyHasDivData: bool = False, CurKeyHasExtIdx: bool = False, CurKeyIdx: int, NewKeyDivData: Optional[bytes] = None, CurKeyDivData: Optional[bytes] = None, NewKeyExtIdx: Optional[int] = None, CurKeyExtIdx: Optional[int] = None, KeySet: Optional[int] = None, BrpTimeout: int = 1000) -> bytes:
        ...
        if IsKeySet:
            ...
            _send_buffer.write(KeySet.to_bytes(length=1, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, IsKeySet: bool = False, IsMasterKey: bool = False, IsChangeKey: bool = False, *, KeyNr: int, NewKeyDivMode: Desfire_ChangeKey_NewKeyDivMode = "SamAv1OneRound", NewKeyHasDivData: bool = False, NewKeyHasExtIdx: bool = False, NewKeyIdx: int, CurKeyDivMode: Desfire_ChangeKey_CurKeyDivMode = "SamAv1OneRound", CurKeyHasDivData: bool = False, CurKeyHasExtIdx: bool = False, CurKeyIdx: int, NewKeyDivData: Optional[bytes] = None, CurKeyDivData: Optional[bytes] = None, NewKeyExtIdx: Optional[int] = None, CurKeyExtIdx: Optional[int] = None, KeySet: Optional[int] = None, BrpTimeout: int = 1000) -> None:
...
class Iso14b_Request(Command):
    CommandGroupId = 0x14
    CommandId = 0x02
    def build_frame(self, ReqAll: bool = False, TimeSlots: Iso14b_Request_TimeSlots = "TimeSlots1", AFI: int = 0, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        ...
        return _send_buffer.getvalue()
    def __call__(self, ReqAll: bool = False, TimeSlots: Iso14b_Request_TimeSlots = "TimeSlots1", AFI: int = 0, BrpTimeout: int = 100) -> List[Iso14b_Request_ValueList_Entry]:
        request_frame = self.build_frame(ReqAll=ReqAll, TimeSlots=TimeSlots, AFI=AFI, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        ...
        while not len(_ValueList) >= _ValueList_len:
            ...
            _CID = (_var_0002_int >> 1) & 0b1
            _NAD = (_var_0002_int >> 0) & 0b1
            _ValueList_Entry = Iso14b_Request_ValueList_Entry(_PUPI, _AppData, _Synced, _Send848, _Send424, _Send212, _Recv848, _Recv424, _Recv212, _FSCI, _ProtType, _FWI, _ADC, _CID, _NAD)
...
class Iso14b_Attrib(Command):
    ...
    def build_frame(self, PUPI: bytes, TR0: Iso14b_Attrib_TR0 = "Numerator64", TR1: Iso14b_Attrib_TR1 = "Numerator80", EOF: Iso14b_Attrib_EOF = "EOFrequired", SOF: Iso14b_Attrib_SOF = "SOFrequired", DSI: DivisorInteger = "Kbps106", DRI: DivisorInteger = "Kbps106", FSDI: Iso14b_Attrib_FSDI = "Bytes64", CID: int = 0, EnHLR: bool = False, EnMBLI: bool = False, EnCID: bool = False, BrpTimeout: int = 100) -> bytes:
        ...
        _send_buffer.write(_var_0001_int.to_bytes(length=1, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, PUPI: bytes, TR0: Iso14b_Attrib_TR0 = "Numerator64", TR1: Iso14b_Attrib_TR1 = "Numerator80", EOF: Iso14b_Attrib_EOF = "EOFrequired", SOF: Iso14b_Attrib_SOF = "SOFrequired", DSI: DivisorInteger = "Kbps106", DRI: DivisorInteger = "Kbps106", FSDI: Iso14b_Attrib_FSDI = "Bytes64", CID: int = 0, EnHLR: bool = False, EnMBLI: bool = False, EnCID: bool = False, BrpTimeout: int = 100) -> Iso14b_Attrib_Result:
        request_frame = self.build_frame(PUPI=PUPI, TR0=TR0, TR1=TR1, EOF=EOF, SOF=SOF, DSI=DSI, DRI=DRI, FSDI=FSDI, CID=CID, EnHLR=EnHLR, EnMBLI=EnMBLI, EnCID=EnCID, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        if EnCID:
...
class Main_Bf2Upload(Command):
    CommandGroupId = 0xF0
    CommandId = 0x01
    def build_frame(self, Lines: List[int], BrpTimeout: int = 5000) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\xF0\x01")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        for _Lines_Entry in Lines:
            _Line = _Lines_Entry
            _send_buffer.write(_Line.to_bytes(length=1, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, Lines: List[int], BrpTimeout: int = 5000) -> Main_Bf2Upload_Result:
        request_frame = self.build_frame(Lines=Lines, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
...
class MobileId_Enable(Command):
    ...
    def __call__(self, Mode: MobileId_Enable_Mode, BrpTimeout: int = 100) -> None:
        ...
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
        return None
class MobileId_Setup(Command):
    CommandGroupId = 0x4C
    CommandId = 0x02
    def build_frame(self, Key: bytes, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4C\x02")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(int(len(Key)).to_bytes(1, byteorder='big'))
        _send_buffer.write(Key)
        return _send_buffer.getvalue()
    def __call__(self, Key: bytes, BrpTimeout: int = 100) -> None:
...
class Sys_HFReset(Command):
    CommandGroupId = 0x00
    CommandId = 0x01
    def build_frame(self, OffDuration: int = 1, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x00\x01")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(OffDuration.to_bytes(length=1, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, OffDuration: int = 1, BrpTimeout: int = 100) -> None:
