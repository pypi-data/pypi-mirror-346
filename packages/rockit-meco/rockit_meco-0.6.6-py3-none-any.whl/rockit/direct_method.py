...
class DirectMethod:
    ...
    def add_variables(self, stage, opti, final=None):
        if final is None:
            V = []
            ...
            if final:
                V = self.V_split
                for i,v in enumerate(stage.variables['']):
                    if stage._catalog[v].get("final", False):
                        V[i] = opti.variable(v.shape[0], v.shape[1], scale=stage._scale[v], domain=stage._catalog[v]['domain'])
                self.V = veccat(*V)
            else:
                V = []
                for v in stage.variables['']:
                    if stage._catalog[v].get("final", False):
                        V.append(MX.zeros(*v.shape))
                    else:
                        V.append(opti.variable(v.shape[0], v.shape[1], scale=stage._scale[v], domain=stage._catalog[v]['domain']))
                self.V = veccat(*V)
                self.V_split = V
