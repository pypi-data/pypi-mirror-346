...
ocp.set_value(p_local,[1+i*0.01 for i in range(11)])

p_global = ocp.parameter()
ocp.set_value(p_global,1.1)
...
x_sol = sol.sample(ocp.x,grid='control')[1]
...
print(ca.norm_fro(x_sol-x_res))

print(ocp.sample(p_local,grid='control-')[1])
print(ocp.sample(p_local,grid='control')[1])
