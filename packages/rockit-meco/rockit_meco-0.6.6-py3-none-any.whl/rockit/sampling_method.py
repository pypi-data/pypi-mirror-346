...
class SamplingMethod(DirectMethod):
    ...
    def transcribe(self, stage, phase=1,**kwargs):
        ...
        if phase==1:
            ...
            # Compute preferenced locations for variables
            for c, meta, args in stage._constraints["point"]:
                # Candidate for 'final' label
                final = any([e.name().endswith("_at_tf") for e in symvar(c)])
                parts = []
                for e in symvar(c):
                    if e.name().endswith("_at_t0"): # Exclude mixed terms
                        final = False
                        break
                    if e in stage._placeholders:
                       e = stage._placeholders[e][1]
                    parts.append(e)
                
                expr = veccat(*parts)

                if final:
                    for e in stage.v_global_list:
                        if ca.depends_on(expr, e):
                            stage._catalog[e]['final'] = True
    ...
    def add_variables_V(self, stage, opti, final=None):
        DirectMethod.add_variables(self, stage, opti, final=final)
