...
.. code-block:: python
    ...
    cv.make_simple_map(field=anpp,
                       layer="Total",
                       output_file="figures/test_json_examples/anpp_map.png",
...
We recommend starting with one of these examples and modifying the parameters to to customize the figure according to your needs.

.. warning::

    The JSON mode does not support the full range of functionalities offered by **canopy**.

    For instance, metadata from your run will not be automatically retrieved. As a result, 
    you must explicitly define variable names, plot titles, and units in the JSON file.

    Additionally, JSON mode does not currently support the use of multiple time series 
    or the specification of function keyword arguments (`kwargs`) as JSON keys.

    You are restricted to the predefined set of parameters accepted 
    by each visualization function (see :ref:`visualization` for reference).