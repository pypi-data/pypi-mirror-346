...
from obiguard.api_resources.apis.create_headers import createHeaders
from .global_constants import OBIGUARD_HEADER_PREFIX, DEFAULT_CONNECTION_LIMITS
...
class APIClient:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.obiguard_api_key = default_api_key(self.base_url, obiguard_api_key)
    ...
    def _serialize_header_values(
        ...
        if headers is None:
            return {}
        return {
            f"{OBIGUARD_HEADER_PREFIX}{k}": json.dumps(v)
    ...
    def _default_headers(self) -> Mapping[str, str]:
        return {
            f"{OBIGUARD_HEADER_PREFIX}api-key": self.api_key,
            f"{OBIGUARD_HEADER_PREFIX}package-version": f"portkey-{VERSION}",
            f"{OBIGUARD_HEADER_PREFIX}runtime": platform.python_implementation(),
            f"{OBIGUARD_HEADER_PREFIX}runtime-version": platform.python_version(),
        }
    ...
    def _should_retry(self, response: httpx.Response) -> bool:
        ...
        retry_trace_id = response.headers.get("x-obiguard-trace-id")
        retry_request_id = response.headers.get("x-obiguard-request-id")
        retry_gateway_exception = response.headers.get("x-obiguard-gateway-exception")
