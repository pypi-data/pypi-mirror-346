...
impl NDS2Cache {
        ...
        -> Result<(), DTTError>
    {
        ...
        let mut data_rx = if online {
            ...
            cache.get_past_ordered(round_interval,  channel_names, ).await
        };
...
//                println!("Ordered: {}: cache data len = {}", count, data_rx.len());
            ...
            -> Result<mpsc::Receiver<Vec<Buffer>>, DTTError>
    {
        ...
        let (span_update_tx, mut data_rx) = if view.span.online {
            ...
            println!("{}.{}: Getting past scope data in {} - {} for {}",
                     ...
                     round_interval.start.to_gpst_seconds(),
                     round_interval.end.to_gpst_seconds(), channel_names[0]);
             (None, cache.get_past_unordered(round_interval, channel_names).await)
        };
    ...
    async fn scope_view_stream_loop(self, rc: Box<RunContext>, view: &mut ScopeView) -> Result<(), DTTError>
    {
        ...
        let mut span_update_rx = view.span_update_tx.as_ref().map(|s| s.subscribe());
        ...
        tokio::spawn(
            async move {
                ...
                'main: loop {
                    ...
                    // // re-set latest record if the span changes
    ...
    async fn find_channels_blocking(self, rc: Box<RunContext>, query: ChannelQuery) {
        let handle = match self.initialize_cache().await {
            Ok(h) => h,
            Err(e) => {
                rc.user_messages.error("Failed to open interface to local cache to request channels.");
                return;
            }
        };
        ...
        let channels = c.into_iter().map(|c| c.into()).collect();
        let rc2 = rc.clone();
