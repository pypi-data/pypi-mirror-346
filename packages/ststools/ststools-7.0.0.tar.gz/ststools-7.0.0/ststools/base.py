...
def Display():
        ...
        with plot_output:
            ...
            if toggle_second_log.value:
                ...
                if plot_mode in ["Forward","Both"]:
                    ...
                    if x_fit_ida is not None and y_fit_ida is not None:
                        ...
                        inset_ax.plot(x_fit_ida, ddI_ida[(x2_ida >= fit_range[0]) & (x2_ida <= fit_range[1])], label="Data", color="blue",alpha = 0.7)
                        inset_ax.plot(x_fit_ida, y_fit_ida, label="Fit (tanh) For.", color="Black", linestyle="--")
                        inset_ax.legend()
                        inset_ax.grid(True)
            ...
            if toggle_histograms.value:
                if toggle_second_log.value:
                    ...
                    n1=3;n2=4;n3 =5
                if plot_mode in ["Forward", "Both"]:
                    axs[n1].hist(hist_gap_ida, bins=bins_gap_slider.value, color="gray",edgecolor='black', alpha=0.7, label="Foward")
                    axs[n1].set_title("Histogram of Gaps")
                    axs[n1].set_xlabel("Gap (eV)")
                    axs[n1].set_ylabel("Frequency")
                    axs[n1].legend()
                    axs[n1].grid(True, linestyle='--', alpha=0.5)
                    axs[n1].tick_params(direction='in', top=True, right=True)
                    axs[n1].ticklabel_format(style='plain')
                    axs[n2].hist(hist_doping_ida, bins=bins_doping_slider.value, color="gray", edgecolor='black',alpha=0.7, label="Foward")
                    axs[n2].set_title("Histogram of Doping")
                    axs[n2].set_xlabel("Doping Type")
                    axs[n2].set_ylabel("Frequency")
                    axs[n2].legend()
                    axs[n2].grid(True, linestyle='--', alpha=0.5)
                    axs[n2].tick_params(direction='in', top=True, right=True)
                    axs[n2].ticklabel_format(style='plain')
                ...
                if plot_mode in ["Backward", "Both"]:
                    axs[n1].hist(hist_gap_volta, bins=bins_gap_slider.value, color='lightblue', edgecolor='black', alpha=0.7, label="Backward")
                    axs[n1].set_title("Histogram of Gaps")
                    axs[n1].set_xlabel("Gap (eV)")
                    axs[n1].set_ylabel("Frequency")
                    axs[n1].legend()
                    ...
                    axs[n1].tick_params(direction='in', top=True, right=True)
                    axs[n1].ticklabel_format(style='plain')
                    axs[n2].hist(hist_doping_volta,bins=bins_doping_slider.value, color='lightblue', edgecolor='black', alpha=0.7, label="Backward")
                    axs[n2].set_title("Histogram of Doping")
                    axs[n2].set_xlabel("Doping Type")
                    axs[n2].set_ylabel("Frequency")
                    axs[n2].legend()
    ...
    if len(STS["Hyper_ida"]["V"])<=100:
        bin_value = int(np.ceil(np.log2(len(STS["Hyper_ida"]["V"])) + 1))
    else:
        bin_value = int(np.ceil(2 * len(STS["Hyper_ida"]["V"])) ** (1/3))
   

    bins_gap_slider = IntSlider( value=bin_value, min=1, max=100, step=1, description="Bins (Gap):", tooltip="Number of bins for Gap histogram")
    bins_doping_slider = IntSlider( value=bin_value, min=1, max=100,step=1,description="Bins (Doping):", tooltip="Number of bins for Doping histogram")
