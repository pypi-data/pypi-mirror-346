...
class VeritoneDagAgent(Workflow):
    def __init__(
        ...
        # Prompts
        #self.system_prompt = f"""You are a Veritone Automate expert tasked to generate Automate Studio Flows, which are JSON representations of node-red flows."""
        self.system_prompt = f"""You are a Veritone expert tasked to generate Job DAGs (Directed Acylic Graphs), which are built using GraphQL calls."""

        # TODO: not sure if this should be system prompt or separate message
        # get content of file "veritone_dag.txt" from the same directory as this code file
        self.system_prompt += "Here are some examples of DAGs creation GraphQL calls:\n"
        with open(os.path.join(os.path.dirname(__file__), "veritone_dag.txt"), "r") as f:
            self.system_prompt += f.read()

        self.system_prompt += "\n\nIf you don't know what to enter in a graphql field, use a placeholder like 'DATE' or 'FILENAME' or 'ASSET_ID'.\n"
