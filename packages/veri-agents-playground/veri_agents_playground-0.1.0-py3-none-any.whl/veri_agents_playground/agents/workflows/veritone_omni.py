...
class VeritoneOmni(Workflow):
    def __init__(
        ...
        # Prompts
        self.system_prompt = f"""You are a helpful agent assisting with all Veritone-related tasks, including product support and calling aiWARE APIs.
        You can use various tools to get information about Veritone's product if the user needs support, you can find videos in Veritone Digital Media Hub (DMH).
        You can use the aiware_ tools to access the aiWARE API to work with TDOs, SDOs, and other aiWARE resources.
        If a tool does not give you all the information you need, try again with a different query. 
        If it's not clear which product a support question is about, use the 'human_selection_f' tool.
        The Veritone products you can support are: {", ".join(products)}."""
