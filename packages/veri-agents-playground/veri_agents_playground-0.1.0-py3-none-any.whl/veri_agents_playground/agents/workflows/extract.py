...
import asyncio
import logging
from typing import Annotated, Any, Callable, List, NotRequired, Optional, Sequence, TypedDict, cast
...
from veri_agents_playground.agents.workflows import utils
...
class ExtractionResponse(BaseModel):
    ...
    entities: Entities = Field(
        description="Entities extracted from the documents. If you can't find any occurrences of the requested entity, return an empty list for that entity. DO NOT call this tool with the provided examples. Don't return multiple variants of the same entity like 'employee' and 'employees'.",
        examples=[
            {
                "persons": [
                    "John Doe",
...
def create_extract_agent(
    extract_llm: BaseLanguageModel | str,
    knowledgebase: Knowledgebase | str,
    filter: KnowledgeFilter | None = None,
    system_prompt: str | None = None,
    summarize_prompt: str | None = None,
    llm_provider: Callable[[str], BaseLanguageModel | None] | None = None,
    knowledgebase_provider: Callable[[str], Knowledgebase | None] | None = None,
) -> CompiledGraph:
    ...
    async def aextract(state: AgentState, config: RunnableConfig) -> AgentState:
        ...
        try:
            ...
            runnable_filter = utils.get_filter_from_config(config, filter, prefix="")
            runnable_kb = utils.get_knowledgebase_from_config(config, "knowledgebase", knowledgebase, knowledgebase_provider)
            runnable_llm = utils.get_llm_from_config(config, "llm", extract_llm, llm_provider)
            docs = runnable_kb.get_documents(runnable_filter)
            llm_structured = runnable_llm.with_structured_output(ExtractionResponse)

            extraction_results = await utils.aprocess_docs(
                docs,
                messages,
                llm_structured,
...
class Extract(Workflow):
    """Extract entities from documents using an LLM."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.workflow = create_extract_agent(
            extract_llm=self.llm_name,
            knowledgebase=self.knowledgebase_name or "",
            system_prompt=system_prompt,
            summarize_prompt=self.summarize_prompt,
            llm_provider=LLMProvider.get_llm,
            knowledgebase_provider=KnowledgebaseProvider.get_knowledgebase)
