...
class BuildParser(BaseLogParser):
    ...
    def parse_log(self, log_file, unique, result_file):
        ...
        for make_cmd, snippet in self.split_log_by_make(log_file):
            blocks = self.split_by_regex(snippet, split_regex_gcc)
            snippet_blocks = self.process_blocks(blocks, regexes)

            has_issues = any(
                snippet_blocks[regex_id] for regex_id in range(len(regexes))
            )
            if not has_issues:
            ...
            for regex_id in range(len(regexes)):
                snippets[(suite_name, regex_id)] += snippet_blocks[regex_id]
