...
fn_tag="tag"
fn_version="src/latex_runner/about.py"
...
if [ "$build_doc" = 'true' ]; then
	...
	sed -Ei "s/\(tested on [^)]*\)/(tested on `python3 --version` and `python3.8 --version`)/" 'docs/source/index.rst'

	# update version number
	echo "setting version in documentation to $newversion"
	sed -Ei "s/(release = ')[^']*(')/\\1$newversion\\2/" 'docs/source/conf.py'
	sed -Ei 's/^(\s*"latex-runner >= )[^"]*(")/'"\\1${newversion#v}, < ${nextbreakingreleaseversion#v}\\2/" 'docs/source/index.rst'

	# update table of contents
	modules="$(find src/latex_runner/ -name '*.py' -type f | sed -E 's:src/PKGNAME/(.*)\.py:PKGNAME.\1:' | sed 's:/:.:g' | sed 's/PKGNAME.__init__/PKGNAME/' | sed 's/^/   /' | sort)"
	newline="$(printf '\n ')"  # the space is required to protect the newline. it does not seem to be inserted into the file.
	sed -i '/^Reference/,/^[A-Z]/ { /^\.\. toctree::/,/^===/ { /^ *latex_runner/d } }' 'docs/source/index.rst'
	sed -i "/^Reference/,/^[A-Z]/ { /^\.\. toctree::/,/^$/ { /^$/a \\${modules//$newline/\\$newline}$newline} }" 'docs/source/index.rst'

	# build documentation
	sphinx-apidoc --separate -o 'docs/source' 'src/latex_runner'
...
#if grep -ERI -ho 'latex-runner v[0-9]+\.[0-9]+\.[0-9]+(-[^ ]*)?' docs/ | grep -v "NAME $version\$"; then
