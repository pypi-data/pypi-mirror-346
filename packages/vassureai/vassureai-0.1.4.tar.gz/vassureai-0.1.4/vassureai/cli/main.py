...
def init_framework(force: bool = False) -> bool:
    ...
    try:
        ...
        # Ensure Sample_Testcase1.pdf is present
        sample_pdf_src = os.path.join(framework_dir, 'input_pdfs', 'Sample_Testcase1.pdf')
        sample_pdf_dst = os.path.join('input_pdfs', 'Sample_Testcase1.pdf')
        if os.path.exists(sample_pdf_src) and not os.path.exists(sample_pdf_dst):
            shutil.copy2(sample_pdf_src, sample_pdf_dst)
        # Ensure test_sample_testcase1.py is present
        sample_test_src = os.path.join(framework_dir, 'tests', 'test_sample_testcase1.py')
        sample_test_dst = os.path.join('tests', 'test_sample_testcase1.py')
        if os.path.exists(sample_test_src) and not os.path.exists(sample_test_dst):
            shutil.copy2(sample_test_src, sample_test_dst)
        # Ensure README.html is present
        ...
        missing = []
        if not os.path.exists(sample_pdf_dst):
            missing.append('input_pdfs/Sample_Testcase1.pdf')
        if not os.path.exists(sample_test_dst):
            missing.append('tests/test_sample_testcase1.py')
        if not os.path.exists(readme_html_dst):
            missing.append('README.html')
        if missing:
            print(f"Warning: The following files could not be created/copied: {', '.join(missing)}")
...
def get_workspace_root() -> str:
    """Detect the workspace root by looking for a marker file (README.md or pyproject.toml),
    or fallback to current directory if not found."""
    cwd = os.getcwd()
    for marker in ["README.md", "pyproject.toml"]:
        if os.path.exists(os.path.join(cwd, marker)):
            return cwd
    # If neither marker is found, treat cwd as workspace root
    return cwd
...
def main() -> int:
    ...
    # Always check for required folders, even if marker files are missing
    required_dirs = ["input_pdfs", "tests", "reports", "logs"]
    missing_dirs = [d for d in required_dirs if not os.path.exists(os.path.join(workspace_root, d))]
    if missing_dirs:
        print(f"Warning: The following required directories are missing in the workspace: {', '.join(missing_dirs)}. Run 'vassure init' if this is a new project.")
