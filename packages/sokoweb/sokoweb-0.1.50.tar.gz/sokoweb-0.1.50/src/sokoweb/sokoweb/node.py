...
class Node:
    ...
    async def suggest_category(self, category_name):
            logger.info(f"{self.node_id}: Suggesting new category '{category_name}'")
            validator_nodes = self.get_validator_nodes() # This will be empty if routing table is empty

            if not validator_nodes:
                if self.is_validator_node(): # Check if the current node IS a validator
                    logger.info(f"{self.node_id}: No other validator nodes found. Performing self-validation for '{category_name}'.")
                    # Directly validate and add if approved
                    approved = self.validate_category(category_name) # Local validation logic
                    if approved:
                        logger.info(f"{self.node_id}: Category '{category_name}' approved by self-validation.")
                        try:
                            await self.add_category(category_name) # Add to the (potentially non-existent) shared list
                            return True
                        except Exception as e:
                            logger.error(f"{self.node_id}: Error adding category '{category_name}' after self-approval: {e}", exc_info=True)
                            return False # Failed to add even if approved
                    else:
                        logger.info(f"{self.node_id}: Category '{category_name}' not approved by self-validation.")
                        return False
                else:
                    # Not a validator and no other validators found
                    logger.warning(f"{self.node_id}: No validator nodes available and self is not a validator.")
                    return False

            # Existing logic for when validator_nodes is NOT empty
            tasks = []
            for node in validator_nodes:
                # Avoid sending to self if self is already in validator_nodes and we are processing it here.
                # However, get_validator_nodes() returns RemoteNode objects.
                # For simplicity, if validator_nodes is not empty, we assume it contains others or self as a remote.
                tasks.append(self.send_category_suggestion(node, category_name))

            if not tasks: # Should not happen if validator_nodes was not empty, but as a safeguard
                logger.warning(f"{self.node_id}: No tasks created for category suggestion despite validator_nodes list not being empty.")
                return False

            responses = await asyncio.gather(*tasks, return_exceptions=True)

            valid_responses = [r for r in responses if isinstance(r, Message) and r.message_type == "CATEGORY_APPROVAL"]

            if not valid_responses:
                logger.info(f"{self.node_id}: Category '{category_name}' not approved as no valid CATEGORY_APPROVAL responses were received from validators.")
                return False

            approvals = sum(1 for r in valid_responses if r.data.get("approved"))

            # Approval if more than half of the *responding* validators approved
            if approvals > len(valid_responses) // 2:
                logger.info(
                    f"{self.node_id}: Category '{category_name}' approved by {approvals}/{len(valid_responses)} responding validators."
                )
                try:
                    await self.add_category(category_name)
                    return True
                except Exception as e:
                    logger.error(f"{self.node_id}: Error adding category '{category_name}' after validator approval: {e}", exc_info=True)
                    return False
            else:
                logger.info(
                    f"{self.node_id}: Category '{category_name}' not approved by validators ({approvals}/{len(valid_responses)} approvals)."
                )
                return False
