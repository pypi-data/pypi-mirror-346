...
class OrcaCalculation:
    ...
    def create_scratch_dir(self) -> Iterator[Path]:
        ...
        if slurm_id := os.getenv("SLURM_JOB_ID"):
            scratch_dir = (
                self.scratch_dir / slurm_id / f"{self.directory.name}_{run_uuid}"
            )
        else:
        ...
        try:
            ...
            error(f"Error setting up scratch directory: {e}")
            raise
        finally:
    ...
    def run(self) -> bool:
        ...
        if self.directory.exists() and not self.overwrite:
            ...
            if self.input_file.exists():
                try:
                    ...
                    warning(f"Error reading existing input file: {e}")
            else:
            ...
            if input_match:
                try:
                    ...
                    warning(f"Error checking previous calculation status: {e}")
        ...
        try:
            ...
            error(f"Error during calculation execution: {e}", exc_info=True)
            return False

        success = self.completed_normally()
        if success:
            info(f"Calculation completed successfully: {self.directory}")
        else:
            warning(f"Calculation did not complete normally: {self.directory}")
    ...
    def chain(
        ...
        if xyz_file.exists():
            try:
                ...
                error(f"Failed to load molecule from {xyz_file}: {e}", exc_info=True)
                raise
        else:
