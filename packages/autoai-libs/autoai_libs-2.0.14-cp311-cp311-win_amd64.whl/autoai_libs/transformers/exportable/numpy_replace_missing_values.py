...
class NumpyReplaceMissingValues(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, missing_values, filling_value=np.nan):
        """
        ...
        if filling_value is None:
            ...
            self.filling_value = filling_value  # filling value for the missing values
    ...
    def transform(self, X):
        ...
        Y = numpy_replace_values(
            X, filling_value=self.filling_value, reference_values_list=self.missing_values, invert_flag=False
        )
