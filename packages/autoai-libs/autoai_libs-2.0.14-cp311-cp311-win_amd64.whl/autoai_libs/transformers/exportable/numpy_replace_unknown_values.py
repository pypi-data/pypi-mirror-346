...
class NumpyReplaceUnknownValues(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, known_values_list=None, filling_value=None, missing_values_reference_list=None):
        """

        :param known_values_list: reference list of lists of known values for each column
        :param filling_value: special value assigned to unknown values
        """
        ...
        if filling_value is None:
            self.filling_value = np.nan
        else:
            self.filling_value = filling_value  # filling value for the unknown values
    ...
    def transform(self, X):
        ...
        Y = numpy_replace_values(
            X, filling_value=self.filling_value, reference_values_list=self.known_values_list, invert_flag=True
        )
