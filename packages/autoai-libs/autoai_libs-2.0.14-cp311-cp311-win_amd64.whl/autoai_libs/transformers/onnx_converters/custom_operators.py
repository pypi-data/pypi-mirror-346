...
@onnx_op(
    ...
    attrs={"filling_value": PyCustomOpDef.dt_float, "invert_flag": PyCustomOpDef.dt_string},
)
def op_replace_missing_values_num(X, reference_values_list, **kwargs):
    invert_flag = bool(kwargs.get("invert_flag", False))
    filling_value = kwargs.get("filling_value", np.nan)
    return numpy_replace_values(
        X, filling_value=filling_value, reference_values_list=list(reference_values_list), invert_flag=invert_flag
    )
