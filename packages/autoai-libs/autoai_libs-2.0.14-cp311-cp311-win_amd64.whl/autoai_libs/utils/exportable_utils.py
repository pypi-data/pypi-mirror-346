...
def numpy_replace_values(
    ...
    :param filling_value: Value assigned to array entries that match (or do not match if invert_flag is True) the reference values.
    ...
    if reference_values_list:
        ...
        for j in range(X.shape[1]):
            ...
            Y[:, j] = np.where(mask, filling_value, X[:, j])
