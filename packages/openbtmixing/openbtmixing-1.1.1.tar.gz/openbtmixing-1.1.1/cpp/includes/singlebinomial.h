//     singlebinomial.h: Binomial tree model class definition.
...
#ifndef GUARD_singlebinomial_h
#define GUARD_singlebinomial_h
...
class singlebinomialsinfo : public sinfo { //sufficient statistics (will depend on end node model)
public:
   singlebinomialsinfo():sinfo(),sumy(0.0) {}
   singlebinomialsinfo(const singlebinomialsinfo& is):sinfo(is),sumy(is.sumy) {}
   virtual ~singlebinomialsinfo() {}  //need this so memory is properly freed in derived classes.
   double sumy;
   // compound addition operator needed when adding suff stats
   virtual sinfo& operator+=(const sinfo& rhs) {
      sinfo::operator+=(rhs);
      const singlebinomialsinfo& mrhs=static_cast<const singlebinomialsinfo&>(rhs);
      sumy+=mrhs.sumy;
      return *this;
   }
   ...
   const singlebinomialsinfo operator+(const singlebinomialsinfo& other) const {
      singlebinomialsinfo result = *this; //copy of myself.
      result += other;
      return result;
   }
};

class singlebinomialbrt : public brt 
{
public:
   //--------------------
   //classes
   ...
   virtual sinfo* newsinfo() { return new singlebinomialsinfo; }
   virtual std::vector<sinfo*>& newsinfovec() { std::vector<sinfo*>* si= new std::vector<sinfo*>; return *si; }
   virtual std::vector<sinfo*>& newsinfovec(size_t dim) { std::vector<sinfo*>* si = new std::vector<sinfo*>; si->resize(dim); for(size_t i=0;i<dim;i++) si->push_back(new singlebinomialsinfo); return *si; }
