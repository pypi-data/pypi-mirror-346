...
class MXADeployer:
    ...
    def __init__(
        ...
        self._sudo_info = {
            "has_sudo": False,
            "sudo_type": None,  # "nopasswd" 或 "password"
        }
    ...
    async def detect_sudo(self) -> bool:
        """
        检测是否有 sudo 权限以及sudo类型（无密码或需要密码）
        ...
        try:
            async with self.connection() as conn:
                # 首先尝试使用sudo -n true检查是否可以免密码执行sudo
                try:
                    result = await conn.run("sudo -n true")
                    if result.exit_status == 0:
                        logging.info("用户可以免密码使用sudo")
                        self._sudo_info = {
                            "has_sudo": True,
                            "sudo_type": "nopasswd"
                        }
                    ...
                    # 如果sudo -n true失败，这是正常的，表示需要密码
                    logging.info(f"免密sudo检测结果: {str(e)}")
            
                # 如果提供了密码，尝试使用密码进行sudo
                if self.ssh_password:
                    try:
                        # 使用-S参数让sudo从标准输入读取密码，并请求PTY以解决终端问题
                        cmd = f"echo '{self.ssh_password}' | sudo -S true"
                        result = await conn.run(cmd, term_type="xterm", check=True)
                        if result.exit_status == 0:
                            logging.info("使用提供的密码成功获取sudo权限")
                            self._sudo_info = {
                                "has_sudo": True,
                                "sudo_type": "password"
                            }
                        ...
                        logging.error(f"使用密码获取sudo权限失败: {str(e)}")
                        self._sudo_info = {
                            "has_sudo": False,
                            "sudo_type": None
                        }
                ...
                # 如果没有密码可用
                logging.warning("没有可用的密码来获取sudo权限，若您使用秘钥登录，请确保有无密码的sudo权限，或传入密码")
                self._sudo_info = {
                    "has_sudo": False,
                    "sudo_type": None
                }
            ...
            logging.error(f"执行sudo检测脚本失败: {str(e)}")
            raise e
        except Exception as e:
            logging.error(f"sudo权限检测错误: {str(e)}")
            raise e

    async def run_command(self, cmd: str, **kwargs) -> asyncssh.SSHCompletedProcess:
        """
        执行远程命令的辅助函数

        Args:
            cmd: 要执行的命令
            **kwargs: 传递给conn.run的额外参数

        Returns:
            asyncssh.SSHCompletedProcess: 命令执行结果
        """
        async with self.connection() as conn:
            return await conn.run(cmd, **kwargs)

    async def run_sudo_command(self, cmd: str, **kwargs) -> asyncssh.SSHCompletedProcess:
        """
        根据sudo类型执行sudo命令的辅助函数

        Args:
            cmd: 要执行的命令（不包含sudo前缀）
            **kwargs: 传递给conn.run的额外参数

        Returns:
            asyncssh.SSHCompletedProcess: 命令执行结果
        """
        if not self._sudo_info.get("has_sudo", False):
            await self.detect_sudo()
            if not self._sudo_info.get("has_sudo", False):
                raise RuntimeError("没有sudo权限，无法执行命令")
        
        async with self.connection() as conn:
            sudo_type = self._sudo_info.get("sudo_type")
            
            if sudo_type == "nopasswd":
                # 无密码sudo
                return await conn.run(f"sudo {cmd}", **kwargs)
            elif sudo_type == "password" and self.ssh_password:
                # 需要密码的sudo
                # 使用-S参数让sudo从标准输入读取密码
                full_cmd = f"echo '{self.ssh_password}' | sudo -S {cmd}"
                return await conn.run(full_cmd, term_type="xterm", **kwargs)
            else:
                raise RuntimeError("无法确定sudo执行方式或缺少所需密码")
        ...
        try:
            ...
            async with self.connection() as conn:
                try:
                    ...
                    mkdir_result = await self.run_sudo_command(mkdir_cmd, check=True)
                    logger.debug(f"创建目录结果: {mkdir_result.stdout} {mkdir_result.stderr}")

                    # 复制到系统目录并设置可执行权限
                    remote_mxa_path = f"{remote_dir}/mxa"
                    mv_cmd = f"mv {temp_mxa_path} {remote_mxa_path} && sudo chmod +x {remote_mxa_path}"
                    mv_result = await self.run_sudo_command(mv_cmd, check=True)
                    logger.debug(f"移动文件结果: {mv_result.stdout} {mv_result.stderr}")

                    # 验证安装
                    result = await self.run_sudo_command("mxa --version")
                    if result.exit_status == 0:
    ...
    async def systemd_run_mxa(
        ...
        try:
            ...
            async with self.connection() as conn:
                # 创建临时服务文件
                temp_service_file = f"/tmp/mxa_service_{os.getpid()}"
                logger.info(f"创建临时服务文件: {temp_service_file}")
                
                # 写入服务文件内容到临时文件
                async with conn.start_sftp_client() as sftp:
                    async with sftp.open(temp_service_file, 'w') as f:
                        await f.write(service_content)
                
                # 使用sudo移动服务文件到系统目录
                mv_cmd = f"mv {temp_service_file} {service_file}"
                result = await self.run_sudo_command(mv_cmd, check=True)
                logger.debug(f"移动服务文件结果: {result.stdout} {result.stderr}")

                # 重新加载systemd并启动服务
                await self.run_sudo_command("systemctl daemon-reload")  # 重新加载 systemd 配置
                await self.run_sudo_command("systemctl stop mxa || true")  # 确保停止已有服务
                await self.run_sudo_command("systemctl start mxa")  # 启动服务
                await self.run_sudo_command("systemctl enable mxa")  # 设置开机自启
            ...
            # 清理临时文件
            try:
                cleanup_cmd = "rm -f /tmp/mxa_temp_*"
                await self.run_sudo_command(cleanup_cmd)
            except Exception as e:
    ...
    async def systemd_remove_mxa(self) -> bool:
        ...
        """
        try:
            await self.run_sudo_command("systemctl stop mxa")
            await self.run_sudo_command("systemctl disable mxa")
            await self.run_sudo_command(f"rm -f {service_file}")
            await self.run_sudo_command("systemctl daemon-reload")
