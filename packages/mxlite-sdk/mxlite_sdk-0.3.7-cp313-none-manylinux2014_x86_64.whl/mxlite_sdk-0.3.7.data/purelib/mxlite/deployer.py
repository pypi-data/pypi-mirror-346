...
class MXADeployer:
    ...
    async def detect_sudo(self) -> bool:
        ...
        try:
            ...
            # 首先尝试使用sudo -n true检查是否可以免密码执行sudo
                try:
                    ...
                    if result.exit_status == 0:
                        logger.info("用户可以免密码使用sudo")
                    return True
                except asyncssh.ProcessError as e:
                    # 如果sudo -n true失败，这是正常的，表示需要密码
                    logger.info(f"免密sudo检测结果: {str(e)}")
            
                # 如果系统支持，尝试使用echo密码 | sudo -S方式执行
                if self.ssh_password:
                    try:
                        ...
                        if result.exit_status == 0:
                            logger.info("使用提供的密码成功获取sudo权限")
                            return True
                        return False
                    except asyncssh.ProcessError as e:
                        logger.error(f"使用密码获取sudo权限失败: {str(e)}")
                        return False
                
                # 如果没有密码可用
                logger.warning("没有可用的密码来获取sudo权限")
                return False
            ...
            logger.error(f"sudo权限检测错误: {str(e)}")
            raise e
