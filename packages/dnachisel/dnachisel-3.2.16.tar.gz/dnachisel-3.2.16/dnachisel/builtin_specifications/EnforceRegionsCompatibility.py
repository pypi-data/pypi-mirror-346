...
class EnforceRegionsCompatibility(Specification):
    ...
    def __init__(
        self, locations, compatibility_condition, condition_label="", boost=1.0
    ):
        self.locations = [Location.from_tuple(location) for location in locations]
    ...
    def evaluate(self, problem):
        ...
        all_locations_with_incompatibility = sorted(
            list(set(all_locations_with_incompatibility)), key=counter.get
        )
    ...
    def __repr__(self):
        return "CompatRegions(%s%s...)" % (self.condition_label, self.locations[0])
