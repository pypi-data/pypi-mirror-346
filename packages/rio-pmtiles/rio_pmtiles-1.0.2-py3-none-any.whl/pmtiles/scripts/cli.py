...
from pmtiles import __version__ as pmtiles_version
...
@click.version_option(version=pmtiles_version, message="%(version)s")
@click.option(
...
@click.option("--covers", help="Restrict PMTiles output to cover a quadkey")
@click.option(
    "--cutline",
...
def pmtiles(
    ...
    if implementation == "cf" and sys.version_info < (3, 7):
        ...
        from pmtiles.cf import process_tiles
    elif implementation == "mp":
        from pmtiles.mp import process_tiles
    ...
    with ctx.obj["env"]:
        ...
        def insert_results(tile, contents, img_ext=None, image_dump=None):
            ...
            # PMTiles have a different origin than Mercantile/tilebelt.
