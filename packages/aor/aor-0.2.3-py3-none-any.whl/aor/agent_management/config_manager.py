...
class AgentConfigManager:
    ...
    def validate_agent_inputs_outputs(self, agent: Agent) -> Tuple[bool, Optional[str]]:
        ...
        # Check for reserved keywords in input names and ensure descriptions are provided
        for input_def in agent.inputs:
            ...
            if not input_def.get("desc"):
                return False, f"Input '{name}' is missing a description. All inputs must have descriptions."
                
        # Check for reserved keywords in output names and ensure descriptions are provided
        for output_def in agent.outputs:
            ...
            # Ensure description is provided
            if not output_def.get("desc"):
                return False, f"Output '{name}' is missing a description. All outputs must have descriptions."
    ...
    def _generate_example_comments(self, indent: str, agent_name: str) -> List[str]:
        ...
        examples.append(
            f"{indent}    #     desc: Example text input - first input must be required (description is required)"
        )
        examples.append(f"{indent}    # -   name: EXAMPLE_second_input")
        examples.append(f"{indent}    #     type: text")
        examples.append(f"{indent}    #     required: false")
        examples.append(
            f"{indent}    #     desc: Another example text input - additional inputs can be optional (description is required)"
        )
        examples.append(f"{indent}    # outputs:")
        examples.append(f"{indent}    # -   name: EXAMPLE_text_output")
        examples.append(f"{indent}    #     type: text")
        examples.append(
            f"{indent}    #     desc: Example text output - replace with your own output name and description (description is required)"
        )
        examples.append(f"{indent}    # -   name: EXAMPLE_second_output")
        examples.append(f"{indent}    #     type: text")
        examples.append(
            f"{indent}    #     desc: Another example text output - you must define at least one output (description is required)"
        )
        examples.append(f"{indent}    # ")
