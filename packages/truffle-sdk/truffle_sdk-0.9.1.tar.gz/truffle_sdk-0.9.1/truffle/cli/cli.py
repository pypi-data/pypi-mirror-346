...
def get_base_url() -> str:
    if os.getenv("TRUFFLE_DEV"):
        return "https://forecast.itsalltruffles.com:2087"
    return "https://overcast.itsalltruffles.com:2087"
...
def init_project(path : Path):
    ...
    if project.exists():
        if not project.is_dir():
            logger.error(f"Path {path} exists and is not a directory, cannot init")
            sys.exit(1)

        if any(project.iterdir()):
            logger.error(f"Directory {path} is not empty, cannot init")
            logger.info("Usage: truffle init <project_name>")
            sys.exit(1)
    else:
    ...
    def get_input(prompt : str, default : str = None, field : str = 'Input'):
        ...
        txt = input(f"{prompt}: ") if not has_default else input(f"{prompt} ['{default}']: ")
        if len(txt) > 0:
            return txt
    ...
    proj_name = get_input("Name for your app", project.name, "App name")
    ...
    for f in APP_FILES:
        if f == "icon.png":
            try:
                with open(project / f, "wb") as icon:
                    icon.write(requests.get("https://pngimg.com/d/question_mark_PNG68.png").content)
            except Exception as e:
            ...
            with open(project / f, "w") as reqs:
                reqs.write("requests\n")
            logger.debug(f"-> Created requirements.txt")
        elif f == "main.py":
            class_name = proj_name.replace(" ", "").replace("-", "").replace("_", "").replace(".", "").replace(",", "").replace("!", "").replace("'", "").replace('"', "")
            with open(project / f, "w") as main:
                main.write(default_mainpy(class_name))
...
def build(builddir: Path):
    ...
    logger.success(f"Built project {builddir} to {bundle.relative_to(Path.cwd())}")
    logger.info(f"upload with 'truffle upload {bundle.relative_to(Path.cwd())}'")
    sys.exit(0)
