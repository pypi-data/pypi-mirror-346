...
class TipRack(Labware):
    def __init__(self, id: str, labware_name: str, slot: int, definition: dict, context: 'Context'):
        ...
        self.__row_order = [chr(i) for i in range(ord('A'), ord(self._max_row) + 1)]
        self.__total_tips = len(self._wells)
        self.__tip_status = {well: False for well in self._wells}  # False means unused
        # self.__context = context
        # self.alias = None

    def _get_new_tip(self, count: int = 1) -> Union[str, List[str]]:
        ...
        try:
            ...
            return self._get_single_tip() if count == 1 else self._get_column_tips()
    ...
    def _get_single_tip(self) -> Tuple['TipRack', str]:
        """get single tip
        ...
        Returns:
            tuple: self instance, tip name
        """
        for col in range(1, self._max_col + 1):
            for row in self.__row_order:
                tip = f"{row}{col}"
