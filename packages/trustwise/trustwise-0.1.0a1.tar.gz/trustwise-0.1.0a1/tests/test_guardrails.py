...
class TestGuardrail:
    ...
    def test_evaluate_response(
        ...
        result = guardrail.evaluate_response(
            query=sample_query,
        ...
        for metric_name, metric_result in results.items():
            ...
            assert "result" in metric_result
            assert "passed" in metric_result
            assert isinstance(metric_result["result"], dict)
            assert "score" in metric_result["result"]
            assert isinstance(metric_result["result"]["score"], float)
            if metric_name == "prompt_injection":
                assert 0 <= metric_result["result"]["score"] <= 100
            else:
                assert 0 <= metric_result["result"]["score"] <= 100

    @patch("trustwise.sdk.metrics.safety.v3.FaithfulnessMetric.evaluate")
    @patch("trustwise.sdk.client.TrustwiseClient._post")
    def test_evaluate_response_with_custom_metrics(
        self,
        mock_post: Any,
        mock_faithfulness_evaluate: Any,
        sdk: TrustwiseSDK,
    ...
    @patch("trustwise.sdk.metrics.safety.v3.FaithfulnessMetric.evaluate")
    @patch("trustwise.sdk.client.TrustwiseClient._post")
    def test_evaluate_response_without_blocking(
        self,
        mock_post: Any,
        mock_faithfulness_evaluate: Any,
        sdk: TrustwiseSDK,
    ...
    @patch("trustwise.sdk.client.TrustwiseClient._post")
    def test_evaluate_response_with_prompt_injection(
        self,
        mock_post: Any,
        sdk: TrustwiseSDK,
        sample_query: str,
