...
def generate_cmd(project_dir: str, package_dir: str, override: bool = False,
    ...
    Args:
        ...
        package_dir: 包目录
        override: 是否覆盖文件
        command: 命令名称, 生成的cmd命令可使用 uv run {command} 运行
...
''', package_name=package_name, command=command, extra_import=extra_import, extra_run=extra_run), override=override)
    add_uv_script(project_dir, str_format('${command} = "${package_name}.cmd.${cmd_main_name}:main"',
...
# 执行命令
echo "开始执行命令"
nohup uv run ${command} --env pro >> /dev/null 2>&1 &
echo "执行成功"
echo "退出虚拟环境"
deactivate
