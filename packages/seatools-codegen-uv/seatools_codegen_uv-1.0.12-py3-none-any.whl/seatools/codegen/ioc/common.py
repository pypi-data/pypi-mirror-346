...
def add_uv_script(project_dir: str, script: str):
    """新增uv启动脚本."""
    pyproject_toml = project_dir + os.sep + 'pyproject.toml'
    if not os.path.exists(pyproject_toml):
        logger.error('pyproject.toml文件不存在, 无法添加uv执行脚本')
        return
    with open(pyproject_toml, 'r', encoding='utf-8') as f:
        content = f.read()
    if script in content:
        ...
        return
    content = content.replace(
        '[project.scripts]', f'[project.scripts]\n{script}')
    with open(pyproject_toml, 'w', encoding='utf-8') as f:
        f.write(content)
    logger.success('新增uv执行脚本: [uv run {}]', script.split('=')[0].strip())
