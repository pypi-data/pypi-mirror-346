...
from .common import mkdir, create_file, str_format, add_docker_compose_script, unwrapper_dir_name, add_uv_script
...
'''.format(name=name), override=override)
    ...
    def gen_django_cmd():
        ...
        uv_script_name = '{}_django'.format(app) if app else 'django'
...
def main(args = None) -> None:
    ...
    Usage:
        uv run ${uv_script_name} [runserver|migrate|...]
    """
    # 启动项目依赖
    start()
    # django 基于sys.argv[0]处理, 这里覆盖sys.argv[0]为当前文件来保证uv正常运行
    sys.argv[0] = __file__
...
if __name__ == "__main__":
    main()
''', package_name=package_name, uv_script_name=uv_script_name), override=override)
        add_uv_script(project_dir, str_format('${uv_script_name} = "${package_name}.cmd.django_cmd:main"',
...
''', package_name=package_name), override=override)
        ...
        runserver_uv_script_name = '{}_django_runserver'.format(app) if app else 'django_runserver'

        add_uv_script(project_dir, str_format(
            '${runserver_uv_script_name} = "${package_name}.cmd.django_main:main"',
            package_name=package_name, runserver_uv_script_name=runserver_uv_script_name,
        ))
...
echo "开始执行命令"
# 执行命令
nohup uv run ${runserver_uv_script_name} --host 0.0.0.0 --port 8000 --env pro --workers 2 >> /dev/null 2>&1 &
echo "执行成功"
echo "退出虚拟环境"
deactivate
''', project_name=project_name, runserver_uv_script_name=runserver_uv_script_name), override=override)
...
CMD uv run ${runserver_uv_script_name} --host 0.0.0.0 --port 8000 --env pro --workers 2
''', project_name=project_name, runserver_uv_script_name=runserver_uv_script_name), override=override)
