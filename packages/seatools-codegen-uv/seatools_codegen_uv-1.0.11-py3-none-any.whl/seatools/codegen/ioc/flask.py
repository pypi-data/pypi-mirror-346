...
from .common import mkdir, create_file, add_docker_compose_script, unwrapper_dir_name, str_format, add_uv_script
...
''', package_name=package_name), override=override)
        ...
        uv_script_name = '{}_flask'.format(app) if app else 'flask'

        add_uv_script(project_dir, str_format(
            '${uv_script_name} = "${package_name}.cmd.flask_main:main"',
            package_name=package_name, uv_script_name=uv_script_name
        ))
...
nohup uv run ${uv_script_name} --host 0.0.0.0 --port 8000 --env pro --workers 4 >> /dev/null 2>&1 &
echo "执行成功"
echo "退出虚拟环境"
deactivate
''', project_name=project_name, uv_script_name=uv_script_name), override=override)
...
RUN pip install --upgrade -i https://pypi.tuna.tsinghua.edu.cn/simple uv

RUN uv sync --no-dev

RUN uv add flask uvicorn[standard]

CMD uv run ${uv_script_name} --env pro --host 0.0.0.0 --port 8000 --workers 4
''', project_name=project_name, uv_script_name=uv_script_name), override=override)
