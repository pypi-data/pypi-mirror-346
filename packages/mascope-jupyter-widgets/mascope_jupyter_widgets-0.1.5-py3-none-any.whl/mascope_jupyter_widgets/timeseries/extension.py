...
class TimeSeriesDataExtension:
    ...
    def timeseries(self) -> pd.DataFrame:
        ...
        Total ion current (tic) has been also set to rows
        which makes tic easily comparable
        to compound-level data e.g. when visualizing time series traces.
    ...
    def add_untarget_peak(self, peak: float) -> None:
        ...
        def _get_reference_row(sample_item_id: str) -> dict:
            ...
            :type peak: float
            """
            match = self.timeseries[self.timeseries["sample_item_id"] == sample_item_id]
            return match.iloc[0].to_dict() if not match.empty else {}

        # Error handling
        if not isinstance(peak, (int, float)):
            raise ValueError(f"Invalid peak value: {peak}. Must be a number.")
        ...
        if peak not in binning_intensity.index:
            raise ValueError(f"Peak {peak} not found in the binning intensity matrix.")
