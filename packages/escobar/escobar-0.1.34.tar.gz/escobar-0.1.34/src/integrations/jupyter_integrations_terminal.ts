...
export function init_terminal() {
    ...
    functions["runCommandInTerminalAsync"] = {
        "def": {
            ...
            "description": "Runs a command in a terminal without waiting for completion. Returns immediately with 'ok' status. Use this for long-running commands where you don't need to wait for the result.",
            "arguments": {
                "name": {
                    "type": "string",
                    "name": "Name for the terminal to run the command in"
        ...
        "func": async (args: any): Promise<string> => {
            ...
            // Set up a listener to capture output
            const listener = (_, msg: any) => {
                if (msg.type === 'stdout') {
                    if (Array.isArray(msg.content)) {
                        for (var i = 0; i < msg.content.length; i++) {
                            const content = stripAnsi(msg.content[i]);
                            terminalOutputs[name] += content;
                        }
                    }
                }
            };
    ...
    functions["getAsyncTerminalOutput"] = {
        ...
        "func": async (args: any): Promise<string> => {
            ...
            console.log(`Getting output from async terminal: ${name}`);
