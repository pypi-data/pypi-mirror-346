...
def save_current_flock_to_file_service(new_filename: str) -> tuple[bool, str]:
    global CURRENT_FLOCK_INSTANCE, CURRENT_FLOCK_FILENAME
    if not CURRENT_FLOCK_INSTANCE: return False, "No flock loaded to save."
    if not new_filename.strip(): return False, "Filename cannot be empty."
    ...
    except Exception as e: return False, f"Error saving flock: {e}"
...
def add_agent_to_current_flock_service(agent_config: dict) -> bool:
    ...
    if agent_config.get("tools_names"):
        for tool_name in agent_config["tools_names"]:
            ...
            except KeyError: print(f"Warning: Tool '{tool_name}' not found. Skipping.")
    try:
        agent = FlockFactory.create_default_agent(
            name=agent_config["name"], description=agent_config.get("description"),
            model=agent_config.get("model"), input=agent_config["input"],
            output=agent_config["output"], tools=tools_instances or None
        )
        ...
        return True
    except Exception as e: print(f"Error adding agent: {e}"); return False
...
def update_agent_in_current_flock_service(original_agent_name: str, agent_config: dict) -> bool:
    ...
    try:
        ...
        elif agent_to_update.handoff_router: agent_to_update.handoff_router = None
        ...
        return True
    except Exception as e: print(f"Error updating agent: {e}"); return False


def remove_agent_from_current_flock_service(agent_name: str) -> bool:
    # ... (same as before) ...
    global CURRENT_FLOCK_INSTANCE
    if not CURRENT_FLOCK_INSTANCE or agent_name not in CURRENT_FLOCK_INSTANCE.agents: return False
...
async def run_current_flock_service(start_agent_name: str, inputs: dict) -> dict | str:
    # ... (same as before) ...
    global CURRENT_FLOCK_INSTANCE
    if not CURRENT_FLOCK_INSTANCE: return "Error: No flock loaded."
    ...
    try:
        ...
        return result
    except Exception as e: print(f"Error during flock execution: {e}"); return f"Error: {str(e)}"
