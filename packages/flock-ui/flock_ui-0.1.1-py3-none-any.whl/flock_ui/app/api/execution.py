...
async def htmx_get_agent_input_form(request: Request, agent_name: str):
    # ... (same as before) ...
    flock = get_current_flock_instance()
    if not flock: return HTMLResponse("") 
...
async def htmx_run_flock(request: Request):
    ...
    if not start_agent_name: return HTMLResponse("<p class='error'>Starting agent not selected.</p>")
    agent = flock.agents.get(start_agent_name)
    if not agent: return HTMLResponse(f"<p class='error'>Agent '{start_agent_name}' not found.</p>")
    inputs = {}
    if agent.input and isinstance(agent.input, str):
        try:
            parsed_spec = parse_schema(agent.input)
        ...
        except Exception as e: return HTMLResponse(f"<p class='error'>Error processing inputs for {start_agent_name}: {e}</p>")
