...
class BackendBaseSettings(pydantic.BaseSettings):
    ...
    class Config:
        ...
        def customise_sources(cls, init_settings, env_settings, file_secret_settings):
            ...
            # 4. 默认值
            return (
                init_settings,  # 命令行或程序中设置的值
                env_settings,   # 环境变量
                file_secret_settings,  # .env文件
            )
    
    @pydantic.validator("JWT_ACCESS_TOKEN_EXPIRATION_TIME", pre=True)
    def compute_expiration_time(cls, v: int, values: Dict[str, Any]) -> int:
        """计算JWT令牌过期时间"""
