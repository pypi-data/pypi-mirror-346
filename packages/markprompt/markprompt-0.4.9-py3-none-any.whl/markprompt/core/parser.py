...
class TemplateParser:
    ...
    def _process_response_format(self, response_format, template_dir) -> Dict[str, Any]:
        ...
        # 如果template_dir为None，直接返回原始的response_format
        if template_dir is None:
            logger.info(f"template_dir为None，返回原始的response_format: {response_format}")
            return response_format
            
        # 仅尝试最直接的解析方式 - 基于模板目录
        file_path = template_dir / response_format
        logger.info(f"_process_response_format template_dir: {template_dir}")
        logger.info(f"_process_response_format response_format: {response_format}")
    ...
    def parse(self, content: str, template_dir = None) -> PromptTemplate:
        ...
        Args:
            content: 模板文件内容
            template_dir: 模板目录路径，用于解析response_format的相对路径。如果为None，则不处理response_format路径。
        """
        ...
        if "generation_config" in metadata and "response_format" in metadata["generation_config"] and template_dir is not None:
