from _typeshed import Incomplete

class BINARIESmodel:
    Cprior: Incomplete
    Jprior: Incomplete
    KO_Kprior: Incomplete
    KO_Pprior: Incomplete
    KO_cosiprior: Incomplete
    KO_eprior: Incomplete
    KO_phiprior: Incomplete
    KO_qprior: Incomplete
    KO_wdotprior: Incomplete
    KO_wprior: Incomplete
    binary_mass: Incomplete
    binary_radius: Incomplete
    conditional: Incomplete
    cubic_prior: Incomplete
    degree: Incomplete
    directory: Incomplete
    double_lined: Incomplete
    eclipsing: Incomplete
    enforce_stability: Incomplete
    fix: Incomplete
    known_object: Incomplete
    n_known_object: Incomplete
    nu_prior: Incomplete
    offsets_prior: Incomplete
    quadr_prior: Incomplete
    relativistic_correction: Incomplete
    slope_prior: Incomplete
    star_mass: Incomplete
    star_radius: Incomplete
    studentt: Incomplete
    tidal_correction: Incomplete
    trend: Incomplete
    def __init__(self, *args, **kwargs) -> None: ...
