...
class AgentFlowPilot:
    ...
    async def _notify(self, status: TaskStatus, task: Optional[Task] = None):
        """
        通知外部服务任务状态变化
        """
        self.logger.info(f"通知外部服务任务状态变化: {status}, task: {task.status}")
        notification = TaskNotification(status=status, task=copy.deepcopy(task), context=self.context)
        task = asyncio.create_task(self.external_service.notify(notification))
        asyncio.gather(task)
    ...
    async def _arun(self) -> None:
        ...
        await self._notify(self.final_status(), task)
