...
from ...services.continuous_conversation_service.continuous_conversation_helper import ContinuousConversationHelper
...
class Chat(CompanyAssetModel):
    ...
    def model_dump_json(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        if serializer == SerializerType.FRONTEND:
            #we don't want to dump templates that are part of a continuous conversation as they're represented by the continuous conversation
            messages_to_dump = [message for message in self.messages if not (message.subtype == MessageSubtype.TEMPLATE and message.context.continuous_conversation_id is not None)]
            dump["messages"] = [message.model_dump_json(serializer=SerializerType.FRONTEND) for message in messages_to_dump]
            #we handle the continuos conversation for the frontend
            dump.pop("continuous_conversations", None)
            active_cc = ContinuousConversationHelper.get_active_cc(chat=self)
            dump["continuos_conversation"] = active_cc.model_dump_json(serializer=SerializerType.FRONTEND) if active_cc else None
            #we handle the scheduled messages for the frontend (pending)
            dump["time_left"] = json.loads(self.time_left.model_dump_json())
            dump["client"] = self.client.model_dump_json(serializer=SerializerType.FRONTEND)
