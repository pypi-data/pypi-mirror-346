...
2025/05/11 1.4.0 FolderSymlinksUtilityを改良
```
...
class FolderSymlinksUtility:
    ...
    def create_folder_symlinks(self):
        ...
        for sd_rel, gd_rel in self.folder_pairs.items():
            ...
            # 1. もし既にシンボリックリンクが張られていたら、まず削除
            if gd_dir.exists() and gd_dir.is_symlink():
                gd_dir.unlink()
                logger.info(f"既存のシンボリックリンク削除: {gd_dir}")
            # 2. それ以外（存在しない or 本物のディレクトリ）は mkdir (exist_ok=True で安全)
