...
from konfuzio_sdk.utils import get_ascii_string, get_spans_from_bbox, is_file
...
_restored_project_id = None  # cache result here


def restored_project_id():
    """
    Restores the snapshot only once per test session,
    returns the cached project ID to all tests that need it.
    """
    global _restored_project_id
    if _restored_project_id is None:
        print('⏳ Restoring snapshot...')
        _restored_project_id = restore_snapshot(snapshot_id=65)
    return _restored_project_id
...
class TestOnlineProject(unittest.TestCase):
    ...
    def test_get_sentence_spans_from_bbox(self):
        ...
        file_path = os.path.join(BASE_PATH, 'test_data', 'textposition.pdf')
        document = Document.from_file(path=file_path, project=self.project)
    ...
    def test_create_invalid_file_type_document(self):
        ...
        with pytest.raises(NotImplementedError, match='We do not support file'):
            file_path = os.path.join(BASE_PATH, 'test_data', 'invalid_pdf.pdf')
            Document.from_file(file_path, self.project)
    ...
    def test_create_modify_and_delete_document(self):
        ...
        file_path = os.path.join(BASE_PATH, 'test_data', 'pdf.pdf')
        doc = Document.from_file(file_path, self.project)
        doc_id = doc.id_
        doc.dataset_status = 1
        doc.save_meta_data()
    ...
    def test_get_image_png(self):
        ...
        file_path = os.path.join(BASE_PATH, 'test_data', 'png.png')
        document = Document.from_file(path=file_path, project=self.project)
    ...
    def test_get_image_jpeg(self):
        ...
        file_path = os.path.join(BASE_PATH, 'test_data', 'jpg.jpg')
        document = Document.from_file(path=file_path, project=self.project)
        image = document.pages()[0].get_image()
        assert isinstance(image, PIL.PngImagePlugin.PngImageFile)
        document.delete(delete_online=True)
    ...
    def tearDownClass(cls) -> None:
        ...
        folder_path = os.path.join(BASE_PATH, 'test_data')
        for document in project._documents:
            if document.name in os.listdir(folder_path):
                document.delete(delete_online=True)
...
class TestOfflineExampleData(unittest.TestCase):
    ...
    def test_project_labels(self):
        # Expected list of (name, data_type) tuples
        expected_labels = [
            ...
            ('EMPTY_LABEL', 'Text'),
            ('Faktor', 'Number'),
            ('Gesamt-Brutto', 'Number'),
            ('Lohnart', 'Text'),
            ('Menge', 'Number'),
            ('Nachname', 'Text'),
            ('Netto-Verdienst', 'Number'),
            ('Personalausweis', 'Text'),
            ('Sozialversicherung', 'Text'),
            ('Steuer-Brutto', 'Text'),
            ('Steuerklasse', 'Text'),
            ('Steuerrechtliche Abzüge', 'Text'),
            ('Vorname', 'Text'),
            ('Ansprechpartner', 'Text'),
            ('Anzahl / Menge', 'Positive Number'),
            ('Artikelbezeichnung', 'Text'),
            ('Artikelnummer', 'Text'),
            ('Ausstelldatum', 'Date'),
            ('Bedienung Nr', 'Text'),
            ('BonNr', 'Text'),
            ('Brutto (Ergebnis der MwSt. Berechnung)', 'Text'),
            ('Einheit', 'Text'),
            ('Einzelpreis (Brutto)', 'Number'),
            ('Filial-/Markt-Nummer', 'Text'),
            ('Firmenname', 'Text'),
            ('Gesamtpreis (Brutto)', 'Number'),
            ('Hausnummer', 'Text'),
            ('Kassennummer', 'Text'),
            ('Mehrwertsteuerbetrag', 'Text'),
            ('Mehrwertsteuersatz', 'Text'),
            ('Netto (Basis der MwSt. Berechnung)', 'Text'),
            ('Ort', 'Text'),
            ('Postleitzahl', 'Text'),
            ('Referenz auf MwSt', 'Text'),
            ('Steuernummer', 'Text'),
            ('Straße', 'Text'),
            ('Telefonnummer', 'Text'),
            ('Uhrzeit', 'Text'),
            ('Umsatzsteuer-Identifikationsnummer', 'Text'),
            ('Währung', 'Text'),
        ...
        # Assuming self.project.labels is a list of objects with name and data_type attributes
        actual_labels = [(x.name, x.data_type) for x in self.project.labels]

        # Assert that the actual list matches the expected list
        self.assertEqual(actual_labels, expected_labels, 'The project labels do not match the expected list.')
...
class TestOfflineDataSetup(unittest.TestCase):
    ...
    def test_project_label_names(self):
        """Test that no_label exists in the Labels of the Project and has the expected name."""
        assert self.label.name == 'First Offline Label'
        assert self.label.name_clean == get_ascii_string(self.label.name)
    ...
    def test_to_return_a_custom_offset_string(self):
        ...
        annotation = Annotation(
            ...
            label=self.label,
            is_correct=True,
            custom_offset_string='other_string',
            offset_string='custom_string',
        )
        assert annotation.offset_string == 'other_string'
