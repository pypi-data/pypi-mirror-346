...
@cache
def convert_proto_message_type_to_pyarrow_type(proto_message_class: Descriptor, max_depth: int) -> pa.DataType:
    ...
    def _convert_proto_message_type_to_pyarrow_type(
        proto_message_class: Descriptor,
        max_depth: int,
        visited_messages: set[Descriptor],
        ...
        for fd in proto_message_class.fields:
            if fd.type == FieldDescriptor.TYPE_MESSAGE:
                ...
                self_referential = new_message_type in visited_messages
                if self_referential and max_depth == 1:
                    # Skip self-referential types when max depth is 1
                    continue
                visited_messages.add(new_message_type)
                ...
                visited_messages.remove(new_message_type)
            else:
    ...
    return _convert_proto_message_type_to_pyarrow_type(
        proto_message_class, max_depth=default_max_depth, visited_messages=set(), self_referential=False
    )
