...
def get_metadata_host_response(features_metadata_url: str, job_id: str, job_token: str) -> dict:
    ...
    assert response.status_code == 200, f"custodian request failed with status code {response.status_code}: {response.json()}"
...
def get_features_from_response_v2(json_response, fg_to_consider):
    ...
    return feature_list, offline_col_to_default_values_map, onfs_fg_to_onfs_feat_map, onfs_fg_to_ofs_feat_map, fg_to_datatype_map, entity_label, entity_column_names
