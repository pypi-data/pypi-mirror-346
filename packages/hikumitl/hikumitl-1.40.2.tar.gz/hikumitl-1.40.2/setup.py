...
* gen, to generate the classes required for hikumitl to run or docs
...
API_REF_URL = 'https://tl.hikumitl.dev/'

GENERATOR_DIR = Path('hikumitl_generator')
LIBRARY_DIR = Path('hikumitl')
...
def generate(which, action='gen'):
    from hikumitl_generator.parsers import\
        parse_errors, parse_methods, parse_tl, find_layer

    from hikumitl_generator.generators import\
...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        # Make sure tl.hikumitl.dev is up-to-date first
        with urllib.request.urlopen(API_REF_URL) as resp:
            ...
            from hikumitl_generator.parsers import find_layer
            layer = next(filter(None, map(find_layer, TLOBJECT_IN_TLS)))
        ...
        # Try importing the hikumitl module to assert it has no errors
        try:
            import hikumitl
        ...
        remove_dirs = ['__pycache__', 'build', 'dist', 'hikumitl.egg-info']
        ...
        for x in ('build', 'dist', 'hikumitl.egg-info'):
            shutil.rmtree(x, ignore_errors=True)
        ...
        with open('hikumitl/version.py', 'r', encoding='utf-8') as f:
        ...
        setup(
            ...
            url='https://github.com/Plovchikdeval/hikumitl',
            download_url='https://github.com/Plovchikdeval/hikumitl/releases',

            author='Pavel Nedelko',
            author_email='dev3quest@gmail.com',
            ...
            install_requires=['pyaes','rsa==4.9','aiofile==3.8.1','gitpython==3.1.27','meval==2.5','aiohttp_jinja2==1.5','aiohttp==3.8.3','Jinja2==3.0.3','requests==2.27.1','aiogram==2.19','grapheme==0.6.0','emoji==2.1.0','werkzeug==2.2.2','psutil==5.9.4','tgcrypto==1.2.5','ruamel.yaml==0.17.21'],
            extras_require={
                'cryptg': ['cryptg']
            }
        )
