...
# Adapted from transformers.models.jamba.modeling_jamba.FalconHybridMambaAttentionDynamicCache for the v2 mixer
...
class FalconH1Model(FalconH1PreTrainedModel):
    ...
    def _init_mup_vector(self):
        """
        FalconH1 applies different MuP mulitplier for each dimension of the hidden states. Therefore we create a MuP
        vector that we will use to multiply the projected hidden states with that
        """
        mup_vector = None

        for layer in self.layers:
            mamba_layer = layer.mamba
            vector_shape = 2 * mamba_layer.intermediate_size + 2 * mamba_layer.groups_time_state_size + mamba_layer.num_heads
            ...
            if mup_vector is None:
                ...
                mup_vector[:, :, mamba_layer.intermediate_size : 2 * mamba_layer.intermediate_size] *= mamba_layer.zxbcdt_multipliers[1]
                mup_vector[:, :, 2 * mamba_layer.intermediate_size : 2 * mamba_layer.intermediate_size + mamba_layer.groups_time_state_size] *= (
                    mamba_layer.zxbcdt_multipliers[2]
                )
                mup_vector[
                    :,
                    :,
                    2 * mamba_layer.intermediate_size + mamba_layer.groups_time_state_size : 2 * mamba_layer.intermediate_size
                    + 2 * mamba_layer.groups_time_state_size,
                ] *= mamba_layer.zxbcdt_multipliers[3]
                mup_vector[:, :, 2 * mamba_layer.intermediate_size + 2 * mamba_layer.groups_time_state_size :] *= mamba_layer.zxbcdt_multipliers[4]
