...
from petcmd.autocompletion.zsh import ZshAutocompletion
...
class Commander:
	...
	def __init__(self, error_handler: Callable[[Exception], None] = None, compact_commands_list: bool = False):
		...
		def show_shell_completion(alias: str = None):
			...
			print(ZshAutocompletion.generate(self.__commands, alias))
		...
		def setup_shell_completion(alias: str = None):
			...
			with open(os.path.join(completions, f"_{alias}"), "w") as f:
				f.write(ZshAutocompletion.generate(self.__commands, alias))
	...
	def command(self, *cmds: str, shell_complete: Iterable | Callable | str = None):
