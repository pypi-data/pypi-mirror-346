...
import argparse
import os
from ordered_logit import OrderedLogit
#import time
...
def optimise_orderered():
   ...
   # ORDLOG(X, y, ncat, start=None, normalize=True, fit_intercept=False)
    FIT = True
    if FIT:
        mod = OrderedLogit(X=X, y=y, J=ncat, distr='logit', start=None, normalize=False, fit_intercept=True)
        mod.fit()
        mod.report()
    else:
    ...
    print('now do a multinomial logit')
    ...
    isvars = ['carat', 'depth', 'table', 'depth']
    asvars = ['carat']
    varnames = asvars
    ...
    ids = df_long['ids']
    mnl = MultinomialLogit()
    mnl.setup(X, y, ids=ids, varnames=varnames, isvars=asvars, transvars=transvars, alts=alt_var)
    mnl.fit()
    mnl.summarise()
    mnl.get_loglik_null()
