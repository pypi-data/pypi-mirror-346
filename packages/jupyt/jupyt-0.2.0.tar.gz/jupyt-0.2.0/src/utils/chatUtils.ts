export function extractCellReferences(text: string): { cleanQuery: string; cellNumbers: Set<number> } {
...
export function extractJsonFromContent(content: string): { json: any | null, rest: string } {
...
export function removeCellOperationTags(input: string): string {
  return input.replace(/<cell_operation>[\s\S]*?<\/cell_operation>/g, '').trim();
}
