...
export const injectOrUpdateCellUI = (cell: CodeCell, panel: NotebookPanel) => {
  if (!cell.model || !(cell instanceof CodeCell) || !cell.inputArea /* Check needed for typing */) {
    return;
  }

  const model = cell.model as CodeCellModel;
  const pendingMetadata = model.sharedModel.getMetadata(PENDING_OPERATION_METADATA_KEY) as JupytPendingOperation | undefined;

  // Find container within the main cell node
  let container = cell.node.querySelector(`:scope > .${JUPYT_UI_CONTAINER_CLASS}`) as HTMLElement;
  ...
  ReactDOM.render(
    React.createElement(CellApprovalControls, {
      ...
      originalCode: cell.model.sharedModel.source,
 ...
 */
export const handleApprove = async (cell: CodeCell, panel: NotebookPanel, metadata: JupytPendingOperation) => {
  ...
  if (!notebookModel) {
      ...
      try { model.sharedModel.deleteMetadata(APPROVED_OPERATION_METADATA_KEY); } catch{} 
      return;
  }
  try {
    let approvedMetadata: JupytApprovedOperation | null = null;
    let cellIndexToRun: number | null = null;
    let runAfterDelete = false;
    ...
    switch (metadata.type) {
      case 'create_cell':
        if (metadata.code !== undefined) {
          ...
          if (index !== -1 && metadata.runNeeded) cellIndexToRun = index;
      ...
      case 'update_cell':
       ...
       case 'delete_cell':
        ...
        if (indexToDelete !== -1) {
          // If we're deleting the current cell, we want to select the next one
          // or the previous one if we're deleting the last cell
          let nextCellIndex: number | null = null;  // Change type to allow for null
          
          // Set next cell index to select after deletion
          if (indexToDelete < notebook.widgets.length - 1) {
          ...
          // Select the next cell if there is one
          if (nextCellIndex !== null && notebook.widgets.length > 1) {
    ...
    if (approvedMetadata) {
      ...
      model.sharedModel.setMetadata(APPROVED_OPERATION_METADATA_KEY, approvedMetadata as any);
    }
    ...
    showNotification(`Failed to approve operation: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
 ...
 */
export const handleReject = (cell: CodeCell, panel: NotebookPanel, metadata: JupytPendingOperation) => {
  if (!cell.model) return;
  const model = cell.model as CodeCellModel;
  ...
  try {
    switch (metadata.type) {
      case 'create_cell':
        ...
        if (indexToDelete !== -1) {
          notebookModel.sharedModel.deleteCell(indexToDelete);
          showNotification('Cell creation rejected and placeholder removed.', 'success');
        } else {
      ...
      default:
        ...
        showNotification(`Cannot reject unsupported operation: ${metadata.type}`, 'error');
        return; 
    }
    // Metadata was cleared at the start
  } catch (error) {
    console.error('Error rejecting operation:', error);
    showNotification(`Failed to reject operation: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
    ...
    notebooks.widgetAdded.connect((sender, panel: NotebookPanel) => {
      ...
      const addAIButton = (cell: Cell) => {
        ...
        if (!cellNumberLabel) {
          ...
          (cellNumberLabel as HTMLElement).style.color = 'var(--jp-ui-font-color2)';
          (cellNumberLabel as HTMLElement).style.fontFamily = 'var(--jp-ui-font-family)';
        ...
        panel.content.widgets.forEach((cell, index) => {
          ...
          if (toolbar) {
            ...
            if (!numberLabel) {
              ...
              (numberLabel as HTMLElement).style.color = 'var(--jp-ui-font-color2)';
              (numberLabel as HTMLElement).style.fontFamily = 'var(--jp-ui-font-family)';
