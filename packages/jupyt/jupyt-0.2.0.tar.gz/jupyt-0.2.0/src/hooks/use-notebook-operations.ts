...
export function useNotebookOperations({
  ...
  const copyToNotebook = async (
    ...
    try {
      ...
      showNotification(
        shouldExecute ? 'Code executed in notebook.' : 'Code copied to notebook.',
        'success'
      );
    } catch (error) {
      showNotification(
  ...
  const modifyCell = async (code: string, cell_index: number, run_needed?: boolean): Promise<void> => {
  ...
  const handleAddCell = async (code: string, cell_index: number, run_needed: boolean): Promise<void> => {
    if (!notebookPanel?.model) {
      showNotification('No notebook is open. Please open a notebook first.', 'error');
      return;
    }
    try {
      ...
      showNotification(
        run_needed ? `Code executed at cell index ${cell_index + 1}.` : `Code added at cell index ${cell_index + 1}.`,
        'success'
      );
    } catch (error) {
      showNotification(
  ...
  const handleRevertOperation = async (cell: CodeCell, metadata: JupytApprovedOperation): Promise<boolean> => {
