...
const getJupyterLabTheme = (): 'light' | 'dark' => {
  if (typeof document === 'undefined') return 'light'; // Default in SSR/non-browser
  // Check for JupyterLab theme attribute on body or html
  const themeLight = document.body.getAttribute('data-jp-theme-light') === 'true' || document.documentElement.getAttribute('data-jp-theme-light') === 'true';
  return themeLight ? 'light' : 'dark';
};

// Theme provider component
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children, commands }) => {
  ...
  const [currentTheme, setCurrentTheme] = useState<'light' | 'dark'>(() => 
    themeMode === 'system' ? getJupyterLabTheme() : themeMode as 'light' | 'dark'
  );
    ...
    if (mode === 'light' || mode === 'dark') {
      ...
      try {
        if (commands.hasCommand('apputils:change-theme')) {
          await commands.execute('apputils:change-theme', { theme: targetTheme });
  ...
  useEffect(() => {
    if (themeMode !== 'system' || typeof document === 'undefined') return;

    // Use MutationObserver to detect changes in JupyterLab theme attribute
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.type === 'attributes' && (mutation.attributeName === 'data-jp-theme-light' || mutation.attributeName === 'data-jp-theme-name')) {
          ...
          if (currentSetting !== 'system' && currentSetting !== newJupyterLabTheme) {
