...
import { DEFAULT_MODEL_CONFIG, getModelConfig, saveModelConfig } from '../config';
...
import { extractCellReferences, removeCellOperationTags } from '../utils/chatUtils';
...
import { JupytPendingOperation, PENDING_OPERATION_METADATA_KEY } from '../types/cell-metadata';
import { JupytApprovedOperation, APPROVED_OPERATION_METADATA_KEY } from '../types/cell-metadata';
import { CodeCell, CodeCellModel } from '@jupyterlab/cells';
import { injectOrUpdateCellUI, handleApprove, handleReject } from '../plugins/cell-toolbar';
...
export function Chat({
  ...
  const [modelConfig, setModelConfig] = useState<IModelConfig>(getModelConfig());
  ...
  const handleModelConfigChange = useCallback((config: IModelConfig) => {
    setModelConfig(config);
    saveModelConfig(config);
    showNotification(`Model updated to ${config.provider} - ${config.model}`, 'success');
  }, [showNotification]);
  ...
  const { copyToNotebook, modifyCell, handleAddCell, handleDeleteCell, handleRevertOperation } = useNotebookOperations({
    notebookPanel,
    sessionContext,
    showNotification
  });
  ...
  const executeCellOperation = async (op: CellOperation): Promise<string | undefined> => {
    try {
      if (op.type === 'create_cell' && op.code !== undefined && op.cell_index !== undefined) {
        await handleAddCell(op.code, op.cell_index, !!op.run_needed);
        return undefined;
      } else if (op.type === 'update_cell' && op.code !== undefined && op.cell_index !== undefined) {
      ...
      showNotification(`Cell operation failed: ${err instanceof Error ? err.message : 'Unknown error'}`, 'error');
  ...
  const { startAgenticLoop, cancelAgenticLoop, isLooping, revertAllChanges } = useAgenticLoopManager({
    plan,
    setPlan,
    planStage,
    setPlanStage,
  ...
  const handleNewChat = useCallback(() => {
    if (isStreaming || isLooping) return; // Prevent starting new chat while streaming
  ...
  }, [isStreaming, isLooping, setPlan, setPlanStage, setCellOutput, showNotification, scrollToBottom]);
  ...
  const cleanMessageContent = (content: string): string => {
    ...
    const cleanedOfStatus = content.replace(/\n?\[COMPLETION_STATUS:[^\]]*\]/gi, '').trim();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lastChunk && lastChunk.next_action && lastChunk.next_action.length > 0 && lastChunk.completion_status === 'continue') {
          ...
          if (hasOutputs(cell.model)) {
            ...
            setTimeout(() => {
              if (!resolved) {
                ...
                initialCellOutput = extractTextOutputFromCell(cell.model as any);
              }
    ...
    if (lastChunk && lastChunk.completion_status === 'continue') {
      ...
      if (operations && operations.length > 0) {
        ...
        for (const op of operations) {
          ...
          try {
            if (op.type === 'create_cell' && op.cell_index !== undefined && op.code !== undefined) {
              // Insert a placeholder cell FIRST
              model.sharedModel.insertCell(op.cell_index, { cell_type: 'code', source: '' }); // Insert empty initially
              ...
              if (cellWidget) {
                ...
                cellWidget.model.sharedModel.setMetadata(PENDING_OPERATION_METADATA_KEY, pendingMetadata as any); // Cast to any
                
                // Delay UI injection and selection slightly to allow DOM rendering
                setTimeout(() => {
                    // Add checks for safety and type narrowing
                    if (cellWidget.isAttached && cellWidget.node && op.cell_index !== undefined) { 
                        ...
                        console.warn(`Cell widget at index ${op.cell_index ?? 'unknown'} not ready after delay for UI injection.`);
                    }
                }, 0); 

              } else {
                console.error(`Failed to get cell widget after creation at index ${op.cell_index}`);
                showNotification(`Error preparing cell creation at index ${op.cell_index + 1}.`, 'error');
              }
            } else if (op.type === 'update_cell' && op.cell_index !== undefined && op.code !== undefined) {
              const cellWidget = notebook.widgets[op.cell_index];
              if (cellWidget) {
                ...
                showNotification(`Error: Could not find cell ${op.cell_index + 1} to update.`, 'error');
              }
            } else if (op.type === 'delete_cell' && op.cell_index !== undefined) {
              const cellWidget = notebook.widgets[op.cell_index];
              if (cellWidget) {
                ...
                showNotification(`Error: Could not find cell ${op.cell_index + 1} to delete.`, 'error');
              }
            }
          } catch (metaError) {
            console.error("Error setting metadata:", metaError);
            showNotification(`Failed to set metadata for operation on cell ${op.cell_index !== undefined ? op.cell_index + 1 : 'N/A'}.`, 'error');
          }
        }
        ...
        setMessages(prev => {
          ...
          if (assistantMsgIndex >= 0 && assistantMsgIndex < updated.length && updated[assistantMsgIndex]) {
  ...
  const checkCanRevert = useCallback(async (operation: CellOperation): Promise<boolean> => {
      ...
      if (cellWidget instanceof CodeCell && cellWidget.model) {
          const approvedMetadata = cellWidget.model.sharedModel.getMetadata(APPROVED_OPERATION_METADATA_KEY) as JupytApprovedOperation | undefined;
  ...
  const handleRevertRequest = useCallback(async (operation: CellOperation, cell: CodeCell) => {
      if (!cell.model) return;
      const model = cell.model as CodeCellModel;
      const approvedMetadata = model.sharedModel.getMetadata(APPROVED_OPERATION_METADATA_KEY) as JupytApprovedOperation | undefined;
      ...
      if (approvedMetadata && approvedMetadata.type === operation.type) {
          ...
          showNotification('Revert impossible: Cell state changed or metadata missing.', 'error');
      }
  ...
  const checkPendingOperations = useCallback(() => {
    if (!notebookPanel?.content) return false;
    const notebook = notebookPanel.content;
    for (const cell of notebook.widgets) {
      if (cell instanceof CodeCell && cell.model) {
        const pendingMetadata = cell.model.sharedModel.getMetadata(PENDING_OPERATION_METADATA_KEY);
        if (pendingMetadata) {
          return true;
        }
      }
  ...
  const handleApproveAll = useCallback(async () => {
    if (!notebookPanel?.content) return;
    const notebook = notebookPanel.content;
    ...
    pendingCells.sort((a, b) => {
      const aMetadata = (a.cell as CodeCell).model?.sharedModel.getMetadata(PENDING_OPERATION_METADATA_KEY) as unknown as JupytPendingOperation;
      const bMetadata = (b.cell as CodeCell).model?.sharedModel.getMetadata(PENDING_OPERATION_METADATA_KEY) as unknown as JupytPendingOperation;
      // Put delete operations at the end
      if (aMetadata.type === 'delete_cell' && bMetadata.type !== 'delete_cell') return 1;
      if (bMetadata.type === 'delete_cell' && aMetadata.type !== 'delete_cell') return -1;
      return 0;
    });
    ...
    for (const { cell } of pendingCells) {
      if (cell instanceof CodeCell && cell.model) {
        const pendingMetadata = cell.model.sharedModel.getMetadata(PENDING_OPERATION_METADATA_KEY) as JupytPendingOperation | undefined;
        if (pendingMetadata) {
    ...
    >
      ...
      >       
        {messages.map((message, index) => {
          const { cellNumbers } = extractCellReferences(message.role === 'user' ? message.content : messages[index - 1]?.content || '');
